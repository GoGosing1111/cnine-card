...
async function api(path,options={}){
  ...
  if(!adminToken) throw new Error('숲켓몬 관리자 인증을 먼저 가져오세요.');
  let response;
  try{
    ...
    throw new Error(`숲켓몬 서버 연결 실패: ${error?.message||'네트워크 또는 CORS 오류'}`);
  }
...
async function syncAdminTokenFromTabs(){
  ...
  if(!tabs.length) lastError='열려 있는 숲켓몬 CMS 탭을 찾지 못했습니다.';
  return {connected:false,error:lastError};
}

chrome.runtime.onMessage.addListener((message,sender,sendResponse)=>{
  (async()=>{
    if(message?.type==='SOOP_SAVE_ADMIN_TOKEN'){
      const token=String(message.token||'').trim();
      if(!token) throw new Error('관리자 토큰이 없습니다.');
      await setStore({adminToken:token,adminTokenSavedAt:new Date().toISOString()});
      return {ok:true};
    }
    if(message?.type==='SOOP_STATUS'){
    ...
    if(message?.type==='SOOP_SYNC_AUTH'){const result=await syncAdminTokenFromTabs();return {ok:true,...result};}
    if(message?.type==='SOOP_LOGOUT'){await chrome.storage.local.remove(['adminToken','adminTokenSavedAt']);return {ok:true};}
    if(message?.type==='SOOP_RESOLVE')return api('admin/wago-extension/resolve',{method:'POST',body:JSON.stringify({wagoNickname:message.wagoNickname,sourceUrl:message.sourceUrl})});
    if(message?.type==='SOOP_GRANT')return api('admin/wago-extension/grant',{method:'POST',body:JSON.stringify(message.payload||{})});
    if(message?.type==='SOOP_SAVE_SETTINGS'){await setStore(message.settings||{});return {ok:true};}
