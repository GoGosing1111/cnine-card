(()=>{
  ...
  async function open(nick,node){
    ...
    const resolved=await send({type:'SOOP_RESOLVE',wagoNickname:nick,sourceUrl:location.href});
    ...
    if(!resolved.ok){account.innerHTML='연결된 숲켓몬 계정<strong>확인 실패</strong>';msg.className='cnw-msg cnw-error';msg.textContent=resolved.error;return;}
    current.resolved=resolved;account.innerHTML=`연결된 숲켓몬 계정<strong>${esc(resolved.gameUser.nickname)}</strong><span>현재 ${Number(resolved.gameUser.coin).toLocaleString()}코인</span>`;msg.textContent='지급 금액과 사유를 확인하세요.';submit.disabled=false;
    submit.onclick=async()=>{
      ...
      const result=await send({type:'SOOP_GRANT',payload:{requestId,targetUserId:resolved.gameUser.id,wagoNickname:nick,amount,reason,sourceUrl:location.href,sourceKey:sourceKey(node)}});
      if(!result.ok){msg.className='cnw-msg cnw-error';msg.textContent=result.error;submit.disabled=false;return;}
      msg.className='cnw-msg cnw-success';msg.textContent=result.delivery==='MESSAGE'?`메시지 발송 완료\n${amount.toLocaleString()}코인은 유저가 메시지에서 수령합니다.`:`지급 완료`;account.innerHTML=`연결된 숲켓몬 계정<strong>${esc(result.gameUser.nickname)}</strong><span>보상 메시지 발송 완료</span>`;
    };
  }
  ...
  function attachMessageButtons(root=document){
    ...
    for(const row of rows){
      ...
      btn.type='button';btn.className='cnine-msg-reward-btn';btn.title='숲켓몬 코인 지급';btn.setAttribute('aria-label','숲켓몬 코인 지급');btn.textContent='🪙';
