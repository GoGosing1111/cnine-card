...
import {RAGNIEL,MODES,makePlan} from '../skill.mjs';
import {RagnielSkillFX,loadRagnielAssets} from './RagnielSkillFX.js';
const parentDoc=window.parent.document,$=id=>parentDoc.getElementById(id)||document.getElementById(id),ROOT='/preview/mercenary-ragniel-v1/';
...
function plan(){const mode=$('mode').value,at=mode==='ultimate'?2.05:mode==='slash'?.55:.30;return makePlan({mode,cancelAt:$('scenario').value==='interrupt'?at:null,targetLostAt:$('scenario').value==='lost'?at:null});}
...
async function boot(){
 try{
  ...
  const prepared=window.ProjectVBattleV3Live.prepareLoading({modal:$('lab-modal'),mode:'PVP',playerName:'라그니엘 · SSS',opponentName:'연출 검수',autoText:'전장 준비 중'});
  renderer=await window.ProjectVBattleV3Live.createRenderer({...prepared,modal:$('lab-modal'),data:payload,mode:'PVP',playerName:'라그니엘 · SSS'});api.mountForBattle=mountForBattle;
  ...
  const art=adapter.resolveForConsumer('BATTLE_FIELD',manifest.code);if(!art)throw Error('라그니엘 전투 SD 누락');
  const [sd,cutin,assets]=await Promise.all([Assets.load(art.spriteUrl),Assets.load(ROOT+'assets/source-art-preview.webp'),loadRagnielAssets(manifest)]);
  merc=new BattleCharacter({id:'RAGNIEL_PREVIEW',name:RAGNIEL.name,team:TEAM.ALLY,fullBodyTexture:sd,cutInTexture:cutin,fullBodyHeight:380,accent:0xf0cd7d});
  ...
  fx=new RagnielSkillFX(engine,merc,targets,assets,manifest,plan(),update);
  ...
  $('health').textContent='재생 준비 완료';engine.app.renderer.on('resize',resize);prepared.phase.textContent='라그니엘 · 종언의 백금성역';prepared.stage.querySelector('#pvBattleStatus').textContent='SSS 용병 · 동작과 스킬 연출 검수';
  const review={get fx(){return fx;},get engine(){return engine;},get merc(){return merc;},manifest,diagnostics:()=>fx.diagnostics(),dispose};window.RagnielPreview=review;window.parent.RagnielPreview=review;
 ...
 }catch(error){$('health').textContent='시연 준비 실패: '+error.message;console.error('[Ragniel]',error);}
}
