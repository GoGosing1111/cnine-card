...
2026-09-11: 배치 표시는 [V3 공통 점유 그리드](../../docs/project-v-common-grid-v1.md)를 상속한다. 승인 이펙트·총기·아틀라스를 재생성하지 않고 아군 중앙 슈트의 새 기준점으로 재생·복귀를 검수했다. 이번 그리드 변경의 라이브 배포는 사용자 지시로 보류한다.

상태: **USER_APPROVED_LIVE_INTEGRATED_V2046**. 승인 이펙트를 공용 V3 스킬칩 런타임에 연결했다. 이 프리뷰 자체는 로컬 시각 검수용이며 계정 지급·저장·실제 피해가 없다. 전투·장착 계약의 최신 기준은 [V2046 문서](../../docs/battle-suit-skill-chip-v2046.md)다. 아래 독립 프리뷰 설계 설명 중 운영 미연결 표기는 최초 검수 시점의 기록이다.
...
- 로켓런처: 0.12초 총구 발사 → 탄체/배기/연기 이동 → 0.36초 충돌 → 잔불·연기. 전체 2.25초. 원본 GIF의 짧은 후반부보다 잔향을 길게 조정했다. 투사체는 승인된 궤적(대상 Y−62)을 유지하며 폭발은 별도 발끝 기준점(V3 `SOLE_CENTER`, 대상 Y)에 고정한다. 프레임별로 측정한 폭발 바닥·먼지·지면광이 발끝에 맞춰지고 헬기 폭격 좌표는 유지한다.
