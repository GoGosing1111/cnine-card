...
import {MANGISA_SKILL,sampleMangisaPreview,mangisaPoseAt} from '../skill.mjs';

const clamp=(n,a=0,b=1)=>Math.min(b,Math.max(a,n));
const mix=(a,b,p)=>a+(b-a)*p;
export async function loadMangisaAssets(manifest){
 const base='/preview/mercenary-mangisa-v1/';
...
export class MangisaSkillFX{
 constructor(engine,merc,targets,assets,manifest,plan,onUpdate=()=>{},{authoritative=false,useAuthoredPose=true}={}){
  ...
  this.layer=new Container({label:'MangisaSkill:MS-045'});this.layer.eventMode='none';engine.effectLayer.addChild(this.layer);
 ...
 muzzle(){
  const s=this.merc.fullBodySprite,p=this.frame>=0?this.poses[this.frame].muzzle:{x:.984,y:.196};
 ...
 render(time){
  ...
  const state=sampleMangisaPreview(this.plan,time);this.sample=state;
  const lastShot=this.plan.events.filter(e=>e.kind==='SHOT').at(-1)?.at??0;
  this.applyPose(lastShot<1.23&&time>lastShot+.13?5:mangisaPoseAt(time),time>0&&time<Math.min(1.82,lastShot+.59)&&!state.cancelled);
  this.targets.forEach((a,i)=>{if(!a?.root||a.root.destroyed)return;if(!this.authoritative){a.hp=Math.max(0,100-(state.damage['E'+(i+1)]||0)*17);a.layoutHudBars();}a.fullBodySprite.tint=0xffffff});
  ...
  const draw=(texture,p,size,{alpha=1,angle=0,tint=0xffffff,blend='normal',height=size}={})=>{
   if(this.used>=this.pool.length)throw Error('Mangisa effect pool exceeded');const s=this.pool[this.used++];s.texture=texture;s.visible=true;s.position.set(p.x,p.y);s.width=size;s.height=height;s.alpha=clamp(alpha);s.rotation=angle;s.tint=tint;s.blendMode=blend;
  };
  const muzzle=this.muzzle(),target=this.point(this.targets[0]);
  for(const e of this.plan.events.filter(e=>e.kind==='SHOT')){
   const age=time-e.at,travel=MANGISA_SKILL.visual.travel;
   if(age>=0&&age<travel){const q=age/travel,r=clamp(q-.34);for(const [width,color,alpha]of [[7,0xd74a32,.24],[2,0xffe5a6,.95]])this.lines.moveTo(mix(muzzle.x,target.x,r),mix(muzzle.y,target.y,r)).lineTo(mix(muzzle.x,target.x,q),mix(muzzle.y,target.y,q)).stroke({width,color,alpha});}
   if(age>=0&&age<.07)draw(this.assets.flash,muzzle,48,{height:25,alpha:(1-age/.07)*.95,tint:0xffdf9a,blend:'add'});
