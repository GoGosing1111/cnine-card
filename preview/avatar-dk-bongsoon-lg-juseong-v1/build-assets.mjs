...
const result={generator:'built-in image_gen',processing:'Native alpha retained. Uniform resize, transparent padding and WebP encoding only. Source PNGs unchanged.',avatars:{}};
for(const stem of ['avatar-dk-bongsoon','avatar-lg-juseong']){
 const file=name=>new URL('./assets/'+stem+'-'+name,import.meta.url);
 const lobby=await readFile(file('lobby-source-art-v1.png')),equipment=await readFile(file('equipment-source-art-v1.png'));
 const m=await sharp(equipment).metadata();if(!m.hasAlpha)throw Error(stem+': native alpha missing');
 ...
 if(clear/(info.width*info.height)<.4||solid/(info.width*info.height)<.1||borderMax>8)throw Error(stem+': invalid transparent silhouette');
 ...
 const files={};for(const name of ['lobby-source-art-v1.png','equipment-source-art-v1.png','lobby-v1-1024.webp','lobby-v1-640.webp','equipment-v1-640.webp']){
  const b=await readFile(file(name)),meta=await sharp(b).metadata();files[stem+'-'+name]={sha256:sha(b),bytes:b.length,width:meta.width,height:meta.height,hasAlpha:meta.hasAlpha};
 }
 result.avatars[stem]={alpha:{clearPixels:clear,solidPixels:solid,borderMax,crop},runtime:{width:640,height:1088,uniformScale:scale,bodyWidth:width,bodyHeight:height},files};
}
