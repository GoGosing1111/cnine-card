(() => {
  ...
  async function prepare(next) {
    ...
    const prepared = ProjectVBattleV3Live.prepareLoading({modal, mode: payload.mode,
    ...
    const nextRenderer = await ProjectVBattleV3Live.createRenderer({...prepared, modal, data: payload,
      mode: payload.mode, playerName: payload.playerName || '폐차장 회수대', playUltimateCinematics: false});
  ...
  async function play(onEvent) {
    ...
    try {
      ...
      if (token === epoch) {
        runEngine.previewPlaybackBusy = false;
  ...
  function cancel() {
    if (engine) engine.previewPlaybackBusy = false;
  ...
  window.WideGridBattleBridge = {prepare, play, pause, resume, cancel, dispose,
