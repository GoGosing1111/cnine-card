import {connect} from '/js/clan-faction-v1.mjs?v=2127';
async function boot(){
...
const user={id:1,serverUserId:1,nickname:'DK 지휘관',coin:2400000000,cardShards:284000,masterStars:13400,pigCoin:0};
const apiRequest=async(path,options={})=>{const res=await fetch('/api/'+path,options),data=await res.json();if(!res.ok)throw Error(data.error||'검수 서버 연결 실패');return data;};
const renderShell=()=>{document.getElementById('review-mount').innerHTML=window.ClanV1.view();window.ClanV1.bind(ctx);};
const ctx={apiRequest,renderShell,clearApiCache(){},async playFactionBattle(data){
