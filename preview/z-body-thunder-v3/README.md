...
- 마지막 요청 `날라갈대 대쉬 이펙트좀 쌔게줘봐 화려하게`: 돌진 전류 12프레임을 추가하고 5겹 잔상으로 강화했다. 이 증폭본은 **USER_REVIEW_PENDING**.
- `shared/z-body-area-skill.mjs`의 운영 게이트는 false다. 신규 FX 등록은 이 프리뷰에만 있다. API 요청은 로컬 검수용 Symbol 옵션을 만들 수 없다.
- 운영 배포·기능 활성화는 하지 않았다. 최종 승인 후 운영 등록·캐시 버전·배포 기준 SHA를 확인해 범위 배포한다. 기술 검사 통과를 시각 승인으로 대체하지 않는다.
...
합계 **72개의 서로 다른 프레임**. 원본 RGBA PNG와 SHA-256은 `assets.json`·`normal-assets.json`에 기록했다. Sharp 0.35.2는 셀 추출·균일 배율·투명 여백 패킹만 수행하며 원본 알파를 보존한다. 승인된 슈트 몸체·검 원본과 크기 `0.563696948902027`은 변경하지 않는다.

잠금 버전은 PixiJS **8.20.0**, GSAP **3.13.0**. `build-report.json`은 하나의 Pixi/GSAP 번들만 사용함을 검증한다.

- `source/ZBodyNormalFX.js`: 공용 `backgroundLayer`에 번개 꼬리·전류 증폭·잔상, `effectLayer`에 검격과 방전을 표시한다. 별도 타이머/렌더러/GSAP 인스턴스 없이 공용 `ZBodySwordAnimation`의 GSAP 밀리초를 샘플링한다.
- 일반 공격: 이동 55–190ms, 접촉 **245ms**, 복귀 완료 540ms, 전체 640ms. 검격 5번째 프레임과 방전 3번째 프레임이 245ms에 맞는다. 돌진 증폭은 40–302ms이며 준비→확장→분기→소멸을 각각 그렸다.
- `source/ZBodyThunderFX.js`: 공용 `BattleSuitSkillChipPlayback`의 GSAP 초 단위 전투 시계를 샘플링한다. 기존 뇌검 자세를 사용하고 1080/1210/1340/1430/1540ms 서버 타격을 확인한 뒤 접촉 프레임을 진행한다. 대상 좌표·ID를 고정하고 교체된 적 슬롯에 새 피해를 만들지 않는다.
...
관련 검사만 수행했다. 전체 출시 게이트/운영 배포는 수행하지 않았다.
...
서버 Z-BODY 스킬 계산, 해당 공용 스킬 재생·몸체 충돌 방지, 검수 자산과 페이지만 변경했다. DB·권한·의존성·다른 콘텐츠 정책 변경은 없다. 최종 운영 배포 시 `docs/scoped-release-policy-20260923.md`에 따라 실제 직전 배포 SHA·캐시·출시 플래그·원격 main 일치를 확인하고 `npm run deploy:production -- --scoped`를 사용한다. 이미 통과한 동일 빌드 검사를 불필요하게 반복하지 않는다.
