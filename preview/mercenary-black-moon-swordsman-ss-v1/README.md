# 흑월 검객 · SS 원화 / SD / 전용 스킬 검수판

- 상태: 원화 `APPROVED_SOURCE_ART`, 새 SD·모션·스킬 `USER_REVIEW_PENDING`
- 사용자 지정 등급: **SS**
...
2026-09-22 후속 **“승인 SD,스킬까지 만들것”**에 따라 원화 승인을 기록하고 전투 리소스를 제작했다. 새 SD·모션·스킬은 사용자 시각 검수 대기다.

이름 **흑월 검객**, 스킬명 **흑월 삼연참**은 가칭이다. `V-998`은 독립 프리뷰의 아트 어댑터 식별자이며 운영 카드 코드가 아니다. SS 등급은 사용자 지정이다.
능력치·피해 배율·쿨다운·획득 경로·운영 스킬 배정은 설정하지 않았다. 이 폴더는 독립 프리뷰에서만 사용하며 운영 DB/API·라이브 로스터·전투 버전·플래그를 변경하지 않는다.
