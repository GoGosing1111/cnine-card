...
// 2026-09-08 user approval: “배경만 스크립트로 제거”.
...
const source = path.join(assets, 'avatar-cheon-equipment-draft-v1.png');
const output = path.join(assets, 'avatar-cheon-equipment-source-art-v1.png');
const expectedHash = '62FB9857862015C4B24AB166C070A6A2222F1913BDDD5EE726FF9046174C6153';
...
async function main() {
  ...
  // Measured checkerboard: neutral (channel range <= 5), minimum luminance 99.7.
  // The hair/arm aperture has warm checker spill (range <= 28). Trousers keep
  // the stricter neutral threshold. Flood connected areas, not whites globally.
  const background = p => {
    const r = data[p * 4], g = data[p * 4 + 1], b = data[p * 4 + 2];
    const maxRange = Math.floor(p / width) < 515 ? 28 : 8;
    return Math.max(r, g, b) - Math.min(r, g, b) <= maxRange && (r + g + b) / 3 >= 96;
  };
  let read = 0, write = 0;
  const enqueue = p => {
  ...
  const enclosedSeeds = [[353, 746], [354, 759]];
  ...
  await sharp(output).resize({width: 640}).webp({quality: 92, alphaQuality: 100, effort: 4})
    .toFile(path.join(assets, 'avatar-cheon-equipment-v1-640.webp'));
  ...
  console.log(JSON.stringify({sourceSha256: expectedHash, outputSha256: sha(output), width, height,
    hasAlpha: true, transparent, opaque, partial, connectedRemoved, mattePixels, opaqueRgbChanged, bounds, enclosedSeeds}, null, 2));
}
