...
2026-09-25. 상태: **USER_REVIEW_PENDING**. V1의 도형·밝기 변화 위주 연출은 사용자 반려다. 이 폴더는 재제작한 독립 시각 검수본이다. 기존 보상 정책·CMS·API·확률·최소 지급량을 바꾸지 않는다. 게임 실행 파일에는 연결하지 않았다.
...
- `reliquary-stage.mjs`: 기존 `/js/ui-fx-vendor-v2045.bundle.js`의 **PixiJS 8.20.0 / GSAP 3.13.0**을 재사용한다. 별도 라이브러리·CDN·엔진 의존성을 추가하지 않는다. 전투 장면을 복제하지 않는 단일 UI 캔버스다.
- `picker.mjs`: 접근 가능한 선택 버튼, 결과 표시, 동일 선택 재시도, 12초 자산 로딩/15초 결과 응답 제한, 창 닫기·화면 비활성화 정리. 보상은 주입된 `claim` 반환값만 표시한다. 프리뷰에서는 고정 샘플만 반환한다.
- `reward-preview.css`: 기존 Noto Sans KR·Barlow Condensed와 라임 확인 버튼을 계승하며, 금속·청록 광원은 전리품 원화의 재질을 따른다. 320×568까지 화면 겹침을 확인했다.
...
- 기술 검사와 자체 시각 점검은 완료했지만 사용자 시각 승인을 대신하지 않는다. V1 반려를 취소하거나 V2를 운영에 자동 연결한 상태가 아니다.

검사 결과·스크린샷·WebM은 저장소 바깥 `../qa-core-rewards-v2/`에 보관한다. 게임 서버·거래 로직·CMS 변경이 없는 독립 프리뷰라 전체 release gate/운영 재배포를 실행하지 않는다.
