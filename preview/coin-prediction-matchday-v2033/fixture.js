(() => {
  ...
  const sample=[['SOCCER','토트넘 vs 아스널','토트넘 승리','무승부','아스널 승리'],['LOL','T1 vs Gen.G · 결승전','T1 승리','Gen.G 승리'],['BASEBALL','LG 트윈스 vs KIA 타이거즈','LG 승리','KIA 승리'],['BASKETBALL','서울 SK vs 부산 KCC','서울 SK 승리','부산 KCC 승리'],['SETKA','세트카 · 이바노프 vs 페트로프','이바노프 승리','페트로프 승리'],['OTHER','영토전 · 희야파 vs 히나파','희야파 승리','히나파 승리']];
  const events=Array.from({length:16},(_,i)=>{
    const [category,title,...labels]=sample[i%6],id=i+1;
    ...
    return {id,category,title:i>5?`${title} · ${i+1}경기`:title,description:i===0?'정규 시간 종료 기준 승리 팀을 예측하세요. 연장전과 승부차기는 포함하지 않습니다.':'최종 경기 결과를 기준으로 정산합니다.',image_url:'',status:'OPEN',closes_at:new Date(Date.now()+(i+1)*1800000).toISOString(),total_pool:options.reduce((n,o)=>n+o.total_bet,0),treasury_subsidy:i===0?300000000:0,fee_percent:10,min_bet:100000,max_bet:500000000,participant_count:223,options,myBet:i===0||i===3?{event_id:id,option_id:options[0].id,amount:50000000,payout:0,status:'ACTIVE'}:null};
  });
