...
const file=name=>new URL('./assets/avatar-fm-dimwoos-'+name,import.meta.url);
const sha=b=>createHash('sha256').update(b).digest('hex');
const lobby=await readFile(file('lobby-source-art-v3.png')),equipment=await readFile(file('equipment-source-art-v1.png'));
if(sha(lobby)!=='1bccb2cc6d22dbb061784e1297e740c86418af39fbcc24535ab725442ae71664')throw Error('Approved lobby changed');
if(sha(equipment)!=='53ae25adf9bc0d76936a2697d70ad3a70b43ef029d8345363b7a604868528e16')throw Error('Approved equipment changed');
...
const files={};
for(const name of ['lobby-source-art-v3.png','equipment-source-art-v1.png','lobby-v3-1024.webp','lobby-v3-640.webp','equipment-v1-640.webp']){
 const b=await readFile(file(name)),m=await sharp(b).metadata();files['avatar-fm-dimwoos-'+name]={sha256:sha(b),bytes:b.length,width:m.width,height:m.height,hasAlpha:m.hasAlpha};
}
