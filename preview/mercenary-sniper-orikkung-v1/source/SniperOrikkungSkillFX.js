...
import {SNIPER_ORIKKUNG_TRAVEL,sniperOrikkungPoseAt} from '../../../shared/mercenary-sniper-orikkung-v1.mjs';
...
export async function loadSniperOrikkungAssets(manifest){
 const base='/preview/mercenary-sniper-orikkung-v1/';
...
export class SniperOrikkungSkillFX{
 constructor(engine,merc,targets,assets,manifest,plan,onUpdate=()=>{},{authoritative=false,useAuthoredPose=true}={}){
  ...
  this.layer=new Container({label:'SniperOrikkungSkill:MS-050'});this.layer.eventMode='none';engine.effectLayer.addChild(this.layer);
 ...
 render(time){
  ...
  this.applyPose(sniperOrikkungPoseAt(time,this.plan),time>0&&time<(this.plan.basic?1:1.8)&&!state.cancelled);
  ...
  const draw=(texture,p,size,{alpha=1,angle=0,tint=0xffffff,blend='normal',height=size}={})=>{
   if(this.used>=this.pool.length)throw Error('SniperOrikkung effect pool exceeded');const s=this.pool[this.used++];s.texture=texture;s.visible=true;s.position.set(p.x,p.y);s.width=size;s.height=height;s.alpha=clamp(alpha);s.rotation=angle;s.tint=tint;s.blendMode=blend;
  };
  const muzzle=this.muzzle(),target=this.point(this.targets[0]);
  for(const e of this.plan.events.filter(e=>e.kind==='SHOT')){
   const age=time-e.at,travel=SNIPER_ORIKKUNG_TRAVEL/(this.plan.basic?1.8:1);
   if(age>=0&&age<travel){const q=age/travel,r=clamp(q-.34);for(const [width,color,alpha]of [[10,0x4faa88,.32],[3,0xffedb5,.98]])this.lines.moveTo(mix(muzzle.x,target.x,r),mix(muzzle.y,target.y,r)).lineTo(mix(muzzle.x,target.x,q),mix(muzzle.y,target.y,q)).stroke({width,color,alpha});}
   if(age>=0&&age<.07)draw(this.assets.flash,muzzle,76,{height:36,alpha:(1-age/.07)*.95,tint:0xffe9a8,blend:'add'});
  ...
  for(const e of this.plan.events.filter(e=>e.kind==='HIT'||e.kind==='SPLASH')){
   ...
   const age=time-e.at,p=this.point(actor),final=!this.plan.basic;
   if(!final){if(age>=0&&age<.11)draw(this.assets.impact[4+Math.min(2,Math.floor(age/.04))],p,100,{alpha:1-age/.13});}
