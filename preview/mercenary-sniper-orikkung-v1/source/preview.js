...
import {ORIKKUNG_SKILL,compileOrikkungPreview} from '../skill.mjs';
import {SniperOrikkungSkillFX,loadSniperOrikkungAssets} from './SniperOrikkungSkillFX.js';

const parentDoc=window.parent.document,$=id=>parentDoc.getElementById(id)||document.getElementById(id);
const ROOT='/preview/mercenary-sniper-orikkung-v1/';
...
const plan=()=>compileOrikkungPreview({mode:$('mode').value,basic:$('targets').value==='basic',cancelAt:$('scenario').value==='interrupt'?.4:null});
...
async function boot(){
 try{
  ...
  const prepared=window.ProjectVBattleV3Live.prepareLoading({modal:$('lab-modal'),mode:'PVP',playerName:'저격 오리꿍 스킬 검수',opponentName:'승인 카드 모의 표적',autoText:'공유 V3 전장 준비 중'});
  renderer=await window.ProjectVBattleV3Live.createRenderer({...prepared,modal:$('lab-modal'),data:payload,mode:'PVP',playerName:'저격 오리꿍 스킬 검수'});api.mountForBattle=mountForBattle;
  ...
  const art=adapter.resolveForConsumer('BATTLE_FIELD',manifest.code);if(!art)throw Error('저격 오리꿍 전투 SD 해석 실패');
  const [sd,cutin,assets]=await Promise.all([Assets.load(art.spriteUrl),Assets.load(ROOT+'assets/source-art-preview.webp'),loadSniperOrikkungAssets(manifest)]);
  merc=new BattleCharacter({id:'ORIKKUNG_PREVIEW',name:'저격 오리꿍',team:TEAM.ALLY,fullBodyTexture:sd,cutInTexture:cutin,fullBodyHeight:260,accent:0x8ce5ba});
  ...
  fx=new SniperOrikkungSkillFX(engine,merc,engine.enemies.slice(0,1),assets,manifest,plan(),update);
  $('play').onclick=()=>fx.playing?fx.pause():fx.play();$('restart').onclick=()=>{fx.seek(0);fx.play()};$('impact').onclick=()=>fx.seek(fx.plan.basic?(.6/1.8):ORIKKUNG_SKILL.visual.impactAt);$('cancel').onclick=()=>fx.cancel();
  ...
  prepared.phase.textContent='저격 오리꿍 · 에메랄드 대물저격';prepared.stage.querySelector('#pvBattleStatus').textContent='SD·스킬 검수 · 실제 계정 변경 없음';
  const review={get fx(){return fx},get engine(){return engine},diagnostics:()=>fx.diagnostics(),dispose};window.SniperOrikkungPreview=review;window.parent.SniperOrikkungPreview=review;
 ...
 }catch(error){$('health').textContent='전투 시연 준비 실패: '+error.message;console.error('[SniperOrikkung]',error)}
}
