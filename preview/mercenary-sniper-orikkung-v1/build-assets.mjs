...
const motion=await inspect(path.join(root,'assets/source/sniper-orikkung-fire-motion-v1.png'));
...
for(const [index,b]of components.entries()){
 ...
 for(let y=b.y0;y<=b.y1;y++)for(let x=b.x0;x<=b.x1;x++)if(motion.data[(y*w+x)*4+3]>=128){
  ...
  if(y<b.y0+(b.y1-b.y0)*.68){if(x>mx){mx=x;my=y;mc=1;}else if(x===mx){my+=y;mc++;}}
 }
...
const sd='assets/ui/project-v/characters/mercenary/mercenary-v050-sniper-orikkung-sd-v1.png';
await fs.copyFile(path.join(root,motionFrames[0].file),path.join(project,sd));
const sprite=await inspect(path.join(project,sd));
const art='assets/ui/project-v/mercenaries/approved-20260926/mercenary-v050-sniper-orikkung-source-art-v1.png',source=await inspect(path.join(project,art));
await sharp(source.bytes).resize({width:640}).webp({quality:92}).toFile(path.join(root,'assets/source-art-preview.webp'));
const effect=await inspect(path.join(root,'assets/source/emerald-antimateriel-impact-v1.png'));
...
const manifest={version:1,code:'V-050',name:'저격 오리꿍',rank:'SS',sourceArt:art,sourceArtInfo:source.record,battleSprite:sd,battleSpriteInfo:sprite.record,battleSpriteFootAnchor:motionFrames[0].footAnchor,battleSpriteFacing:'RIGHT',battleSpriteMuzzle:motionFrames[0].muzzle,status:'USER_REQUESTED_LIVE',runtimeEnabled:true,
 renderer:{pixi:'8.20.0',gsap:'3.13.0',engine:'preview/project-v-v3/source/battle/BattleEngine.js',clockOwner:'V3_REGISTERED_GSAP',audio:'EXISTING_LICENSED_SNIPER_AND_IMPACT_PROFILES'},
 impact:{source:'assets/source/emerald-antimateriel-impact-v1.png',sourceInfo:effect.record,atlas:'assets/impact-atlas.webp',cellSize:effectCell,columns:4,rows:4,frameCount:16,collisionFrame:4,frames:effectFrames},
 motion:{source:'assets/source/sniper-orikkung-fire-motion-v1.png',sourceInfo:motion.record,atlas:'assets/motion-atlas.webp',cellSize:cell,columns:3,rows:2,frameCount:6,frames:motionFrames},
 processing:'Approved illustration unchanged. Native-alpha motion components extracted and transparently padded, no retouching or synthetic intermediate poses. Idle uses pose 0. Approved goggles, khaki outfit, yellow duck anatomy and straight horizontal rifle retained.',
 generation:{mode:'BUILT_IN_IMAGEGEN',prompts:['prompt-impact-v1.txt','prompt-motion-v1.txt']}};
