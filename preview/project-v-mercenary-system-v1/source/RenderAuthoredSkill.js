...
export function renderAuthored(fx,time){
  ...
  if(!fx.authoritative)for(const [id,a]of actors){const s=states.get(id);a.root.visible=!!s;a.root.alpha=s?.hp>0?1:.22;
  ...
  const approach=(at,id,{reach=67,returnAt=at+.18,returnDuration=.44}={})=>{
    ...
    const direction=p.x>=o.x?1:-1;a.root.x=mix(o.x,p.x-reach*direction,q);a.root.y=mix(o.y,p.y+9,q);a.root.rotation=-.055*direction*Math.sin(q*Math.PI);
  ...
  switch(mode){
    ...
    case 'EVENT_HORIZON': {
      ...
      for(const id of plan.targets){
        ...
        const alpha=terminal||fx.authoritative&&plan.effectPhase===0?1:1-smooth((time-loss)/.18);
    ...
    case 'INTERCEPT': {
      approach(.62,target,{reach:35,returnAt:1.37});const p=point(target),contact={x:p.x+(actors.get('M')?.team==='ENEMY'?-24:24),y:p.y};
      material(contact,time-1.05,{size:345,lead:.65,life:plan.scenario==='counter'?.42:1.25,alpha:plan.scenario==='counter'?.45:1});
      if(plan.scenario!=='counter'&&(!fx.authoritative||plan.eventType==='MERCENARY_INTERCEPT')){if(actors.has('E_SOURCE'))trace(point('E_SOURCE'),contact,1.05);else if(actors.has('E1'))trace(point('E1'),contact,1.05);debris(contact,time-1.05,{count:26,strength:.8});light(point(target,true),time-1.05,350);}break;
    }
    case 'PRECISION':
    ...
    case 'INFILTRATE':
      if(atPhase(0))approach(.9,target,{reach:42,returnAt:1.03,returnDuration:.22});hits.forEach((e,i)=>impact(e.targets[0],e.at,{size:phase(e,i)?330:275,lead:phase(e,i)?.12:.3,life:phase(e,i)?.9:.6,angle:phase(e,i)?.25:0,particles:13}));break;
    case 'VEIL':
    ...
    case 'STITCH':
      if(atPhase(0))material(point(target),time-.8,{size:230,lead:.3,life:.65});if(atPhase(1)){material(point(target),time-1.65,{size:plan.scenario==='counter'?210:310,lead:.35,life:1.1});light(point(target,true),time-1.65,300,.75);debris(point(target),time-1.65,{count:16,strength:.4,soft:true});}break;
    case 'RIPOSTE':
      if(plan.scenario==='counter'){flash(point('M'),time-1,65);debris(point('M'),time-1,{count:9,strength:.3});}
      else{if(atPhase(0))material(point('M'),time-1,{size:225,lead:.4,life:.48,angle:-.55});if(atPhase(1)){approach(1.4,target,{returnAt:1.6});impact(target,1.4,{size:395,lead:.22,life:1.03,particles:22,grounded:true});}}break;
    case 'STILLNESS':
    ...
    case 'RESTRAIN': {
      const p=point(target);if(atPhase(0)){aim(p,.93,28);trace(muzzle(),p,.95,{travel:.13,width:2.5});flash(p,time-.95,95);debris(p,time-.95,{count:11,strength:.55});}
      if(plan.scenario!=='counter'&&atPhase(1)){material(p,time-1.45,{size:310,lead:.5,life:1.1});light(point(target,true),time-1.45,300,.5);}break;
    }
    case 'DOUBLE_BEAT':
      hits.forEach((e,i)=>{const n=phase(e,i);trace(muzzle(),point(e.targets[0]),e.at,{travel:n?.14:.1,width:n?4:2.3,smoke:n===1});impact(e.targets[0],e.at,{size:n?340:225,lead:.07,life:n?1.07:.62,angle:n?.2:0,particles:n?21:11});});break;
  }
  let shake=0;
