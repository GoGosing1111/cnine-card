...
2026-09-08 · 제작 연출 독립 검수본 → **2026-09-09 사용자 라이브 연결 승인 반영 (v2073)**

## v2073 라이브 연결

- 사용자 지시: `제작UI 연출 개편한거 라이브 적용해`. 제작소의 차량 제작·배틀슈트 제작 결과에 승인 연출을 연결했다. 제작법 공개 상태·확률·비용·아이템 지급·전투 V3 외형은 변경하지 않았다.
- 실제 진입: `js/workshop-v1881.js` → `js/workshop-assembly-live-v2073.src.js` → 필요할 때만 `workshopAssemblyFx` 기능 로드 → `js/workshop-assembly-fx-v2073.src.js` → 기존 `source/AssemblyFilm.js`. PixiJS **8.20.0** / GSAP **3.13.0**와 공유 vendor를 재사용한다.
- 서버 응답·영수증 저장 및 잔액 동기화 후에만 재생한다. 요청 번호·분류·제작법·출력 아이템 ID를 대조한다. 실패 응답에는 output이 없으므로 해당 서버 제작법의 아이템으로 외형을 선택한다. 보상 재추첨·추가 API 요청은 없다.
- 연결 모델: E/F/G/H-BODY(장비 ID 36/37/38/42), 이그니스 X(차량 ID 27), 베네노(승인 CMS 원본 이미지 경로 일치). 미준비 모델은 다른 차량/슈트로 대체하지 않고 기존의 실제 아이템 결과창을 유지한다.
- 연출·자산은 해당 모델만 로드한다. 공유 자산 URL은 현재 페이지가 아닌 `/preview/workshop-assembly-v1/` 기준이다. 승인 자산의 픽셀·분할·접점·13.4초 타임라인은 그대로다. 자산 해시는 `asset-manifest.json`, 번들 해시는 `live-build-report.json`에 기록한다.
- 건너뛰기, 확인/닫기, 일시정지/계속 재생, 클릭으로만 활성화되는 녹음 효과음 제공. 모션 줄이기는 GPU 자산을 내려받지 않고 저장 결과를 즉시 표시한다. 로딩 12초 초과·모듈/이미지 로드 실패·WebGL 컨텍스트 소실은 저장된 결과 화면으로 복구한다.
- 화면 이탈·닫기·세션 변경 시 GSAP 타임라인/오디오/Pixi Application/감시자/키보드 리스너를 정리한다. 부품 공유 텍스처는 파괴하지 않는다. 중복 제작 잠금은 결과창을 닫을 때까지 유지되며, 연출 오류를 제작 재시도 오류로 처리하지 않는다.
- 코드 정식 재빌드: `npm run build:workshop-assembly` (원화·부품 자산 재생성 없음). 원화 분할을 재생성하는 기존 `build.mjs`도 이후 라이브/프리뷰 양쪽 번들을 다시 빌드한다.
- 라이브 클라이언트 검수: `/preview/workshop-assembly-v1/live.html`. `?item=g&result=failure&play=1` / `?item=f&loadFailure=1&play=1`. 실제 `workshop-v1881.js`를 메모리 전용 응답에 연결해 검증하며 운영 API·계정·재화에 접근하지 않는다. `qa-report.json`은 최초 독립 프리뷰 이력이고 최신 연결 검증은 `live-qa-report.json`을 따른다.
