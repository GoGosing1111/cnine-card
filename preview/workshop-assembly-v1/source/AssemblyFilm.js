...
export class AssemblyFilm {
  ...
  async init(){
    ...
    await Promise.all(Object.entries(resources).map(async([key,url])=>{this.buffers[key]=await Assets.load(url);}));
  ...
  prepare(mode,result){
    ...
    this.result=acceptResult(result);this.mode=mode;this.generation++;this.resetScene();
  ...
  drawMachines(t){
    ...
    if(this.mode==='suit'){
      ...
      g.roundRect(562,145,23,612,4).fill({color:0x111a1c,alpha:a});g.roundRect(906,145,23,612,4).fill({color:0x111a1c,alpha:a});
      for(const y of[238,445,706]){g.rect(557-this.state.gate*100,y,70,10).fill({color:0x8b7d61,alpha:a});g.rect(866+this.state.gate*100,y,70,10).fill({color:0x8b7d61,alpha:a});}
    }
  }
  drawEffects(t){
    const g=this.fx;g.clear();this.glow.clear();
    for(const p of this.impacts)this.spark(g,p.x,p.y,t-p.at,p.at);
    if(this.mode==='vehicle'&&t>=7.2&&t<8.8){
      const u=(t-7.2)/1.6,x=435+u*640,y=634-u*65;
      g.moveTo(435,634).lineTo(x,y).stroke({width:1,color:0xf8d69a,alpha:.6});
      this.spark(g,x,y,(t*7%1)*.35,3);this.glow.circle(x,y,21).fill({color:0xf7bb6e,alpha:.65});
    }
    if(t>=this.scanStart&&t<10.15){
      const u=this.state.scan,sy=140+u*625,left=this.mode==='suit'?575:290,right=this.mode==='suit'?907:1200;
    ...
    if(t>=10.3){
      const q=clamp((t-10.3)/.65),success=this.result.success,c=success?(this.mode==='suit'?0xffa82e:0xcaeced):0xda603f;
      const pulse=this.reducedMotion?0:Math.sin((t-10.3)*1.3)*.035;
      if(this.mode==='suit'){
        this.glow.circle(758,287,25+q*20).fill({color:c,alpha:(.18+q*.24+pulse)});
        g.circle(758,287,6).fill({color:success?0xffe8ad:c,alpha:q*.9});
        if(success){g.moveTo(716,196).lineTo(733,204).moveTo(778,200).lineTo(790,190).stroke({width:2,color:0xffc17a,alpha:q});}
      }else{
        this.glow.ellipse(365,580,50,13).fill({color:c,alpha:q*.3});this.glow.ellipse(584,594,46,12).fill({color:c,alpha:q*.5});
        if(success)g.poly([347,565,70,650,80,718,355,593]).fill({color:0xc5e6df,alpha:q*.065});
      }
    }
