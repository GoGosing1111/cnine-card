...
function render(d){
  ...
  if(d.complete!==lastComplete||d.complete){
    ...
    $('completion-item').textContent=d.success?config.item:'최종 품질 검사에서 제작이 중단되었습니다.';
    $('completion-note').textContent='검수용 연출입니다. 실제 재화는 사용되지 않습니다.';lastComplete=d.complete;
  }
  const i=Math.max(0,config.phases.findLastIndex(p=>p[0]<=d.time));$('part-index').textContent=`${String(i+1).padStart(2,'0')} / ${String(config.phases.length).padStart(2,'0')}`;$('part-label').textContent=config.phases[i][2];
...
function select(next){
  mode=next;lastPhase='';const c=MODES[mode];
  document.querySelectorAll('[data-mode]').forEach(el=>el.setAttribute('aria-pressed',el.dataset.mode===mode));
  $('line-code').textContent=mode==='suit'?'EXOSUIT / ASSEMBLY BAY 04':'AUTOMOTIVE / ASSEMBLY LINE 01';
  $('item-name').textContent=c.item;$('item-description').textContent=c.descriptor;
  $('phase-detail').textContent=mode==='suit'?'부품 결합 → 장갑 장착 → 코어 점화':'프레임 → 엔진 · 휠 → 차체 결합 → 시동';
  $('phase-rail').replaceChildren(...c.phases.map((p,i)=>{const el=document.createElement('div');el.className='rail-step';const num=document.createElement('b');num.textContent=String(i+1).padStart(2,'0');el.append(num,document.createTextNode(p[1]));return el;}));
  film.prepare(mode,previewResult($('outcome').value==='success'));
...
async function boot(){
  ...
  globalThis.WorkshopAssemblyPreview=Object.freeze({diagnostics:()=>film.diagnostics(),seek:t=>film.seek(t),selectMode:select,play:()=>film.play(),pause:()=>film.pause(),skip:()=>film.skip(),destroy:()=>film.destroy()});
