...
test('Hanbok Diim originals and runtime assets retain their declared dimensions and hashes',async()=>{
  ...
  for(const [name,expected] of Object.entries(manifest.files)){
    ...
    if(name==='avatar-hanbok-diim-equipment-v1-640.webp')assert.deepEqual([meta.width,meta.height],[640,1664]);
    else assert.equal(meta.width*3,meta.height*2,name);
  }
  assert.equal(manifest.files['avatar-hanbok-diim-lobby-source-art-v1.png'].sha256,'2f8ca42c38ea3865be2628e2455455eee5b0ea828ef62eaae7f534eec0569be0');
});

test('equipment PNG and WebP contain a complete isolated figure with transparent borders and opaque clothing',async()=>{
  for(const name of ['avatar-hanbok-diim-equipment-source-art-v1.png','avatar-hanbok-diim-equipment-v1-640.webp']){
    ...
    assert.ok(minY>=8&&maxY<info.height*.99,`${name}: head or feet clipped`);
    for(const [x,y] of (name.endsWith('.png')?[[.48,.1],[.49,.3],[.5,.68]]:[[.36,.1],[.36,.3],[.37,.64]]))assert.ok(data[(Math.floor(y*info.height)*info.width+Math.floor(x*info.width))*4+3]>=240,`${name}: translucent face, hanbok or leg`);
  }
});

test('background extraction leaves opaque character RGB identical to the generated art',async()=>{
  const original=await sharp(await readFile(new URL('./assets/avatar-hanbok-diim-equipment-draft-v1.png',import.meta.url))).ensureAlpha().raw().toBuffer();
  const final=await sharp(await readFile(new URL('./assets/avatar-hanbok-diim-equipment-source-art-v1.png',import.meta.url))).ensureAlpha().raw().toBuffer();
