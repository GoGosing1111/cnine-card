...
export function playBerkanSkill(engine,event){
 ...
 return playback(engine,actor,rows.map(r=>r.target),{dodge:rows.every(r=>r.impact.dodge),targetDodges:Object.fromEntries(rows.map(r=>[r.target.id,!!r.impact.dodge])),apply(){
  for(const {impact,target}of rows){
   ...
   if(Number.isFinite(impact.targetShieldAfter))engine.syncTargetShield(target,impact.targetShieldAfter,impact.targetMaxShield);
   if(impact.dodge)engine.queueBanner('빗나감',0xedc878,target.name);else engine.showAccountBattleUnitDamage(target,{damage:impact.damage});
  }
 }});
}
