...
const DAMAGE_STYLE=Object.freeze({
  ATTACK:{fill:0xffe0a3,stroke:0x250207,tag:'ARMOR BREAK',tagColor:0xff5a64},
  DEFENSE:{fill:0xeaf9ff,stroke:0x00182d,tag:'GUARD CRUSH',tagColor:0x42c7ff},
  SPEED:{fill:0xf5eaff,stroke:0x130028,tag:'MULTI STRIKE',tagColor:0xb778ff},
  HP:{fill:0xeaffd8,stroke:0x00251a,tag:'VITAL DRAIN',tagColor:0x4ce89d}
});
...
export function configureDamageText(view,{kind='ATTACK',damage=0,critical=false,healing=0,hitCount=1}={}){
  ...
  view.numberLabel.style.stroke={color:profile.stroke,width:critical?11:9,join:'round'};
  view.numberLabel.style.fontSize=critical?68:56;
  view.roleTag.text=profile.tag;
  view.roleTag.style.fill=profile.tagColor;
  ...
  view.hitLabel.text=Number(hitCount)>1?`${Math.max(2,Math.floor(Number(hitCount)))} HIT`:'';
...
export function createDamageTextPool(size=24){
  return new ObjectPool({
    ...
    factory:()=>{
      ...
      root.addChild(underline,numberLabel,roleTag,criticalLabel,healLabel,hitLabel);
      root.numberLabel=numberLabel;root.roleTag=roleTag;root.criticalLabel=criticalLabel;root.healLabel=healLabel;root.hitLabel=hitLabel;root.underline=underline;
