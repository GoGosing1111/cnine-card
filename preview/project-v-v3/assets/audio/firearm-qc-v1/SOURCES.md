...
These three immutable CC0 assets serve four preview weapon profiles used only by the independent PROJECT V V3 Battle Suit QC preview. They are not wired to live automatic combat.
...
The SKS sprite remains the exact database weapon asset. Its preview audio explicitly reuses the existing real AK-47 single-shot recording as a 7.62×39mm proxy because no compliant SKS recording exists in the repository. It is not represented as an exact SKS receiver recording. The SKS profile uses a lower `0.46` master gain and reduced impact/tail gains so the shared recording remains a conservative DMR preview mix; the source bytes are not copied, regenerated, or altered.
