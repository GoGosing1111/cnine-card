...
function pace(){const workload=Object.fromEntries([...document.querySelectorAll('[data-xp-source]')].map(input=>[input.dataset.xpSource,input.value]));const {dailyXp,days}=estimateGrowth(workload);$('daily-xp').textContent=`${fmt(dailyXp)} XP`;$('target-days').textContent=days===null?'—':fmt(days);$('pace-comparison').textContent=days===null?'플레이 횟수를 입력해 주세요.':`입력한 플레이 기준 약 ${fmt(days)}일`;$('milestones').innerHTML=[50,125,250].map(l=>`<div><small>Lv.${l} · ${rankForLevel(l).name}</small><strong>${dailyXp>0?fmt(Math.ceil(totalXpForLevel(l)/dailyXp))+'일':'—'}</strong></div>`).join('');}
