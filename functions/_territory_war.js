...
function truceState(round){const endsAt=round?.truce_ends_at||null,active=Boolean(round?.status==='ACTIVE'&&sqlMs(endsAt)>Date.now());return{active,endsAt:active?endsAt:null,minutes:15,canRefresh:active}}
...
async function simulateTerritoryBattle(env,deps,attackerUser,mine,opponent,requestId,seedOverride=null){
  ...
  const battleV2=deps.createPvpBattleV2({attackerCards:attackerEngineCards,defenderCards:defenderEngineCards,attackerEquipmentBonus:Number(aBonus?.pvp||0),defenderEquipmentBonus:Number(bBonus?.pvp||0),seed:battleSeed,singleHealerBonus:battle?.engine?.singleHealerBonus});
...
async function activateOperation(env,round,mine,operation,cfg){
  ...
  statements.push(env.DB.prepare('DELETE FROM territory_war_v3_operation_votes WHERE round_id=? AND side=?').bind(round.id,side));await env.DB.batch(statements);await addNotice(env,round.id,'COUNTER_OPERATION',side,`${OPERATIONS[operation].name} 발동`,`${configuredTeamLabel(cfg,side)}이 역습 작전 ‘${OPERATIONS[operation].name}’을 개시했습니다.`,{operation,endsAt});
}
async function voteOperation(env,deps,user,cfg,body){
  const operation=String(body.operation||'').toUpperCase();if(!OPERATIONS[operation])return deps.json({error:'선택할 수 없는 역습 작전입니다.'},400);const round=await lifecycle(env,cfg);if(!round||round.status!=='ACTIVE')return deps.json({error:'현재 진행 중인 영토전이 없습니다.'},409);const mine=await env.DB.prepare('SELECT * FROM territory_war_v3_users WHERE round_id=? AND user_id=?').bind(round.id,user.id).first();if(!mine?.side||mine.status!=='ACTIVE')return deps.json({error:'활성 영토전 참가자만 투표할 수 있습니다.'},403);const max=Number(cfg.counterGaugeMax||1000),gauge=Number(round[sideField(mine.side,'counter_gauge')]||0);if(gauge<max)return deps.json({error:'역습 게이지가 아직 가득 차지 않았습니다.'},409);
  const lock=await acquireLock(env,`counter_vote_${round.id}_${mine.side}`,30000);if(!lock.ok)return deps.json({error:'작전 투표를 집계 중입니다.'},409);try{const used=await env.DB.prepare('SELECT 1 FROM territory_war_v3_operation_uses WHERE round_id=? AND side=? AND operation=?').bind(round.id,mine.side,operation).first();if(used)return deps.json({error:'이번 영토전에서 이미 발동한 작전입니다.'},409);await env.DB.prepare(`INSERT INTO territory_war_v3_operation_votes(round_id,user_id,side,operation) VALUES(?,?,?,?) ON CONFLICT(round_id,user_id) DO UPDATE SET operation=excluded.operation,side=excluded.side,updated_at=CURRENT_TIMESTAMP`).bind(round.id,user.id,mine.side,operation).run();const counts=await env.DB.prepare(`SELECT SUM(CASE WHEN operation=? THEN 1 ELSE 0 END) votes,(SELECT COUNT(*) FROM territory_war_v3_users WHERE round_id=? AND side=? AND status='ACTIVE') participants FROM territory_war_v3_operation_votes WHERE round_id=? AND side=?`).bind(operation,round.id,mine.side,round.id,mine.side).first(),required=operationRequiredVotes(counts?.participants);if(Number(counts?.votes||0)>=required)await activateOperation(env,await roundById(env,round.id),mine,operation,cfg);return deps.json({ok:true,activated:Number(counts?.votes||0)>=required,state:await publicState(env,user.id)})}finally{await releaseLock(env,lock)}
}
...
export async function handleTerritoryWar({path,request,env,deps}){
  ...
  if(path==='territory-war/refresh-loadout'&&request.method==='POST'){
    ...
    const deck=await deps.pvpDeckSnapshot(env,user.id);if(deck.length!==5)return deps.json({error:'PVP 덱 5장을 먼저 편성하세요.'},400);const bs=await deps.battleSettings(env),power=deck.reduce((sum,card)=>sum+deps.cardBattlePower(card,card.breakthrough_level,bs),0),loadoutBonus=typeof deps.userEquipmentBonuses==='function'?await deps.userEquipmentBonuses(env,user.id):{pvp:0};await env.DB.prepare('UPDATE territory_war_v3_users SET deck_snapshot=?,deck_power=?,loadout_bonus_json=?,loadout_refreshed_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE round_id=? AND user_id=?').bind(JSON.stringify(deck.map(card=>String(card.id))),power,JSON.stringify(loadoutBonus||{pvp:0}),round.id,user.id).run();for(const key of participantDeckCache.keys())if(key.startsWith(`${round.id}:${user.id}:`))participantDeckCache.delete(key);publicStateSharedCache=null;return deps.json({ok:true,deckPower:power,state:await publicState(env,user.id)});
  }
  ...
  if(path==='admin/territory-war/truce'&&request.method==='POST'){
    ...
    try{const endsAt=iso(Date.now()+15*60000);await env.DB.prepare('UPDATE territory_war_v3_rounds SET truce_ends_at=?,truce_started_by=?,version=version+1,updated_at=CURRENT_TIMESTAMP WHERE id=? AND status=\'ACTIVE\'').bind(endsAt,user.id,round.id).run();await addNotice(env,round.id,'TRUCE',null,'15분 임시 휴전','관리자 명령으로 15분간 공격이 중단됩니다. 휴전 중 PVP 덱과 장비·칭호를 최신화할 수 있습니다.',{endsAt,durationMinutes:15});publicStateSharedCache=null;const response={ok:true,endsAt,state:await publicState(env,user.id,true)};await completeAdmin(env,key,response);if(deps.writeAdminLog)await deps.writeAdminLog(env,user,'TERRITORY_TRUCE','TERRITORY_WAR_ROUND',String(round.id),null,{endsAt,durationMinutes:15});return deps.json(response)}catch(error){await failAdmin(env,key,error);return deps.json({error:error.message||'임시 휴전 발동에 실패했습니다.'},409)}
  }
