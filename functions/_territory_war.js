...
export async function handleTerritoryWar({path,request,env,deps}){
  ...
  if(path==='admin/territory-war/truce'&&request.method==='POST'){
    ...
    try{const durationMinutes=clampInt(body.durationMinutes,1,360,15),endsAt=iso(Date.now()+durationMinutes*60000);await env.DB.prepare('UPDATE territory_war_v3_rounds SET truce_ends_at=?,truce_duration_minutes=?,truce_started_by=?,version=version+1,updated_at=CURRENT_TIMESTAMP WHERE id=? AND status=\'ACTIVE\'').bind(endsAt,durationMinutes,user.id,round.id).run();await addNotice(env,round.id,'TRUCE',null,`${durationMinutes}분 임시 휴전`,`관리자 명령으로 ${durationMinutes}분간 공격이 중단됩니다. 휴전 중 PVP 덱과 장비·칭호를 최신화할 수 있습니다.`,{endsAt,durationMinutes,image:'assets/ui/territory-war/truce-v1.png'});publicStateSharedCache=null;const response={ok:true,endsAt,durationMinutes,state:await publicState(env,user.id,true)};await completeAdmin(env,key,response);if(deps.writeAdminLog)await deps.writeAdminLog(env,user,'TERRITORY_TRUCE','TERRITORY_WAR_ROUND',String(round.id),null,{endsAt,durationMinutes});return deps.json(response)}catch(error){await failAdmin(env,key,error);return deps.json({error:error.message||'임시 휴전 발동에 실패했습니다.'},409)}
  }
