...
async function ensureFoundation(env){
  ...
  const commandWarfareMarker=await env.DB.prepare("SELECT value FROM app_meta WHERE key='safe_runtime_upgrade_v1811_territory_command_warfare'").first();
  if(!commandWarfareMarker){
    await env.DB.batch([
      ...
      env.DB.prepare("INSERT INTO app_meta(key,value,updated_at) VALUES('safe_runtime_upgrade_v1811_territory_command_warfare','1',CURRENT_TIMESTAMP) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=CURRENT_TIMESTAMP")
    ]);
  }
...
async function sharedPublicState(env,round,cfg){
  ...
  ]).then(([front,counts,ranking,recentResults,recentActions,notice,votes,participants,operationUses,commanders,commandMessages])=>({front,counts:counts||{},ranking:ranking.results||[],recentResults:recentResults.results||[],recentActions:recentActions.results||[],notice,votes:votes.results||[],participants:participants.results||[],operationUses:operationUses.results||[],commanders,commandMessages}));
