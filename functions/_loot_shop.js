import {LOOT_SHOP_DEFAULTS,validateLootShopPolicy,upgradeLootShopPolicy,PIG_COIN_IMAGE,pigCoinRewardWeek,LOOT_MYSTIC_RARITY,LOOT_MYSTIC_NAME_PREFIX,LOOT_MYSTIC_NAMES,LOOT_MYSTIC_CHANCE_PERCENT,lootMysticPoolComplete,lootEquipmentMatchesProduct,lootMercenaryRanks} from '../shared/loot-shop-policy-v1.mjs';
...
const legacyMysticEquipmentSql=`(rarity='${LOOT_MYSTIC_RARITY}' AND name LIKE '${LOOT_MYSTIC_NAME_PREFIX}%')`;
const mysticEquipmentSql=`(rarity='${LOOT_MYSTIC_RARITY}' AND name IN (${LOOT_MYSTIC_NAMES.map(name=>`'${name}'`).join(',')}))`;
async function mysticEquipmentPool(env){
 const rows=(await env.DB.prepare(`SELECT id,code,name,rarity,image_url AS image FROM character_equipment_items WHERE is_active=1 AND is_public=1 AND ${mysticEquipmentSql}`).all()).results;
 if(!lootMysticPoolComplete(rows))throw fail('미스틱 4종 구성을 확인 중입니다. 코인은 차감되지 않습니다.');
 return LOOT_MYSTIC_NAMES.map(name=>({...rows.find(row=>row.name===name),chancePercent:LOOT_MYSTIC_CHANCE_PERCENT}));
}
...
async function mercenaryPool(env,product){const {document,revision}=await readMercenaryDocument(env);const ranks=lootMercenaryRanks(product.type),pool=document.mercenaries.filter(c=>product.mercenaryCodes.includes(c.code)&&ranks.includes(c.rank)).map(c=>({code:c.code,name:c.name,rank:c.rank,image:MERCENARY_CMS_SEED.catalog.cards.find(a=>a.code===c.code)?.sourceArt}));if(product.type==='MERCENARY_SS_PACK'&&(!pool.length||pool.length!==product.mercenaryCodes.length))throw fail('등록된 후보가 모두 SS등급 용병인지 확인하세요.');for(const rank of ranks)if((rank==='SS'||product.mercenaryWeights[rank]>0)&&!pool.some(c=>c.rank===rank))throw fail(`${rank}등급 용병 후보가 없습니다.`);return {pool,revision};}
async function validateProductCatalog(env,product){if(product.type.endsWith('_CHOICE')){if(!(await cardPool(env,product)).length)throw fail('획득 가능한 선택 카드가 없습니다.');}else if(lootMercenaryRanks(product.type).length)await mercenaryPool(env,product);else if(product.type==='MYSTIC_EQUIPMENT')return mysticEquipmentPool(env);else await equipment(env,product);}
export async function lootShopCatalog(env){
 ...
 const {document}=await readMercenaryDocument(env);return {cards:cards.results,equipment:gear.results,mercenaries:document.mercenaries.filter(c=>['A','S','SS'].includes(c.rank)).map(c=>({code:c.code,name:c.name,rank:c.rank,image:MERCENARY_CMS_SEED.catalog.cards.find(a=>a.code===c.code)?.sourceArt}))};
}
...
export async function lootShopState(env,user){
 ...
 let mysticPool;
 for(const product of policy.products){const bought=counts.get(product.id)||0;let image='',available=false,options=[];try{if(product.type.endsWith('_CHOICE')){image=product.type==='SUPERSTAR_CHOICE'?'assets/ui/packs/superstar-card-pack-v1.png':'assets/ui/packs/limited-pack.png';options=await cardPool(env,product);available=options.length>0;}else if(lootMercenaryRanks(product.type).length){image='assets/ui/packs/hyper-pack-v2076.png';if(product.enabled){options=(await mercenaryPool(env,product)).pool;available=options.length>0;}}else if(product.type==='MYSTIC_EQUIPMENT'){mysticPool??=mysticEquipmentPool(env);options=await mysticPool;image='assets/ui/packs/prime-armory-equipment-box-v1.png';available=true;}else if(product.equipmentId){image=(await equipment(env,product)).image;available=true;}}catch{available=false;}
...
function gearGrant(DB,userId,product,requestId,reward){const p=pFor(DB),equipmentId=reward?.id??product.equipmentId,condition=product.type==='F_BODY'?"code='BATTLE_SUIT_02'":reward?mysticEquipmentSql:legacyMysticEquipmentSql;return guarded(DB,`EXISTS(SELECT 1 FROM character_equipment_items WHERE id=? AND is_active=1 AND is_public=1 AND ${condition})`,[equipmentId],[p("INSERT INTO user_equipment_instances(user_id,equipment_id,source_type,source_id,request_id) VALUES(?,?,'LOOT_SHOP',?,?)",userId,equipmentId,product.id,requestId)]);}
function mysticPoolGuard(DB,pool){
 const eligible=`is_active=1 AND is_public=1 AND ${mysticEquipmentSql}`;
 // Check the entire frozen pool in the payment transaction, not just the winner.
 // Missing/hidden/replaced candidates abort every write; never shrink or reroll.
 return [...(DB.dialect==='postgres'?[DB.prepare(`SELECT id FROM character_equipment_items WHERE id IN (${pool.map(()=>'?').join(',')}) ORDER BY id FOR SHARE`).bind(...pool.map(item=>item.id))]:[]),...guarded(DB,
  `(SELECT COUNT(*) FROM character_equipment_items WHERE ${eligible})=4 AND ${pool.map(()=>`EXISTS(SELECT 1 FROM character_equipment_items WHERE id=? AND code=? AND name=? AND ${eligible})`).join(' AND ')}`,
  pool.flatMap(item=>[item.id,item.code,item.name]),[])];
}
export async function purchaseLootProduct(env,user,body,{randomInt=mercenaryRandomInt}={}){
 const {productId,requestId}=body;
 const result=await runJointOperation(env,user,{requestId,kind:'LOOT_PURCHASE',input:{productId},prepare:async()=>{const {policy}=await readLootShopPolicy(env),product=policy.products.find(p=>p.id===productId);if(!policy.salesEnabled||!product?.enabled)throw fail('현재 판매 준비 중인 상품입니다.');const pool=await validateProductCatalog(env,product);if(await pigCoinBalance(env,user.id)<product.price)throw fail('피그 코인이 부족합니다.');if(product.type!=='MYSTIC_EQUIPMENT')return {product};const roll=randomInt(pool.length);if(!Number.isSafeInteger(roll)||roll<0||roll>=pool.length)throw fail('장비 추첨을 완료하지 못했습니다.');return {product,equipmentPool:pool.map(({id,code,name})=>({id,code,name})),equipmentReward:{...pool[roll],kind:'EQUIPMENT',quantity:1}};},statements:async plan=>{
  ...
  return [...policyGuard(DB,raw),...(plan.equipmentPool?mysticPoolGuard(DB,plan.equipmentPool):[]),...guarded(DB,'(SELECT COUNT(*) FROM loot_shop_purchases_v1 WHERE user_id=? AND product_id=?)<?',[user.id,productId,product.accountLimit],[]),...debitPigCoins(DB,user.id,product.price,requestId),p('INSERT INTO loot_shop_purchases_v1(request_id,user_id,product_id,price,product_json,created_at) VALUES(?,?,?,?,?,?)',requestId,user.id,productId,product.price,JSON.stringify(plan.equipmentReward?{...product,equipmentReward:plan.equipmentReward}:product),now),...(['F_BODY','MYSTIC_EQUIPMENT'].includes(product.type)?gearGrant(DB,user.id,product,requestId,plan.equipmentReward):[p('INSERT INTO loot_shop_packs_v1(id,user_id,product_json,created_at) VALUES(?,?,?,?)',requestId,user.id,JSON.stringify(product),now)])];
 }});return {ok:true,requestId:result.requestId,replayed:result.replayed,product:result.plan.product,...(result.plan.equipmentReward?{reward:result.plan.equipmentReward}:{}),state:await lootShopState(env,user)};
}
...
export async function lootPackOptions(env,user,packId){const pack=await ownedPack(env,user,packId);if(pack.opened_at)throw fail('이미 개봉한 팩입니다.');return {packId,product:pack.product,cards:pack.product.type.endsWith('_CHOICE')?await cardPool(env,pack.product):[],mercenaries:lootMercenaryRanks(pack.product.type).length?(await mercenaryPool(env,pack.product)).pool:[]};}
...
export async function openLootPack(env,user,body,{randomInt=mercenaryRandomInt}={}){
 ...
 const r=await runJointOperation(env,user,{requestId,kind:'LOOT_PACK_OPEN',input:{packId,cardId},prepare:async()=>{
  ...
  if(cardId!==null)throw fail('용병 팩에는 선택 카드 ID를 지정할 수 없습니다.');const {pool,revision}=await mercenaryPool(env,product),weights=product.mercenaryWeights,total=weights.A+weights.S,rank=product.type==='MERCENARY_SS_PACK'?'SS':randomInt(total)<weights.A?'A':'S',rankPool=pool.filter(c=>c.rank===rank),mercenary=rankPool[randomInt(rankPool.length)];return {packId,product,mercenary,cmsRevision:revision};
