...
async function equipment(env,product){const row=await env.DB.prepare('SELECT id,code,name,rarity,image_url AS image FROM character_equipment_items WHERE id=? AND is_active=1 AND is_public=1').bind(product.equipmentId).first();if(!row||(product.type==='F_BODY'?row.code!=='BATTLE_SUIT_02':row.rarity!=='MYSTIC'))throw fail('선택한 장비가 공개 상태 또는 상품 종류와 맞지 않습니다.');return row;}
