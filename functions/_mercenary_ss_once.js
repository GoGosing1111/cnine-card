...
export function mercenarySsOnceState({userId,operationId,actorId,reason,mercenaryCode,slotIndex,now=new Date().toISOString()}){
  ...
  return {version:2104,operationId,userId,actorId,reason,createdAt:now,status:'ARMED',rank:'SS',batchCount:10,...(targeted?{mercenaryCode,slotIndex}:{})};
}
...
export async function prepareMercenarySsOnce(env,user,count){
  ...
  return {key,before:row.value,operationId:state.operationId,index:state.slotIndex??9,...(state.mercenaryCode?{mercenaryCode:state.mercenaryCode}:{})};
}
...
export async function consumeMercenarySsOnce(env,user,requestId,plan,draw){
  ...
  if(plan.key!==mercenarySsOnceKey(user.id)||plan.operationId!==state.operationId||plan.index!==(state.slotIndex??9)||plan.mercenaryCode!==state.mercenaryCode||
     state.mercenaryCode&&draw?.mercenaryCode!==state.mercenaryCode||draw?.rank!=='SS'||draw?.grantKind!=='ONE_TIME_SS_GUARANTEE')throw jointError('MERCENARY_SS_ONCE_CONFIG','1회 보장 결과를 확인하세요.',409);
