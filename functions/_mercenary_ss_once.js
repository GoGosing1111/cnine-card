...
// RAGNIEL-0921: `rank` 는 SS(기본) 또는 SSS. 기존 저장 상태(rank:'SS')는 그대로 유효하다.
export const MERCENARY_ONCE_RANKS=Object.freeze(['SS','SSS']);
export function mercenarySsOnceState({userId,operationId,actorId,reason,mercenaryCode,slotIndex,batchesRemaining,rank='SS',now=new Date().toISOString()}){
  ...
  if(!MERCENARY_ONCE_RANKS.includes(rank))throw Error('Invalid one-time grant rank');
  return {version:2104,operationId,userId,actorId,reason,createdAt:now,status:'ARMED',rank,batchCount:10,...(targeted?{mercenaryCode,slotIndex}:{}),...(batchesRemaining!==undefined?{batchesRemaining}:{})};
}
function validateArmed(value,userId){
  const state=JSON.parse(value);
  const expected=mercenarySsOnceState({...state,now:state.createdAt});
  if(state.userId!==Number(userId)||state.version!==2104||state.status!=='ARMED'||!MERCENARY_ONCE_RANKS.includes(state.rank)||state.batchCount!==10||
...
export function pickMercenarySsOnce({policy,mercenaries,randomInt,mercenaryCode,rank='SS'}){
  ...
  const result=pickMercenaryDraw({policy:{...policy,outcomes:policy.outcomes.map(row=>({...row,chancePpm:row.id===`CARD_${rank}`?DRAW_TOTAL:0}))},mercenaries,randomInt});
...
export async function consumeMercenarySsOnce(env,user,requestId,plan,draw){
  ...
  if(plan.key!==mercenarySsOnceKey(user.id)||plan.operationId!==state.operationId||(plan.rank??'SS')!==state.rank||plan.index!==(deferred?-1:state.slotIndex??9)||plan.mercenaryCode!==state.mercenaryCode||
     (deferred?draw!==undefined:(state.mercenaryCode&&draw?.mercenaryCode!==state.mercenaryCode||draw?.rank!==state.rank||draw?.grantKind!=='ONE_TIME_SS_GUARANTEE')))throw jointError('MERCENARY_SS_ONCE_CONFIG','1회 보장 결과를 확인하세요.',409);
