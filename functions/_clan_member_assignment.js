...
const TRANSFER_CONFIRMATION = 'TRANSFER_EXPLICIT_CLAN_MEMBER';
const GIFT_CONFIRMATION = 'APPLY_CLAN_CHANGE_AND_COIN_MESSAGES';
const confirmationFor = target => target.clanGift ? GIFT_CONFIRMATION : target.fromClanId ? TRANSFER_CONFIRMATION
  : target.removeMembers?.length ? REPLACEMENT_CONFIRMATION : CONFIRMATION;
...
async function assignmentState(q, target) {
  ...
  return {target,seasonNo:Number(season.season_no),memberCount,maxMembers,deck:cards.map(String),draftPool:draftPool || null,removals,
    membership:membership||null,sourceClan,sourceIdentity};
}
...
export async function handleClanMemberAssignment({request,env,user,deps,now=Date.now()}) {
  ...
  try {
    ...
    if (body.action === 'preview') {
      ...
      if(body.fromClanId!==undefined||body.fromClanName!==undefined) {
        check(Number.isSafeInteger(body.fromClanId)&&body.fromClanId>0&&body.fromClanId!==target.clanId
          &&typeof body.fromClanName==='string'&&body.fromClanName.length>0&&body.fromClanName.length<=80,'정확한 이적 출발 클랜을 지정하세요.',400);
        target.fromClanId=body.fromClanId;target.fromClanName=body.fromClanName;
      }
      const gift=clanGiftSpec(body.clanGift);
      if(gift)target.clanGift=gift;
      return deps.json(await clanAdminTransaction(env.DB, async q => {
        ...
        const giftRecipients=gift?await clanGiftRecipients(q,target,{projectChange:true}):[];
        const moneyUpgrade=gift?await clanGiftMoneyStorage(q):[];
        const record = {status:'PREVIEW',ownerId:Number(user.id),createdAt:now,target,removalIdentities:state.removals.identities,
          sourceIdentity:state.sourceIdentity,giftRecipients};
        ...
        return {ok:true,previewId,...target,seasonNo:state.seasonNo,memberCount:state.memberCount,maxMembers:state.maxMembers,
          currentMembership:state.sourceIdentity,verified:true,rankedDeckReady:true,removals:state.removals.identities,
          afterCount:state.memberCount-state.removals.members.length+1,
          gift:gift?{...gift,recipients:giftRecipients,sent:giftRecipients.length,totalCoin:gift.coin*giftRecipients.length,moneyUpgrade}:null,
          confirmation:confirmationFor(target)};
      }));
    }
    check([CONFIRMATION,REPLACEMENT_CONFIRMATION,TRANSFER_CONFIRMATION,GIFT_CONFIRMATION].includes(body.confirmation) && /^[0-9a-f-]{36}$/.test(String(body.previewId || '')), '편입 확인값과 확인 ID가 필요합니다.',400);
    return deps.json(await clanAdminTransaction(env.DB, async q => {
      ...
      check(body.confirmation === confirmationFor(record.target), '확인한 인원 교체 요청과 확인값이 다릅니다.',400);
      ...
      const affectedIds = [...new Set([record.target.userId,...(record.target.removeMembers || []).map(m => m.userId),
        ...(record.giftRecipients||[]).map(m=>m.userId)])].sort((a,b) => a-b);
      ...
      check(pack(state.sourceIdentity)===pack(record.sourceIdentity||null),'이적 출발 소속이 확인 이후 변경되었습니다.');
      if(target.clanGift) {
        const freshRecipients=await clanGiftRecipients(q,target,{projectChange:true});
        check(pack(freshRecipients)===pack(record.giftRecipients),'확인한 클랜 보상 수신 명단이 변경되었습니다.');
        await clanGiftMoneyStorage(q,{upgrade:true});
      }
      ...
      const inserted = state.sourceClan
        ?await q(`UPDATE clan_members SET clan_id=$3,draft_pick_no=0,joined_at=$4,updated_at=$4
          WHERE season_id=$1 AND user_id=$2 AND clan_id=$5 AND member_role='MEMBER' RETURNING *`,[target.seasonId,target.userId,target.clanId,at,target.fromClanId])
        :await q(`INSERT INTO clan_members(season_id,clan_id,user_id,member_role,preferred_role,draft_pick_no,joined_at,updated_at)
      ...
      let sourceAfterCount=null;
      if(state.sourceClan) {
        const [sourceAfter]=await q('SELECT COUNT(*) n FROM clan_members WHERE season_id=$1 AND clan_id=$2',[target.seasonId,target.fromClanId]);
        sourceAfterCount=Number(sourceAfter.n);
        check(sourceAfterCount===Number(state.sourceClan.member_count)-1,'이적 출발 클랜 인원 검증에 실패했습니다.');
      }
      const giftResult=target.clanGift?await sendClanGift(q,target,body.previewId,record.giftRecipients):null;
      const result = {ok:true,previewId:body.previewId,...target,memberRole:'MEMBER',beforeCount:state.memberCount,
        memberCount:Number(after.n),maxMembers:state.maxMembers,sourceAfterCount,gift:giftResult,removed:state.removals.identities,removedCount:removedIds.length,completedAt:at,recoveryKey:key};
      const before = {membership:state.membership,draftPool:state.draftPool,removedMemberships:state.removals.members,removedDraftPool:state.removals.draftPool};
      ...
      await q(`INSERT INTO admin_logs(admin_id,action_type,target_type,target_id,before_data,after_data)
        VALUES($1,$2,'USER',$3,$4,$5)`,[user.id,target.clanGift?'CLAN_MEMBER_CHANGE_AND_GIFT':state.sourceClan?'CLAN_MEMBER_TRANSFER':removedIds.length ? 'CLAN_MEMBER_REPLACEMENT' : 'CLAN_MEMBER_ASSIGNMENT',String(target.userId),pack(before),pack({...afterState,result})]);
      return result;
    }));
  } catch (error) {
