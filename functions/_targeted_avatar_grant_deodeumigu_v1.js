...
export const TARGETED_AVATAR_GRANT_MARKER_KEY='targeted_avatar_grant_deodeumigu_terran_empress_joeun_20260909_v1';
export const TARGETED_AVATAR_GRANT_NICKNAME='더듬이구';
export const TARGETED_AVATAR_GRANT_CODE='TERRAN_EMPRESS_JOEUN';

const TARGETED_AVATAR_GRANT_NAME='테란여제 조은';
const TARGETED_AVATAR_GRANT_ACTION='SYSTEM_AVATAR_GRANT_DEODEUMIGU_V1';
const TARGETED_AVATAR_GRANT_VERIFICATION_TABLE='targeted_avatar_grant_deodeumigu_v1_verifications';
...
export async function ensureTargetedAvatarGrantDeodeumiguV1(env){
  ...
  if(existing){
    const current=await env.DB.prepare(`SELECT COUNT(*) owned FROM avatar_user_ownership_v1 o JOIN users u ON u.id=o.user_id
      WHERE u.nickname=? AND UPPER(u.status)='ACTIVE' AND o.avatar_code=? AND o.expires_at IS NULL`).bind(TARGETED_AVATAR_GRANT_NICKNAME,TARGETED_AVATAR_GRANT_CODE).first();
    return{...existing,ownershipVerified:integer(current?.owned)===1};
  }
  ...
  const summary={
    ...
    avatarCode:TARGETED_AVATAR_GRANT_CODE,permanent:true,ownershipVerified:true,alreadyOwned:Boolean(ownership&&ownership.expires_at==null)
  };
  ...
  console.log('TARGETED_AVATAR_GRANT_DEODEUMIGU_V1',JSON.stringify({status:stored.status,version:stored.version,avatarCode:stored.avatarCode,permanent:stored.permanent,replayed:stored.replayed}));
  return stored;
}
