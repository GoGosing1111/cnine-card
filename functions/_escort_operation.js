...
export async function handleEscortOperation({path,request,env,deps}){
  ...
  if(path==='escort/fight'&&request.method==='POST'){
    ...
    try{
      ...
      const escortUniqueById=new Map((unique?.cards||[]).map(card=>[String(card.id),card]));
      const cards=baseDeck.map(card=>{const uniqueCard=escortUniqueById.get(String(card.id));return {...card,uniqueAbility:uniqueCard?.uniqueAbility||card.uniqueAbility||null,uniqueAdvancement:uniqueCard?.uniqueAdvancement||null,power:Math.max(1,Math.round(Number(card.power||1)*(1+attackRoleBoost+tacticBoost))),startingHpPercent:clamp(cardHp[String(card.id)]??100,0,100)}});
      ...
      const response={ok:true,run:publicRun(fresh,cfg),sectorSummary,battleV2,monster,characterBonus:equipment,objective:{id:'ESCORT_OBJECTIVE',name:'장갑 수송차',image:'/assets/ui/escort/escort-armored-carrier-v1.webp?v=1830',hp:Number(run.vehicle_hp),hpAfter:vehicleHp,maxHp:run.vehicle_max_hp,targetPriority:'ABSOLUTE'}};
