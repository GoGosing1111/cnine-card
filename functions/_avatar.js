...
function cleanEffectValue(type,value){const n=Math.floor(Number(value)||0),max=type==='COIN_GAIN_PERCENT'?20:type==='BATTLE_POWER_PERCENT'?100:type==='RAID_EXTRA_ENTRY'?20:1;return Math.max(type==='SCRAPYARD_FREE_ENTRY'?1:0,Math.min(max,n))}
...
function publicAvatar(row){
  ...
  return{
    ...
    effect:{type:String(row.effect_type||''),value:Number(row.effect_value||0)},owned:Boolean(Number(row.owned||0)),equipped:Boolean(Number(row.equipped||0)),
...
async function catalogForUser(env,user){
  ...
  return rows(result).map(publicAvatar);
}
...
async function equipAvatar(env,user,body){
  ...
  await env.DB.prepare(`INSERT INTO avatar_user_loadout_v1(user_id,avatar_code,updated_at) VALUES(?,?,CURRENT_TIMESTAMP)
    ON CONFLICT(user_id) DO UPDATE SET avatar_code=excluded.avatar_code,updated_at=CURRENT_TIMESTAMP`).bind(user.id,code).run();
  return{data:{ok:true,equippedAvatarCode:code}};
}

export async function equippedAvatarEffect(env,userId){
  await ensureAvatarFoundation(env);
  const row=await env.DB.prepare(`SELECT a.code,a.effect_type,a.effect_value,a.lobby_image,a.lobby_mobile_image,a.equipment_image
    FROM avatar_user_loadout_v1 l JOIN avatar_user_ownership_v1 o ON o.user_id=l.user_id AND o.avatar_code=l.avatar_code
    JOIN avatar_catalog_v1 a ON a.code=l.avatar_code WHERE l.user_id=? AND a.is_active=1 AND a.is_public=1`).bind(userId).first();
  return row?{code:String(row.code),type:String(row.effect_type),value:Number(row.effect_value||0),lobbyImage:String(row.lobby_image||''),lobbyMobileImage:String(row.lobby_mobile_image||''),equipmentImage:String(row.equipment_image||'')}:null;
}
...
async function adminPayload(env){
  const [settings,result]=await Promise.all([avatarSettings(env,{fresh:true}),env.DB.prepare('SELECT * FROM avatar_catalog_v1 ORDER BY sort_order,code').all()]);
  return{settings,avatars:rows(result).map(publicAvatar),modes:MODES,acquisitionTypes:ACQUISITION_TYPES,effectTypes:EFFECT_TYPES};
}
...
async function saveAdminAvatar(env,admin,body,writeAdminLog){
  const code=cleanText(body.code,80).toUpperCase(),before=await env.DB.prepare('SELECT * FROM avatar_catalog_v1 WHERE code=?').bind(code).first();if(!before)return{error:'아바타 데이터를 찾을 수 없습니다.',status:404};
  ...
  const acquisitionType=cleanAcquisition(body.acquisitionType),coinPrice=cleanPrice(body.coinPrice),sourceLabel=cleanText(body.sourceLabel,80),sourceDetail=cleanText(body.sourceDetail,500),effectType=cleanEffect(body.effectType),effectValue=cleanEffectValue(effectType,body.effectValue),isActive=cleanBool(body.active,false),isPublic=cleanBool(body.public,false),saleEnabled=cleanBool(body.saleEnabled,false),sortOrder=Math.max(0,Math.min(9999,Math.floor(Number(body.sortOrder)||0)));
  if(!effectType)return{error:'아바타 효과 유형이 올바르지 않습니다.',status:400};
  ...
  const updated=await env.DB.prepare(`UPDATE avatar_catalog_v1 SET acquisition_type=?,coin_price=?,source_label=?,source_detail=?,effect_type=?,effect_value=?,is_active=?,is_public=?,sale_enabled=?,sort_order=?,version=version+1,updated_at=CURRENT_TIMESTAMP WHERE code=? AND version=?`).bind(acquisitionType,coinPrice,sourceLabel,sourceDetail,effectType,effectValue,isActive?1:0,isPublic?1:0,saleEnabled?1:0,sortOrder,code,expectedVersion).run();
  if(Number(updated?.meta?.changes||0)!==1)return{error:'아바타 설정이 동시에 변경되었습니다. 새로고침 후 다시 저장해 주세요.',code:'AVATAR_VERSION_CONFLICT',status:409};
  const after=await env.DB.prepare('SELECT * FROM avatar_catalog_v1 WHERE code=?').bind(code).first();await writeAdminLog(env,admin,'AVATAR_CATALOG_SAVE','AVATAR',code,publicAvatar(before),publicAvatar(after));
  return{data:{ok:true,avatar:publicAvatar(after)}};
}
...
export async function handleAvatar({path,request,env,deps}){
  ...
  if(path==='avatar/catalog'&&request.method==='GET'){
    ...
    return json({profile:{id:Number(user.id),nickname:String(user.nickname||''),role:String(user.role||'USER')},coin:Number(user.coin||0),access,avatars:await catalogForUser(env,user)});
  }
  ...
  if(path==='avatar/equip'&&request.method==='POST'){
    const result=await equipAvatar(env,user,await readBody(request));return result.data?json(result.data):json({error:result.error,code:result.code},result.status||400);
  }
