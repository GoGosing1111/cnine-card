import {readClanRedraft,applyClanRedraftQuotas,clanDraftCapacity,assertClanRedraftComplete} from './_clan_redraft.js';
...
const CLAN_CONCURRENT_RESERVATION_RUNTIME_VERSION='safe_runtime_upgrade_v1999_clan_concurrent_war_reservations_v1';
const CLAN_MAX_MEMBERS=22;
const CLAN_DRAFT_DURATION_MINUTES=60;
const CLAN_LATE_REGISTRATION_EXTENSION_MS=2*60*60*1000;
const CLAN_LATE_DRAFT_HOUR_KST=19;
const CLAN_WAR_OPEN_HOUR_KST=21;
...
function seoulDayTimestamp(nowMs,hour,minute=0){
  const local=new Date(Number(nowMs)+SEOUL_OFFSET_MS);
...
function randomMatchCandidates(candidates=[],selectionKey='CLAN_RANDOM_MATCH'){
  ...
  const selectedIndex=available.length?seedOf(selectionKey)%available.length:-1,selected=selectedIndex>=0?available[selectedIndex]:null,selectedId=Number(selected?.userId||0),fallbackOrder=selectedIndex>=0?[...available.slice(selectedIndex),...available.slice(0,selectedIndex)]:[],orderById=new Map(fallbackOrder.map((candidate,index)=>[Number(candidate.userId),index]));
  return prepared.map(candidate=>({...candidate,matchEligible:Boolean(selectedId)&&Number(candidate.userId)===selectedId,matchReason:Boolean(selectedId)&&Number(candidate.userId)===selectedId?'RANDOM_SELECTED':candidate.matchReason})).sort((a,b)=>{
    const aAvailable=a.available!==false,bAvailable=b.available!==false;if(aAvailable!==bAvailable)return aAvailable?-1:1;
    if(aAvailable&&bAvailable)return Number(orderById.get(Number(a.userId))??Number.MAX_SAFE_INTEGER)-Number(orderById.get(Number(b.userId))??Number.MAX_SAFE_INTEGER);
...
const FOUNDATION_SQL=Object.freeze([
  ...
  `CREATE TABLE IF NOT EXISTS clan_war_reservation_locks(
    war_id INTEGER NOT NULL,lock_role TEXT NOT NULL,user_id INTEGER NOT NULL,token TEXT NOT NULL,expires_at TEXT NOT NULL,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY(war_id,lock_role,user_id))`,
  ...
  `CREATE INDEX IF NOT EXISTS idx_clan_war_reservation_locks_expiry ON clan_war_reservation_locks(expires_at,war_id)`
]);
...
async function ensureClanCapacityRuntimeUpgrade(env){
  ...
  try{
    ...
    if(season){
      ...
      if(canExtend){
        const schedule=clanLateRegistrationSchedule(season),settings=await clanSettings(env),nextPick=phase==='DRAFT'?iso(Date.parse(schedule.registrationEndsAt)+Number(settings.draftPickSeconds||30)*1000):season.next_pick_deadline;
...
async function ensureClanLateDraftRuntimeUpgrade(env){
  if(lateDraftRuntimeReady)return;
  const existing=await env.DB.prepare('SELECT value,updated_at FROM app_meta WHERE key=?').bind(CLAN_LATE_DRAFT_RUNTIME_VERSION).first(),existingState=safeJson(existing?.value,{});
  if(existingState.status==='COMPLETED'){console.info('[CLAN_LATE_DRAFT_V1994_STATE]',JSON.stringify(existingState));lateDraftRuntimeReady=true;return}
  if(existing&&Date.now()-sqlMs(existing.updated_at)<60000)return;
  if(existing)await env.DB.prepare('DELETE FROM app_meta WHERE key=?').bind(CLAN_LATE_DRAFT_RUNTIME_VERSION).run();
  const token=crypto.randomUUID(),pending=JSON.stringify({status:'PROCESSING',token,startedAt:iso()});
  await env.DB.prepare('INSERT OR IGNORE INTO app_meta(key,value,updated_at) VALUES(?,?,CURRENT_TIMESTAMP)').bind(CLAN_LATE_DRAFT_RUNTIME_VERSION,pending).run();
  const claim=await env.DB.prepare('SELECT value FROM app_meta WHERE key=?').bind(CLAN_LATE_DRAFT_RUNTIME_VERSION).first();
  if(safeJson(claim?.value,{}).token!==token)return;
  try{
    const now=Date.now(),season=await env.DB.prepare("SELECT * FROM clan_seasons WHERE phase<>'COMPLETE' ORDER BY season_no DESC,id DESC LIMIT 1").first(),result={status:'COMPLETED',version:'v1994',appliedAt:iso(now),draftAtKst:'19:00',warAtKst:'21:00',seasonId:Number(season?.id||0),seasonNo:Number(season?.season_no||0),phase:String(season?.phase||'NONE'),action:'NO_ACTIVE_SEASON'};
    if(season){
      const phase=String(season.phase||'').toUpperCase(),settings=await clanSettings(env),roundRows=rows(await env.DB.prepare('SELECT DISTINCT round_no FROM clan_wars WHERE season_id=? ORDER BY round_no').bind(season.id).all()),schedule=clanLateDraftFixedSchedule(settings,now,roundRows.length||7),warStart=sqlMs(schedule.startsAt),warStats=await env.DB.prepare("SELECT COUNT(*) total,SUM(CASE WHEN status<>'SCHEDULED' OR COALESCE(score_a,0)<>0 OR COALESCE(score_b,0)<>0 OR COALESCE(battle_count,0)<>0 OR winner_clan_id IS NOT NULL THEN 1 ELSE 0 END) unsafe FROM clan_wars WHERE season_id=?").bind(season.id).first(),battleStats=await env.DB.prepare('SELECT COUNT(*) count FROM clan_war_battles WHERE season_id=?').bind(season.id).first(),unsafeWars=Number(warStats?.unsafe||0),battleCount=Number(battleStats?.count||0);
      Object.assign(result,{previousPhase:phase,registrationEndsAt:schedule.registrationEndsAt,draftEndsAt:schedule.draftEndsAt,startsAt:schedule.startsAt,endsAt:schedule.endsAt,scheduledWarCount:Number(warStats?.total||0),unsafeWars,battleCount});
      if(now>=warStart)result.action='SKIP_WAR_START_REACHED';
      else if(phase==='ACTIVE'&&(unsafeWars>0||battleCount>0))result.action='SKIP_STARTED_WAR_SAFETY';
      else if(['ACTIVE','DRAFT','REGISTRATION'].includes(phase)){
        const nextSettings={...schedule.settings,warOpenTime:'21:00',scheduleEnabled:true},writes=[env.DB.prepare('INSERT INTO app_meta(key,value,updated_at) VALUES(?,?,CURRENT_TIMESTAMP) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=CURRENT_TIMESTAMP').bind(CLAN_ADMIN_SETTINGS_KEY,JSON.stringify(nextSettings))];
        if(phase==='REGISTRATION')writes.push(env.DB.prepare("UPDATE clan_seasons SET max_members=?,registration_ends_at=?,draft_ends_at=?,starts_at=?,ends_at=?,next_pick_deadline=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=? AND phase='REGISTRATION'").bind(CLAN_MAX_MEMBERS,schedule.registrationEndsAt,schedule.draftEndsAt,schedule.startsAt,schedule.endsAt,season.id));
        else writes.push(env.DB.prepare("UPDATE clan_seasons SET phase='DRAFT',max_members=?,registration_ends_at=?,draft_ends_at=?,starts_at=?,ends_at=?,next_pick_deadline=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND phase IN ('DRAFT','ACTIVE')").bind(CLAN_MAX_MEMBERS,schedule.registrationEndsAt,schedule.draftEndsAt,schedule.startsAt,schedule.endsAt,schedule.nextPickDeadline,season.id));
        roundRows.forEach((row,index)=>{const start=schedule.roundStarts[index];if(!start)return;writes.push(env.DB.prepare("UPDATE clan_wars SET status='SCHEDULED',starts_at=?,ends_at=?,updated_at=CURRENT_TIMESTAMP WHERE season_id=? AND round_no=? AND status='SCHEDULED'").bind(start,iso(sqlMs(start)+schedule.warDurationMs),season.id,row.round_no))});
        await env.DB.batch(writes);
        result.action=phase==='ACTIVE'?'PAUSE_SCHEDULED_SEASON_FOR_LATE_DRAFT':phase==='DRAFT'?'ALIGN_LATE_DRAFT_TO_1900':'ALIGN_REGISTRATION_TO_1900';
      }else result.action=`SKIP_PHASE_${phase||'UNKNOWN'}`;
    }
    await env.DB.prepare('UPDATE app_meta SET value=?,updated_at=CURRENT_TIMESTAMP WHERE key=? AND value=?').bind(JSON.stringify(result),CLAN_LATE_DRAFT_RUNTIME_VERSION,pending).run();
    console.info('[CLAN_LATE_DRAFT_V1994]',JSON.stringify(result));
    lateDraftRuntimeReady=true;
  }catch(error){
    await env.DB.prepare('DELETE FROM app_meta WHERE key=? AND value=?').bind(CLAN_LATE_DRAFT_RUNTIME_VERSION,pending).run().catch(()=>{});
    throw error;
  }
}
...
async function ensureClanConcurrentReservationRuntimeUpgrade(env){
  if(concurrentReservationRuntimeReady)return;
  const existing=await env.DB.prepare('SELECT value FROM app_meta WHERE key=?').bind(CLAN_CONCURRENT_RESERVATION_RUNTIME_VERSION).first();
  if(existing){concurrentReservationRuntimeReady=true;return}
  const postgres=env.DB?.dialect==='postgres',idType=postgres?'BIGINT':'INTEGER',nowDefault=postgres?'sqlite_now()':'CURRENT_TIMESTAMP',statements=[`CREATE TABLE IF NOT EXISTS clan_war_reservation_locks(
    war_id ${idType} NOT NULL,lock_role TEXT NOT NULL,user_id ${idType} NOT NULL,token TEXT NOT NULL,expires_at TEXT NOT NULL,updated_at TEXT NOT NULL DEFAULT ${nowDefault},
    PRIMARY KEY(war_id,lock_role,user_id))`,'CREATE INDEX IF NOT EXISTS idx_clan_war_reservation_locks_expiry ON clan_war_reservation_locks(expires_at,war_id)'];
  if(postgres&&typeof env.DB.execSchema==='function')await env.DB.execSchema(statements);else await env.DB.batch(statements.map(sql=>env.DB.prepare(sql)));
  await env.DB.prepare('INSERT INTO app_meta(key,value,updated_at) VALUES(?,?,CURRENT_TIMESTAMP) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=CURRENT_TIMESTAMP').bind(CLAN_CONCURRENT_RESERVATION_RUNTIME_VERSION,JSON.stringify({status:'COMPLETED',version:'v1999',appliedAt:iso(),scope:'PER_WAR_USER_AND_TARGET'})).run();
  concurrentReservationRuntimeReady=true;
}
...
function cleanClanAdminSettings(raw={},current=CLAN_ADMIN_SETTINGS_DEFAULTS){
  ...
  return{
    ...
    battleWinBonusPercent:clampInt(raw.battleWinBonusPercent,0,100,base.battleWinBonusPercent),
    participationMilestoneCoin:clampInt(raw.participationMilestoneCoin,0,100000000,base.participationMilestoneCoin),
...
function clanDraftDeadlineMs(season){
  const registration=sqlMs(season?.registration_ends_at),planned=sqlMs(season?.draft_ends_at),limit=registration+CLAN_DRAFT_DURATION_MINUTES*60000;
  return Number.isFinite(limit)?(Number.isFinite(planned)?Math.min(planned,limit):limit):planned;
}
function nextDraftCheckAt(season){
  if(clanRegistrationOpen(season))return season.registration_ends_at;
  const deadline=clanDraftDeadlineMs(season),turn=sqlMs(season.next_pick_deadline);
  return iso(Number.isFinite(turn)?Math.min(turn,deadline):deadline);
}
...
async function acquireWarReservationLock(env,warId,lockRole,userId,ttlMs=8000){
  const role=String(lockRole||'').toUpperCase(),token=crypto.randomUUID(),now=iso(),expiresAt=iso(Date.now()+Math.max(2000,Number(ttlMs)||8000));
  if(!['ATTACKER','DEFENDER'].includes(role))return{ok:false};
  await env.DB.prepare('DELETE FROM clan_war_reservation_locks WHERE war_id=? AND lock_role=? AND user_id=? AND expires_at<?').bind(warId,role,userId,now).run();
  await env.DB.prepare('INSERT OR IGNORE INTO clan_war_reservation_locks(war_id,lock_role,user_id,token,expires_at,updated_at) VALUES(?,?,?,?,?,CURRENT_TIMESTAMP)').bind(warId,role,userId,token,expiresAt).run();
  const row=await env.DB.prepare('SELECT token FROM clan_war_reservation_locks WHERE war_id=? AND lock_role=? AND user_id=?').bind(warId,role,userId).first();
  return row?.token===token?{ok:true,warId,lockRole:role,userId,token}:{ok:false};
}
async function releaseWarReservationLock(env,lock){if(lock?.ok)await env.DB.prepare('DELETE FROM clan_war_reservation_locks WHERE war_id=? AND lock_role=? AND user_id=? AND token=?').bind(lock.warId,lock.lockRole,lock.userId,lock.token).run()}

async function beginDraft(env,season,settings=CLAN_ADMIN_SETTINGS_DEFAULTS,{forceMasterUserId=0}={}){
  const lock=await acquireDraftLock(env,season.id);if(!lock.ok)return env.DB.prepare('SELECT * FROM clan_seasons WHERE id=?').bind(season.id).first();
  try{
...
function draftTurnMs(settings){return clampInt(settings?.draftPickSeconds,30,1800,30)*1000}
// Count denotes snake turns, including passes. Full rosters never block the queue.
function nextDraftTurn(pickCount,teams,{skipClanId=0}={}){
  const eligible=teams.filter(team=>Number(team.member_count||0)<clanDraftCapacity(team));
  if(!eligible.length)return null;
  const alternatives=eligible.filter(team=>Number(team.clan_id)!==Number(skipClanId));
  const allowed=skipClanId&&alternatives.length?alternatives:eligible;
  for(let step=1;step<=teams.length*2;step++){
    const count=Number(pickCount)+step,position=currentDraftPosition(count,teams.length);
    if(allowed.some(team=>Number(team.draft_position)===position))return count;
  }
  return null;
}
async function draftContext(env,season){
  const teams=rows(await env.DB.prepare('SELECT t.*,o.name,o.mark_key,o.primary_color,o.accent_color,(SELECT COUNT(*) FROM clan_members m WHERE m.season_id=t.season_id AND m.clan_id=t.clan_id) member_count FROM clan_season_teams t JOIN clan_organizations o ON o.id=t.clan_id WHERE t.season_id=? ORDER BY t.draft_position').bind(season.id).all());
  const plan=await readClanRedraft(env,season.id),limited=applyClanRedraftQuotas(teams,plan);
  const pickCount=Number(season.draft_pick_count||0),position=currentDraftPosition(pickCount,Math.max(1,limited.length));return{teams:limited,current:limited.find(t=>Number(t.draft_position)===position)||null,pickNo:pickCount+1,redraft:plan};
}
async function makeDraftPick(env,season,team,candidate,settings=CLAN_ADMIN_SETTINGS_DEFAULTS,{auto=false,expectedPickNo=Number(season.draft_pick_count||0)+1}={}){
  ...
  try{
    ...
    if(Number(expectedPickNo)!==ctx.pickNo)throw new Error('지명 순서가 변경됐습니다. 현재 차례를 다시 확인하세요.');
    if(!auto&&(Date.now()>=sqlMs(fresh.next_pick_deadline)||Date.now()>=clanDraftDeadlineMs(fresh)))throw new Error('지명 제한 시간이 끝났습니다. 다음 클랜으로 차례를 넘깁니다.');
    ...
    const count=await env.DB.prepare('SELECT COUNT(*) count FROM clan_members WHERE season_id=? AND clan_id=?').bind(season.id,team.clan_id).first();if(Number(count?.count||0)>=clanDraftCapacity(ctx.current))throw new Error('클랜 정원이 가득 찼습니다.');
    const nextTeams=ctx.teams.map(row=>Number(row.clan_id)===Number(team.clan_id)?{...row,member_count:Number(row.member_count)+1}:row),nextCount=nextDraftTurn(fresh.draft_pick_count,nextTeams)??Number(fresh.draft_pick_count)+1;
    await env.DB.batch([
      ...
      env.DB.prepare('UPDATE clan_seasons SET draft_pick_count=?,next_pick_deadline=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').bind(nextCount,iso(Math.min(Date.now()+draftTurnMs(settings),clanDraftDeadlineMs(fresh))),season.id)
    ]);
    return{pickNo:ctx.pickNo,auto,userId:Number(available.user_id),clanId:Number(team.clan_id)};
  }finally{await releaseDraftLock(env,lock)}
}

async function skipExpiredDraftTurns(env,season,settings=CLAN_ADMIN_SETTINGS_DEFAULTS){
  const lock=await acquireDraftLock(env,season.id);if(!lock.ok)return env.DB.prepare('SELECT * FROM clan_seasons WHERE id=?').bind(season.id).first();
  try{
    const fresh=await env.DB.prepare('SELECT * FROM clan_seasons WHERE id=?').bind(season.id).first(),now=Date.now();
    if(fresh?.phase!=='DRAFT'||clanRegistrationOpen(fresh))return fresh;
    const ctx=await draftContext(env,fresh);if(!ctx.current)return fresh;
    let count=Number(fresh.draft_pick_count),deadline=sqlMs(fresh.next_pick_deadline);
    const turnMs=draftTurnMs(settings);
    if(!Number.isFinite(deadline))deadline=now+turnMs;
    if(Number(ctx.current.member_count)>=clanDraftCapacity(ctx.current)){
      const next=nextDraftTurn(count,ctx.teams);if(next===null)return fresh;
      count=next;deadline=now+turnMs;
    }
    // Catch up using the original deadline, never granting extra time after a late poll.
    // At most one hour of expired turns, all in one CPU-only loop / DB write.
    const expired=now>=clanDraftDeadlineMs(fresh)?0:Math.max(0,Math.floor((now-deadline)/turnMs)+1);
    for(let i=0;i<expired;i++){
      const current=ctx.teams.find(team=>Number(team.draft_position)===currentDraftPosition(count,ctx.teams.length));
      const next=nextDraftTurn(count,ctx.teams,{skipClanId:current?.clan_id});if(next===null)break;
      count=next;deadline+=turnMs;
    }
    deadline=Math.min(deadline,clanDraftDeadlineMs(fresh));
    if(count!==Number(fresh.draft_pick_count)||iso(deadline)!==fresh.next_pick_deadline){
      await env.DB.prepare("UPDATE clan_seasons SET draft_pick_count=?,next_pick_deadline=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND phase='DRAFT' AND draft_pick_count=? AND EXISTS(SELECT 1 FROM clan_draft_locks WHERE season_id=? AND token=?)").bind(count,iso(deadline),fresh.id,fresh.draft_pick_count,fresh.id,lock.token).run();
    }
    return env.DB.prepare('SELECT * FROM clan_seasons WHERE id=?').bind(fresh.id).first();
  }finally{await releaseDraftLock(env,lock)}
}
...
async function createWars(env,season,teams,settings=CLAN_ADMIN_SETTINGS_DEFAULTS,{immediateFirst=false}={}){
  ...
  await batchChunks(env,writes);const persisted=await env.DB.prepare("SELECT MIN(starts_at) starts_at,MAX(ends_at) ends_at,COUNT(DISTINCT round_no) round_count FROM clan_wars WHERE season_id=? AND status IN ('SCHEDULED','ACTIVE')").bind(season.id).first(),persistedStart=sqlMs(persisted?.starts_at),persistedEnd=sqlMs(persisted?.ends_at);return{startsAt:Number.isFinite(persistedStart)?persistedStart:starts[0],endsAt:Number.isFinite(persistedEnd)?persistedEnd:starts.at(-1)+duration,roundCount:Number(persisted?.round_count||rounds.length)};
}
async function activateSeason(env,season,settings=CLAN_ADMIN_SETTINGS_DEFAULTS,{immediateFirst=false}={}){
  const plan=await readClanRedraft(env,season.id);if(plan){const ctx=await draftContext(env,season);assertClanRedraftComplete(ctx.teams,plan);const pending=await env.DB.prepare("SELECT COUNT(*) count FROM clan_draft_pool WHERE season_id=? AND status='AVAILABLE'").bind(season.id).first();if(Number(pending?.count||0))throw new Error('재드래프트 미편성 참가자가 남아 있습니다.');}
...
async function payClanSeasonRewards(env,season,settings,rankedTeams){
  ...
  for(const receipt of pending){const writes=[];if(env.DB.dialect==='postgres')writes.push(env.DB.prepare('SELECT id FROM users WHERE id=? FOR UPDATE').bind(receipt.user_id));writes.push(...await clanPigCoinStatements(env,{userId:Number(receipt.user_id),seasonId:season.id}));writes.push(env.DB.prepare("UPDATE users SET coin=coin+?,card_shards=card_shards+? WHERE id=? AND EXISTS(SELECT 1 FROM clan_reward_receipts WHERE season_id=? AND user_id=? AND status='PENDING')").bind(receipt.coin,receipt.card_shards,receipt.user_id,season.id,receipt.user_id));writes.push(env.DB.prepare("UPDATE clan_reward_receipts SET status='COMPLETED',completed_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE season_id=? AND user_id=? AND status='PENDING'").bind(season.id,receipt.user_id));await env.DB.batch(writes)}
  const left=await env.DB.prepare("SELECT COUNT(*) count FROM clan_reward_receipts WHERE season_id=? AND status<>'COMPLETED'").bind(season.id).first();if(Number(left?.count||0)>0)throw new Error('클랜전 보상 영수증을 모두 완료하지 못했습니다.');return'PAID';
}
async function settleSeason(env,season,settings=CLAN_ADMIN_SETTINGS_DEFAULTS){
  ...
  try{
    ...
    if(season?.phase!=='SETTLEMENT')return season; // A stale request must not cancel newly created playoffs.
    ...
    if(settlement?.status==='COMPLETED'){await startChampions(env,season,settings,rankedTeams);return env.DB.prepare('SELECT * FROM clan_seasons WHERE id=?').bind(season.id).first()}
...
async function autoDraftDue(env,season,settings=CLAN_ADMIN_SETTINGS_DEFAULTS){
  let fresh=season;
  if(fresh.phase==='DRAFT'&&clanRegistrationOpen(fresh))return fresh;
  fresh=await skipExpiredDraftTurns(env,fresh,settings);
  if(fresh.phase!=='DRAFT')return fresh;
  ...
  // The overall deadline is registration close + 60 minutes, not draft start + 60.
  if(Date.now()>=clanDraftDeadlineMs(fresh))return finishDraftAtDeadline(env,fresh,settings);
  return fresh;
}

async function finishDraftAtDeadline(env,season,settings){
  const lock=await acquireDraftLock(env,season.id);if(!lock.ok)return env.DB.prepare('SELECT * FROM clan_seasons WHERE id=?').bind(season.id).first();
  try{
    const fresh=await env.DB.prepare('SELECT * FROM clan_seasons WHERE id=?').bind(season.id).first();
    if(fresh?.phase!=='DRAFT'||clanRegistrationOpen(fresh)||Date.now()<clanDraftDeadlineMs(fresh))return fresh;
    const {teams}=await draftContext(env,fresh),candidates=rows(await env.DB.prepare("SELECT * FROM clan_draft_pool WHERE season_id=? AND status='AVAILABLE' ORDER BY total_score DESC,user_id").bind(fresh.id).all()),assignments=[];
    let count=Number(fresh.draft_pick_count||0);
    for(const candidate of candidates){
      let team=teams.find(row=>Number(row.draft_position)===currentDraftPosition(count,teams.length));
      if(!team||Number(team.member_count)>=clanDraftCapacity(team)){const next=nextDraftTurn(count,teams);if(next===null)throw new Error('드래프트 잔여 후보를 편성할 클랜 정원이 부족합니다.');count=next;team=teams.find(row=>Number(row.draft_position)===currentDraftPosition(count,teams.length))}
      assignments.push({userId:candidate.user_id,clanId:team.clan_id,role:cleanRole(candidate.preferred_role),pickNo:count+1});
      team.member_count=Number(team.member_count)+1;count=nextDraftTurn(count,teams)??count+1;
    }
    // Batch rows under one lock/transaction instead of reopening the pick flow 168 times.
    // Chunks stay below 100 SQL parameters for the D1 compatibility contract.
    const writes=[];
    for(let i=0;i<assignments.length;i+=10){
      const chunk=assignments.slice(i,i+10),cases=chunk.map(()=> 'WHEN ? THEN CAST(? AS INTEGER)').join(' '),ids=chunk.map(()=>'?').join(',');
      writes.push(env.DB.prepare(`UPDATE clan_draft_pool SET status='DRAFTED',drafted_clan_id=CASE user_id ${cases} END,pick_no=CASE user_id ${cases} END,updated_at=CURRENT_TIMESTAMP WHERE season_id=? AND status='AVAILABLE' AND user_id IN (${ids})`).bind(...chunk.flatMap(row=>[row.userId,row.clanId]),...chunk.flatMap(row=>[row.userId,row.pickNo]),fresh.id,...chunk.map(row=>row.userId)));
      writes.push(env.DB.prepare(`INSERT OR IGNORE INTO clan_members(season_id,clan_id,user_id,member_role,preferred_role,draft_pick_no) VALUES ${chunk.map(()=>"(?,?,?,'MEMBER',?,?)").join(',')}`).bind(...chunk.flatMap(row=>[fresh.id,row.clanId,row.userId,row.role,row.pickNo])));
    }
    const draftEnd=iso(clanDraftDeadlineMs(fresh));
    writes.push(env.DB.prepare("UPDATE clan_seasons SET draft_pick_count=?,draft_ends_at=?,next_pick_deadline=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=? AND phase='DRAFT'").bind(count,draftEnd,fresh.id));
    await env.DB.batch(writes);
    return await activateSeason(env,{...fresh,draft_ends_at:draftEnd},settings);
  }finally{await releaseDraftLock(env,lock)}
}

async function startScheduledRedraft(env,season,settings){
  if(season?.phase!=='REDRAFT_WAIT'||Date.now()<sqlMs(season.registration_ends_at))return season;
  const lock=await acquireDraftLock(env,season.id);if(!lock.ok)return env.DB.prepare('SELECT * FROM clan_seasons WHERE id=?').bind(season.id).first();
  try{
    const fresh=await env.DB.prepare('SELECT * FROM clan_seasons WHERE id=?').bind(season.id).first();
    if(fresh?.phase!=='REDRAFT_WAIT'||Date.now()<sqlMs(fresh.registration_ends_at))return fresh;
    const ctx=await draftContext(env,fresh),plan=ctx.redraft;
    if(!plan||Date.parse(plan.startsAt)!==sqlMs(fresh.registration_ends_at)||ctx.teams.some(team=>Number(team.member_count)!==1))throw new Error('재드래프트 시작 상태가 일치하지 않습니다.');
    const pool=await env.DB.prepare('SELECT COUNT(*) count FROM clan_draft_pool WHERE season_id=?').bind(fresh.id).first();
    if(Number(pool?.count)!==plan.participantCount)throw new Error('재드래프트 참가 명단이 변경됐습니다.');
    await calculateSeasonScores(env,fresh);
    await env.DB.prepare("UPDATE clan_seasons SET phase='DRAFT',draft_pick_count=0,next_pick_deadline=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND phase='REDRAFT_WAIT'").bind(iso(Math.min(sqlMs(fresh.registration_ends_at)+draftTurnMs(settings),clanDraftDeadlineMs(fresh))),fresh.id).run();
    return env.DB.prepare('SELECT * FROM clan_seasons WHERE id=?').bind(fresh.id).first();
  }finally{await releaseDraftLock(env,lock)}
}
...
async function advanceLifecycle(env,season,settings=CLAN_ADMIN_SETTINGS_DEFAULTS){
  let fresh=season;
  if(fresh.phase==='REDRAFT_WAIT')fresh=await startScheduledRedraft(env,fresh,settings);
...
async function clanAdminState(env,settings){
  ...
  const [organizationsResult,warsResult,poolResult,battleStatusResult,recentResult,settlement,rankedResult]=await Promise.all([
    ...
    seasonId?env.DB.prepare(`SELECT b.id,b.request_id,b.war_id,b.status,b.created_at,b.updated_at,b.error_message,
      ...
      pr.points participation_points,pr.base_coin+pr.win_bonus_coin+pr.milestone_coin participation_coin
  ...
  const ruleWar=rows(warsResult).find(w=>w.status==='ACTIVE')||rows(warsResult).find(w=>w.status==='SCHEDULED'),roundSettings=ruleWar?await clanWarParticipationSettings(env,ruleWar,await championsBattleSettings(env,season,settings)):null;
  return{
    ok:true,settings,champions:await championsPublicState(env,seasonId,settings),participationRuntime:roundSettings?{roundNo:Number(ruleWar.round_no),scorePolicy:roundSettings.scorePolicy,scorePerWin:roundSettings.warWinScore,scorePerLoss:roundSettings.warLossScore,startsAt:ruleWar.starts_at,rewardsEnabled:roundSettings.mode==='ON'&&roundSettings.battleParticipationRewardsEnabled,battleCoin:roundSettings.battleParticipationCoin}:null,season:publicSeason(season),settlement:settlement?{status:settlement.status,championClanId:Number(settlement.champion_clan_id||0),rewardStatus:settlement.reward_status,completedAt:settlement.completed_at}:null,clans,wars,recentBattles,
    ...
    releaseGates:[
      ...
      {key:'CONCURRENT_ENTRY',status:'READY',label:'유저별 동시 진입 · 대상별 원자 예약'},
...
async function opponentMatchState(env,deps,user,season,war,mine,settings,selectionKey='OVERVIEW'){
  const enemyClan=Number(war.clan_a_id)===Number(mine.clan_id)?Number(war.clan_b_id):Number(war.clan_a_id),[battle,attackerDeckIds,opponentResult]=await Promise.all([
    ...
    env.DB.prepare(`SELECT m.user_id,u.nickname,m.preferred_role,m.battle_wins,m.battle_losses,
      ...
      FROM clan_members m JOIN users u ON u.id=m.user_id LEFT JOIN pvp_decks d ON d.user_id=m.user_id WHERE m.season_id=? AND m.clan_id=? AND (?=0 OR EXISTS(SELECT 1 FROM clan_championship_members cm WHERE cm.season_id=m.season_id AND cm.user_id=m.user_id AND cm.clan_id=m.clan_id)) ORDER BY m.user_id LIMIT ?`).bind(war.id,war.id,user.id,season.id,enemyClan,season.phase==='CHAMPIONS'?1:0,CLAN_MAX_MEMBERS).all()
  ]),attackerPower=await rankedDeckPower(env,deps,user.id,attackerDeckIds,battle),raw=rows(opponentResult);
  const candidates=raw.map(row=>{const defenseCount=Number(row.defense_count||0),alreadyFaced=Number(row.faced_count||0)>=settings.repeatTargetLimit,liveDeck=safeJson(row.live_deck,[]),deckReady=Array.isArray(liveDeck)&&liveDeck.length===5;return{userId:Number(row.user_id),nickname:row.nickname,preferredRole:row.preferred_role,battleWins:Number(row.battle_wins),battleLosses:Number(row.battle_losses),defenseCount,alreadyFaced,deckReady,available:(settings.sharedDefenseLimit===false||defenseCount<settings.defensesPerTarget)&&!alreadyFaced&&deckReady}}),availableOpponentCount=candidates.filter(candidate=>candidate.available).length;
...
async function overview(env,user,season,deps,settings=CLAN_ADMIN_SETTINGS_DEFAULTS){
  ...
  if(membership){
    ...
    if(['ACTIVE','CHAMPIONS'].includes(season.phase)){
      ...
      if(war){
        ...
        const participation=await clanParticipationProgress(env,war.id,user.id);
        ...
        war={id:Number(war.id),stage:Number(war.round_no)===1001?'SEMIFINAL':Number(war.round_no)===1002?'FINAL':'REGULAR',roundNo:Number(war.round_no),status:war.status,clanAId:Number(war.clan_a_id),clanBId:Number(war.clan_b_id),scoreA:Number(war.score_a),scoreB:Number(war.score_b),battleCount:Number(war.battle_count),attacksUsed,attackLimit:settings.totalUseLimit,attacksRemaining:energy.usesRemaining,attackerPower:matched.attackerPower,liveDeckReady:matched.attackerDeckReady,availableOpponentCount:matched.availableOpponentCount,energy,startsAt:war.starts_at,endsAt:war.ends_at,
          scorePolicy:settings.scorePolicy,scorePerWin:settings.warWinScore,scorePerLoss:settings.warLossScore||0,sharedDefenseLimit:settings.sharedDefenseLimit!==false,
          participation:{...participation,rewardsEnabled:settings.mode==='ON'&&settings.battleParticipationRewardsEnabled,battleCoin:settings.battleParticipationCoin,winBonusPercent:settings.battleWinBonusPercent,milestoneCoin:settings.participationMilestoneCoin}};
      }
    }
  ...
  if(membership){
    const progress=rows(await env.DB.prepare('SELECT p.user_id,SUM(p.completed_attacks) attacks,SUM(p.earned_points) points FROM clan_participation_progress p JOIN clan_members m ON m.season_id=p.season_id AND m.user_id=p.user_id WHERE p.season_id=? AND m.clan_id=? GROUP BY p.user_id').bind(season.id,membership.clan_id).all()),byUser=new Map(progress.map(row=>[Number(row.user_id),row]));
...
async function resetSeasonToDraft(env,deps,user,settings,body){
  if(!isOwner(user))return deps.json({error:'클랜전 드래프트 리셋은 OWNER만 실행할 수 있습니다.'},403);
  if(await env.DB.prepare('SELECT 1 ok FROM clan_championships WHERE season_id=(SELECT id FROM clan_seasons ORDER BY season_no DESC LIMIT 1)').first())return deps.json({error:'챔피언스리그 대진·보상 기록이 있는 시즌은 자동 리셋할 수 없습니다.'},409);
  const season=await env.DB.prepare('SELECT * FROM clan_seasons ORDER BY season_no DESC LIMIT 1').first();if(!season)return deps.json({error:'리셋할 클랜 시즌이 없습니다.'},404);
  ...
  try{
    ...
    const now=Date.now(),draftEnd=now+CLAN_DRAFT_DURATION_MINUTES*60000,seasonEnd=draftEnd+Number(settings.seasonDays||28)*86400000,writes=[];
    writes.push(env.DB.prepare('DELETE FROM clan_participation_progress WHERE season_id=?').bind(season.id),env.DB.prepare('DELETE FROM clan_participation_round_rules WHERE season_id=?').bind(season.id));
    ...
    writes.push(env.DB.prepare('DELETE FROM clan_war_reservation_locks WHERE war_id IN (SELECT id FROM clan_wars WHERE season_id=?)').bind(season.id));writes.push(env.DB.prepare('DELETE FROM clan_war_battles WHERE season_id=?').bind(season.id));writes.push(env.DB.prepare('DELETE FROM clan_wars WHERE season_id=?').bind(season.id));writes.push(env.DB.prepare('DELETE FROM clan_reward_receipts WHERE season_id=?').bind(season.id));writes.push(env.DB.prepare('DELETE FROM clan_season_settlements WHERE season_id=?').bind(season.id));writes.push(env.DB.prepare('DELETE FROM clan_members WHERE season_id=?').bind(season.id));writes.push(env.DB.prepare('DELETE FROM clan_season_teams WHERE season_id=?').bind(season.id));writes.push(env.DB.prepare("UPDATE clan_draft_pool SET status='AVAILABLE',drafted_clan_id=NULL,pick_no=NULL,updated_at=CURRENT_TIMESTAMP WHERE season_id=?").bind(season.id));writes.push(env.DB.prepare("UPDATE clan_seasons SET phase='REGISTRATION',draft_pick_count=0,next_pick_deadline=NULL,registration_ends_at=?,draft_ends_at=?,starts_at=?,ends_at=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(iso(now),iso(draftEnd),iso(draftEnd),iso(seasonEnd),season.id));
...
async function resetOfficialSeasonOne(env,deps,user,settings,body){
  ...
  if(await env.DB.prepare('SELECT 1 ok FROM clan_championships LIMIT 1').first())return deps.json({error:'챔피언스리그 기록을 보존해야 하므로 공식 시즌 초기화를 차단합니다.'},409);
  ...
  try{
    ...
    const now=Date.now(),registrationEnd=now+Number(settings.registrationDays||1)*86400000,draftEnd=registrationEnd+CLAN_DRAFT_DURATION_MINUTES*60000,seasonEnd=draftEnd+Number(settings.seasonDays||7)*86400000;
    await env.DB.batch([
      env.DB.prepare('DELETE FROM clan_participation_progress'),env.DB.prepare('DELETE FROM clan_participation_round_rules'),
      env.DB.prepare('DELETE FROM clan_war_reservation_locks'),env.DB.prepare('DELETE FROM clan_war_battles'),env.DB.prepare('DELETE FROM clan_reward_receipts'),env.DB.prepare('DELETE FROM clan_wars'),env.DB.prepare('DELETE FROM clan_season_settlements'),env.DB.prepare('DELETE FROM clan_members'),env.DB.prepare('DELETE FROM clan_season_teams'),env.DB.prepare('DELETE FROM clan_draft_pool'),env.DB.prepare('DELETE FROM clan_draft_locks'),env.DB.prepare('DELETE FROM clan_seasons'),
...
async function clanWarDefender(env,seasonId,clanId,userId){
  return env.DB.prepare('SELECT m.*,u.nickname,u.role FROM clan_members m JOIN users u ON u.id=m.user_id WHERE m.season_id=? AND m.clan_id=? AND m.user_id=?').bind(seasonId,clanId,userId).first();
}

async function clanWarReservationCheck(env,war,userId,defenderUserId,settings=CLAN_ADMIN_SETTINGS_DEFAULTS,excludeId=0){
  const freshWar=await env.DB.prepare('SELECT * FROM clan_wars WHERE id=?').bind(war.id).first(),[attacks,repeat,defenses]=await Promise.all([
    env.DB.prepare("SELECT COUNT(*) count FROM clan_war_battles WHERE war_id=? AND attacker_user_id=? AND id<>? AND status IN ('PENDING','RESOLVING','COMPLETED')").bind(war.id,userId,excludeId).first(),
...
async function fight(env,deps,user,season,body,settings=CLAN_ADMIN_SETTINGS_DEFAULTS){
  const requestId=validRequestId(body.requestId);if(!requestId)return deps.json({error:'전투 요청 키가 올바르지 않습니다.'},400);
  try{const replay=await clanParticipationReplay(env,requestId,user.id);if(replay)return deps.json({...replay,wallet:await clanWallet(env,user.id)})}catch(error){return deps.json({error:error.message},409)}
  if(!['ACTIVE','CHAMPIONS'].includes(season.phase))return deps.json({error:'클랜전 진행 기간이 아닙니다.'},409);
  settings=await championsBattleSettings(env,season,settings);
  ...
  if(!receipt&&!match.attackerDeckReady)return deps.json({error:'현재 랭크전 덱 5장을 저장한 뒤 랜덤 매칭을 시작하세요.',code:'CLAN_LIVE_DECK_REQUIRED'},409);

  if(!receipt){
    const candidates=match.opponents.filter(candidate=>candidate.available!==false);
    if(!candidates.length)return deps.json({error:'현재 공격 가능한 무작위 상대가 없습니다. 잠시 후 다시 시도하세요.',code:'CLAN_RANDOM_POOL_EMPTY'},409);
    const attackerLock=await acquireWarReservationLock(env,war.id,'ATTACKER',user.id);
    if(!attackerLock.ok)return deps.json({error:'같은 계정의 클랜전 진입 요청을 처리 중입니다. 잠시 후 다시 시도하세요.',code:'CLAN_ATTACKER_RESERVATION_BUSY'},409);
    try{
      receipt=await env.DB.prepare('SELECT * FROM clan_war_battles WHERE request_id=?').bind(requestId).first();
      if(!receipt){
        let terminalCheck=null,busyTargets=0;
        for(const candidate of candidates){
          const targetLock=await acquireWarReservationLock(env,war.id,'DEFENDER',candidate.userId);
          if(!targetLock.ok){busyTargets++;continue}
          try{
            const candidateDefender=await clanWarDefender(env,season.id,enemyClan,candidate.userId);if(!candidateDefender)continue;
            const check=await clanWarReservationCheck(env,war,user.id,candidateDefender.user_id,settings);
            if(!check.ok){
              if(['CLAN_REPEAT_TARGET_LIMIT','CLAN_DEFENSE_LIMIT'].includes(check.code))continue;
              terminalCheck=check;break;
            }
            const reservedAt=iso();
            await env.DB.prepare("INSERT OR IGNORE INTO clan_war_battles(request_id,season_id,war_id,attacker_clan_id,defender_clan_id,attacker_user_id,defender_user_id,battle_seed,status) SELECT ?,?,?,?,?,?,?,?,'PENDING' WHERE EXISTS(SELECT 1 FROM clan_wars WHERE id=? AND status='ACTIVE' AND starts_at<=? AND ends_at>?)").bind(requestId,season.id,war.id,mine.clan_id,enemyClan,user.id,candidateDefender.user_id,battleSeed,war.id,reservedAt,reservedAt).run();
            receipt=await env.DB.prepare('SELECT * FROM clan_war_battles WHERE request_id=?').bind(requestId).first();
            if(receipt&&Number(receipt.attacker_user_id)===Number(user.id)&&Number(receipt.defender_user_id)===Number(candidateDefender.user_id)){defender=candidateDefender;break}
            if(receipt)break;
          }finally{await releaseWarReservationLock(env,targetLock)}
        }
        if(!receipt){
          if(terminalCheck)return deps.json(terminalCheck,409);
          return deps.json({error:busyTargets?'무작위 상대 예약 요청이 몰렸습니다. 바로 다시 시도해 주세요.':'현재 공격 가능한 무작위 상대가 없습니다. 잠시 후 다시 시도하세요.',code:busyTargets?'CLAN_RANDOM_TARGET_BUSY':'CLAN_RANDOM_POOL_EMPTY'},409);
        }
      }
    }finally{await releaseWarReservationLock(env,attackerLock)}
  }

  if(Number(receipt.attacker_user_id)!==Number(user.id)||Number(receipt.season_id)!==Number(season.id)||Number(receipt.war_id)!==Number(war.id))return deps.json({error:'다른 전투에 사용된 요청 키입니다.'},409);
  if(!defender)defender=await clanWarDefender(env,season.id,enemyClan,receipt.defender_user_id);
  if(!defender)return deps.json({error:'공격할 상대 클랜원을 찾지 못했습니다.'},404);
  const seed=Number(receipt.battle_seed);
  ...
  if(receipt?.status==='FAILED'){
    const attackerLock=await acquireWarReservationLock(env,war.id,'ATTACKER',user.id);if(!attackerLock.ok)return deps.json({error:'같은 계정의 클랜전 진입 요청을 처리 중입니다. 잠시 후 다시 시도하세요.',code:'CLAN_ATTACKER_RESERVATION_BUSY'},409);
    let targetLock=null;
    try{
      targetLock=await acquireWarReservationLock(env,war.id,'DEFENDER',defender.user_id);if(!targetLock.ok)return deps.json({error:'무작위 상대 예약 요청이 몰렸습니다. 바로 다시 시도해 주세요.',code:'CLAN_RANDOM_TARGET_BUSY'},409);
      const check=await clanWarReservationCheck(env,war,user.id,defender.user_id,settings,receipt.id);if(!check.ok)return deps.json(check,409);
      const retry=await env.DB.prepare("UPDATE clan_war_battles SET status='PENDING',error_message=NULL,updated_at=CURRENT_TIMESTAMP WHERE id=? AND status='FAILED' AND EXISTS(SELECT 1 FROM clan_wars WHERE id=? AND status='ACTIVE')").bind(receipt.id,war.id).run();if(Number(retry?.meta?.changes||0)!==1)return deps.json({error:'재시도할 공격권이 남아 있지 않습니다.'},409);receipt={...receipt,status:'PENDING',error_message:null};
    }finally{await releaseWarReservationLock(env,targetLock);await releaseWarReservationLock(env,attackerLock)}
  }
  ...
  try{
    const defenderUser={id:Number(defender.user_id),nickname:defender.nickname,role:defender.role},simulation=await buildClanBattle(env,deps,user,defenderUser,seed);
    if(!['A','B'].includes(simulation.battleV2?.result?.winner))throw new Error('전투 판정이 완료되지 않았습니다. 공격 횟수와 보상은 반영하지 않습니다.');
    const simulatedWin=simulation.battleV2.result.winner==='A',won=receipt.status==='COMPLETED'?Number(receipt.winner_clan_id)===Number(mine.clan_id):simulatedWin,winnerClan=won?Number(mine.clan_id):enemyClan;
    let participationResult=null;
    if(settings.scorePolicy==='ATTACKER_PARTICIPATION_V1'){
      participationResult=await settleClanParticipationBattle(env,{war,receipt,userId:user.id,clanId:mine.clan_id,defenderId:defender.user_id,winnerClanId:winnerClan,won,settings,result:{winner:won?'ATTACKER':'DEFENDER',reason:simulation.battleV2?.result?.reason||'',actions:Number(simulation.battleV2?.result?.actions||0)}});
    ...
    const retention=Math.max(1,Number(settings.battleReceiptRetentionDays||30));await env.DB.prepare(`DELETE FROM clan_war_battles WHERE id IN (SELECT id FROM clan_war_battles WHERE updated_at<datetime('now','-${retention} days') AND status IN ('COMPLETED','FAILED') AND war_id IN (SELECT id FROM clan_wars WHERE status IN ('COMPLETED','CANCELLED')) ORDER BY id LIMIT 200)`).run();const attacks=await env.DB.prepare("SELECT COUNT(*) count FROM clan_war_battles WHERE war_id=? AND attacker_user_id=? AND status IN ('PENDING','RESOLVING','COMPLETED')").bind(war.id,user.id).first(),attacksUsed=Number(attacks?.count||0),freshWar=await env.DB.prepare('SELECT * FROM clan_wars WHERE id=?').bind(war.id).first(),energy=clanEnergySnapshot(freshWar,attacksUsed,settings);
    return deps.json({ok:true,result:won?'WIN':'LOSE',wallet:participationResult?await clanWallet(env,user.id):undefined,battleEngine:{active:true,version:'PROJECT_V_V3',playbackSpeed:settings.playbackSpeed},battleV2:simulation.battleV2,attackerDeck:simulation.attackerDeck,defenderDeck:simulation.defenderDeck,attackerPower:simulation.attackerPower,defenderPower:simulation.defenderPower,opponent:{id:Number(defender.user_id),nickname:defender.nickname},clanWar:{id:Number(war.id),winnerClanId:winnerClan,pointsAwarded:settings.warWinScore,...(participationResult?.clanWar||{}),matchMode:settings.matchMode,deckPolicy:settings.powerSnapshot,attackLimit:settings.totalUseLimit,attacksUsed,attacksRemaining:energy.usesRemaining,energy}});
  }catch(error){
    if(claimed)await env.DB.prepare("UPDATE clan_war_battles SET status='FAILED',error_message=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND status='RESOLVING'").bind(String(error?.message||error).slice(0,240),receipt.id).run();
    const persisted=await env.DB.prepare('SELECT status FROM clan_war_battles WHERE id=?').bind(receipt.id).first();
    return deps.json({error:error.message||'클랜전 V3 전투를 구성하지 못했습니다.',requestFinalized:persisted?.status==='FAILED'},409)
  }
}
...
export async function handleClan({path,request,env,deps}){
  ...
  if(path==='admin/clan-war/settings'){
    ...
    if(request.method==='PATCH'||request.method==='POST'){
      ...
      await prepareClanParticipationSettings(env,settings,next);
  ...
  if(path==='admin/clan-war/champions-rewards'&&request.method==='POST'){
    if(!owner)return deps.json({error:'챔피언스리그 보상 지급은 OWNER만 실행할 수 있습니다.'},403);
    const body=await deps.readBody(request),seasonId=Number(body.seasonId);
    if(!Number.isSafeInteger(seasonId)||seasonId<=0||body.confirmation!=='SEND_CHAMPIONS_REWARDS')return deps.json({error:'대회 시즌과 지급 확인값이 필요합니다.'},400);
    const before=await championsPublicState(env,seasonId,settings);
    if(before.seasonId!==seasonId||before.status!=='COMPLETED')return deps.json({error:'최종 우승이 확정된 챔피언스리그만 보상을 지급할 수 있습니다.'},409);
    try{await deliverChampionsRewards(env,seasonId,settings,{configurePending:true})}catch(error){return deps.json({error:error.message},409)}
    const after=await championsPublicState(env,seasonId,settings);
    if(deps.writeAdminLog)await deps.writeAdminLog(env,user,'CLAN_CHAMPIONS_REWARDS','CLAN_SEASON',String(seasonId),before,after);
    return deps.json(await clanAdminState(env,settings));
  }
...
export const __clanTest={startScheduledRedraft,activateSeason,clanDraftDeadlineMs,nextDraftCheckAt,createSeason,finishDraftAtDeadline,beginDraft,autoDraftDue,skipExpiredDraftTurns,makeDraftPick,draftContext,nextDraftTurn,draftTurnMs,settleSeason,advanceLifecycle,fight,normalizeScores,currentDraftPosition,roundRobinRounds,scheduledWindowStarts,seoulDayTimestamp,clanLateDraftFixedSchedule,clanEnergySnapshot,randomMatchCandidates,cleanRole,isOwner,publicSeason,clanRegistrationOpen,clanLateRegistrationSchedule,warWinnerClanId,cleanClanAdminSettings,clanAdminState,CLAN_ADMIN_SETTINGS_DEFAULTS,CLAN_MAX_MEMBERS,CLAN_MAX_PARTICIPANTS,CLAN_LATE_REGISTRATION_EXTENSION_MS,CLAN_LATE_DRAFT_HOUR_KST,CLAN_WAR_OPEN_HOUR_KST,CLAN_ATTACKS_PER_WAR,CLAN_DEFENSES_PER_TARGET,CLAN_REPEAT_TARGET_LIMIT,CLAN_MARKS,OFFICIAL_CLAN_CATALOG,FOUNDATION_SQL};
