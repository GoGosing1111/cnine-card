...
async function payClanSeasonRewards(env,season,settings,rankedTeams){
  ...
  for(const receipt of pending){const writes=[];if(env.DB.dialect==='postgres')writes.push(env.DB.prepare('SELECT id FROM users WHERE id=? FOR UPDATE').bind(receipt.user_id));writes.push(...await pigCoinRewardStatements(env,{userId:Number(receipt.user_id),source:'CLAN',referenceId:String(season.id),guardSql:"EXISTS(SELECT 1 FROM clan_reward_receipts WHERE season_id=? AND user_id=? AND status='PENDING')",guardBindings:[season.id,receipt.user_id]}));writes.push(env.DB.prepare("UPDATE users SET coin=coin+?,card_shards=card_shards+? WHERE id=? AND EXISTS(SELECT 1 FROM clan_reward_receipts WHERE season_id=? AND user_id=? AND status='PENDING')").bind(receipt.coin,receipt.card_shards,receipt.user_id,season.id,receipt.user_id));writes.push(env.DB.prepare("UPDATE clan_reward_receipts SET status='COMPLETED',completed_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE season_id=? AND user_id=? AND status='PENDING'").bind(season.id,receipt.user_id));await env.DB.batch(writes)}
...
export const __clanTest={settleSeason,advanceLifecycle,fight,normalizeScores,currentDraftPosition,roundRobinRounds,scheduledWindowStarts,seoulDayTimestamp,clanLateDraftFixedSchedule,clanEnergySnapshot,randomMatchCandidates,cleanRole,isOwner,publicSeason,clanRegistrationOpen,clanLateRegistrationSchedule,warWinnerClanId,cleanClanAdminSettings,clanAdminState,CLAN_ADMIN_SETTINGS_DEFAULTS,CLAN_MAX_MEMBERS,CLAN_MAX_PARTICIPANTS,CLAN_LATE_REGISTRATION_EXTENSION_MS,CLAN_LATE_DRAFT_HOUR_KST,CLAN_WAR_OPEN_HOUR_KST,CLAN_ATTACKS_PER_WAR,CLAN_DEFENSES_PER_TARGET,CLAN_REPEAT_TARGET_LIMIT,CLAN_MARKS,OFFICIAL_CLAN_CATALOG,FOUNDATION_SQL};
