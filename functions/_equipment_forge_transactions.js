...
import {forgeRuntimeDraft,validateForgePolicy,FORGE_RUNTIME_KEY,forgePower} from '../shared/equipment-forge-policy-v1.mjs';
...
export async function executeForge(env,user,body,kind,{randomInt=mercenaryRandomInt,now=Date.now()}={}){
 ...
 const r=await runJointOperation(env,user,{requestId,kind:`FORGE_${kind}`,input:{quoteId},prepare:async()=>{
   allow(await readForgeRuntime(env),user);const plan=await quotePlan(env,user,quoteId,kind,now);
   ...
   for(const [code,quantity]of[[plan.cost.itemCode,plan.cost.itemQuantity],...(plan.protectedAttempt?[[plan.protection.itemCode,plan.cost.protectionQuantity]]:[])])if(code){const row=await env.DB.prepare('SELECT quantity FROM cnine_user_inventory WHERE user_id=? AND item_code=?').bind(user.id,code).first();if(Number(row?.quantity||0)<quantity)throw jointError('FORGE_MATERIAL','강화 재료 또는 보호권이 부족합니다.',409);}
...
export async function forgeAccountState(env,user,options){
 ...
 const available=policy.mode==='ON'||policy.mode==='TEST'&&user.role==='OWNER';const items=inventory.items.map(item=>{const g=growth.results.find(g=>String(g.instance_id)===item.instanceId),level=Number(g?.level||0);return {...item,enhancement:{level,revision:Number(g?.revision||0),power:forgePower(item.basePower.total,level)}};});
