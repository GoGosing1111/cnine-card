...
export async function onRequest(context){
  ...
  try{
    ...
    const hotRequestV1407=path==='auth/login'||path==='auth/logout'||path==='cards'||path==='packs'||path==='draw'||path==='draw/status'||path==='burning-event/status'||path==='user/runtime-command'||path==='me/summary'||path==='shell/summary'||path==='raid/status'||path.startsWith('territory-war/');
    ...
    const runtimeGateBypassV1407=path==='auth/login'||path==='auth/logout'||path==='cards'||path==='packs'||path==='draw'||path==='draw/status'||path==='burning-event/status'||path==='user/runtime-command'||path.startsWith('territory-war/');
    ...
    if(path==='pvp/fight'&&request.method==='POST'){
      ...
      return json({result:attackerWin?'WIN':'LOSE',burningEvent:burningPublicState(burning),battleEngine:engineState,battleV2,cubeReward,weeklyPremiumCube,magicReward,equipmentReward,attackerCharacterBonus:aCharacterBonus,defenderCharacterBonus:dCharacterBonus,attackerCardPower:aCardPower,defenderCardPower:dCardPower,scoreChange:attackerWin?change:-change,scoreAfter:aAfter,coinReward:attackerCoinReward,coinAfter:freshCoinUser?.coin??coinUser.coin,magicCrystalsAfter:Number(freshCoinUser?.magic_crystals||0),rewardRecipient:'ATTACKER',attackerPower:aPower,defenderPower:dPower,attackerTitle:titleMap[String(user.id)]||null,defenderTitle:titleMap[String(defenderId)]||null,opponent:defUser.nickname,attackerDeck:aUnique.cards||aDeck,defenderDeck:dUnique.cards||dDeck,uniqueAbility:{attacker:uniqueBattleResponsePayload(aUnique,aUniqueRuntime),defender:uniqueBattleResponsePayload(dUnique,dUniqueRuntime)},scoreAdjustment:aAdj,opponentScoreAdjustment:dAdj,energy:pvpEnergy,serverNow:new Date().toISOString()});
    }
    if(path==='pvp/history'){
