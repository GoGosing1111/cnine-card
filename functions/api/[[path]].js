...
import {ensureChuseok,handleChuseok} from '../_chuseok.js';
...
async function safePveUnifiedDrop(env,payload){
  ...
  if(payload.isApocalypse){
    const apocalypseSource=sourceType==='PVE_AUTO'?'PVE_APOCALYPSE_AUTO':'PVE_APOCALYPSE';
    const dedicated=await safeUnifiedDrop(env,{...payload,sourceType:apocalypseSource});
...
const BATTLE_POWER_DEFAULT={C:100,U:160,R:250,SR:400,HR:620,UR:900,SSR:1300,MA:1850,LIMITED:2800,PRESTIGE:3100,FUR:3200,ZENITH:5500,SUPERSTAR:15500};
const BATTLE_BREAKTHROUGH_DEFAULT=[0,18,42,72,108,150,198,252,312,378,450,528,612,702];
const MA_MASTER_STAR_BREAKTHROUGH_DEFAULT={enabled:true,steps:[{cost:1,duplicateCards:0,rate:85,retirementShardRefund:3000},{cost:3,duplicateCards:0,rate:50,retirementShardRefund:4000},{cost:5,duplicateCards:0,rate:25,retirementShardRefund:5000}]};
const LIMITED_MASTER_STAR_BREAKTHROUGH_DEFAULT={enabled:true,steps:[{cost:5,duplicateCards:0,rate:50,retirementShardRefund:3000},{cost:10,duplicateCards:0,rate:30,retirementShardRefund:4000},{cost:15,duplicateCards:0,rate:20,retirementShardRefund:5000}]};
let maMasterStarBreakthroughCache=null;
let limitedMasterStarBreakthroughCache=null;
// V1802 / V1940: FUR / ZENITH / SUPERSTAR +11~+13 고급 강화.
// 재료는 실패해도 돌려주지 않는다(재화 회수 목적). 대신 단계마다 천장을 둬 무한 실패를 막는다.
// duplicateCards = 같은 카드 중복분을 몇 장 태우는지. FUR 는 1장, ZENITH 는 기본 0장(전수조사 후 조정).
// uniqueBoostPercent = 그 단계에서 고유효과 수치를 몇 % 끌어올리는지.
const FUR_MASTER_STAR_BREAKTHROUGH_DEFAULT={enabled:false,steps:[{cost:100,duplicateCards:1,rate:35,pityThreshold:3,uniqueBoostPercent:30,retirementShardRefund:6000},{cost:150,duplicateCards:1,rate:25,pityThreshold:4,uniqueBoostPercent:60,retirementShardRefund:8000},{cost:200,duplicateCards:1,rate:15,pityThreshold:6,uniqueBoostPercent:100,retirementShardRefund:10000}]};
...
const ALL_LEVEL_MASTER_STAR_GRADES=['ZENITH','SUPERSTAR'];
...
function raidRoomListRows(env){
  return raidMemo('rooms',2000,async()=>{
    const rows=await env.DB.prepare("SELECT ri.id,ri.status,ri.starts_at AS startsAt,ri.ends_at AS endsAt,(SELECT COUNT(*) FROM raid_participants rp2 WHERE rp2.instance_id=ri.id AND COALESCE(rp2.is_active,1)=1) AS participantCount,rb.name AS bossName,rb.image_url AS bossImage,rb.max_hp AS maxHp,COALESCE(x.slot_id,'LEGACY') AS slotId,x.settings_json AS settingsJson FROM raid_instances ri JOIN raid_bosses rb ON rb.id=ri.boss_id LEFT JOIN raid_instance_v1293 x ON x.instance_id=ri.id WHERE ri.status IN ('LOBBY','BATTLE') ORDER BY ri.id DESC LIMIT 10").all();
    return rows.results||[];
  });
}
function raidWeeklyBossRows(env){
  return raidMemo('weekly-bosses-v2141',30000,()=>readWeeklyRaidCms(env));
}
async function raidWeeklyToday(env,nowMs=Date.now(),fresh=false){
  const weekly=await (fresh?readWeeklyRaidCms(env):raidWeeklyBossRows(env)),boss=weeklyRaidBossForKst(nowMs,weekly.config.rotation,weekly.bosses);
  if(!boss?.id)throw new Error('요일 레이드 보스 등록을 찾지 못했습니다.');
  return {boss,rows:weekly.days};
}
function raidParticipantRows(env,instanceId){
  return raidMemo(`participants:${instanceId}`,1000,async()=>{
    const rows=await env.DB.prepare(`SELECT rp.user_id AS userId,u.nickname,rp.deck_cards AS deckCards,rp.total_power AS totalPower,rp.total_damage AS totalDamage,rp.reward_claimed AS rewardClaimed,rp.joined_at AS joinedAt,
        t.id AS titleId,t.name AS titleName,t.badge_text AS titleBadgeText,t.style_preset AS titleStylePreset,t.unlock_config_json AS titleUnlockConfig
...
const COUPON_REWARD_MAX={COIN:10000000000,MASTER_STAR:1000000,PREMIUM_CUBE:100000,EQUIPMENT_SUPPLY_BOX:100000,HIGH_GRADE_REROLL_TICKET:100000,CHUSEOK_COIN:100000};
function couponRewardSpec(value){const type=String(value||'').trim().toUpperCase(),spec=type==='CHUSEOK_COIN'?{type,label:'추석 코인',inventory:true}:verifiedMessageRewardSpec(type);return spec&&!spec.messageOnly?{...spec,max:Number(COUPON_REWARD_MAX[spec.type]||spec.max)}:null}
...
function chiefBurningEndsAt(startAt,durationMinutes){const startMs=Date.parse(String(startAt||'')),minutes=Number(durationMinutes);if(!Number.isFinite(startMs)||!Number.isInteger(minutes)||minutes<1||minutes>1440)throw new TypeError('족장 버닝 고정 시간이 올바르지 않습니다.');return new Date(startMs+minutes*60000).toISOString();}
...
async function pvpChallengerRank(env,userId,{fresh=false}={}){
  ...
  const rows=await env.DB.prepare(`SELECT u.id FROM pvp_profiles p JOIN users u ON u.id=p.user_id
    ...
    ORDER BY p.season_score DESC,p.wins DESC,u.nickname,u.id LIMIT 20`).all();
...
// OWNER는 비경쟁 운영 계정으로 계속 제외한다. ADMIN은 CMS 권한만 제한하며
// 일반 플레이에서는 다른 유저와 동일하게 매칭·랭킹·시즌 정산 대상이다.
const PVP_RANKED_ROLE_SQL="UPPER(TRIM(COALESCE(u.role,'USER'))) <> 'OWNER'";
...
async function pvpSettlementPreviewRows(env,settings){
  const [rankedRows,tierClaims,rankClaims]=await Promise.all([
    env.DB.prepare(`SELECT u.id AS user_id,u.nickname,p.season_score,p.highest_score,p.wins,p.losses FROM pvp_profiles p JOIN users u ON u.id=p.user_id
      WHERE u.status='ACTIVE' AND ${PVP_RANKED_ROLE_SQL} AND (u.banned_until IS NULL OR u.banned_until<=datetime('now'))
      ORDER BY p.season_score DESC,p.wins DESC,u.nickname,u.id`).all(),
...
async function deliverAutomaticPvpRewards(env,settlementId,settings){
  ...
  for(const row of rows.results||[])for(const [rewardType,amount] of [['COIN',Number(row.reward_coin||0)],['SHARDS',Number(row.reward_shards||0)]]){
    ...
    const unit=rewardType==='COIN'?'코인':'카드조각',seasonTitle=rewardType==='COIN'?(row.tier_id==='challenger'?'챌린저★★★★':row.tier_id==='grandmaster'?'승부사':row.tier_id==='master'?'결투가':''):'',title=`${settings.seasonName} 랭크전 정산 보상`,body=`${settings.seasonName} 최종 ${row.final_rank}위 (${row.tier_name}) 정산 보상입니다.\n\n${unit} ${amount.toLocaleString()}개${seasonTitle?`\n기간제 칭호 '${seasonTitle}' (다음 시즌 정산까지 5일간 활성)`:''}\n\n아래 보상 수령 버튼을 눌러주세요.`;
    ...
    if(status==='MESSAGES_READY'){
      ...
      await env.DB.batch([
        env.DB.prepare(`INSERT OR IGNORE INTO pvp_season_title_grants_v1671(settlement_id,user_id,title_id,season_key,expires_at)
          SELECT r.settlement_id,r.user_id,t.id,?,? FROM pvp_season_settlement_ranks r JOIN character_titles t ON t.code=CASE r.tier_id WHEN 'challenger' THEN 'TITLE_RANKED_CHALLENGER' WHEN 'grandmaster' THEN ? WHEN 'master' THEN ? ELSE '' END
          WHERE r.settlement_id=? AND r.tier_id IN ('challenger','grandmaster','master')`).bind(seasonKey,expiresAt,PVP_RANKED_TITLE_CODES.grandmaster,PVP_RANKED_TITLE_CODES.master,sid),
...
async function createRankedMatchTicket(env,user,settings){
  ...
  const searchRange=1000000,[recentRows,candidates]=await Promise.all([
    ...
    WHERE u.id<>? AND u.status='ACTIVE' AND ${PVP_RANKED_ROLE_SQL}
  ...
  return {token,expiresAt,opponent:{id:Number(selected.id),nickname:selected.nickname,season_score:Number(selected.season_score),wins:Number(selected.wins||0),losses:Number(selected.losses||0),title:titleMap[String(selected.id)]||null,tier:resolvePvpTier(Number(selected.season_score),settings,await pvpChallengerRank(env,selected.id)),expectedWin:winPreview.change,expectedLoss:lossPreview.change,balance:{tierDistance:selected.tierDistance,scoreDifference:selected.scoreDiff,powerDifferencePercent:Math.round(selected.powerDiff*10)/10}}};
}
...
function superstarDeckCount(cards=[]){return cards.filter(card=>String(card?.rarity||card?.grade||'').trim().toUpperCase()==='SUPERSTAR').length}
function deckRulesContract(scope='PVE'){
  return {
    schemaVersion:1,
    scope:String(scope||'PVE').toUpperCase()==='PVP'?'PVP':'PVE',
    deckSize:5,
    gradeLimits:{PRESTIGE:PRESTIGE_DECK_LIMIT,FUR:FUR_DECK_LIMIT,ZENITH:ZENITH_DECK_LIMIT,SUPERSTAR:SUPERSTAR_DECK_LIMIT},
...
async function deckGradeCounts(env,cardIds=[]){const ids=[...new Set((cardIds||[]).map(String).filter(Boolean))];if(!ids.length)return {prestigeCount:0,furCount:0,zenithCount:0,superstarCount:0};const marks=ids.map(()=>'?').join(',');const row=await env.DB.prepare(`SELECT SUM(CASE WHEN UPPER(rarity)='PRESTIGE' THEN 1 ELSE 0 END) prestige_count,SUM(CASE WHEN UPPER(rarity)='FUR' THEN 1 ELSE 0 END) fur_count,SUM(CASE WHEN UPPER(rarity)='ZENITH' THEN 1 ELSE 0 END) zenith_count,SUM(CASE WHEN UPPER(rarity)='SUPERSTAR' THEN 1 ELSE 0 END) superstar_count FROM cards_effective_v1210 WHERE id IN (${marks})`).bind(...ids).first();return {prestigeCount:Math.max(0,Number(row?.prestige_count||0)),furCount:Math.max(0,Number(row?.fur_count||0)),zenithCount:Math.max(0,Number(row?.zenith_count||0)),superstarCount:Math.max(0,Number(row?.superstar_count||0))}}
async function validateDeckGradeLimits(env,cardIds=[],deckName='덱'){const {prestigeCount,furCount,zenithCount,superstarCount}=await deckGradeCounts(env,cardIds);if(prestigeCount>PRESTIGE_DECK_LIMIT){const error=new Error(`${deckName}에는 PRESTIGE 카드를 최대 ${PRESTIGE_DECK_LIMIT}장까지만 편성할 수 있습니다.`);error.status=400;error.code='PRESTIGE_DECK_LIMIT';error.grade='PRESTIGE';error.count=prestigeCount;error.limit=PRESTIGE_DECK_LIMIT;throw error}if(furCount>FUR_DECK_LIMIT){const error=new Error(`${deckName}에는 FUR 카드를 최대 ${FUR_DECK_LIMIT}장까지만 편성할 수 있습니다.`);error.status=400;error.code='FUR_DECK_LIMIT';error.grade='FUR';error.count=furCount;error.limit=FUR_DECK_LIMIT;throw error}if(zenithCount>ZENITH_DECK_LIMIT){const error=new Error(`${deckName}에는 ZENITH 카드를 최대 ${ZENITH_DECK_LIMIT}장까지만 편성할 수 있습니다.`);error.status=400;error.code='ZENITH_DECK_LIMIT';error.grade='ZENITH';error.count=zenithCount;error.limit=ZENITH_DECK_LIMIT;throw error}if(superstarCount>SUPERSTAR_DECK_LIMIT){const error=new Error(`${deckName}에는 SUPERSTAR 카드를 최대 ${SUPERSTAR_DECK_LIMIT}장까지만 편성할 수 있습니다. 덱을 다시 저장해주세요.`);error.status=400;error.code='SUPERSTAR_DECK_LIMIT';error.grade='SUPERSTAR';error.count=superstarCount;error.limit=SUPERSTAR_DECK_LIMIT;throw error}return {prestigeCount,furCount,zenithCount,superstarCount}}
...
async function refreshRaidForOwner(env,instance,cfg){
  ...
  if(cfg.bossProfile)instance.max_hp=cfg.bossProfile.maxHp;
...
async function ensureRaidFinalizedV1293(env,instanceId,fallbackCfg,nowMs=Date.now()){
  ...
  if(!finalized)await finalizeRaidV1293(env,instance.id,snapshot);
  return {instance,cfg,snapshot,alreadyFinalized:Boolean(finalized)};
}
...
async function raidDeckPower(env,userId,cardIds,mode='RAID'){
  ...
  if(zenithCount>ZENITH_DECK_LIMIT){const e=new Error(`PvE 덱에는 ZENITH 카드를 최대 ${ZENITH_DECK_LIMIT}장까지만 편성할 수 있습니다.`);e.status=400;e.code='ZENITH_DECK_LIMIT';throw e}
  if(superstarDeckCount(owned.results)>SUPERSTAR_DECK_LIMIT){const e=new Error(`PvE 덱에는 SUPERSTAR 카드를 최대 ${SUPERSTAR_DECK_LIMIT}장까지만 편성할 수 있습니다. 덱을 다시 저장해주세요.`);e.status=400;e.code='SUPERSTAR_DECK_LIMIT';throw e}
...
function normalizeMonsterPveTab(value,fallback='NORMAL'){
  const key=String(value||fallback).trim().toUpperCase();
  return ({ELITE:'HARD',BOSS:'HELL',EVENT:'HELL'})[key]||(['NORMAL','HARD','HELL','NIGHTMARE','APOCALYPSE'].includes(key)?key:'NORMAL');
}
function defaultBattleSettings(){return {enabled:true,deckSize:5,engine:normalizeBattleEngineSettings(),nightmare:normalizeNightmareSettings(),apocalypse:normalizeApocalypseSettings(),powerByGrade:{...BATTLE_POWER_DEFAULT},breakthroughBonus:[...BATTLE_BREAKTHROUGH_DEFAULT],highBreakthroughBonus:{FUR:[...HIGH_BREAKTHROUGH_BONUS_DEFAULT.FUR],ZENITH:[...HIGH_BREAKTHROUGH_BONUS_DEFAULT.ZENITH],SUPERSTAR:[...HIGH_BREAKTHROUGH_BONUS_DEFAULT.ZENITH]},cardDrop:{enabled:true,defaultRate:3,gradeRates:{C:40,U:25,R:15,SR:10,HR:6,UR:3,SSR:1,MA:0,FUR:0}},energy:{enabled:true,maxEnergy:10,dailyRestore:10,rechargeMinutes:15,costPerBattle:1,adminUnlimited:true,testUnlimited:true},ultimateRules:[{enabled:true,name:'SSR AWAKENING',requiredGrade:'SSR',minBreakthrough:5,requiredCount:1,activationChance:100,mediaUrl:'/assets/effects/SKILL-v1497.webp',durationMs:3000,coefficientPercent:500}]};}
function cleanBattleSettingsPayload(x={},base=defaultBattleSettings()){
  const highBreakthroughBonus=Object.fromEntries(Object.keys(base.highBreakthroughBonus).map(g=>[g,base.highBreakthroughBonus[g].map((v,i)=>Math.max(0,Math.min(1000000,Number(x.highBreakthroughBonus?.[g]?.[i]??v))))]));
  // 강화 재료/고유효과 표와의 호환은 유지한다. 실제 SUPERSTAR 전투력은 cardBattlePower에서 등급 비교 후 가산한다.
  highBreakthroughBonus.SUPERSTAR=[...highBreakthroughBonus.ZENITH];
  const powerByGrade=Object.fromEntries(Object.keys(base.powerByGrade).map(g=>[g,Math.max(0,Math.floor(Number(x.powerByGrade?.[g]??base.powerByGrade[g])))]));
  powerByGrade.SUPERSTAR=Math.max(powerByGrade.FUR,powerByGrade.ZENITH)+10000;
...
function pveBattleEngineState(settings,user,characterBonus={}){
  ...
  return {...base,active,version:active?'V2':'LEGACY',ownerTest:battleSuitLiveOverride?false:base.ownerTest,battleSuitLiveOverride,activationScope:battleSuitLiveOverride?'PVE_BATTLE_SUIT_ONLY':base.active?'BATTLE_ENGINE_SETTING':'NONE',battleSuitLive};
}
const CARD_POWER_TYPES={SSR:{NORMAL:1300,HIGH:1375,TOP:1450},MA:{NORMAL:1850,HIGH:2050,TOP:2250},LIMITED:{NORMAL:2350,HIGH:2600,TOP:2850},PRESTIGE:{CUSTOM:3100},FUR:{FIXED:3200},ZENITH:{FIXED:5500},SUPERSTAR:{FIXED:15500}};
...
async function consumeApocalypseEnergy(env,user,maintenanceOverride=null){
  ...
  if(state.energy<state.costPerBattle){const error=new Error(`아포칼립스 행동력이 부족합니다. ${state.rechargeMinutes}분마다 1회 충전됩니다.`);error.code='NO_APOCALYPSE_ENERGY';error.energy=state;throw error;}
  const nowSql=sqlUtcNow(),result=await env.DB.prepare('UPDATE user_apocalypse_energy SET energy=energy-?,last_recharged_at=CASE WHEN energy>=? THEN ? ELSE last_recharged_at END,updated_at=CURRENT_TIMESTAMP WHERE user_id=? AND energy>=?').bind(state.costPerBattle,state.maxEnergy,nowSql,user.id,state.costPerBattle).run();
  if(!result.meta.changes){const error=new Error('아포칼립스 행동력이 부족합니다.');error.code='NO_APOCALYPSE_ENERGY';error.energy=await apocalypseEnergyState(env,user,maintenanceOverride);throw error;}
  return apocalypseEnergyState(env,user,maintenanceOverride);
}
async function pveEnergyStateForDifficulty(env,user,settings,difficulty,maintenanceOverride=null){return difficulty?.isApocalypse?apocalypseEnergyState(env,user,maintenanceOverride):battleEnergyState(env,user,settings,maintenanceOverride)}
async function consumePveEnergyForDifficulty(env,user,settings,difficulty,maintenanceOverride=null){return difficulty?.isApocalypse?consumeApocalypseEnergy(env,user,maintenanceOverride):consumeBattleEnergy(env,user,settings)}
...
async function highBreakthroughConfigs(env){
  return cachedRuntimeSetting('highBreakthroughConfigs',30000,async()=>{
    ...
    const FUR=cleanFurMasterStarBreakthrough(pick('fur_master_star_breakthrough_v1802'));
    const ZENITH=cleanZenithMasterStarBreakthrough(pick('zenith_master_star_breakthrough_v1802'));
    return {FUR,ZENITH,SUPERSTAR:ZENITH};
  });
}
...
const CORS_HEADERS={'access-control-allow-origin':'*','access-control-allow-methods':'GET,POST,PATCH,PUT,DELETE,OPTIONS','access-control-allow-headers':'authorization,content-type,x-cnine-draw-receipt,x-cnine-auto-draw,x-cnine-draw-client,x-cnine-client-id,x-cnine-d1-bookmark','access-control-allow-credentials':'false','access-control-expose-headers':'x-cnine-response-ms,x-cnine-d1-bookmark,x-cnine-db-backend','access-control-max-age':'86400'};
...
// V1996: 한 Worker 요청에서 수십 회를 직렬 처리하면 브라우저의 요청 제한보다
// 오래 걸릴 수 있다. 클라이언트가 짧은 묶음으로 이어서 요청하고, 서버도 구버전
...
function pveSweepReceiptState(row,now=Date.now()){
...
async function pveSweepReceiptResponse(env,user,requestId,row,options={}){
  ...
  if(Number(row.user_id)!==Number(user.id))return json({error:'잘못된 소탕 요청입니다.'},403);
  const state=pveSweepReceiptState(row);
  if(state==='COMPLETED'&&row.response_json){
    try{
      const replay=JSON.parse(row.response_json);
      if(String(replay?.difficulty||'').toUpperCase()==='APOCALYPSE')return json({error:'아포칼립스는 소탕할 수 없습니다.',code:'PVE_SWEEP_APOCALYPSE_EXCLUDED'},400);
      return json({...replay,requestId:String(replay?.requestId||requestId),replayed:true});
    }catch(error){
      console.error('pve sweep completed receipt parse failed',error);
      return json({error:'완료된 소탕 결과를 불러오지 못했습니다.',code:'PVE_SWEEP_RECEIPT_INVALID',status:'COMPLETED',requestId,retryable:false},503);
    }
  }
  if(state==='RUNNING'){
    if(options.legacyRunningError)return json({error:'동일한 소탕 요청이 이미 처리 중입니다. 새 소탕 복구 버전을 받으려면 게임을 새로고침해 주세요.',code:'PVE_SWEEP_RUNNING',status:'RUNNING',requestId,retryable:true,retryAfterMs:PVE_SWEEP_RETRY_AFTER_MS},409,{'Retry-After':'2'});
    return json({ok:true,mode:'PVE_SWEEP_STATUS',status:'RUNNING',code:'PVE_SWEEP_RUNNING',requestId,retryable:true,retryAfterMs:PVE_SWEEP_RETRY_AFTER_MS,updatedAt:row.updated_at||null},202,{'Retry-After':'2'});
  }
  if(state==='STALE'){
    const stale=await env.DB.prepare("UPDATE pve_auto_runs SET status='FAILED',error_message='STALE_RUNNING_RECOVERED',updated_at=CURRENT_TIMESTAMP WHERE request_id=? AND user_id=? AND status='RUNNING' AND updated_at=?").bind(requestId,user.id,row.updated_at).run();
    if(Number(stale?.meta?.changes||0)===0){
      const refreshed=await env.DB.prepare('SELECT user_id,status,response_json,error_message,created_at,updated_at FROM pve_auto_runs WHERE request_id=?').bind(requestId).first();
      return pveSweepReceiptResponse(env,user,requestId,refreshed,options);
    }
    await env.DB.prepare('DELETE FROM pve_auto_locks WHERE user_id=? AND request_id=?').bind(user.id,requestId).run();
    console.warn('PVE_SWEEP_STALE_RECEIPT',JSON.stringify({userId:user.id,requestId,updatedAt:row.updated_at||null}));
    return json({error:'이전 소탕 처리가 중단되었습니다. 지급된 보상과 사용한 행동력은 보존되며, PVE 화면에서 남은 행동력으로 다시 소탕해 주세요.',code:'PVE_SWEEP_STALE',status:'FAILED',requestId,retryable:false},409);
  }
  if(state==='FAILED'){
    await env.DB.prepare('DELETE FROM pve_auto_locks WHERE user_id=? AND request_id=?').bind(user.id,requestId).run();
    console.warn('PVE_SWEEP_FAILED_RECEIPT',JSON.stringify({userId:user.id,requestId,error:String(row.error_message||'').slice(0,160)}));
    return json({error:'이전 소탕 처리가 완료되지 않았습니다. 지급된 보상과 사용한 행동력은 보존됩니다.',code:'PVE_SWEEP_FAILED',status:'FAILED',requestId,retryable:false},409);
  }
  return json({error:'소탕 요청 상태를 확인할 수 없습니다.',code:'PVE_SWEEP_STATE_INVALID',status:state,requestId,retryable:false},409);
}
...
async function ensureUpgrades(env){
  ...
  upgradePromise=(async()=>{
    ...
    if(unlimitedLimitedDone?.value!=='1'){
      await env.DB.batch([
        ...
        env.DB.prepare(`INSERT INTO inventory_items(code,name,subtitle,description,category,rarity,image_url,sort_order,is_active)
          VALUES('HIGH_GRADE_REROLL_TICKET','고등급 재뽑기권','HIGH GRADE REROLL TICKET','PRESTIGE·LIMITED·FUR·ZENITH 카드 1장을 같은 등급의 다른 고유효과 특성 카드로 재뽑습니다.','REROLL','SPECIAL','',109,1)
        ...
        env.DB.prepare("INSERT OR REPLACE INTO app_meta(key,value,updated_at) VALUES('high_grade_reroll_settings_v1',?,CURRENT_TIMESTAMP)").bind(JSON.stringify({enabled:true,dexButtonEnabled:false,prestigeEnabled:true,limitedEnabled:true,furEnabled:true,zenithEnabled:true})),
    ...
    if(pveDifficultyDone?.value!=='1'){
      if(await columnExists(env,'battle_monsters','pve_tab')){
        await env.DB.prepare("UPDATE battle_monsters SET pve_tab=CASE UPPER(COALESCE(pve_tab,'')) WHEN 'NORMAL' THEN 'NORMAL' WHEN 'HARD' THEN 'HARD' WHEN 'HELL' THEN 'HELL' WHEN 'NIGHTMARE' THEN 'NIGHTMARE' WHEN 'APOCALYPSE' THEN 'APOCALYPSE' WHEN 'ELITE' THEN 'HARD' WHEN 'BOSS' THEN 'HELL' WHEN 'EVENT' THEN 'HELL' ELSE 'NORMAL' END").run();
      }
...
function dailyQuestAdminExcluded(user,settings){
  ...
  const operatingAccount=role==='ADMIN'||role==='OWNER';
  return operatingAccount&&settings.adminTestAllowed===false;
}
...
async function liveOperationAlerts(env){
  ...
  try{
    const [result,coup,duo]=await Promise.all([env.DB.prepare(`WITH
    territory AS (
    ...
    UNION ALL SELECT * FROM auction UNION ALL SELECT * FROM raid ORDER BY sort_order`).all(),coupLiveOperation(env,now).catch(error=>{console.error('Coup lobby summary unavailable',error?.message);return null;}),rankedDuoLiveOperation(env,now).catch(error=>{console.error('Duo lobby summary unavailable',error?.message);return null;})]);
...
function canUseTestAccess(user,maintenance){return Boolean(user&&!isDedicatedPredictionAdmin(user)&&maintenance?.active===true&&maintenance?.testUsers?.includes(user.nickname))}
...
function userMutationLeaseMs(actionPath){
  ...
  const fastBattleAction=['battle/fight','tower/fight','escort/fight','pvp/match','pvp/fight','clan/war/fight'].includes(actionPath);
...
let prisonFoundationPromise=null;
function prisonSchemaStatements(env){
  const postgres=env.DB?.dialect==='postgres';
  const userIdType=postgres?'BIGINT':'INTEGER';
  const chatId=postgres?'BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY':'INTEGER PRIMARY KEY AUTOINCREMENT';
  const nowDefault=postgres?"to_char(timezone('UTC',CURRENT_TIMESTAMP),'YYYY-MM-DD HH24:MI:SS')":'CURRENT_TIMESTAMP';
  return [
    `CREATE TABLE IF NOT EXISTS user_prison_status (
      user_id ${userIdType} PRIMARY KEY,active INTEGER NOT NULL DEFAULT 0,reason TEXT NOT NULL DEFAULT '',
      jailed_by ${userIdType},jailed_at TEXT,jailed_until TEXT,released_by ${userIdType},released_at TEXT,
      release_reason TEXT NOT NULL DEFAULT '',updated_at TEXT NOT NULL DEFAULT ${nowDefault})`,
    'CREATE INDEX IF NOT EXISTS idx_user_prison_active_until ON user_prison_status(active,jailed_until)',
    `CREATE TABLE IF NOT EXISTS prison_chat_messages (
      id ${chatId},user_id ${userIdType} NOT NULL,body TEXT NOT NULL,
      sender_was_incarcerated INTEGER NOT NULL DEFAULT 0,created_at TEXT NOT NULL DEFAULT ${nowDefault})`,
    'CREATE INDEX IF NOT EXISTS idx_prison_chat_recent ON prison_chat_messages(id DESC,user_id)'
  ];
}
async function ensurePrisonFoundation(env){
  if(prisonFoundationPromise)return prisonFoundationPromise;
  prisonFoundationPromise=(async()=>{
    const [statusReady,chatReady]=await Promise.all([
...
async function handleRequest(context){
  ...
  try{
    ...
    if(path==='health'){
      ...
      if(databaseInitialized){
        ...
        targetedCardTransfer=transfer?{
          ...
          sourceCardId:transfer.source?.cardId||null,targetCardId:transfer.target?.cardId||null
        }:null;
        await ensureAvatarFoundation(env);
        const avatarGrant=await ensureTargetedAvatarGrantV2007(env);
        targetedAvatarGrant=avatarGrant?{
        ...
        const deodeumiguAvatarGrant=await ensureTargetedAvatarGrantDeodeumiguV1(env);
        targetedAvatarGrantDeodeumiguV1=deodeumiguAvatarGrant?{
        ...
        const xsudaengHighGradeRerollGrant=await ensureTargetedInventoryGrantV2026(env);
        targetedInventoryGrantV2026=xsudaengHighGradeRerollGrant?{
          status:xsudaengHighGradeRerollGrant.status,version:xsudaengHighGradeRerollGrant.version,replayed:Boolean(xsudaengHighGradeRerollGrant.replayed),
        ...
        const skillChipGrant=await ensureTargetedSkillChipGrantV2055(env);
        targetedSkillChipGrantV2055=skillChipGrant?{
          status:skillChipGrant.status,version:skillChipGrant.version,replayed:Boolean(skillChipGrant.replayed),
          permanent:Boolean(skillChipGrant.permanent),accountCount:Number(skillChipGrant.accountCount||0),
    ...
    const chuseokResponse=await handleChuseok({path,request,env,deps:{authenticate,readBody,json,requirePermission}});if(chuseokResponse)return chuseokResponse;
    ...
    if(path==='card/breakthrough/auto'&&request.method==='POST'){
      const user=await authenticate(request,env);if(!user)return json({error:'로그인이 필요합니다.'},401);
      const payload=await readBody(request),cardId=String(payload.cardId||'').trim(),requestId=String(payload.requestId||'').trim(),maxAttempts=Math.max(1,Math.min(50,Math.floor(Number(payload.maxAttempts)||50))),requestedTargetLevel=Math.max(0,Math.floor(Number(payload.targetLevel)||0));
      ...
      // 스키마 변경 없이 card_id 칸에 불변 요청 옵션까지 묶는다. 같은 requestId를
      // 다른 목표/묶음 크기로 재사용해 이전 결과를 잘못 재생하는 일을 차단한다.
      const receiptFingerprint=`${cardId}|target:${requestedTargetLevel}|attempts:${maxAttempts}`;
      await env.DB.prepare("INSERT OR IGNORE INTO breakthrough_auto_receipts_v1616(request_id,user_id,card_id,status) VALUES(?,?,?,'PENDING')").bind(requestId,user.id,receiptFingerprint).run();
      ...
      if(Number(receipt?.userId)!==Number(user.id)||String(receipt?.cardId)!==receiptFingerprint)return json({error:'이미 다른 자동 강화 조건에 사용된 요청 번호입니다.',code:'BREAKTHROUGH_AUTO_REQUEST_MISMATCH',retryable:false},409);
      ...
      // V1802/V1940: FUR/ZENITH/SUPERSTAR +11~+13은 자동 강화에서 제외한다. MA/LIMITED도
      // 운영 설정상 중복 카드를 요구하는 단계라면 해당 단계부터 수동으로 넘긴다.
      const autoHighBlocked=['FUR','ZENITH','SUPERSTAR'].includes(grade),maxLevel=['MA','LIMITED'].includes(grade)?13:10,targetLevel=requestedTargetLevel>0?Math.max(1,Math.min(maxLevel,requestedTargetLevel)):maxLevel,targetExplicit=requestedTargetLevel>0;
      const maxLevelStop=()=>{stopReason=autoHighBlocked?'HIGH_MANUAL_ONLY':'MAX_LEVEL';stopMessage=autoHighBlocked?`${grade} +11~+13 강화는 고급 강화 보호 정책상 자동으로 진행할 수 없습니다. 카드 상세에서 한 단계씩 진행해 주세요.`:'최대 강화 단계에 도달했습니다.'};
      const targetLevelStop=()=>{if(!targetExplicit||(autoHighBlocked&&requestedTargetLevel>maxLevel))return maxLevelStop();stopReason='TARGET_REACHED';stopMessage=`목표 강화 단계 +${targetLevel}에 도달했습니다.`};
      while(attempts<maxAttempts){
        if(level>=targetLevel){targetLevelStop();break}
        const highStep=['MA','LIMITED'].includes(grade)&&level>=10;
        const masterStarStep=ALL_LEVEL_MASTER_STAR_GRADES.includes(grade)||highStep;
        ...
        const duplicateCost=highStep?Math.max(0,Math.floor(Number(rule.duplicateCards)||0)):0;
        if(duplicateCost>0){stopReason='HIGH_MANUAL_ONLY';stopMessage=`${grade} +${level+1} 강화는 중복 카드 ${duplicateCost}장이 소모되어 카드 상세에서 수동으로 진행해야 합니다.`;break}
      ...
      if(attempts===maxAttempts&&level>=targetLevel)targetLevelStop();
      ...
      const finalPityRule=breakthroughPityRule(grade,level,pity),response={ok:true,requestId,cardId,grade,maxAttempts,attempts,successes,failures,spent:{cardShards:shardSpent,masterStars:starSpent},level,failCount,maxLevel,targetLevel,requestedTargetLevel:requestedTargetLevel||maxLevel,canContinue,stopReason,stopMessage,pity:{...finalPityRule,failCount,nextGuaranteed:finalPityRule.enabled&&failCount>=finalPityRule.threshold},cinematic,user:partialUser};
      if(attempts===0){
      ...
      if(Number(results[receiptIndex]?.meta?.changes||0)!==1){
        await env.DB.batch([
          env.DB.prepare('UPDATE users SET card_shards=? WHERE id=? AND card_shards=? AND EXISTS(SELECT 1 FROM user_cards WHERE user_id=? AND card_id=? AND breakthrough_level=? AND breakthrough_fail_count=?)').bind(initial.shards,user.id,shards,user.id,cardId,initial.level,marker),
          ...(starSpent>0?[env.DB.prepare("UPDATE cnine_user_inventory SET quantity=?,unseen_quantity=MIN(unseen_quantity,?),updated_at=CURRENT_TIMESTAMP WHERE user_id=? AND item_code='MASTER_STAR' AND quantity=? AND EXISTS(SELECT 1 FROM user_cards WHERE user_id=? AND card_id=? AND breakthrough_level=? AND breakthrough_fail_count=?)").bind(initial.stars,initial.stars,user.id,stars,user.id,cardId,initial.level,marker)]:[]),
          env.DB.prepare('UPDATE user_cards SET breakthrough_level=?,breakthrough_fail_count=? WHERE user_id=? AND card_id=? AND breakthrough_level=? AND breakthrough_fail_count=?').bind(initial.level,initial.failCount,user.id,cardId,initial.level,marker),
          env.DB.prepare("UPDATE breakthrough_auto_receipts_v1616 SET status='FAILED',response_json=NULL,error_message='STATE_CONFLICT',updated_at=CURRENT_TIMESTAMP WHERE request_id=? AND user_id=? AND status='PENDING'").bind(requestId,user.id)
        ]);
        return json({error:'강화 상태가 변경되어 자동 강화를 중단했습니다. 최신 상태를 다시 확인해주세요.'},409);
      }
      // 경제 로그는 이 실행이 PENDING 영수증을 실제로 COMPLETED로 바꾼 뒤에만
      // 남긴다. 동일 requestId 후발 batch가 선행 완료 영수증을 보고 중복 기록할 수 없다.
      try{
        const logStatements=[];
        if(shardSpent>0)logStatements.push(env.DB.prepare("INSERT INTO shard_logs(user_id,change_amount,balance_after,reason,card_id) VALUES(?,?,?,?,?)").bind(user.id,-shardSpent,shards,'BREAKTHROUGH_AUTO',cardId));
        if(starSpent>0)logStatements.push(env.DB.prepare("INSERT INTO inventory_logs(user_id,item_code,change_amount,balance_after,reason,reference_type,reference_id) VALUES(?,'MASTER_STAR',?,?,?,'CARD_BREAKTHROUGH_AUTO',?)").bind(user.id,-starSpent,stars,ALL_LEVEL_MASTER_STAR_GRADES.includes(grade)?`${grade}_BREAKTHROUGH_AUTO`:`${grade}_HIGH_BREAKTHROUGH_AUTO`,cardId));
        if(logStatements.length)await env.DB.batch(logStatements);
      }catch(logError){console.error('automatic breakthrough economy log failed',logError)}
      return json(response);
    }
    ...
    if(path==='card/breakthrough'&&request.method==='POST'){
      ...
      if(usesMasterStars){
        ...
        // V1802: 중복 카드 소모. 실패해도 돌려주지 않으므로, 강화 대상 1장은 반드시 남겨두고 그 위의 중복분만 태운다.
        const duplicateCost=isMasterStarHigh?Math.max(0,Math.floor(Number(rule.duplicateCards)||0)):0,ownedQuantity=Math.max(0,Number(owned.quantity||0)),quantityAfter=ownedQuantity-duplicateCost;
        if(duplicateCost>0&&ownedQuantity<duplicateCost+1)return json({error:`중복 카드가 부족합니다. 같은 카드가 ${duplicateCost+1}장 필요합니다. (보유 ${ownedQuantity}장)`,code:'DUPLICATE_CARD_SHORTAGE',required:duplicateCost+1,owned:ownedQuantity},400);
        const pity=await breakthroughPity(env),pityRule=highBreakthroughStepPity(grade,level,isMasterStarHigh?rule:null,pity),guaranteed=pityRule.enabled&&failCount>=pityRule.threshold;
        ...
        if(changes.some(value=>value!==1)){
          ...
          await env.DB.batch([
            ...
            env.DB.prepare('UPDATE user_cards SET breakthrough_fail_count=?,quantity=? WHERE user_id=? AND card_id=? AND breakthrough_level=? AND breakthrough_fail_count=?').bind(failCount,ownedQuantity,user.id,cardId,level,cardMarker)
          ]);
        ...
        if(duplicateCost>0){try{await env.DB.batch([
          env.DB.prepare("CREATE TABLE IF NOT EXISTS card_material_logs_v1802(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,card_id TEXT NOT NULL,grade TEXT NOT NULL,level INTEGER NOT NULL,change_amount INTEGER NOT NULL,balance_after INTEGER NOT NULL,reason TEXT NOT NULL,created_at TEXT DEFAULT CURRENT_TIMESTAMP)"),
          env.DB.prepare("INSERT INTO card_material_logs_v1802(user_id,card_id,grade,level,change_amount,balance_after,reason) VALUES(?,?,?,?,?,?,?)").bind(user.id,cardId,grade,level,-duplicateCost,quantityAfter,success?`${grade}_HIGH_BREAKTHROUGH_SUCCESS`:`${grade}_HIGH_BREAKTHROUGH_FAIL`)
        ])}catch(logError){console.error(`${grade} high breakthrough duplicate-card log failed`,logError)}}
        const updated=await env.DB.prepare('SELECT * FROM users WHERE id=?').bind(user.id).first();
        const finalLevel=success?level+1:level,cinematic=await breakthroughCinematicFor(env,{success,grade,level:finalLevel,cardId,cardTitle:owned.title});return json({ok:true,success,cost,rate,material:'MASTER_STAR',masterStarsAfter:starAfter,duplicateCardsSpent:duplicateCost,cardQuantityAfter:quantityAfter,level:finalLevel,guaranteed,pity:{...pityRule,failCount:nextFailCount,nextGuaranteed:!success&&pityRule.enabled&&nextFailCount>=pityRule.threshold},cinematic,user:await profile(env,updated)});
      }
    ...
    if(path==='raid/status'){
      ...
      const raidCfgPromise=raidSettings(env),raidUniqueCfgPromise=cardUniqueSettings(env),raidWeeklyPromise=raidWeeklyToday(env);
      ...
      for(const pending of [raidTitlePromise,raidCfgPromise,raidUniqueCfgPromise,raidWeeklyPromise,raidTransitionPromise])pending.catch(()=>{});
      const user=await authenticate(request,env);if(!user)return json({error:'로그인이 필요합니다.'},401);
      const avatarEffectPromise=equippedAvatarEffect(env,user.id);avatarEffectPromise.catch(()=>{});
      const [,cfg,uniqueCfg,weekly,raidTransitionResult]=await Promise.all([raidTitlePromise,raidCfgPromise,raidUniqueCfgPromise,raidWeeklyPromise,raidTransitionPromise]);
      const owner=isRaidOwner(user),ownerTestMode=isRaidOwnerTest(user,cfg),uniqueVisible=cardUniqueVisibleTo(user,uniqueCfg);let schedule=raidScheduleState(cfg,user);if(schedule.currentSlot)schedule={...schedule,currentSlot:{...schedule.currentSlot,bossId:Number(weekly.boss.id)}};const entryDateKey=String(schedule.entryDateKey||kstDateKey()),configuredSlots=Array.isArray(cfg.timeSlots)?cfg.timeSlots:[];
      const [{todayEntryCount,slotEntries:baseSlotEntries},avatarEffect]=await Promise.all([raidEntryStatusCounts(env,user.id,entryDateKey,configuredSlots),avatarEffectPromise]);
      const dailyEntryAllowance=applyAvatarRaidEntryBonus(cfg.dailyEntries,avatarEffect),dailyEntryLimit=dailyEntryAllowance.limit,dailyEntry={count:todayEntryCount,baseLimit:dailyEntryAllowance.base,avatarBonus:dailyEntryAllowance.bonus,limit:dailyEntryLimit,remaining:Math.max(0,dailyEntryLimit-todayEntryCount),dateKey:entryDateKey,unlimited:ownerTestMode};
      const slotEntries=baseSlotEntries.map(row=>{const allowance=applyAvatarRaidEntryBonus(row.limit,avatarEffect);return{...row,baseLimit:allowance.base,avatarBonus:allowance.bonus,limit:allowance.limit,remaining:Math.max(0,allowance.limit-Number(row.count||0))}});
      const scheduleSlot=schedule.currentSlot||null,scheduleSlotId=String(scheduleSlot?.id||''),scheduleSlotAllowance=applyAvatarRaidEntryBonus(scheduleSlot?.entriesPerSlot,avatarEffect),slotEntry=scheduleSlotId&&scheduleSlotId!=='ALWAYS'?(slotEntries.find(row=>String(row.id)===scheduleSlotId)||{id:scheduleSlotId,label:String(scheduleSlot?.label||scheduleSlotId),count:0,baseLimit:scheduleSlotAllowance.base,avatarBonus:scheduleSlotAllowance.bonus,limit:scheduleSlotAllowance.limit,remaining:scheduleSlotAllowance.limit,unlimited:ownerTestMode}):{id:scheduleSlotId||'DAILY',label:String(scheduleSlot?.label||'오늘 합계'),count:todayEntryCount,baseLimit:dailyEntryAllowance.base,avatarBonus:dailyEntryAllowance.bonus,limit:dailyEntryLimit,remaining:Math.max(0,dailyEntryLimit-todayEntryCount),unlimited:ownerTestMode},slotEntryUsed=Boolean(scheduleSlotId&&scheduleSlotId!=='ALWAYS'&&Number(slotEntry.count||0)>=Number(slotEntry.limit||1));
      ...
      const rooms=roomRows.map((room,i)=>{let savedProfile=null;try{savedProfile=JSON.parse(room.settingsJson||'{}').bossProfile||null}catch{}const {settingsJson,...roomInfo}=room;const slot=(cfg.timeSlots||[]).find(x=>String(x.id)===String(room.slotId)),activeSlotId=String(schedule.currentSlot?.id||''),bossProfile=savedProfile||weeklyRaidBossByName(room.bossName),slotOpen=owner||room.slotId==='LEGACY'||room.slotId==='ALWAYS'||(activeSlotId&&activeSlotId===String(room.slotId));return {...roomInfo,maxHp:Number(bossProfile?.maxHp??room.maxHp),bossImage:bossProfile?.battleSprite||room.bossImage,powerRating:Number(bossProfile?.powerRating||0),slotLabel:slot?.label||room.slotId,roomNumber:roomRows.length-i,joinable:slotOpen&&room.status==='LOBBY'&&Date.parse(room.startsAt)>Date.now()&&Number(room.participantCount)<Number(cfg.maxParticipants||30)}});
      if(!current){
        const policies=await raidBossOpenPolicies(env),today=weekly.boss,availableBosses=today.isActive&&(owner||policies[String(today.id)]?.enabled!==false)?[{...today,openCost:Number(policies[String(today.id)]?.cost||0),ownerTestVisible:false}]:[];
        return json({settings:cfg,schedule,current:null,rooms,participants:[],me:null,availableBosses,weeklyBosses:weekly.rows,todayBoss:today,ownerTestMode,dailyEntryUsed:ownerTestMode?false:(todayEntryCount>=dailyEntryLimit||slotEntryUsed),dailyEntry,slotEntry,slotEntries,serverNow:new Date().toISOString()});
      }
      ...
      if(instanceCfg.bossProfile)current.max_hp=instanceCfg.bossProfile.maxHp;
      ...
      const participants=participantRows.map((r,i)=>{const {titleUnlockConfig,...publicRow}=r;return {...publicRow,rank:i+1,title:r.titleId?{id:Number(r.titleId),name:r.titleName,badgeText:r.titleBadgeText||r.titleName,stylePreset:String(r.titleStylePreset||'DEFAULT').toUpperCase(),fontPreset:(()=>{try{const value=String(JSON.parse(titleUnlockConfig||'{}')?.fontPreset||'DEFAULT').toUpperCase();return ['DEFAULT','SERIF','DISPLAY','ARCADE','ROUNDED','SCIFI','BRUSH','HANDWRITING','MONO','CLASSIC'].includes(value)?value:'DEFAULT'}catch{return 'DEFAULT'}})()}:null,deckCards:(()=>{try{return raidDisplayDeckIdsV1311(JSON.parse(r.deckCards||'[]'))}catch{return []}})()}});
      ...
      const instanceSlot=await instanceSlotPromise,instanceSlotCfg=(instanceCfg.timeSlots||[]).find(row=>String(row.id)===String(instanceSlot))||null,instanceSlotAllowance=applyAvatarRaidEntryBonus(instanceSlotCfg?.entriesPerSlot,avatarEffect),instanceSlotEntry=instanceSlot&&instanceSlot!=='ALWAYS'&&instanceSlot!=='LEGACY'?(slotEntries.find(row=>String(row.id)===String(instanceSlot))||{id:String(instanceSlot),label:String(instanceSlotCfg?.label||instanceSlot),count:0,baseLimit:instanceSlotAllowance.base,avatarBonus:instanceSlotAllowance.bonus,limit:instanceSlotAllowance.limit,remaining:instanceSlotAllowance.limit,unlimited:ownerTestMode}):slotEntry,instanceSlotUsed=Boolean(instanceSlot&&instanceSlot!=='ALWAYS'&&instanceSlot!=='LEGACY'&&Number(instanceSlotEntry.count||0)>=Number(instanceSlotEntry.limit||1)),entryUsedForCurrent=ownerTestMode?false:(todayEntryCount>=dailyEntryLimit||instanceSlotUsed);
      ...
      if(current.status==='ENDED'&&me&&Number(me.rewardClaimed||0)!==1){
        ...
        const fixedPlan=await ensureRaidUserRewardPlanV1293(env,{instanceId:current.id,userId:user.id,cfg:instanceCfg,totalDamage:finalDamage,finalRank,cleared,minionsDefeated:Number(combat.minionsDefeated||0)}),rewardDisplay=raidRewardDisplayV1293(fixedPlan.plan);
      ...
      const battleProfile=instanceCfg.bossProfile||weeklyRaidBossByName(current.boss_name)||null;
      return json({settings:cfg,schedule,dailyEntryUsed:entryUsedForCurrent,dailyEntry,slotEntry:instanceSlotEntry,slotEntries,rooms,weeklyBosses:weekly.rows,todayBoss:weekly.boss,current:{id:current.id,status:current.status,startsAt:current.starts_at,endsAt:current.ends_at,currentHp:hp,maxHp:Number(current.max_hp),participantCount:participants.length,bossName:current.boss_name,bossImage:current.boss_image,bossBattleSprite:battleProfile?.battleSprite||current.boss_image,powerRating:Number(battleProfile?.powerRating||0),bossTitle:String(battleProfile?.title||''),bossAccent:String(battleProfile?.accent||''),ultimate:battleProfile?.ultimate||null,minions:combat.minions||[],minionsDefeated:Number(combat.minionsDefeated||0),bossDamageReduction:Number(combat.bossDamageReduction||0),ultimateCasts:Number(combat.ultimateCasts||0),lastUltimate:combat.lastUltimate||null,progress,result:current.status==='ENDED'?result:null,attackTicks,enraged,slotId:instanceSlot,phase:Number(combat.phase||1),phaseLabel:combat.phaseLabel||'',shieldHp:Number(combat.shieldHp||0),shieldMaxHp:Number(combat.shieldMaxHp||0),shieldBroken:combat.shieldBroken===true,breakProgress:Number(combat.breakProgress||0)},participants:visibleParticipants,me,claimableReward,serverNow:new Date().toISOString()});
    }
    if(path==='raid/open'&&request.method==='POST'){
      const user=await authenticate(request,env);if(!user)return json({error:'로그인이 필요합니다.'},401);
      const [cfg,weekly]=await Promise.all([raidSettings(env),raidWeeklyToday(env,Date.now(),true)]),owner=isRaidOwner(user),ownerTestMode=isRaidOwnerTest(user,cfg);if(!cfg.enabled)return json({error:'현재 레이드가 중지되어 있습니다.'},503);if(cfg.ownerOnlyTest&&!owner)return json({error:'현재 레이드는 OWNER 테스트 전용입니다.'},403);if(!cfg.userOpenEnabled&&!owner)return json({error:'유저 레이드 개방이 중지되어 있습니다.'},403);
      let schedule=raidScheduleState(cfg,user);if(schedule.currentSlot)schedule={...schedule,currentSlot:{...schedule.currentSlot,bossId:Number(weekly.boss.id)}};if(!schedule.canEnter)return json({error:schedule.reason==='ENTRY_CLOSED'?'레이드 입장 마감 시간이 지났습니다.':'현재는 레이드 개방 시간이 아닙니다.',schedule},403);
      ...
      const body=await readBody(request),requestId=String(body.requestId||crypto.randomUUID()).trim().slice(0,100),bossId=Number(body.bossId||weekly.boss.id),dateKey=String(schedule.entryDateKey||kstDateKey());
      ...
      const [activeCount,activeMine,entryCount,slotEntryCount,policies,boss,fresh,avatarEffect]=await Promise.all([env.DB.prepare("SELECT COUNT(*) count FROM raid_instances WHERE status IN ('LOBBY','BATTLE')").first(),env.DB.prepare("SELECT rp.instance_id FROM raid_participants rp JOIN raid_instances ri ON ri.id=rp.instance_id WHERE rp.user_id=? AND COALESCE(rp.is_active,1)=1 AND ri.status IN ('LOBBY','BATTLE') LIMIT 1").bind(user.id).first(),raidDailyEntryCount(env,user.id,dateKey),raidSlotEntryCountV1293(env,user.id,dateKey,slotId),raidBossOpenPolicies(env),env.DB.prepare('SELECT * FROM raid_bosses WHERE id=? AND is_active=1').bind(bossId).first(),env.DB.prepare('SELECT coin FROM users WHERE id=?').bind(user.id).first(),equippedAvatarEffect(env,user.id)]);
      const dailyEntryAllowance=applyAvatarRaidEntryBonus(cfg.dailyEntries,avatarEffect),slotEntryAllowance=applyAvatarRaidEntryBonus(slot.entriesPerSlot,avatarEffect);
      if(Number(activeCount?.count||0)>=10)return json({error:'동시에 개설 가능한 레이드 방 10개가 모두 사용 중입니다.'},409);if(activeMine)return json({error:'이미 참가 중인 레이드 방이 있습니다.'},409);if(!ownerTestMode&&entryCount>=dailyEntryAllowance.limit)return json({error:`오늘의 레이드 입장 횟수 ${dailyEntryAllowance.limit}회를 모두 사용했습니다.`},409);if(!ownerTestMode&&slotId!=='ALWAYS'&&slotEntryCount>=slotEntryAllowance.limit)return json({error:`${slot.label||slotId} 레이드 참여 횟수 ${slotEntryAllowance.limit}회를 모두 사용했습니다. 다음 개방 타임에 다시 참여할 수 있습니다.`},409);if(!boss)return json({error:'개방 가능한 레이드 보스를 찾을 수 없습니다.'},404);
      if(Number(weekly.boss.id)!==bossId)return json({error:`오늘은 ${weekly.boss.name} 레이드만 개방할 수 있습니다.`},403);
      const policy=policies[String(bossId)]||{},cost=Math.max(0,Number(policy.cost||0));if(policy.enabled===false&&!owner)return json({error:'CMS에서 이 보스의 유저 개방을 중지했습니다.'},403);if(Number(fresh?.coin||0)<cost)return json({error:'레이드 개방에 필요한 코인이 부족합니다.'},400);
      ...
      const instanceId=Number(created.meta.last_row_id),roomCfg=weeklyRaidBossSettings(cfg,weekly.boss);await snapshotRaidInstanceV1293(env,instanceId,slotId,roomCfg);await raidRewardSnapshot(env,instanceId,roomCfg,true);
      try{
      ...
      return json({ok:true,instanceId,cost,totalPower:deck.power,cardPower:deck.cardPower,characterBonus:deck.characterBonus,participantCount:1,dailyEntry:{baseLimit:dailyEntryAllowance.base,avatarBonus:dailyEntryAllowance.bonus,limit:dailyEntryAllowance.limit},slot:{id:slotId,label:slot.label,baseLimit:slotEntryAllowance.base,avatarBonus:slotEntryAllowance.bonus,limit:slotEntryAllowance.limit}});
    }
    ...
    if(path==='raid/join'&&request.method==='POST'){
      ...
      if(Number(current.participant_count||0)>=Number(instanceCfg.maxParticipants||30))return json({error:'레이드 참가 인원이 가득 찼습니다.'},409);const dateKey=String(schedule.entryDateKey||kstDateKey());let entryCount,slotEntryCount,deck,avatarEffect;try{[entryCount,slotEntryCount,deck,avatarEffect]=await Promise.all([raidDailyEntryCount(env,user.id,dateKey),raidSlotEntryCountV1293(env,user.id,dateKey,instanceSlot),raidDeckPower(env,user.id,body.cardIds),equippedAvatarEffect(env,user.id)])}catch(e){return json({error:e.message},e.status||400)}const dailyEntryAllowance=applyAvatarRaidEntryBonus(instanceCfg.dailyEntries,avatarEffect),instanceSlotAllowance=applyAvatarRaidEntryBonus(instanceSlotCfg?.entriesPerSlot,avatarEffect),instanceSlotLimit=instanceSlotAllowance.limit;if(!ownerTestMode&&entryCount>=dailyEntryAllowance.limit)return json({error:`오늘의 레이드 입장 횟수 ${dailyEntryAllowance.limit}회를 모두 사용했습니다.`},409);if(!ownerTestMode&&instanceSlot!=='LEGACY'&&instanceSlot!=='ALWAYS'&&slotEntryCount>=instanceSlotLimit)return json({error:`${instanceSlotCfg?.label||instanceSlot} 레이드 참여 횟수 ${instanceSlotLimit}회를 모두 사용했습니다.`},409);
    ...
    if(path==='raid/claim'&&request.method==='POST'){
      ...
      const cleared=Number(row.current_hp||0)<=0,instanceCfg=await raidInstanceSettingsV1293(env,row.instance_id,cfg),finalized=await ensureRaidFinalizedV1293(env,row.instance_id,instanceCfg),rewardCfg=await raidRewardSnapshot(env,row.instance_id,instanceCfg,true),finalState=await raidFinalParticipantV1293(env,row.instance_id,user.id),finalRank=Math.max(0,Number(finalState?.finalRank||await raidUserFinalRank(env,row.instance_id,user.id))),finalDamage=Math.max(0,Number(finalState?.finalDamage||0)),minionsDefeated=Math.max(0,Number(finalized?.snapshot?.minionsDefeated||0));
      const fixedPlan=await ensureRaidUserRewardPlanV1293(env,{instanceId:row.instance_id,userId:user.id,cfg:instanceCfg,totalDamage:finalDamage,finalRank,cleared,minionsDefeated}),rewardDisplay=raidRewardDisplayV1293(fixedPlan.plan);
    ...
    if(path==='battle/auto/status'&&request.method==='POST'){
      ...
      const payload=await readBody(request),requestId=String(payload.requestId||'').trim();
      if(!/^[a-zA-Z0-9-]{16,80}$/.test(requestId))return json({error:'소탕 요청 정보가 올바르지 않습니다.'},400);
      const receipt=await env.DB.prepare('SELECT user_id,status,response_json,error_message,created_at,updated_at FROM pve_auto_runs WHERE request_id=?').bind(requestId).first();
      return pveSweepReceiptResponse(env,user,requestId,receipt);
    }
    if(path==='battle/auto'&&request.method==='POST'){
      const user=await authenticate(request,env);if(!user)return json({error:'다른 기기 또는 창에서 다시 로그인되어 현재 로그인이 종료되었습니다.',code:'SESSION_REPLACED'},401);
      const payload=await readBody(request),requestId=String(payload.requestId||'').trim(),monsterId=Number(payload.monsterId),requestedBattles=Math.floor(Number(payload.requestedBattles)),sweepProtocolVersion=Math.max(1,Math.floor(Number(payload.protocolVersion||1))),ids=[...new Set((payload.cardIds||[]).map(String))];
      if(!/^[a-zA-Z0-9-]{16,80}$/.test(requestId))return json({error:'소탕 요청 정보가 올바르지 않습니다.'},400);
      const previous=await env.DB.prepare('SELECT user_id,status,response_json,error_message,created_at,updated_at FROM pve_auto_runs WHERE request_id=?').bind(requestId).first();
      if(previous)return pveSweepReceiptResponse(env,user,requestId,previous,{legacyRunningError:sweepProtocolVersion<2});
      ...
      if(sweepProtocolVersion<2&&requestedBattles>PVE_SWEEP_BATCH_LIMIT)return json({error:'소탕 안정화 업데이트가 적용되었습니다. 게임을 새로고침한 뒤 다시 소탕해 주세요.',code:'PVE_SWEEP_CLIENT_UPDATE_REQUIRED',retryable:false},409);
      ...
      const availableBattles=Math.floor(Number(energyBefore.energy||0)/Math.max(1,Number(energyBefore.costPerBattle||1))),battleCount=Math.min(requestedBattles,availableBattles,PVE_SWEEP_BATCH_LIMIT);if(battleCount<1)return json({error:'전투 횟수가 부족합니다.',code:'NO_BATTLE_ENERGY',energy:energyBefore,energyKind:'STANDARD'},429);
      ...
      if(Number(claimed?.meta?.changes||0)!==1){
        ...
        if(Number(duplicate?.user_id)!==Number(user.id))await env.DB.prepare('DELETE FROM pve_auto_locks WHERE user_id=? AND request_id=?').bind(user.id,requestId).run();
    ...
    if(path==='battle/fight'&&request.method==='POST'){
      ...
      const bossConfiguredPct=apocalypseSkillCast?Math.max(0,Number(apocalypseSkill.damagePercent||0)):Math.max(0,Number(monster.ultimate_pve_damage_percent??monster.ultimate_damage_percent??0)||0);
      ...
      const bossUltimate=bossShouldCast?{name:apocalypseSkillCast?String(apocalypseSkill.name||'종말 집행'):String(monster.ultimate_name||'보스 궁극기'),description:apocalypseSkillCast?String(apocalypseSkill.description||''):String(monster.ultimate_description||''),warningText:apocalypseSkillCast?'APOCALYPSE SKILL':String(monster.ultimate_warning_text||'BOSS ULTIMATE'),damagePercent:bossPveDamagePercent,forceCast:apocalypseSkillCast||bossForceCast,target:String(monster.ultimate_target||'ALL'),theme:apocalypseSkillCast?'VOID':String(monster.ultimate_theme||'CRIMSON'),shake:apocalypseSkillCast||Boolean(monster.ultimate_shake),zoom:apocalypseSkillCast||Boolean(monster.ultimate_zoom),mediaUrl:String(monster.ultimate_media_url||''),soundUrl:String(monster.ultimate_sound_url||''),durationMs:Math.max(600,Math.min(25000,Number(monster.ultimate_duration_ms||2400))),volumePercent:Math.max(0,Math.min(100,Number(monster.ultimate_volume_percent??35))),penalty:bossUltimatePenalty,capPercent:difficulty.bossUltimateCapPercent,damageCapUnlocked:difficulty.bossUltimateUnlocked,apocalypseExclusive:apocalypseSkillCast}:null;
    ...
    if((path==='admin/verified-reward-message-send'||path==='admin/verified-coin-message-send')&&request.method==='POST'){
      ...
      // 구버전 CMS가 값을 생략해도 포함하고, 운영자가 명시적으로 false를 보낸 경우만 제외한다.
      const includeOwner=body.includeOwner===true,includeAdmin=body.includeAdmin!==false;
    ...
    if(path==='admin/tiers'){
      ...
      if(request.method==='GET'){
        ...
        const pvpRows=await env.DB.prepare(`SELECT u.nickname,p.season_score,p.highest_score,p.wins,p.losses FROM pvp_profiles p JOIN users u ON u.id=p.user_id WHERE u.status='ACTIVE' AND ${PVP_RANKED_ROLE_SQL} AND (u.banned_until IS NULL OR u.banned_until<=datetime('now')) ORDER BY p.season_score DESC,p.wins DESC,u.nickname,u.id LIMIT 100`).all();
        ...
        return json({settings,ranking:[...map.values()].sort((a,b)=>b.score-a.score).slice(0,100),pvpRanking:pvpRows.results.map((x,i)=>({...x,rank:i+1,tier:resolvePvpTier(Number(x.season_score),livePvp,i+1)})),pvpStats,pvpSettlement:settlement||null,pvpLifecycle:lifecycle});
      }
    ...
    if(path==='admin/pvp-settlement'){
      ...
      if(needsPreview){
        const [rankedRows,tierClaims,rankClaims]=await Promise.all([
          env.DB.prepare(`SELECT u.id AS user_id,u.nickname,p.season_score,p.highest_score,p.wins,p.losses FROM pvp_profiles p JOIN users u ON u.id=p.user_id WHERE u.status='ACTIVE' AND ${PVP_RANKED_ROLE_SQL} AND (u.banned_until IS NULL OR u.banned_until<=datetime('now')) ORDER BY p.season_score DESC,p.wins DESC,u.nickname,u.id`).all(),
          env.DB.prepare('SELECT user_id FROM pvp_reward_claims WHERE season_name=?').bind(settings.seasonName).all(),
          env.DB.prepare('SELECT user_id FROM pvp_rank_reward_claims WHERE season_name=?').bind(settings.seasonName).all()
        ]);
        const tierClaimed=new Set(tierClaims.results.map(x=>Number(x.user_id))),rankClaimed=new Set(rankClaims.results.map(x=>Number(x.user_id)));
    ...
    if(path==='admin/breakthrough-settings'){
      const admin=await requirePermission(request,env,'SETTINGS'); if(!admin)return json({error:'관리자 권한이 없습니다.'},403);
      if(request.method==='GET'){
        const zenithHigh=await zenithMasterStarBreakthroughConfig(env);
        return json({config:await breakthroughConfig(env),grades:BREAKTHROUGH_GRADES,pity:await breakthroughPity(env),maHigh:await maMasterStarBreakthroughConfig(env),limitedHigh:await limitedMasterStarBreakthroughConfig(env),furHigh:await furMasterStarBreakthroughConfig(env),zenithHigh,superstarHigh:zenithHigh,highBonus:{...HIGH_BREAKTHROUGH_BONUS_DEFAULT,SUPERSTAR:[...HIGH_BREAKTHROUGH_BONUS_DEFAULT.ZENITH]},cinematic:await breakthroughCinematicConfig(env)});
      }
      if(request.method==='PATCH'){
        ...
        for(const grade of BREAKTHROUGH_GRADES){
          ...
          for(let i=0;i<maxLevel;i++){
            ...
            if(!Number.isInteger(cost)||cost<1||cost>10000000)return json({error:`${grade} ★${i}→★${i+1} 강화 비용을 확인하세요.`},400);
        ...
        await env.DB.batch([
          ...
          env.DB.prepare("INSERT OR REPLACE INTO app_meta(key,value,updated_at) VALUES('fur_master_star_breakthrough_v1802',?,CURRENT_TIMESTAMP)").bind(JSON.stringify(furHigh)),
    ...
    if(path==='admin/raid'){
      const admin=await requirePermission(request,env,'DASHBOARD');if(!admin)return json({error:'관리자 권한이 없습니다.'},403);if(admin.role!=='OWNER')return json({error:'레이드 관리는 OWNER 전용입니다.'},403);await Promise.all([ensureRaidOverhaulV1293(env),ensureWeeklyRaidBossesV1(env)]);
      const payload=request.method==='GET'?{}:await readBody(request);
      if(request.method==='PATCH'&&payload.weekly){
        try{const weekly=await saveWeeklyRaidCms(env,admin,payload.weekly);raidSharedCache.delete('weekly-bosses-v2141');return json({ok:true,weekly});}
      ...
      if(request.method==='GET'){
        ...
        const weekly=await readWeeklyRaidCms(env);
        return json({settings:await raidSettings(env),weekly,bosses:bosses.results.filter(b=>weeklyRaidBossByName(b.name)).map(b=>({...b,userOpenEnabled:policies[String(b.id)]?.enabled!==false,openCost:Number(policies[String(b.id)]?.cost||0)})),current:current||null});
      }
      ...
      if(request.method==='POST'&&payload.action==='CREATE_BOSS')return json({error:'요일 레이드는 나가토·요리이치·이치고 3종만 사용합니다.'},400);
      if(request.method==='PATCH'&&payload.boss){const b=payload.boss,id=Number(b.id);if(!id)return json({error:'보스 ID가 필요합니다.'},400);if(!Number.isSafeInteger(Number(b.maxHp))||Number(b.maxHp)<1||Number(b.maxHp)>2000000000||!Number.isFinite(Number(b.defenseRate))||Number(b.defenseRate)<0||Number(b.defenseRate)>99||!Number.isSafeInteger(Number(b.openCost))||Number(b.openCost)<0||Number(b.openCost)>100000000)return json({error:'보스 HP(1~20억), 방어(0~99%), 개방 비용(0~1억)을 확인하세요.'},400);const existing=await env.DB.prepare('SELECT name FROM raid_bosses WHERE id=?').bind(id).first();if(!existing||!weeklyRaidBossByName(existing.name)||b.name!==existing.name)return json({error:'요일 보스 3종의 이름은 변경할 수 없습니다.'},400);await env.DB.prepare('UPDATE raid_bosses SET name=?,image_url=?,max_hp=?,defense_rate=?,is_active=?,sort_order=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').bind(String(b.name||'').trim().slice(0,40),String(b.image||'').trim().slice(0,500),Math.max(1,Math.floor(Number(b.maxHp)||1)),Math.max(0,Math.min(99,Number(b.defenseRate)||0)),b.isActive===false?0:1,Math.floor(Number(b.sortOrder)||0),id).run();const policies=await raidBossOpenPolicies(env);policies[String(id)]={enabled:b.userOpenEnabled===true,cost:Math.max(0,Math.min(100000000,Math.floor(Number(b.openCost)||0)))};await env.DB.prepare("INSERT OR REPLACE INTO app_meta(key,value,updated_at) VALUES('raid_user_open_bosses_v1',?,CURRENT_TIMESTAMP)").bind(JSON.stringify(policies)).run();invalidateMetaSnapshot();raidSharedCache.delete('weekly-bosses-v2141');raidSharedCache.delete('rooms');await writeAdminLog(env,admin,'RAID_BOSS_UPDATE','RAID_BOSS',String(id),null,b);return json({ok:true,boss:{...b,...policies[String(id)]}});}
      if(request.method==='POST'&&payload.action==='START'){const bossId=Number(payload.bossId),boss=await env.DB.prepare('SELECT * FROM raid_bosses WHERE id=? AND is_active=1').bind(bossId).first();if(!boss||!weeklyRaidBossByName(boss.name))return json({error:'활성 레이드 보스를 선택하세요.'},400);const active=await env.DB.prepare("SELECT COUNT(*) count FROM raid_instances WHERE status IN ('LOBBY','BATTLE')").first();if(Number(active?.count||0)>=10)return json({error:'동시에 개설 가능한 레이드 방 10개가 모두 사용 중입니다.'},409);const cfg=await raidSettings(env),schedule=raidScheduleState(cfg,admin);if(!schedule.isOpen)return json({error:'현재는 CMS에서 설정한 레이드 개방 시간이 아닙니다.',schedule},403);const startsAt=new Date(Date.now()+cfg.lobbySeconds*1000).toISOString(),endsAt=new Date(Date.now()+(cfg.lobbySeconds+cfg.battleSeconds)*1000).toISOString(),r=await env.DB.prepare("INSERT INTO raid_instances(boss_id,status,starts_at,ends_at,current_hp,participant_count) VALUES(?,'LOBBY',?,?,?,0)").bind(bossId,startsAt,endsAt,boss.max_hp).run();const weeklyProfile=(await readWeeklyRaidCms(env)).bosses.find(row=>Number(row.id)===bossId),roomCfg=weeklyRaidBossSettings(cfg,weeklyProfile);await snapshotRaidInstanceV1293(env,Number(r.meta.last_row_id),String(schedule.currentSlot?.id||'ADMIN'),roomCfg);await raidRewardSnapshot(env,Number(r.meta.last_row_id),roomCfg,true);await writeAdminLog(env,admin,'RAID_START','RAID_INSTANCE',String(r.meta.last_row_id),null,{bossId,weeklyCode:weeklyProfile?.code||null});return json({ok:true,id:r.meta.last_row_id});}
    ...
    if(path==='admin/coupon-create-permanent-v3'){
      ...
      if(rewardType==='CHUSEOK_COIN'){if(code.startsWith('SLD-'))return json({error:'SLD-는 숲켓랜드 전용 접두어입니다. 다른 쿠폰 코드를 입력하세요.'},400);await ensureChuseok(env)}
    ...
    if(path==='admin/coupons'||path==='admin/coupons-v2'){
      ...
      if(request.method==='POST'){
        ...
        if(!/^[A-Z0-9_-]{4,40}$/.test(code))return json({error:'쿠폰 코드는 영문 대문자·숫자·_·- 조합 4~40자로 입력하세요.'},400);
        if(!spec||!['COIN','MASTER_STAR','PREMIUM_CUBE','EQUIPMENT_SUPPLY_BOX','HIGH_GRADE_REROLL_TICKET','CHUSEOK_COIN'].includes(rewardType))return json({error:'쿠폰 보상 종류를 확인하세요.'},400);
    ...
    if(path==='admin/users/card-grant'){
      ...
      if(request.method==='GET'){
        ...
        const rows=await env.DB.prepare(`SELECT c.id,c.title,UPPER(c.rarity) AS grade,c.image_url AS image,m.name,
          ...
          ORDER BY CASE UPPER(c.rarity) WHEN 'SUPERSTAR' THEN 1 WHEN 'ZENITH' THEN 2 WHEN 'FUR' THEN 3 WHEN 'PRESTIGE' THEN 4 WHEN 'LIMITED' THEN 5 WHEN 'MA' THEN 6 WHEN 'SSR' THEN 7 WHEN 'UR' THEN 8 WHEN 'HR' THEN 9 WHEN 'SR' THEN 10 WHEN 'R' THEN 11 WHEN 'U' THEN 12 WHEN 'C' THEN 13 ELSE 14 END,m.sort_order,c.title,c.id LIMIT 80`).bind(...binds).all();
    ...
    if(path==='admin/card-retirement'&&request.method==='POST'){
      ...
      // V1802/V1940: +11~+13 을 쓰는 등급(MA/LIMITED/FUR/ZENITH/SUPERSTAR) 전부 퇴사 환급 구간을 잇는다.
      const cardGrade=String(card.rarity||'').toUpperCase();
      const cfg=await breakthroughConfig(env),highCfg=await highBreakthroughConfigFor(env,cardGrade);
      const gradeRules=(Array.isArray(cfg[card.rarity])?cfg[card.rarity]:[]).map(rule=>({refundShards:Math.max(0,Number(rule?.cost)||0)}));
      if(highCfg)for(const step of highCfg.steps)gradeRules.push({refundShards:Math.max(0,Number(step?.retirementShardRefund)||0)});
      const cumulative=[0];
      ...
      const unsupported=ownedRows.filter(row=>{const level=Math.max(0,Number(row.breakthrough_level)||0);if(level>gradeRules.length)return true;if(highCfg&&level>10)return highCfg.steps.slice(0,level-10).some(step=>Number(step?.retirementShardRefund||0)<=0);return false});
      ...
      if(request.method==='POST'){
        ...
        try{
          ...
          const snapshots=await env.DB.prepare(`SELECT COALESCE(u.nickname,'') AS nickname,COALESCE(c.title,'') AS title,COALESCE(c.issued_count,0) AS issuedCount,COALESCE(uc.quantity,0) AS ownedQuantity
            FROM (SELECT CAST(? AS BIGINT) AS user_id,CAST(? AS TEXT) AS card_id) x LEFT JOIN users u ON u.id=x.user_id LEFT JOIN cards_effective_v1210 c ON c.id=x.card_id LEFT JOIN user_cards uc ON uc.user_id=x.user_id AND uc.card_id=x.card_id`).bind(userId,cardId).first();
    ...
    if(path==='admin/cards'){
      ...
      const normalizeCard=async payload=>{
        ...
        else if(['FUR','ZENITH','SUPERSTAR'].includes(grade)){powerType='FIXED';basePower=grade==='SUPERSTAR'?15500:grade==='ZENITH'?5500:3200}
  ...
  if(serializedGameAction(actionPath,request.method)){
    ...
    if(user){
      ...
      if(acquired==='LOCK_GUARD_TIMEOUT'||acquired==='LOCK_ERROR'){
        ...
        mutationLock=null;
        if(acquired==='LOCK_GUARD_TIMEOUT'){
