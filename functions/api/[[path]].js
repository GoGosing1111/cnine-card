...
export async function onRequest(context){
  ...
  try{
    ...
    if(path==='battle/fight'&&request.method==='POST'){
      ...
      const eligible=(settings.ultimateRules||[]).filter(u=>u.enabled!==false&&cards.filter(c=>String(c.rarity||'').toUpperCase()===u.requiredGrade&&Number(c.breakthrough_level||0)>=Number(u.minBreakthrough||0)).length>=Number(u.requiredCount||1)).sort((a,b)=>Number(ORDER[b.requiredGrade]||0)-Number(ORDER[a.requiredGrade]||0)||Number(b.minBreakthrough||0)-Number(a.minBreakthrough||0)||Number(b.requiredCount||0)-Number(a.requiredCount||0));
      const activatedUltimate=eligible.find(u=>Math.random()*100<Math.max(0,Math.min(100,Number(u.activationChance??100))))||null;
      const bonusDamage=activatedUltimate?Math.max(0,Math.floor(activatedUltimate.damageType==='FIXED'?Number(activatedUltimate.damageValue||0):basePlayerPower*Number(activatedUltimate.damageValue||0)/100)):0;
      const playerPower=basePlayerPower+bonusDamage;
      ...
      const updated=await env.DB.prepare('SELECT * FROM users WHERE id=?').bind(user.id).first();
      return json({result,reward,cardReward,playerPower,basePlayerPower,bonusDamage,activatedUltimate,monsterPower,monster:{id:monster.id,name:monster.name,image:monster.image_url,isBoss:Boolean(monster.is_boss)},cards,energy:energyAfter,serverNow:new Date().toISOString(),user:await profile(env,updated)});
    }
    ...
    if(path==='pvp/opponents'){
      const user=await authenticate(request,env);if(!user)return json({error:'로그인이 필요합니다.'},401);const settings=await pvpSettings(env);if(!settings.enabled&&!isAdminRole(user))return json({error:'현재 PvP 콘텐츠가 중지되어 있습니다.'},503);const mine=await ensurePvpProfile(env,user,settings),myCard=await userCardScore(env,user.id);
      ...
      const seasonRange=Number(settings.matchSeasonRange||300),users=await env.DB.prepare(`SELECT u.id,u.nickname,p.season_score,p.wins,p.losses,p.highest_score FROM users u JOIN pvp_profiles p ON p.user_id=u.id JOIN pvp_decks d ON d.user_id=u.id WHERE u.id<>? AND u.status='ACTIVE' AND (u.banned_until IS NULL OR u.banned_until<=datetime('now')) AND ABS(p.season_score-?)<=? ORDER BY ABS(p.season_score-?) ASC LIMIT 60`).bind(user.id,mine.season_score,seasonRange,mine.season_score).all();
      const out=[];for(const x of users.results){if(Number(x.id)===blockedOpponentId)continue;const cs=await userCardScore(env,x.id),pct=myCard?Math.abs(cs-myCard)/myCard*100:0;if(pct<=Number(settings.matchCardRange||15))out.push({...x,cardScore:cs,diffPercent:myCard?((cs-myCard)/myCard*100):0,tier:resolveTier(Number(x.season_score),settings.tiers)});if(out.length>=6)break}return json({opponents:out,myCardScore:myCard,blockedOpponentId:blockedOpponentId||null});
    }
    if(path==='pvp/fight'&&request.method==='POST'){
      ...
      const aDeck=await pvpDeckSnapshot(env,user.id),dDeck=await pvpDeckSnapshot(env,defenderId);if(aDeck.length!==5)return json({error:'먼저 PvP 덱 편성을 완료하세요.'},400);if(dDeck.length!==5)return json({error:'상대의 PvP 덱이 완성되지 않았습니다.'},409);const battle=await battleSettings(env),aPower=aDeck.reduce((s,c)=>s+cardBattlePower(c,c.breakthrough_level,battle),0),dPower=dDeck.reduce((s,c)=>s+cardBattlePower(c,c.breakthrough_level,battle),0);const aCard=await userCardScore(env,user.id),dCard=await userCardScore(env,defenderId);const attackerWin=aPower>=dPower,winnerId=attackerWin?user.id:defenderId,aAdj=pvpScoreAdjustment(attackerWin?Number(settings.winScore||24):Number(settings.loseScore||16),attackerWin,aCard,dCard,settings),dAdj=pvpScoreAdjustment(attackerWin?Number(settings.loseScore||16):Number(settings.winScore||24),!attackerWin,dCard,aCard,settings),change=aAdj.change,defenderChange=dAdj.change,aBefore=Number(attacker.season_score),dBefore=Number(defender.season_score),aAfter=Math.max(0,aBefore+(attackerWin?change:-change)),dAfter=Math.max(0,dBefore+(attackerWin?-defenderChange:defenderChange));
