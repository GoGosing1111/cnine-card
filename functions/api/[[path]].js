...
export async function onRequest(context){
  ...
  try{
    ...
    if(path==='battle/fight'&&request.method==='POST'){
      ...
      const bossUltimate=bossShouldCast?{name:String(monster.ultimate_name||'보스 궁극기'),description:String(monster.ultimate_description||''),warningText:String(monster.ultimate_warning_text||'BOSS ULTIMATE'),damagePercent:bossPveDamagePercent,forceCast:bossForceCast,target:String(monster.ultimate_target||'ALL'),theme:String(monster.ultimate_theme||'CRIMSON'),shake:Boolean(monster.ultimate_shake),zoom:Boolean(monster.ultimate_zoom),mediaUrl:String(monster.ultimate_media_url||''),soundUrl:String(monster.ultimate_sound_url||''),durationMs:Math.max(600,Math.min(15000,Number(monster.ultimate_duration_ms||2400))),penalty:bossUltimatePenalty}:null;
    ...
    if(path==='tower/fight'&&request.method==='POST'){
      ...
      if(Boolean(floor.is_boss||floor.monster_is_boss||floorNo%10===0)){const bm=await env.DB.prepare('SELECT * FROM battle_monsters WHERE is_boss=1 AND is_active=1 AND name=? ORDER BY id DESC LIMIT 1').bind(String(floor.monster_name||'')).first();if(bm&&bm.ultimate_enabled&&String(bm.ultimate_name||'').trim()){const trigger=String(bm.ultimate_trigger||'ON_LOSS').toUpperCase(),pre=playerPower>=monsterPower?'WIN':'LOSE',chance=Math.max(0,Math.min(100,Number(bm.ultimate_chance??100))),cast=Boolean(bm.ultimate_force_cast)||trigger==='ALWAYS'||(trigger==='ON_LOSS'&&pre==='LOSE')||(trigger==='CHANCE'&&Math.random()*100<chance);if(cast){const pct=Math.max(0,Math.min(100,Number(bm.ultimate_tower_damage_percent??bm.ultimate_damage_percent??0))),penalty=Math.max(0,Math.floor(playerPower*pct/100));effectiveTowerPower=Math.max(0,playerPower-penalty);towerBossUltimate={name:String(bm.ultimate_name||'보스 궁극기'),description:String(bm.ultimate_description||''),warningText:String(bm.ultimate_warning_text||'BOSS ULTIMATE'),damagePercent:pct,target:String(bm.ultimate_target||'ALL'),theme:String(bm.ultimate_theme||'CRIMSON'),shake:Boolean(bm.ultimate_shake),zoom:Boolean(bm.ultimate_zoom),mediaUrl:String(bm.ultimate_media_url||''),soundUrl:String(bm.ultimate_sound_url||''),durationMs:Math.max(600,Math.min(15000,Number(bm.ultimate_duration_ms||2400))),penalty};}}}
