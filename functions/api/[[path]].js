...
async function userCardScore(env,userId){const settings=await battleSettings(env);const rows=await env.DB.prepare("SELECT c.rarity,c.power_type,c.base_power,uc.breakthrough_level FROM user_cards uc JOIN cards c ON c.id=uc.card_id WHERE uc.user_id=? AND COALESCE(uc.quantity,0)>0 AND COALESCE(c.card_status,'PUBLIC') NOT IN ('RETIRE_PENDING','RETIRED')").bind(userId).all();return rows.results.reduce((sum,c)=>sum+cardBattlePower(c,Number(c.breakthrough_level||0),settings),0)}
...
/* V1191: 차원의 균열 원정 */
function defaultRiftSettings(){return {enabled:true,maxDifficulty:10,maxStages:7,weeklyRewardLimit:3,baseCoin:300,baseShards:4,baseCrystals:5};}
async function riftSettings(env){const row=await env.DB.prepare("SELECT value FROM app_meta WHERE key='pve_rift_settings_v1'").first();const base=defaultRiftSettings();try{const x=JSON.parse(row?.value||'{}');return {enabled:x.enabled!==false,maxDifficulty:Math.max(1,Math.min(20,Math.floor(Number(x.maxDifficulty||base.maxDifficulty)))),maxStages:7,weeklyRewardLimit:Math.max(1,Math.min(20,Math.floor(Number(x.weeklyRewardLimit||base.weeklyRewardLimit)))),baseCoin:Math.max(0,Math.floor(Number(x.baseCoin||base.baseCoin))),baseShards:Math.max(0,Math.floor(Number(x.baseShards||base.baseShards))),baseCrystals:Math.max(0,Math.floor(Number(x.baseCrystals||base.baseCrystals)))};}catch{return base}}
function riftJson(value,fallback){try{return JSON.parse(value||'')}catch{return fallback}}
function riftBuffCatalog(){return [
  {key:'ATTACK_CORE',name:'공격 코어',icon:'⚔',description:'원정 전투력이 15% 증가합니다.',attackPercent:15},
  {key:'BARRIER_FIELD',name:'방벽 전개',icon:'◆',description:'전투 후 받는 피해가 25% 감소합니다.',damageReducePercent:25},
  {key:'VELOCITY_LINK',name:'속도 연계',icon:'↯',description:'원정 전투력이 10% 증가하고 정예전에서 추가 8%를 얻습니다.',attackPercent:10,elitePercent:8},
  {key:'LIFE_PULSE',name:'생명 파동',icon:'♥',description:'승리할 때마다 생존 카드 체력을 8 회복합니다.',healAfterWin:8},
  {key:'BOSS_HUNTER',name:'보스 사냥꾼',icon:'♛',description:'보스 전투력이 25% 증가합니다.',bossPercent:25},
  {key:'SECOND_WIND',name:'두 번째 숨결',icon:'✦',description:'처음 전투 불능이 된 카드 1장을 체력 25로 부활시킵니다.',reviveOnce:true},
  {key:'CRYSTAL_GREED',name:'결정 탐욕',icon:'◇',description:'노드 보상 균열 결정이 40% 증가하지만 받는 피해가 10% 증가합니다.',crystalPercent:40,damageTakenPercent:10}
];}
function riftBuffByKey(key){return riftBuffCatalog().find(x=>x.key===key)||null}
function riftBuffTotals(keys=[]){const out={attackPercent:0,damageReducePercent:0,elitePercent:0,healAfterWin:0,bossPercent:0,crystalPercent:0,damageTakenPercent:0,reviveOnce:false};for(const key of keys){const b=riftBuffByKey(key);if(!b)continue;for(const k of Object.keys(out)){if(typeof out[k]==='boolean')out[k]=out[k]||Boolean(b[k]);else out[k]+=Number(b[k]||0)}}return out}
...
function riftNodeFactor(type,stage,difficulty){const d=Math.max(1,Number(difficulty)||1),s=Math.max(0,Number(stage)||0);if(type==='ELITE')return .91+d*.035+s*.035;if(type==='BOSS')return 1.03+d*.04+s*.04;if(type==='FINAL_BOSS')return 1.19+d*.05+s*.045;return .72+d*.03+s*.03}
async function riftMonsterPool(env,wantBoss=false){let rows=(await env.DB.prepare(`SELECT id,name,image_url AS image,battle_power AS battlePower,is_boss AS isBoss FROM battle_monsters WHERE is_active=1 AND COALESCE(pve_enabled,1)=1 AND COALESCE(tower_only,0)=0 AND COALESCE(is_boss,0)=? ORDER BY COALESCE(pve_display_order,sort_order,0),id LIMIT 80`).bind(wantBoss?1:0).all()).results;if(!rows.length)rows=(await env.DB.prepare(`SELECT id,name,image_url AS image,battle_power AS battlePower,is_boss AS isBoss FROM battle_monsters WHERE is_active=1 ORDER BY id LIMIT 80`).all()).results;return rows}
async function riftBuildChoices(env,runId,stage,difficulty,basePower){const types=riftStageTypes(stage),normal=await riftMonsterPool(env,false),boss=types.some(t=>t==='BOSS'||t==='FINAL_BOSS')?await riftMonsterPool(env,true):[];return types.map((type,index)=>{const node={id:`${runId}:${stage}:${index}:${type}`,stage,type,label:riftNodeLabel(type)};if(['BATTLE','ELITE','BOSS','FINAL_BOSS'].includes(type)){const pool=(type==='BOSS'||type==='FINAL_BOSS')?(boss.length?boss:normal):normal,monster=pool.length?pool[Math.floor(Math.random()*pool.length)]:null;node.monsterId=Number(monster?.id||0);node.name=monster?.name||(type==='FINAL_BOSS'?'균열의 지배자':type==='BOSS'?'균열 수문장':'균열 마물');node.image=monster?.image||'';node.battlePower=Math.max(100,Math.floor(Math.max(100,Number(basePower||1000))*riftNodeFactor(type,stage,difficulty)));node.rewardPreview={coin:Math.max(0,Math.floor((300+stage*90)*difficulty*(type==='ELITE'?1.6:type.includes('BOSS')?2.4:1))),crystals:Math.max(1,Math.floor((5+stage+difficulty)*(type==='ELITE'?1.7:type.includes('BOSS')?2.5:1)))};}else if(type==='REST'){node.name='별빛 휴식처';node.description='생존 카드의 체력을 25 회복합니다.';}else if(type==='EVENT'){node.name='불안정한 균열';node.eventKind=['HEAL','CRYSTAL','POWER'][Math.floor(Math.random()*3)];node.description=({HEAL:'전 카드 체력을 15 회복합니다.',CRYSTAL:'균열 결정 20개를 확보합니다.',POWER:'공격 코어 효과를 획득합니다.'})[node.eventKind];}else if(type==='RISK'){node.name='붕괴 직전의 보물';node.description='전 카드 체력 10을 대가로 균열 결정 35개를 확보합니다.';}return node;});}
...
async function resolveAutoBattle(env,user,settings,monster,cards,ids,uniqueBattle=null){
  ...
  const uniquePlayerPower=Math.max(0,Number(uniqueRuntime?.effectivePower||basePlayerPower));
  ...
  return {result,reward,cardReward,playerPower:uniquePlayerPower,uniqueAbility:uniqueBattleResponsePayload(uniqueBattle,uniqueRuntime)};
}
...
async function ensureRuntimeUpgrades(env){
  ...
  runtimeUpgradeGatePromise=(async()=>{
    ...
    const markers=await env.DB.prepare("SELECT key,value FROM app_meta WHERE key IN ('safe_runtime_upgrade_v1144_stability_gate','safe_runtime_upgrade_v1189_weekly_premium_atomic_receipts','safe_runtime_upgrade_v1191_rift_expedition')").all();
    const markerMap=Object.fromEntries((markers.results||[]).map(row=>[String(row.key),String(row.value||'')]));
    if(markerMap.safe_runtime_upgrade_v1144_stability_gate==='1'&&markerMap.safe_runtime_upgrade_v1189_weekly_premium_atomic_receipts==='1'&&markerMap.safe_runtime_upgrade_v1191_rift_expedition==='1')return true;
  ...
  upgradePromise=(async()=>{
    ...
    if(riftExpeditionDone?.value!=='1'){
      await env.DB.batch([
        ...
        env.DB.prepare("INSERT OR IGNORE INTO inventory_items(code,name,subtitle,description,category,rarity,image_url,sort_order,is_active) VALUES('RIFT_CRYSTAL','균열 결정','RIFT CRYSTAL','차원의 균열 원정에서 획득하는 전용 결정입니다.','MATERIAL','SPECIAL','',6,1)"),
...
export async function onRequest(context){
  ...
  try{
    ...
    if(path==='draw'&&request.method==='POST'){
      ...
      const finalizeAppliedDraw=async (draft,{recoveryVerify=false}={})=>{
        ...
        if(recoveryVerify){
          ...
          response={...draft,user:drawResponseProfileFromRows(actualUser,ownedRows.results||[],masterRow)};
        }
    ...
    if(path==='rift/status'&&request.method==='GET'){
      const user=await authenticate(request,env);if(!user)return json({error:'로그인이 필요합니다.'},401);
      const settings=await riftSettings(env),weekly=await riftWeeklyRow(env,user.id,settings),run=await riftLatestRun(env,user.id),runCards=run?await riftDeckCardsInfo(env,user.id,run.deck):[],savedDeck=await pveDeckCards(env,user.id),crystal=await env.DB.prepare("SELECT quantity FROM cnine_user_inventory WHERE user_id=? AND item_code='RIFT_CRYSTAL'").bind(user.id).first();
      return json({settings,weekly,run:riftPublicRun(run,runCards),savedDeck,riftCrystals:Number(crystal?.quantity||0),weekKey:weekly.weekKey});
    }
    if(path==='rift/start'&&request.method==='POST'){
      ...
      const weekly=await riftWeeklyRow(env,user.id,settings),runId=`rift-${user.id}-${Date.now()}-${Math.random().toString(36).slice(2,9)}`,hp=Object.fromEntries(deckInfo.ids.map(id=>[String(id),100])),basePower=Math.max(100,cards.reduce((sum,card)=>sum+Number(card.power||0),0)),currentChoices=await riftBuildChoices(env,runId,0,difficulty,basePower),run={runId,weekKey:weekly.weekKey,difficulty,status:'ACTIVE',deck:deckInfo.ids.map(String),stage:0,maxStages:settings.maxStages,hp,buffs:[],history:[],currentChoices,activeNode:null,pendingBuffChoices:[],stash:{coin:0,shards:0,crystals:0},basePower,rewardEligible:weekly.rewardCount<settings.weeklyRewardLimit,reviveUsed:false};
      await env.DB.batch([
    ...
    if(path==='rift/select'&&request.method==='POST'){
      ...
      try{const row=await env.DB.prepare("SELECT * FROM pve_rift_runs WHERE run_id=? AND user_id=? AND status='ACTIVE'").bind(runId,user.id).first();if(!row){await riftReceiptFail(env,requestId,'진행 중인 원정을 찾을 수 없습니다.');return json({error:'진행 중인 원정을 찾을 수 없습니다.'},404)}const run=riftStateFromRow(row);if(run.activeNode||run.pendingBuffChoices.length){await riftReceiptFail(env,requestId,'현재 선택을 먼저 완료해야 합니다.');return json({error:'현재 전투 또는 강화 선택을 먼저 완료해주세요.'},409)}const node=run.currentChoices.find(x=>String(x.id)===nodeId);if(!node){await riftReceiptFail(env,requestId,'선택할 수 없는 노드입니다.');return json({error:'선택할 수 없는 원정 노드입니다.'},400)}
        if(['BATTLE','ELITE','BOSS','FINAL_BOSS'].includes(node.type)){run.activeNode=node;run.currentChoices=[];}else{const hp={...run.hp},history={stage:run.stage,type:node.type,name:node.name,result:'RESOLVED',at:new Date().toISOString()};if(node.type==='REST'){for(const id of run.deck)if(Number(hp[id]||0)>0)hp[id]=Math.min(100,Number(hp[id]||0)+25);history.detail='생존 카드 체력 25 회복';}else if(node.type==='EVENT'){if(node.eventKind==='HEAL'){for(const id of run.deck)if(Number(hp[id]||0)>0)hp[id]=Math.min(100,Number(hp[id]||0)+15);history.detail='전 카드 체력 15 회복';}else if(node.eventKind==='CRYSTAL'){run.stash.crystals=Number(run.stash.crystals||0)+20;history.detail='균열 결정 20 획득';}else{if(!run.buffs.includes('ATTACK_CORE'))run.buffs.push('ATTACK_CORE');history.detail='공격 코어 획득';}}else if(node.type==='RISK'){for(const id of run.deck)if(Number(hp[id]||0)>0)hp[id]=Math.max(1,Number(hp[id]||0)-10);run.stash.crystals=Number(run.stash.crystals||0)+35;history.detail='체력 10 소모 · 균열 결정 35 획득';}run.hp=hp;run.history.push(history);run.stage++;run.currentChoices=await riftBuildChoices(env,run.runId,run.stage,run.difficulty,run.basePower);}
    ...
    if(path==='rift/fight'&&request.method==='POST'){
      ...
      try{const row=await env.DB.prepare("SELECT * FROM pve_rift_runs WHERE run_id=? AND user_id=? AND status='ACTIVE'").bind(runId,user.id).first();if(!row){await riftReceiptFail(env,requestId,'진행 중인 원정을 찾을 수 없습니다.');return json({error:'진행 중인 원정을 찾을 수 없습니다.'},404)}const run=riftStateFromRow(row),node=run.activeNode;if(!node||!['BATTLE','ELITE','BOSS','FINAL_BOSS'].includes(node.type)){await riftReceiptFail(env,requestId,'전투 노드가 선택되지 않았습니다.');return json({error:'먼저 전투 노드를 선택해주세요.'},409)}const cards=await riftDeckCardsInfo(env,user.id,run.deck);if(cards.length!==5){await riftReceiptFail(env,requestId,'원정 덱 카드를 확인할 수 없습니다.');return json({error:'원정 덱 카드 일부를 확인할 수 없습니다.'},400)}const totals=riftBuffTotals(run.buffs),boss=node.type==='BOSS'||node.type==='FINAL_BOSS',elite=node.type==='ELITE',hpPower=cards.reduce((sum,c)=>sum+Number(c.power||0)*Math.max(0,Math.min(100,Number(run.hp?.[String(c.id)]||0)))/100,0),bonusPct=totals.attackPercent+(boss?totals.bossPercent:0)+(elite?totals.elitePercent:0),playerPower=Math.max(0,Math.floor(hpPower*(1+bonusPct/100)*(0.97+Math.random()*.06))),monsterPower=Math.max(1,Number(node.battlePower||1)),win=playerPower>=monsterPower,ratio=monsterPower/Math.max(1,playerPower),baseDamage=win?(node.type==='BATTLE'?6:elite?11:16):(node.type==='BATTLE'?24:elite?30:36),damage=Math.max(3,Math.min(60,Math.round(baseDamage*Math.max(.75,Math.min(1.45,ratio))*(1-totals.damageReducePercent/100)*(1+totals.damageTakenPercent/100)))),hp={...run.hp};for(const id of run.deck)if(Number(hp[id]||0)>0)hp[id]=Math.max(0,Number(hp[id]||0)-damage);let revivedCardId=null;if(!run.reviveUsed&&totals.reviveOnce){const dead=run.deck.find(id=>Number(hp[id]||0)<=0);if(dead){hp[dead]=25;run.reviveUsed=true;revivedCardId=dead}}if(win&&totals.healAfterWin>0)for(const id of run.deck)if(Number(hp[id]||0)>0)hp[id]=Math.min(100,Number(hp[id]||0)+totals.healAfterWin);run.hp=hp;const alive=run.deck.filter(id=>Number(hp[id]||0)>0).length,history={stage:run.stage,type:node.type,name:node.name,result:win?'WIN':'LOSE',playerPower,monsterPower,damage,at:new Date().toISOString()};run.history.push(history);let completed=false,status='ACTIVE';if(win){const settings=await riftSettings(env),mult=node.type==='BATTLE'?1:elite?1.6:node.type==='BOSS'?2.3:3.4,crystalBonus=1+totals.crystalPercent/100;run.stash.coin=Number(run.stash.coin||0)+Math.floor((settings.baseCoin+run.stage*90)*run.difficulty*mult);run.stash.shards=Number(run.stash.shards||0)+Math.floor(settings.baseShards*run.difficulty*(elite?1.5:boss?2.5:1));run.stash.crystals=Number(run.stash.crystals||0)+Math.floor((settings.baseCrystals+run.stage+run.difficulty)*mult*crystalBonus);run.activeNode=null;if(node.type==='FINAL_BOSS'){completed=true;status='COMPLETED_PENDING';run.currentChoices=[];run.pendingBuffChoices=[];}else{run.pendingBuffChoices=riftPickBuffChoices(run.buffs);}}else if(alive===0){status='FAILED';run.activeNode=null;run.currentChoices=[];run.pendingBuffChoices=[];}
        run.status=status;const publicRun=riftPublicRun(run,cards),response={ok:true,result:win?'WIN':'LOSE',playerPower,monsterPower,damage,revivedCardId,completed,run:publicRun};const statements=[env.DB.prepare("UPDATE pve_rift_runs SET status=?,state_json=?,updated_at=CURRENT_TIMESTAMP,completed_at=CASE WHEN ?='COMPLETED_PENDING' THEN CURRENT_TIMESTAMP ELSE completed_at END WHERE run_id=? AND user_id=?").bind(status,JSON.stringify(riftStateForSave(run)),status,runId,user.id),env.DB.prepare("UPDATE pve_rift_action_receipts SET status='COMPLETED',response_json=?,updated_at=CURRENT_TIMESTAMP WHERE request_id=?").bind(JSON.stringify(response),requestId)];if(completed)statements.splice(1,0,env.DB.prepare('UPDATE pve_rift_weekly SET completed_count=completed_count+1,highest_difficulty=MAX(highest_difficulty,?),updated_at=CURRENT_TIMESTAMP WHERE user_id=? AND week_key=?').bind(run.difficulty,user.id,run.weekKey));await env.DB.batch(statements);return json(response);
      }catch(error){await riftReceiptFail(env,requestId,error.message);throw error}
    }
    if(path==='rift/buff'&&request.method==='POST'){
      ...
      try{const row=await env.DB.prepare("SELECT * FROM pve_rift_runs WHERE run_id=? AND user_id=? AND status='ACTIVE'").bind(runId,user.id).first();if(!row){await riftReceiptFail(env,requestId,'진행 중인 원정을 찾을 수 없습니다.');return json({error:'진행 중인 원정을 찾을 수 없습니다.'},404)}const run=riftStateFromRow(row);if(!run.pendingBuffChoices.includes(buffKey)){await riftReceiptFail(env,requestId,'선택할 수 없는 강화입니다.');return json({error:'선택할 수 없는 원정 강화입니다.'},400)}if(!run.buffs.includes(buffKey))run.buffs.push(buffKey);run.pendingBuffChoices=[];run.stage++;run.currentChoices=await riftBuildChoices(env,run.runId,run.stage,run.difficulty,run.basePower);const cards=await riftDeckCardsInfo(env,user.id,run.deck),response={ok:true,run:riftPublicRun(run,cards)};await env.DB.batch([env.DB.prepare("UPDATE pve_rift_runs SET state_json=?,updated_at=CURRENT_TIMESTAMP WHERE run_id=? AND user_id=? AND status='ACTIVE'").bind(JSON.stringify(riftStateForSave(run)),runId,user.id),env.DB.prepare("UPDATE pve_rift_action_receipts SET status='COMPLETED',response_json=?,updated_at=CURRENT_TIMESTAMP WHERE request_id=?").bind(JSON.stringify(response),requestId)]);return json(response);
      }catch(error){await riftReceiptFail(env,requestId,error.message);throw error}
    }
    if(path==='rift/claim'&&request.method==='POST'){
      ...
      try{const reserved=await env.DB.prepare("UPDATE pve_rift_runs SET status='CLAIMING',updated_at=CURRENT_TIMESTAMP WHERE run_id=? AND user_id=? AND status='COMPLETED_PENDING'").bind(runId,user.id).run();if(!reserved.meta.changes){await riftReceiptFail(env,requestId,'수령 가능한 원정 보상이 없습니다.');return json({error:'수령 가능한 원정 보상이 없습니다.'},409)}const row=await env.DB.prepare("SELECT * FROM pve_rift_runs WHERE run_id=? AND user_id=? AND status='CLAIMING'").bind(runId,user.id).first(),run=riftStateFromRow(row),settings=await riftSettings(env),weekly=await riftWeeklyRow(env,user.id,settings),eligible=run.rewardEligible&&weekly.rewardCount<settings.weeklyRewardLimit,coin=eligible?Math.max(0,Math.floor(Number(run.stash.coin||0))):0,shards=eligible?Math.max(0,Math.floor(Number(run.stash.shards||0))):0,crystals=eligible?Math.max(0,Math.floor(Number(run.stash.crystals||0))):0,before=await env.DB.prepare('SELECT coin,card_shards FROM users WHERE id=?').bind(user.id).first(),inv=await env.DB.prepare("SELECT quantity FROM cnine_user_inventory WHERE user_id=? AND item_code='RIFT_CRYSTAL'").bind(user.id).first(),coinAfter=Number(before?.coin||0)+coin,shardsAfter=Number(before?.card_shards||0)+shards,crystalsAfter=Number(inv?.quantity||0)+crystals,response={ok:true,rewarded:eligible,reward:{coin,shards,crystals},message:eligible?'원정 보상을 수령했습니다.':'이번 주 보상 횟수를 모두 사용해 기록만 반영되었습니다.'};const statements=[env.DB.prepare("UPDATE pve_rift_runs SET status='CLAIMED',updated_at=CURRENT_TIMESTAMP WHERE run_id=? AND user_id=? AND status='CLAIMING'").bind(runId,user.id)];if(eligible){statements.unshift(env.DB.prepare('UPDATE users SET coin=coin+?,card_shards=card_shards+? WHERE id=?').bind(coin,shards,user.id),env.DB.prepare("INSERT INTO cnine_user_inventory(user_id,item_code,quantity,unseen_quantity,updated_at) VALUES(?,'RIFT_CRYSTAL',?,0,CURRENT_TIMESTAMP) ON CONFLICT(user_id,item_code) DO UPDATE SET quantity=cnine_user_inventory.quantity+excluded.quantity,updated_at=CURRENT_TIMESTAMP").bind(user.id,crystals),env.DB.prepare("UPDATE pve_rift_weekly SET reward_count=reward_count+1,updated_at=CURRENT_TIMESTAMP WHERE user_id=? AND week_key=? AND reward_count<?").bind(user.id,run.weekKey,settings.weeklyRewardLimit));if(coin>0)statements.push(env.DB.prepare("INSERT INTO coin_logs(user_id,change_amount,balance_after,reason) VALUES(?,?,?,'차원의 균열 원정 보상')").bind(user.id,coin,coinAfter));if(shards>0)statements.push(env.DB.prepare("INSERT INTO shard_logs(user_id,change_amount,balance_after,reason) VALUES(?,?,?,'차원의 균열 원정 보상')").bind(user.id,shards,shardsAfter));if(crystals>0)statements.push(env.DB.prepare("INSERT INTO inventory_logs(user_id,item_code,change_amount,balance_after,reason,reference_type,reference_id) VALUES(?,'RIFT_CRYSTAL',?,?,?,'RIFT',?)").bind(user.id,crystals,crystalsAfter,'차원의 균열 원정 보상',runId));}statements.push(env.DB.prepare("UPDATE pve_rift_action_receipts SET status='COMPLETED',response_json=?,updated_at=CURRENT_TIMESTAMP WHERE request_id=?").bind(JSON.stringify(response),requestId));await env.DB.batch(statements);const updated=await env.DB.prepare('SELECT * FROM users WHERE id=?').bind(user.id).first();response.user=await profile(env,updated);response.weekly=await riftWeeklyRow(env,user.id,settings);response.riftCrystals=crystalsAfter;await env.DB.prepare("UPDATE pve_rift_action_receipts SET response_json=?,updated_at=CURRENT_TIMESTAMP WHERE request_id=?").bind(JSON.stringify(response),requestId).run();return json(response);
    ...
    if(path==='rift/abandon'&&request.method==='POST'){
      const user=await authenticate(request,env);if(!user)return json({error:'로그인이 필요합니다.'},401);const body=await readBody(request),runId=String(body.runId||'');const result=await env.DB.prepare("UPDATE pve_rift_runs SET status='ABANDONED',updated_at=CURRENT_TIMESTAMP WHERE run_id=? AND user_id=? AND status IN ('ACTIVE','FAILED')").bind(runId,user.id).run();if(!result.meta.changes)return json({error:'포기할 원정을 찾을 수 없습니다.'},404);return json({ok:true});
    }
    ...
    if(path==='pvp/fight'&&request.method==='POST'){
      ...
      const aPower=Math.max(0,Math.floor(Number(aUniqueRuntime?.effectivePower||aBase)*(1+Number(aSyn.totals.attackPercent||0)/100))),dPower=Math.max(0,Math.floor(Number(dUniqueRuntime?.effectivePower||dBase)*(1+Number(dSyn.totals.attackPercent||0)/100)));
    ...
    if(path==='ranking'){
      ...
      const rows=await env.DB.prepare(`SELECT u.id,u.nickname,c.rarity,c.power_type,c.base_power,uc.breakthrough_level,COUNT(uc.card_id) OVER(PARTITION BY u.id) AS card_count
        FROM users u LEFT JOIN user_cards uc ON uc.user_id=u.id AND COALESCE(uc.quantity,0)>0 LEFT JOIN cards c ON c.id=uc.card_id
    ...
    if(path==='admin/dashboard'){
      ...
      const [users,usersToday,cards,draws,coins,banned,coupons,urOwned,ssrOwned]=await Promise.all([
        ...
        env.DB.prepare("SELECT COUNT(*) count FROM user_cards uc JOIN cards c ON c.id=uc.card_id WHERE c.rarity='UR' AND COALESCE(uc.quantity,0)>0").first(),
        env.DB.prepare("SELECT COUNT(*) count FROM user_cards uc JOIN cards c ON c.id=uc.card_id WHERE c.rarity='SSR' AND COALESCE(uc.quantity,0)>0").first()
      ]);
    ...
    if(path==='admin/tiers'){
      ...
      if(request.method==='GET'){
        ...
        const rows=await env.DB.prepare(`SELECT u.nickname,c.rarity,c.power_type,c.base_power,uc.breakthrough_level FROM users u LEFT JOIN user_cards uc ON uc.user_id=u.id AND COALESCE(uc.quantity,0)>0 LEFT JOIN cards c ON c.id=uc.card_id WHERE u.status='ACTIVE' AND (u.banned_until IS NULL OR u.banned_until<=datetime('now'))`).all();
    ...
    if(path==='admin/users'){
      ...
      const sql=`SELECT u.id,u.nickname,u.coin,u.card_shards,u.role,u.status,u.created_at,u.last_login_at,w.status AS verification_status,w.wago_nickname,w.wago_member_no,w.verified_at,COUNT(uc.card_id) AS card_count,COALESCE(SUM(CASE WHEN c.rarity='UR' THEN 1 ELSE 0 END),0) AS ur_count,COALESCE(SUM(CASE WHEN c.rarity='SSR' THEN 1 ELSE 0 END),0) AS ssr_count,COALESCE(SUM(CASE WHEN c.rarity='LIMITED' THEN 1 ELSE 0 END),0) AS limited_count,COALESCE(SUM(CASE WHEN c.rarity='FUR' THEN 1 ELSE 0 END),0) AS fur_count
        FROM users u LEFT JOIN wago_verifications w ON w.user_id=u.id LEFT JOIN user_cards uc ON uc.user_id=u.id AND COALESCE(uc.quantity,0)>0 LEFT JOIN cards c ON c.id=uc.card_id ${filters.length?'WHERE '+filters.join(' AND '):''}
