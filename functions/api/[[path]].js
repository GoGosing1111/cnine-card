...
function defaultPvpSettings(){return {enabled:true,seasonName:'시즌 1',startsAt:null,endsAt:null,initialScore:1000,winScore:24,loseScore:16,matchCardRange:15,historyLimit:100,tiers:[{id:'bronze',name:'브론즈',min:0,color:'#b87333',aura:false,rewardCoin:500},{id:'silver',name:'실버',min:1100,color:'#c9d4e3',aura:false,rewardCoin:1000},{id:'gold',name:'골드',min:1250,color:'#ffd15c',aura:false,rewardCoin:2000},{id:'platinum',name:'플래티넘',min:1450,color:'#5ff0df',aura:true,rewardCoin:4000},{id:'diamond',name:'다이아',min:1700,color:'#69cfff',aura:true,rewardCoin:7000},{id:'master',name:'마스터',min:2050,color:'#bd7cff',aura:true,rewardCoin:12000},{id:'grandmaster',name:'그랜드마스터',min:2500,color:'#ff6f91',aura:true,rewardCoin:20000}]};}
async function pvpSettings(env){const base=defaultPvpSettings(),row=await env.DB.prepare("SELECT value FROM app_meta WHERE key='pvp_settings_v1'").first();if(!row?.value)return base;try{return {...base,...JSON.parse(row.value)}}catch{return base}}
...
export async function onRequest(context){
  ...
  try{
    ...
    if(path==='pvp/opponents'){
      ...
      const users=await env.DB.prepare(`SELECT u.id,u.nickname,p.season_score,p.wins,p.losses,p.highest_score FROM users u JOIN pvp_profiles p ON p.user_id=u.id JOIN pvp_decks d ON d.user_id=u.id WHERE u.id<>? AND u.status='ACTIVE' ORDER BY ABS(p.season_score-?) ASC LIMIT 40`).bind(user.id,mine.season_score).all();
      const out=[];for(const x of users.results){const cs=await userCardScore(env,x.id),pct=myCard?Math.abs(cs-myCard)/myCard*100:0;if(pct<=Number(settings.matchCardRange||15)*2)out.push({...x,cardScore:cs,diffPercent:myCard?((cs-myCard)/myCard*100):0,tier:resolveTier(Number(x.season_score),settings.tiers)});if(out.length>=3)break}return json({opponents:out,myCardScore:myCard});
    }
    ...
    if(path==='pvp/reward/claim'&&request.method==='POST'){
      const user=await authenticate(request,env);if(!user)return json({error:'로그인이 필요합니다.'},401);const settings=await pvpSettings(env);if(!settings.enabled&&!isAdminRole(user))return json({error:'현재 PvP 콘텐츠가 중지되어 있습니다.'},503);const p=await ensurePvpProfile(env,user,settings),tier=resolveTier(Number(p.highest_score),settings.tiers),reward=Number(tier.rewardCoin||0),exists=await env.DB.prepare('SELECT 1 FROM pvp_reward_claims WHERE user_id=? AND season_name=? AND tier_id=?').bind(user.id,settings.seasonName,tier.id).first();if(exists)return json({error:'이미 수령한 시즌 티어 보상입니다.'},409);await env.DB.batch([env.DB.prepare('INSERT INTO pvp_reward_claims(user_id,season_name,tier_id,reward_coin) VALUES(?,?,?,?)').bind(user.id,settings.seasonName,tier.id,reward),env.DB.prepare('UPDATE users SET coin=coin+? WHERE id=?').bind(reward,user.id)]);const updated=await env.DB.prepare('SELECT * FROM users WHERE id=?').bind(user.id).first();return json({ok:true,tier,reward,user:await profile(env,updated)});
    }
    ...
    if(path==='admin/tiers'){
      ...
      if(request.method==='PATCH'){
        ...
        const currentPvp=await pvpSettings(env);
        const livePvp={...currentPvp,enabled:payload.pvp?.enabled!==false,status:String(payload.pvp?.status||currentPvp.status||'ACTIVE').slice(0,30),seasonName:String(payload.pvp?.seasonName||currentPvp.seasonName||'시즌 1').slice(0,40),startsAt:payload.pvp?.startsAt||null,endsAt:payload.pvp?.endsAt||null,tiers:Array.isArray(payload.pvp?.tiers)?payload.pvp.tiers:currentPvp.tiers};
        const clean={cardScoreTiers:tiers,pvp:livePvp};
        await env.DB.batch([env.DB.prepare("INSERT OR REPLACE INTO app_meta(key,value,updated_at) VALUES('tier_settings_v1',?,CURRENT_TIMESTAMP)").bind(JSON.stringify(clean)),env.DB.prepare("INSERT OR REPLACE INTO app_meta(key,value,updated_at) VALUES('pvp_settings_v1',?,CURRENT_TIMESTAMP)").bind(JSON.stringify(livePvp))]);await writeAdminLog(env,admin,'TIER_SETTINGS_UPDATE','SETTINGS','tiers',before,clean);return json({ok:true,settings:clean});
      }
    }
