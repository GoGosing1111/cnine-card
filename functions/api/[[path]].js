...
export async function onRequest(context){
  ...
  try{
    ...
    if(path==='battle/fight'&&request.method==='POST'){
      ...
      const updated=await env.DB.prepare('SELECT * FROM users WHERE id=?').bind(user.id).first();
      return json({result,reward,cardReward,playerPower,monsterPower,monster:{id:monster.id,name:monster.name,image:monster.image_url,isBoss:Boolean(monster.is_boss)},cards,energy:energyAfter,serverNow:new Date().toISOString(),user:await profile(env,updated)});
    }
    ...
    if(path==='admin/verified-coin-message-send'&&request.method==='POST'){
      ...
      const allowedRoles=['USER','CARD_MANAGER','EVENT_MANAGER','SUPPORT'];if(includeOwner)allowedRoles.push('OWNER');if(includeAdmin)allowedRoles.push('ADMIN');
      const placeholders=allowedRoles.map(()=>'?').join(',');
      const users=await env.DB.prepare(`SELECT w.user_id,u.nickname,UPPER(TRIM(COALESCE(u.role,'USER'))) AS role FROM wago_verifications w JOIN users u ON u.id=w.user_id WHERE UPPER(TRIM(COALESCE(w.status,'')))='VERIFIED' AND UPPER(TRIM(COALESCE(u.status,'ACTIVE')))='ACTIVE' AND UPPER(TRIM(COALESCE(u.role,'USER'))) IN (${placeholders})`).bind(...allowedRoles).all();let sent=0,sentUsers=0,sentOwners=0,sentAdmins=0;
