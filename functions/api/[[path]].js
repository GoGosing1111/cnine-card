...
export async function onRequest(context){
  ...
  try{
    ...
    if(path==='pvp/config'){
      const user=await authenticate(request,env);if(!user)return json({error:'로그인이 필요합니다.'},401);
      const settings=await pvpSettings(env),profile=await ensurePvpProfile(env,user,settings),deck=await pvpDeckCards(env,user.id);
      return json({settings,profile:{...profile,tier:resolveTier(Number(profile.season_score),settings.tiers)},deck,energy:await pvpEnergyState(env,user,settings),bypass:isAdminRole(user),serverNow:new Date().toISOString()});
    }
    ...
    if(path==='pvp/opponents'){
      const user=await authenticate(request,env);if(!user)return json({error:'로그인이 필요합니다.'},401);const settings=await pvpSettings(env);if(!settings.enabled&&!isAdminRole(user))return json({error:'현재 PvP 콘텐츠가 중지되어 있습니다.'},503);const mine=await ensurePvpProfile(env,user,settings);
