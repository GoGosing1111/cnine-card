...
export async function onRequest(context){
  ...
  try{
    ...
    if(path==='pvp/opponents'){
      ...
      const seasonRange=Number(settings.matchSeasonRange||300),users=await env.DB.prepare(`SELECT u.id,u.nickname,p.season_score,p.wins,p.losses,p.highest_score FROM users u JOIN pvp_profiles p ON p.user_id=u.id JOIN pvp_decks d ON d.user_id=u.id WHERE u.id<>? AND u.status='ACTIVE' AND ABS(p.season_score-?)<=? ORDER BY ABS(p.season_score-?) ASC LIMIT 60`).bind(user.id,mine.season_score,seasonRange,mine.season_score).all();
      const out=[];for(const x of users.results){const cs=await userCardScore(env,x.id),pct=myCard?Math.abs(cs-myCard)/myCard*100:0;if(pct<=Number(settings.matchCardRange||15))out.push({...x,cardScore:cs,diffPercent:myCard?((cs-myCard)/myCard*100):0,tier:resolveTier(Number(x.season_score),settings.tiers)});if(out.length>=6)break}return json({opponents:out,myCardScore:myCard});
    }
    if(path==='pvp/fight'&&request.method==='POST'){
      ...
      const settings=await pvpSettings(env);if(!settings.enabled&&!isAdminRole(user))return json({error:'현재 PvP 시즌이 중지되어 있습니다.'},503);const attacker=await ensurePvpProfile(env,user,settings),defUser=await env.DB.prepare("SELECT * FROM users WHERE id=? AND status='ACTIVE'").bind(defenderId).first();if(!defUser)return json({error:'상대를 찾을 수 없습니다.'},404);const defender=await ensurePvpProfile(env,defUser,settings);
      ...
      await env.DB.batch([env.DB.prepare('UPDATE pvp_profiles SET season_score=?,highest_score=MAX(highest_score,?),wins=wins+?,losses=losses+?,updated_at=CURRENT_TIMESTAMP WHERE user_id=?').bind(aAfter,aAfter,attackerWin?1:0,attackerWin?0:1,user.id),env.DB.prepare('UPDATE pvp_profiles SET season_score=?,highest_score=MAX(highest_score,?),wins=wins+?,losses=losses+?,updated_at=CURRENT_TIMESTAMP WHERE user_id=?').bind(dAfter,dAfter,attackerWin?0:1,attackerWin?1:0,defenderId),env.DB.prepare('INSERT INTO pvp_match_history(attacker_id,defender_id,attacker_name,defender_name,attacker_deck,defender_deck,attacker_card_score,defender_card_score,attacker_power,defender_power,winner_id,attacker_score_before,attacker_score_after,defender_score_before,defender_score_after,score_change) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)').bind(user.id,defenderId,user.nickname,defUser.nickname,JSON.stringify(aDeck),JSON.stringify(dDeck),aCard,dCard,aPower,dPower,winnerId,aBefore,aAfter,dBefore,dAfter,change)]);return json({result:attackerWin?'WIN':'LOSE',scoreChange:attackerWin?change:-change,scoreAfter:aAfter,attackerPower:aPower,defenderPower:dPower,opponent:defUser.nickname,attackerDeck:aDeck,defenderDeck:dDeck,scoreAdjustment:aAdj,opponentScoreAdjustment:dAdj,energy:pvpEnergy,serverNow:new Date().toISOString()});
    }
    if(path==='pvp/history'){
    ...
    if(path==='pvp/ranking'){
      const user=await authenticate(request,env);if(!user)return json({error:'로그인이 필요합니다.'},401);const settings=await pvpSettings(env);if(!settings.enabled&&!isAdminRole(user))return json({error:'현재 PvP 콘텐츠가 중지되어 있습니다.'},503);const rows=await env.DB.prepare(`SELECT u.id,u.nickname,p.season_score,p.highest_score,p.wins,p.losses FROM pvp_profiles p JOIN users u ON u.id=p.user_id WHERE u.status='ACTIVE' ORDER BY p.season_score DESC,p.wins DESC,u.nickname LIMIT 100`).all();const ranking=rows.results.map((x,i)=>({...x,rank:i+1,tier:resolveTier(Number(x.season_score),settings.tiers)}));return json({settings,ranking,me:ranking.find(x=>Number(x.id)===Number(user.id))||null});
    }
    ...
    if(path==='admin/tiers'){
      ...
      if(request.method==='GET'){
        ...
        const pvpRows=await env.DB.prepare(`SELECT u.nickname,p.season_score,p.highest_score,p.wins,p.losses FROM pvp_profiles p JOIN users u ON u.id=p.user_id WHERE u.status='ACTIVE' ORDER BY p.season_score DESC,p.wins DESC,u.nickname LIMIT 100`).all();
        const pvpStats=await env.DB.prepare(`SELECT COUNT(*) AS profiles,COALESCE(SUM(wins+losses),0)/2 AS matches,COALESCE(MAX(season_score),0) AS top_score FROM pvp_profiles`).first();
