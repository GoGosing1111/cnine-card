...
async function cubeBoostContext(env,userId,source,cfg){if(!cfg.enabled||!cfg[`${String(source).toLowerCase()}Enabled`])return {eligible:false,highGradeCount:0,multiplier:1,pityMissWins:0,pityBonusRate:0};const user=await env.DB.prepare('SELECT role FROM users WHERE id=?').bind(userId).first();if(cfg.excludeAdmins&&['OWNER','ADMIN'].includes(String(user?.role||'').toUpperCase()))return {eligible:false,highGradeCount:0,multiplier:1,pityMissWins:0,pityBonusRate:0};const owned=await env.DB.prepare(`SELECT COALESCE(SUM(COALESCE(uc.quantity,0)),0) AS total FROM user_cards uc JOIN cards c ON c.id=uc.card_id WHERE uc.user_id=? AND COALESCE(uc.quantity,0)>0 AND c.rarity IN ('MA','LIMITED','FUR') AND COALESCE(c.card_status,'PUBLIC') NOT IN ('RETIRE_PENDING','RETIRED')`).bind(userId).first();const highGradeCount=Number(owned?.total||0);if(highGradeCount>=cfg.targetHighGradeCount)return {eligible:false,highGradeCount,multiplier:1,pityMissWins:0,pityBonusRate:0};const multiplier=highGradeCount<=0?cfg.zeroCountMultiplier:cfg.oneCountMultiplier;let pityMissWins=0;if(cfg.pityEnabled){const row=await env.DB.prepare('SELECT premium_miss_wins FROM cube_drop_boost_state WHERE user_id=? AND source=?').bind(userId,source).first();pityMissWins=Number(row?.premium_miss_wins||0)}const pityBonusRate=cfg.pityEnabled&&pityMissWins>=cfg.pityStartWins?Math.min(cfg.pityMaxBonusRate,(pityMissWins-cfg.pityStartWins+1)*cfg.pityIncrementRate):0;return {eligible:true,highGradeCount,multiplier,pityMissWins,pityBonusRate};}
  ...
  try{
    ...
    if(path==='admin/settings'){
      ...
      if(request.method==='GET'){
        ...
        return json({settings:Object.fromEntries(rows.results.map(x=>[x.key,x.value])),attendance:await attendanceSettings(env),cubes:await cubeSettings(env),cubeDrops:await cubeDropSettings(env),cubeBoost:await cubeBoostSettings(env),role:admin.role});
      }
      if(request.method==='POST'){
        const payload=await readBody(request);
