...
function raidWeeklyBossRows(env){
  return raidMemo('weekly-bosses-v2140',30000,async()=>{
    await ensureWeeklyRaidBossesV1(env);
    const names=WEEKLY_RAID_BOSSES_V1.map(row=>row.name),marks=names.map(()=>'?').join(','),rows=(await env.DB.prepare(`SELECT id,name,image_url AS image,max_hp AS maxHp,defense_rate AS defenseRate FROM raid_bosses WHERE is_active=1 AND name IN (${marks})`).bind(...names).all()).results||[],byName=Object.fromEntries(rows.map(row=>[String(row.name),row]));
    return WEEKLY_RAID_BOSSES_V1.map(profile=>({...profile,...(byName[profile.name]||{}),image:profile.sourceArt,battleSprite:profile.battleSprite,maxHp:Number(byName[profile.name]?.maxHp||profile.maxHp),defenseRate:Number(byName[profile.name]?.defenseRate??profile.defenseRate)}));
  });
}
async function raidWeeklyToday(env,nowMs=Date.now()){
  const profile=weeklyRaidBossForKst(nowMs),rows=await raidWeeklyBossRows(env),boss=rows.find(row=>row.code===profile.code)||null;
  if(!boss?.id)throw new Error(`요일 레이드 보스 등록을 찾지 못했습니다: ${profile.name}`);
  return {boss,rows};
}
...
async function handleRequest(context){
  ...
  try{
    ...
    if(path==='raid/status'){
      ...
      const rooms=roomRows.map((room,i)=>{const slot=(cfg.timeSlots||[]).find(x=>String(x.id)===String(room.slotId)),activeSlotId=String(schedule.currentSlot?.id||''),bossProfile=weeklyRaidBossByName(room.bossName),slotOpen=owner||room.slotId==='LEGACY'||room.slotId==='ALWAYS'||(activeSlotId&&activeSlotId===String(room.slotId));return {...room,bossImage:bossProfile?.battleSprite||room.bossImage,powerRating:Number(bossProfile?.powerRating||0),slotLabel:slot?.label||room.slotId,roomNumber:roomRows.length-i,joinable:slotOpen&&room.status==='LOBBY'&&Date.parse(room.startsAt)>Date.now()&&Number(room.participantCount)<Number(cfg.maxParticipants||30)}});
      if(!current){
        const policies=await raidBossOpenPolicies(env),today=weekly.boss,availableBosses=[{...today,openCost:Number(policies[String(today.id)]?.cost||0),ownerTestVisible:false}];
    ...
    if(path==='raid/open'&&request.method==='POST'){
      ...
      const policy=policies[String(bossId)]||{},cost=Math.max(0,Number(policy.cost||0));if(Number(fresh?.coin||0)<cost)return json({error:'레이드 개방에 필요한 코인이 부족합니다.'},400);
