  ...
  try{
    ...
    if(path==='admin/settings'){
      ...
      if(request.method==='GET'){
        ...
        return json({settings:Object.fromEntries(rows.results.map(x=>[x.key,x.value])),attendance:await attendanceSettings(env),cubes:await cubeSettings(env),cubeDrops:await cubeDropSettings(env),role:admin.role});
      }
      if(request.method==='POST'){
        const payload=await readBody(request);
