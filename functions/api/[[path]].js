...
function riftPublicNode(run,node){if(!node)return null;const scale=riftLegacyDifficultyOneScale(run,node);return scale===1?node:{...node,battlePower:Math.max(1,Math.floor(Number(node.battlePower||1)*scale))}}
function riftPublicRun(run,cards=[]){if(!run)return null;return {...run,currentChoices:(run.currentChoices||[]).map(node=>riftPublicNode(run,node)),activeNode:riftPublicNode(run,run.activeNode),buffs:run.buffs.map(key=>riftBuffByKey(key)).filter(Boolean),cards:run.deck.map(id=>{const c=cards.find(x=>String(x.id)===String(id));return c?{id:String(c.id),title:c.title,rarity:c.rarity,powerType:c.power_type,image:c.image,focusX:Number(c.focus_x||50),focusY:Number(c.focus_y||50),breakthroughLevel:Number(c.breakthrough_level||0),power:Number(c.power||0),hp:Number(run.hp?.[id]??100)}:{id:String(id),hp:Number(run.hp?.[id]??100)}})};}
