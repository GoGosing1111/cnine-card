  ...
  try{
    ...
    if(path==='inventory'){
      ...
      const rows=await env.DB.prepare(`SELECT i.code,i.name,i.subtitle,i.description,i.category,i.rarity,i.image_url AS image,COALESCE(ui.quantity,0) AS quantity,COALESCE(ui.unseen_quantity,0) AS unseenQuantity,
          CASE WHEN i.category='MATERIAL' OR i.code IN ('VEHICLE_PART_TIRE','VEHICLE_PART_FRAME','VEHICLE_PART_ENGINE') THEN 0 WHEN i.code='BLACK_MIRACLE_PACK' THEN COALESCE((SELECT CASE LOWER(CAST(json_extract(value,'$.enabled') AS TEXT)) WHEN 'true' THEN 1 WHEN 'false' THEN 0 ELSE CAST(json_extract(value,'$.enabled') AS INTEGER) END FROM app_meta WHERE key='black_miracle_pack_settings_v1485'),1) ELSE 1 END AS usable
