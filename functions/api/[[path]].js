...
export async function onRequest(context){
  ...
  try{
    ...
    if(path==='pvp/config'){
      ...
      return json({settings,profile:{...profile,tier:resolveTier(Number(profile.season_score),settings.tiers)},deck,cardScore:score,serverNow:new Date().toISOString()});
    }
    if(path==='pvp/deck'&&request.method==='POST'){
      const user=await authenticate(request,env);if(!user)return json({error:'로그인이 필요합니다.'},401);
      const body=await readBody(request),ids=[...new Set((body.cardIds||[]).map(String))];if(ids.length!==5)return json({error:'PvP 덱은 보유 카드 5장으로 편성해야 합니다.'},400);
    ...
    if(path==='pvp/opponents'){
      const user=await authenticate(request,env);if(!user)return json({error:'로그인이 필요합니다.'},401);const settings=await pvpSettings(env),mine=await ensurePvpProfile(env,user,settings),myCard=await userCardScore(env,user.id);
