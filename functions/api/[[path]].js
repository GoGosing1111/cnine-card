...
async function raidDeckPower(env,userId,cardIds,mode='RAID'){
  ...
  const prestigeCount=owned.results.filter(card=>String(card.rarity||'').toUpperCase()==='PRESTIGE').length,furCount=owned.results.filter(card=>String(card.rarity||'').toUpperCase()==='FUR').length;
...
async function readBattleSettings(env){const row=await env.DB.prepare("SELECT value FROM app_meta WHERE key='battle_settings_v1'").first();const base=defaultBattleSettings();if(!row?.value)return base;try{return cleanBattleSettingsPayload(JSON.parse(row.value),base)}catch{return base}}
...
async function resolveAutoBattle(env,user,settings,monster,cards,ids,uniqueBattle=null,requestId=''){
  ...
  let cardReward=null,equipmentReward=null,blackMiracleReward=null;if(result==='WIN'){if(settings.cardDrop?.enabled!==false){const cardRate=Math.max(0,Math.min(100,Number(settings.cardDrop?.defaultRate??0)));if(cardRate>0&&Math.random()*100<cardRate)cardReward=await grantBattleCard(env,user.id,settings);}equipmentReward=await safeEquipmentDrop(env,{userId:user.id,sourceType:'PVE_AUTO',sourceId:String(monster.id),requestId:requestId||`${Date.now()}-${monster.id}`});blackMiracleReward=await rollBlackMiracleDrop(env,{userId:user.id,source:'PVE_AUTO',referenceId:requestId||`${Date.now()}-${monster.id}`});}
  ...
  return {result,reward,cardReward,equipmentReward,blackMiracleReward,playerPower:uniquePlayerPower,cardPower,characterBonus,uniqueAbility:uniqueBattleResponsePayload(uniqueBattle,uniqueRuntime)};
}
...
async function handleRequest(context){
  ...
  try{
    ...
    if(path==='battle/config'){
      ...
      return json({settings,deck,characterBonus,energy,serverNow:new Date().toISOString(),monsters:monsters.results,burningEvent:burningPublicState(burning),battleEngine:battleEngineState(settings,user)});
    }
    ...
    if(path==='battle/fight'&&request.method==='POST'){
      ...
      const bossPveDamagePercent=Math.max(0,Math.min(100,Number(monster.ultimate_pve_damage_percent??monster.ultimate_damage_percent??0))),bossUltimatePenalty=bossShouldCast?Math.max(0,Math.floor(playerPower*bossPveDamagePercent/100)):0;
    ...
    if(path==='pvp/deck'&&request.method==='POST'){
      ...
      await env.DB.batch([env.DB.prepare('INSERT INTO pvp_decks(user_id,card_ids,updated_at) VALUES(?,?,CURRENT_TIMESTAMP) ON CONFLICT(user_id) DO UPDATE SET card_ids=excluded.card_ids,updated_at=CURRENT_TIMESTAMP').bind(user.id,JSON.stringify(ids)),env.DB.prepare('INSERT INTO pvp_deck_presets(user_id,preset_no,card_ids,updated_at) VALUES(?,?,?,CURRENT_TIMESTAMP) ON CONFLICT(user_id,preset_no) DO UPDATE SET card_ids=excluded.card_ids,updated_at=CURRENT_TIMESTAMP').bind(user.id,presetNo,JSON.stringify(ids)),env.DB.prepare('INSERT INTO pvp_active_presets(user_id,preset_no,updated_at) VALUES(?,?,CURRENT_TIMESTAMP) ON CONFLICT(user_id) DO UPDATE SET preset_no=excluded.preset_no,updated_at=CURRENT_TIMESTAMP').bind(user.id,presetNo)]);return json({ok:true,deck:ids,presetNo,prestigeLimit:PRESTIGE_DECK_LIMIT,furLimit:FUR_DECK_LIMIT});
    }
    ...
    if(path==='pvp/fight'&&request.method==='POST'){
      ...
      const equipmentReward=attackerWin?await safeEquipmentDrop(env,{userId:user.id,sourceType:'PVP',sourceId:'*',requestId}):null,blackMiracleReward=attackerWin?await rollBlackMiracleDrop(env,{userId:user.id,source:'PVP',referenceId:requestId}):null,freshCoinUser=await env.DB.prepare('SELECT coin,magic_crystals FROM users WHERE id=?').bind(user.id).first(),weeklyPremiumCube=await premiumCubeWeeklyStatus(env,user.id);
      return json({result:attackerWin?'WIN':'LOSE',burningEvent:burningPublicState(burning),battleEngine:engineState,battleV2,cubeReward,weeklyPremiumCube,magicReward,equipmentReward,blackMiracleReward,attackerCharacterBonus:aCharacterBonus,defenderCharacterBonus:dCharacterBonus,attackerCardPower:aCardPower,defenderCardPower:dCardPower,scoreChange:attackerWin?change:-change,scoreAfter:aAfter,coinReward:attackerCoinReward,coinAfter:freshCoinUser?.coin??coinUser.coin,magicCrystalsAfter:Number(freshCoinUser?.magic_crystals||0),rewardRecipient:'ATTACKER',attackerPower:aPower,defenderPower:dPower,attackerTitle:titleMap[String(user.id)]||null,defenderTitle:titleMap[String(defenderId)]||null,opponent:defUser.nickname,attackerDeck:aUnique.cards||aDeck,defenderDeck:dUnique.cards||dDeck,uniqueAbility:{attacker:uniqueBattleResponsePayload(aUnique,aUniqueRuntime),defender:uniqueBattleResponsePayload(dUnique,dUniqueRuntime)},scoreAdjustment:aAdj,opponentScoreAdjustment:dAdj,energy:pvpEnergy,serverNow:new Date().toISOString()});
    }
    if(path==='pvp/history'){
