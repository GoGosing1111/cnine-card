...
export async function onRequest(context){
  ...
  try{
    ...
    if(path==='rift/start'&&request.method==='POST'){
      ...
      const weekly=await riftWeeklyRow(env,user.id,settings),runId=`rift-${user.id}-${Date.now()}-${Math.random().toString(36).slice(2,9)}`,hp=Object.fromEntries(deckInfo.ids.map(id=>[String(id),100])),basePower=Math.max(100,cards.reduce((sum,card)=>sum+Number(card.power||0),0)),currentChoices=await riftBuildChoices(env,runId,0,difficulty,basePower),run={runId,weekKey:weekly.weekKey,difficulty,status:'ACTIVE',deck:deckInfo.ids.map(String),stage:0,maxStages:settings.maxStages,hp,buffs:[],history:[],currentChoices,activeNode:null,pendingBuffChoices:[],stash:{coin:0,shards:0,crystals:0},basePower,rewardEligible:weekly.rewardCount<settings.weeklyRewardLimit,reviveUsed:false,battleRewardBonusPercent:0,battleWinCounts:{BATTLE:0,ELITE:0}};
      await env.DB.batch([
