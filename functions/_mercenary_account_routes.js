...
export async function handleMercenaryAccount({path,request,env,deps}){
  ...
  if(!V3_JOINT_RELEASE_ENABLED&&!opening&&!collection&&!(MERCENARY_DEPLOYMENT_RELEASE_ENABLED&&path==='mercenaries/v3/loadout')&&path!=='admin/mercenaries/runtime')return deps.json({code:'MERCENARY_OPENING_DISABLED',error:'용병 공동 업데이트를 준비 중입니다.',userOpeningEnabled:false},423);
...
export async function handleMercenaryAccountReady({path,request,env,deps,publicOpening=false}){
  ...
  try{const user=await authenticate(request,env);if(!user)throw jointError('MERCENARY_AUTH','로그인이 필요합니다.',401);
    ...
    if(request.method==='GET'){
      ...
      if(path==='mercenaries/v3/state'){const state=await mercenaryAccountState(env,user);if(publicOpening){state.available=MERCENARY_DEPLOYMENT_RELEASE_ENABLED||V3_JOINT_RELEASE_ENABLED&&state.available;state.openingAvailable=(await hyperOpeningFeature(env)).userOpeningEnabled;}return json(state);}
