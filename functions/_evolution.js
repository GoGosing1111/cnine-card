...
async function runUpgrade(env){
  ...
  for(const [name,type] of [['evolution_type',"TEXT NOT NULL DEFAULT 'SSR_TO_MA'"],['master_star_cost','INTEGER NOT NULL DEFAULT 0'],['request_id','TEXT'],['source_consumed','INTEGER NOT NULL DEFAULT 0'],['source_quantity_before','INTEGER'],['source_quantity_after','INTEGER']]){
  ...
  await env.DB.prepare("INSERT INTO app_meta(key,value,updated_at) VALUES('safe_runtime_upgrade_v1703_evolution_source_consumption','1',CURRENT_TIMESTAMP) ON CONFLICT(key) DO UPDATE SET value='1',updated_at=CURRENT_TIMESTAMP").run();
}
async function runZenithPityUpgrade(env){
  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS zenith_evolution_pity_compensations_v1718(user_id INTEGER NOT NULL,source_card_id TEXT NOT NULL,reward_card_id TEXT NOT NULL,failed_attempts INTEGER NOT NULL,source_quantity_before INTEGER NOT NULL,claim_token TEXT NOT NULL,applied INTEGER NOT NULL DEFAULT 0,compensated_at TEXT,PRIMARY KEY(user_id,source_card_id))`).run();
  const pool=resultRows(await env.DB.prepare(`SELECT c.id FROM cards_effective_v1210 c JOIN members m ON m.id=c.member_id WHERE c.rarity='ZENITH' AND c.is_active=1 AND m.is_active=1 AND COALESCE(c.card_status,'PUBLIC')='PUBLIC' ORDER BY c.id`).all());
  if(!pool.length)return;
  const eligible=resultRows(await env.DB.prepare(`SELECT p.user_id,p.source_card_id,p.failed_attempts,p.total_attempts,uc.quantity FROM card_evolution_progress p JOIN user_cards uc ON uc.user_id=p.user_id AND uc.card_id=p.source_card_id JOIN cards_effective_v1210 c ON c.id=p.source_card_id WHERE p.is_success=0 AND p.failed_attempts>=? AND c.rarity='LIMITED' AND COALESCE(uc.quantity,0)>0 AND COALESCE(uc.breakthrough_level,0)>=13 AND NOT EXISTS(SELECT 1 FROM zenith_evolution_pity_compensations_v1718 z WHERE z.user_id=p.user_id AND z.source_card_id=p.source_card_id AND z.applied=1) ORDER BY p.user_id,p.source_card_id`).bind(ZENITH_POLICY.pityAttempts).all());
  for(const row of eligible){
    const reward=pickRandom(pool),rewardId=String(reward.id),userId=Number(row.user_id),sourceCardId=String(row.source_card_id),sourceQuantity=Math.max(1,Number(row.quantity||1)),failedAttempts=Math.max(ZENITH_POLICY.pityAttempts,Number(row.failed_attempts||0)),attemptNo=Math.max(failedAttempts,Number(row.total_attempts||0)),claimToken=`${Date.now()}:${userId}:${sourceCardId}:${Math.random().toString(36).slice(2)}`,requestId=`zenith-pity-v1718:${userId}:${sourceCardId}`,ownedReward=await env.DB.prepare('SELECT quantity FROM user_cards WHERE user_id=? AND card_id=? AND COALESCE(quantity,0)>0').bind(userId,rewardId).first(),duplicate=ownedReward?1:0;
    const activeClaim=`EXISTS(SELECT 1 FROM zenith_evolution_pity_compensations_v1718 z WHERE z.user_id=? AND z.source_card_id=? AND z.claim_token=? AND z.applied=0)`;
    await env.DB.batch([
      env.DB.prepare(`INSERT INTO zenith_evolution_pity_compensations_v1718(user_id,source_card_id,reward_card_id,failed_attempts,source_quantity_before,claim_token,applied) VALUES(?,?,?,?,?,?,0) ON CONFLICT(user_id,source_card_id) DO UPDATE SET reward_card_id=excluded.reward_card_id,failed_attempts=excluded.failed_attempts,source_quantity_before=excluded.source_quantity_before,claim_token=excluded.claim_token WHERE zenith_evolution_pity_compensations_v1718.applied=0`).bind(userId,sourceCardId,rewardId,failedAttempts,sourceQuantity,claimToken),
      env.DB.prepare(`UPDATE pve_decks SET card_ids=COALESCE((SELECT json_group_array(j.value) FROM json_each(pve_decks.card_ids) j WHERE CAST(j.value AS TEXT)<>?),'[]'),updated_at=CURRENT_TIMESTAMP WHERE user_id=? AND ${activeClaim}`).bind(sourceCardId,userId,userId,sourceCardId,claimToken),
      env.DB.prepare(`UPDATE pvp_decks SET card_ids=COALESCE((SELECT json_group_array(j.value) FROM json_each(pvp_decks.card_ids) j WHERE CAST(j.value AS TEXT)<>?),'[]'),updated_at=CURRENT_TIMESTAMP WHERE user_id=? AND ${activeClaim}`).bind(sourceCardId,userId,userId,sourceCardId,claimToken),
      env.DB.prepare(`UPDATE pvp_deck_presets SET card_ids=COALESCE((SELECT json_group_array(j.value) FROM json_each(pvp_deck_presets.card_ids) j WHERE CAST(j.value AS TEXT)<>?),'[]'),updated_at=CURRENT_TIMESTAMP WHERE user_id=? AND ${activeClaim}`).bind(sourceCardId,userId,userId,sourceCardId,claimToken),
      env.DB.prepare(`UPDATE user_cards SET quantity=0,breakthrough_level=0,last_obtained_at=CURRENT_TIMESTAMP WHERE user_id=? AND card_id=? AND ${activeClaim}`).bind(userId,sourceCardId,userId,sourceCardId,claimToken),
      env.DB.prepare(`INSERT INTO user_cards(user_id,card_id,quantity,first_obtained_at,last_obtained_at) SELECT ?,?,1,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP WHERE ${activeClaim} ON CONFLICT(user_id,card_id) DO UPDATE SET quantity=user_cards.quantity+1,last_obtained_at=CURRENT_TIMESTAMP`).bind(userId,rewardId,userId,sourceCardId,claimToken),
      env.DB.prepare(`UPDATE card_evolution_progress SET failed_attempts=0,is_success=1,reward_card_id=?,completed_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE user_id=? AND source_card_id=? AND ${activeClaim}`).bind(rewardId,userId,sourceCardId,userId,sourceCardId,claimToken),
      env.DB.prepare(`INSERT INTO card_evolution_logs(user_id,source_card_id,attempt_no,coin_cost,shard_cost,success_rate,is_pity,is_success,reward_card_id,reward_duplicate,reward_shards,evolution_type,master_star_cost,request_id,source_consumed,source_quantity_before,source_quantity_after) SELECT ?,?,?,0,0,100,1,1,?,?,0,'LIMITED_TO_ZENITH',0,?,1,?,0 WHERE ${activeClaim}`).bind(userId,sourceCardId,attemptNo,rewardId,duplicate,requestId,sourceQuantity,userId,sourceCardId,claimToken),
      env.DB.prepare(`UPDATE zenith_evolution_pity_compensations_v1718 SET applied=1,compensated_at=CURRENT_TIMESTAMP WHERE user_id=? AND source_card_id=? AND claim_token=? AND applied=0`).bind(userId,sourceCardId,claimToken)
    ]);
  }
  const remaining=await env.DB.prepare(`SELECT COUNT(*) count FROM card_evolution_progress p JOIN user_cards uc ON uc.user_id=p.user_id AND uc.card_id=p.source_card_id JOIN cards_effective_v1210 c ON c.id=p.source_card_id WHERE p.is_success=0 AND p.failed_attempts>=? AND c.rarity='LIMITED' AND COALESCE(uc.quantity,0)>0 AND COALESCE(uc.breakthrough_level,0)>=13 AND NOT EXISTS(SELECT 1 FROM zenith_evolution_pity_compensations_v1718 z WHERE z.user_id=p.user_id AND z.source_card_id=p.source_card_id AND z.applied=1)`).bind(ZENITH_POLICY.pityAttempts).first();
  if(Number(remaining?.count||0)===0){const summary=await env.DB.prepare('SELECT COUNT(*) count FROM zenith_evolution_pity_compensations_v1718 WHERE applied=1').first();await env.DB.prepare('INSERT INTO app_meta(key,value,updated_at) VALUES(?,?,CURRENT_TIMESTAMP) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=CURRENT_TIMESTAMP').bind(ZENITH_PITY_UPGRADE_KEY,JSON.stringify({pityAttempts:ZENITH_POLICY.pityAttempts,compensated:Number(summary?.count||0)})).run()}
}
let upgradePromise=null;
async function upgrade(env){if(!upgradePromise)upgradePromise=(async()=>{const done=await env.DB.prepare("SELECT value FROM app_meta WHERE key='safe_runtime_upgrade_v1703_evolution_source_consumption'").first();if(done?.value!=='1')await runUpgrade(env);const pityDone=await env.DB.prepare('SELECT value FROM app_meta WHERE key=?').bind(ZENITH_PITY_UPGRADE_KEY).first();if(!pityDone?.value)await runZenithPityUpgrade(env)})().catch(error=>{upgradePromise=null;throw error});return upgradePromise}
...
async function overview(env,userId,settings){
  ...
  return {
    ...
    types:{
      ...
      LIMITED_TO_ZENITH:{...zenithRule,type:'LIMITED_TO_ZENITH',candidates:zenithCandidates,resultPool:zenithPool,eligibleCount:zenithCandidates.filter(card=>card.eligible).length,masterStarCost:ZENITH_POLICY.masterStarCost,coinCost:ZENITH_POLICY.coinCost,successRate:ZENITH_POLICY.successRate,pityAttempts:ZENITH_POLICY.pityAttempts}
    }
  };
}
...
async function prestigeAttempt({env,deps,user,cardId,requestId,settings}){
  ...
  const pool=evolutionResultPool((await env.DB.prepare(`SELECT c.id,c.title,m.name,c.rarity AS grade,c.image_url AS image,c.focus_x AS focusX,c.focus_y AS focusY FROM cards_effective_v1210 c JOIN members m ON m.id=c.member_id WHERE c.rarity='PRESTIGE' AND c.is_active=1 AND m.is_active=1 AND COALESCE(c.card_status,'PUBLIC')='PUBLIC' AND c.limited_total IS NULL AND NOT EXISTS(SELECT 1 FROM user_cards owned_uc WHERE owned_uc.user_id=? AND owned_uc.card_id=c.id AND COALESCE(owned_uc.quantity,0)>0) ORDER BY c.id`).bind(user.id).all()).results||[]);
  if(!pool.length)return deps.json({error:'현재 획득 가능한 신규 PRESTIGE 카드가 없습니다. 공개 PRESTIGE 카드를 모두 보유 중이라 진화 재료는 소모되지 않았습니다.',code:'PRESTIGE_RESULT_POOL_EXHAUSTED'},409);
  ...
  statements.push(
    ...
    env.DB.prepare(`INSERT INTO card_evolution_logs(user_id,source_card_id,attempt_no,coin_cost,shard_cost,success_rate,is_pity,is_success,reward_card_id,reward_duplicate,reward_shards,evolution_type,master_star_cost,request_id,source_consumed,source_quantity_before,source_quantity_after) VALUES(?,?,?,0,0,?,?,?,?,?,?, 'MA_TO_PRESTIGE',?,?,?,?,?)`).bind(user.id,cardId,attemptNo,successRate,isPity?1:0,success?1:0,reward?.id||null,duplicate?1:0,rewardShards,masterStarCost,requestId,success?1:0,Number(source.quantity||0),success?0:Number(source.quantity||0)),
    env.DB.prepare('DELETE FROM card_evolution_atomic_guard WHERE guard_id=?').bind(guardId)
  );
  try{await env.DB.batch(statements)}catch(error){const message=String(error?.message||error||'');if(/request_id|UNIQUE/i.test(message))return deps.json({error:'이미 처리된 진화 요청입니다.',code:'EVOLUTION_DUPLICATE_REQUEST'},409);if(/CHECK constraint|verified/i.test(message))return deps.json({error:'진화 재료 또는 신규 PRESTIGE 결과 후보 상태가 변경되었습니다. 다시 확인해주세요. 재료는 소모되지 않았습니다.',code:'EVOLUTION_STATE_CHANGED'},409);throw error}
...
async function zenithAttempt({env,deps,user,cardId,requestId,settings}){
  ...
  statements.push(
    ...
    env.DB.prepare(`INSERT INTO card_evolution_logs(user_id,source_card_id,attempt_no,coin_cost,shard_cost,success_rate,is_pity,is_success,reward_card_id,reward_duplicate,reward_shards,evolution_type,master_star_cost,request_id,source_consumed,source_quantity_before,source_quantity_after) VALUES(?,?,?, ?,0, ?,?, ?,?,?,0,'LIMITED_TO_ZENITH',?,?,?,?,?)`).bind(user.id,cardId,attemptNo,coinCost,successRate,isPity?1:0,success?1:0,success?reward.id:null,success&&duplicate?1:0,masterStarCost,requestId,success?1:0,Number(source.quantity||0),success?0:Number(source.quantity||0)),
    env.DB.prepare('DELETE FROM card_evolution_atomic_guard WHERE guard_id=?').bind(guardId)
  );
