...
async function rebuild(env,versions,config,deps,hash,now){
 ...
 for(const version of versions){
  ...
  const saved=decks.results.find(d=>Number(d.user_id)===userId),select=raw=>parsed(raw).map(id=>cardMap.get(String(id))).filter(Boolean);
