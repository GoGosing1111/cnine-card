import {ensureGoldenAxe} from './_golden_axe.js';
import {OLD_AXE} from '../js/golden-axe-model-v1.js';

export const AXE_COUPON_MAX=100000;
const label='낡은도끼';
class AxeCouponError extends Error{
 constructor(message,status=409){super(message);this.status=status}
}
const fail=(message,status)=>{throw new AxeCouponError(message,status)};
const validAmount=value=>Number.isSafeInteger(Number(value))&&Number(value)>=1&&Number(value)<=AXE_COUPON_MAX;

// Keep ordinary coupons unchanged. Old axes use the same inventory consumed by the event.
// Serialize the coupon row as well as the user: different users cannot take the last use twice.
export async function redeemOldAxeCoupon({env,user,coupon,body,deps}){
 if(String(coupon?.reward_type||'').toUpperCase()!==OLD_AXE)return null;
 try{
  await ensureGoldenAxe(env);
  ...
  const result=await env.DB.enqueue(async()=>{
   ...
   try{
    ...
    if(prior){
     ...
     if(prior.reward_type!==OLD_AXE||!validAmount(prior.reward_amount))fail('쿠폰 지급 기록을 확인해야 합니다.',500);
     ...
     if(current.reward_type!==OLD_AXE||!validAmount(current.reward_amount))fail('낡은도끼 쿠폰 보상 설정을 확인하세요.',400);
     const [item]=await q('SELECT is_active FROM inventory_items WHERE code=$1 FOR SHARE',[OLD_AXE]);
     if(Number(item?.is_active)!==1)fail('낡은도끼 지급이 중지되어 있습니다. 쿠폰은 사용되지 않았습니다.');
     ...
     const inventory=await q(`INSERT INTO cnine_user_inventory(user_id,item_code,quantity,unseen_quantity,created_at,updated_at)
       ...
       RETURNING quantity`,[user.id,OLD_AXE,rewardAmount]);
     if(inventory.length!==1)fail('낡은도끼 지급에 실패했습니다. 쿠폰은 사용되지 않았습니다.',500);
     ...
     const log=await q("INSERT INTO inventory_logs(user_id,item_code,change_amount,balance_after,reason,reference_type,reference_id) VALUES($1,$2,$3,$4,'COUPON','COUPON',$5) RETURNING user_id",[user.id,OLD_AXE,rewardAmount,inventory[0].quantity,String(current.id)]);
     if(log.length!==1)fail('낡은도끼 지급 기록 저장에 실패했습니다.',500);
     const receipt=await q('INSERT INTO coupon_redemptions(coupon_id,user_id,reward_coin,reward_type,reward_amount,operation_key) VALUES($1,$2,0,$3,$4,$5) RETURNING user_id',[current.id,user.id,OLD_AXE,rewardAmount,operationKey]);
     if(receipt.length!==1)fail('쿠폰 사용 기록 저장에 실패했습니다.',500);
    }
    await q('COMMIT');
    return {ok:true,replayed,rewardType:OLD_AXE,rewardAmount,rewardLabel:label,rewardCoin:0,
     message:`${label} ${rewardAmount.toLocaleString('ko-KR')}개를 받았습니다. 핑두의 금도끼 은도끼에서 사용하세요.`};
 ...
 }catch(error){if(error instanceof AxeCouponError)return deps.json({error:error.message},error.status);throw error}
}
