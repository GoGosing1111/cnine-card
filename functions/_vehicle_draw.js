...
const VEHICLE_CATALOG_CACHE_KEY='vehicle-draw:catalog';
...
// PERF-0919: 경로 확인을 먼저 한다. 이 핸들러는 디스패치 체인 중간에 있어, 예전에는 뒤에서 처리되는
//   모든 라우트(카드뽑기·영토전·장비 등)가 콜드 isolate 에서 이 ensure 를 함께 기다렸다.
function vehicleDrawRoute(path){const value=String(path||'');return value.startsWith('vehicle-draw/')||value.startsWith('admin/vehicle-draw/')}
// PERF-0919: 이동수단 뽑기권 차감 가드(PostgreSQL READ COMMITTED 동시 개봉 대비). 차감이 정확히 1행이
//   아니면 영수증 NOT NULL 컬럼을 NULL 로 만들어 batch 전체(보상 포함)를 되돌린다.
export function vehicleTicketDebitStatements(env,{count,userId,requestId}){
  const debit=`UPDATE cnine_user_inventory SET quantity=quantity-?,updated_at=CURRENT_TIMESTAMP WHERE user_id=? AND item_code=? AND quantity>=? AND EXISTS(SELECT 1 FROM vehicle_draw_receipts WHERE request_id=? AND user_id=? AND status='PENDING')`;
  const values=[count,userId,TICKET_CODE,count,requestId,userId];
  const guard=proof=>`UPDATE vehicle_draw_receipts SET duplicate=CASE WHEN ${proof} THEN duplicate ELSE NULL END WHERE request_id=? AND user_id=? AND status='PENDING'`;
...
function isVehicleTicketGuardError(error){const message=String(error?.message||error||'').toLowerCase();return message.includes('duplicate')&&(message.includes('not null')||message.includes('null value'))}
export async function handleVehicleDraw({path,request,env,deps}){if(!vehicleDrawRoute(path))return null;const {authenticate,readBody,json}=deps;await ensure(env);
 if(path==='vehicle-draw/config'&&request.method==='GET'){const user=await authenticate(request,env);if(!user)return json({error:'로그인이 필요합니다.'},401);const [p,balance]=await Promise.all([payload(env,{fresh:false}),env.DB.prepare('SELECT quantity FROM cnine_user_inventory WHERE user_id=? AND item_code=?').bind(user.id,TICKET_CODE).first()]);return json({...p,ticketQuantity:Number(balance?.quantity||0),coin:Number(user?.coin||0),shop:{enabled:LEGACY_SHOP_ENABLED,unitPrice:SHOP_PRICE,originalUnitPrice:SHOP_PRICE,promotionDiscountPercent:0,discountEndsAt:null}})}
 if(path==='vehicle-draw/purchase'&&request.method==='POST'){
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(env.DB?.dialect==='postgres'?`UPDATE vehicle_draw_purchase_receipts SET status='COMPLETED',response_json=(jsonb_build_object('ok',true,'count',count,'totalPrice',total_price,'coin',(SELECT coin FROM users WHERE id=?),'ticketQuantity',COALESCE((SELECT quantity FROM cnine_user_inventory WHERE user_id=? AND item_code=?),0)))::text,updated_at=CURRENT_TIMESTAMP WHERE request_id=? AND user_id=? AND status='PENDING'`:`UPDATE vehicle_draw_purchase_receipts SET status='COMPLETED',response_json=json_object('ok',json('true'),'count',count,'totalPrice',total_price,'coin',(SELECT coin FROM users WHERE id=?),'ticketQuantity',COALESCE((SELECT quantity FROM cnine_user_inventory WHERE user_id=? AND item_code=?),0)),updated_at=CURRENT_TIMESTAMP WHERE request_id=? AND user_id=? AND status='PENDING'`).bind(user.id,user.id,TICKET_CODE,requestId,user.id)
  ]);
 ...
 if(path==='vehicle-draw/open'&&request.method==='POST'){
  const user=await authenticate(request,env);if(!user)return json({error:'로그인이 필요합니다.'},401);
   const b=await readBody(request),requestId=text(b.requestId,120),rawCount=Number(b.count??1),count=int(rawCount,1,DRAW_BATCH_MAX);
  if(!requestId)return json({error:'요청 ID가 필요합니다.'},400);
   if(!Number.isInteger(rawCount)||rawCount<1||rawCount>DRAW_BATCH_MAX)return json({error:`차량 뽑기는 1회 이상 ${DRAW_BATCH_MAX}회 이하여야 합니다.`},400);
  const existing=await env.DB.prepare('SELECT status,response_json FROM vehicle_draw_receipts WHERE request_id=? AND user_id=?').bind(requestId,user.id).first();
  ...
  // V2062: Independent reads may overlap on D1. The PostgreSQL adapter still
  // serializes them; do not claim fewer PostgreSQL queries. Draws always read fresh.
  const [s,poolRows,ownedResult]=await Promise.all([
    settings(env),
    env.DB.prepare('SELECT * FROM character_garage_items WHERE is_active=1 AND is_public=1 AND draw_enabled=1 AND draw_weight>0 ORDER BY id').all().then(result=>result.results||[]),
    env.DB.prepare('SELECT garage_id FROM user_garage_vehicles WHERE user_id=?').bind(user.id).all()
  ]);
  if(!s.enabled)return json({error:'현재 차량 뽑기가 중지되어 있습니다.'},503);
  const rows=poolRows||[],total=rows.reduce((sum,r)=>sum+Number(r.draw_weight||0),0);if(!rows.length||Math.abs(total-100)>.00001)return json({error:'차량 뽑기 확률 설정이 올바르지 않습니다.'},503);
  const owned=new Set((ownedResult.results||[]).map(x=>Number(x.garage_id)));
  ...
  const statements=[env.DB.prepare(`INSERT INTO vehicle_draw_receipts(request_id,user_id,garage_id,duplicate,shards_gained,master_stars_gained,status) SELECT ?,?,?,?,?,?,'PENDING' WHERE EXISTS(SELECT 1 FROM cnine_user_inventory WHERE user_id=? AND item_code=? AND quantity>=?)`).bind(requestId,user.id,results[0].vehicle.id,results.every(x=>x.duplicate)?1:0,totalShards,totalStars,user.id,TICKET_CODE,count),...vehicleTicketDebitStatements(env,{count,userId:user.id,requestId})];
  const newVehicleIds=[...new Set(results.filter(result=>!result.duplicate).map(result=>Number(result.vehicle.id)))];
  if(newVehicleIds.length)statements.push(env.DB.prepare(`WITH receipt_guard AS (
    SELECT 1 FROM vehicle_draw_receipts WHERE request_id=? AND user_id=? AND status='PENDING'
  ) INSERT OR IGNORE INTO user_garage_vehicles(user_id,garage_id,source_type,source_id)
    SELECT ?,CAST(value AS INTEGER),'VEHICLE_DRAW',?||CAST(value AS INTEGER)
    FROM json_each(?) CROSS JOIN receipt_guard`).bind(requestId,user.id,user.id,`${requestId}:`,JSON.stringify(newVehicleIds)));
  statements.push(env.DB.prepare(`UPDATE users SET card_shards=card_shards+? WHERE id=? AND ?>0 AND EXISTS(SELECT 1 FROM vehicle_draw_receipts WHERE request_id=? AND user_id=? AND status='PENDING')`).bind(totalShards,user.id,totalShards,requestId,user.id),env.DB.prepare(`INSERT INTO cnine_user_inventory(user_id,item_code,quantity,unseen_quantity,created_at,updated_at) SELECT ?,'MASTER_STAR',?,?,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP WHERE ?>0 AND EXISTS(SELECT 1 FROM vehicle_draw_receipts WHERE request_id=? AND user_id=? AND status='PENDING') ON CONFLICT(user_id,item_code) DO UPDATE SET quantity=quantity+excluded.quantity,unseen_quantity=unseen_quantity+excluded.unseen_quantity,updated_at=CURRENT_TIMESTAMP`).bind(user.id,totalStars,totalStars,totalStars,requestId,user.id),env.DB.prepare(`INSERT INTO inventory_logs(user_id,item_code,change_amount,balance_after,reason,reference_type,reference_id) SELECT ?,?,? * -1,quantity,'VEHICLE_DRAW','VEHICLE_DRAW',? FROM cnine_user_inventory WHERE user_id=? AND item_code=? AND EXISTS(SELECT 1 FROM vehicle_draw_receipts WHERE request_id=? AND user_id=? AND status='PENDING')`).bind(user.id,TICKET_CODE,count,requestId,user.id,TICKET_CODE,requestId,user.id),env.DB.prepare(`INSERT INTO shard_logs(user_id,change_amount,balance_after,reason) SELECT ?,?,card_shards,'차량 중복 획득' FROM users WHERE id=? AND ?>0 AND EXISTS(SELECT 1 FROM vehicle_draw_receipts WHERE request_id=? AND user_id=? AND status='PENDING')`).bind(user.id,totalShards,user.id,totalShards,requestId,user.id),env.DB.prepare(`INSERT INTO inventory_logs(user_id,item_code,change_amount,balance_after,reason,reference_type,reference_id) SELECT ?,'MASTER_STAR',?,quantity,'VEHICLE_DRAW_BONUS','VEHICLE_DRAW',? FROM cnine_user_inventory WHERE user_id=? AND item_code='MASTER_STAR' AND ?>0 AND EXISTS(SELECT 1 FROM vehicle_draw_receipts WHERE request_id=? AND user_id=? AND status='PENDING')`).bind(user.id,totalStars,requestId,user.id,totalStars,requestId,user.id),env.DB.prepare(`UPDATE vehicle_draw_receipts SET status='COMPLETED',response_json=?,updated_at=CURRENT_TIMESTAMP WHERE request_id=? AND user_id=? AND status='PENDING' RETURNING status,response_json,COALESCE((SELECT quantity FROM cnine_user_inventory WHERE user_id=? AND item_code=?),0) ticketQuantity,COALESCE((SELECT quantity FROM cnine_user_inventory WHERE user_id=? AND item_code='MASTER_STAR'),0) masterStarQuantity,COALESCE((SELECT card_shards FROM users WHERE id=?),0) cardShards`).bind(JSON.stringify(response),requestId,user.id,user.id,TICKET_CODE,user.id,user.id));
  // PERF-0919: 완료 UPDATE 의 RETURNING 으로 영수증과 잔액을 한 번에 받는다(batch 뒤 재조회 2왕복 제거).
  let batched;
  try{batched=await env.DB.batch(statements)}catch(error){if(isVehicleTicketGuardError(error))return json({error:`차량 뽑기권이 ${count}장 필요합니다.`,code:'VEHICLE_TICKET_STOCK_CHANGED'},409);throw error}
  const completed=batched.at(-1)?.results?.[0];
  if(completed?.status==='COMPLETED'&&completed.response_json){const {status:_status,response_json:stored,...balances}=completed;return json({...parse(stored,{}),ticketQuantity:Number(balances.ticketQuantity||0),masterStarQuantity:Number(balances.masterStarQuantity||0),cardShards:Number(balances.cardShards||0)})}
  const receipt=await env.DB.prepare('SELECT status,response_json FROM vehicle_draw_receipts WHERE request_id=? AND user_id=?').bind(requestId,user.id).first();
