...
export async function handleVehicleDraw({path,request,env,deps}){const {authenticate,readBody,json}=deps,chiefDiscountState=typeof deps.chiefDiscountState==='function'?deps.chiefDiscountState:async()=>({active:false,percent:0});await ensure(env);
 if(path==='vehicle-draw/config'&&request.method==='GET'){const user=await authenticate(request,env);if(!user)return json({error:'로그인이 필요합니다.'},401);const [p,balance,discount]=await Promise.all([payload(env),env.DB.prepare('SELECT quantity FROM cnine_user_inventory WHERE user_id=? AND item_code=?').bind(user.id,TICKET_CODE).first(),chiefDiscountState(env)]),percent=discount.active?Number(discount.percent||25):0,unitPrice=Math.floor(SHOP_PRICE*(100-percent)/100);return json({...p,ticketQuantity:Number(balance?.quantity||0),shop:{enabled:p.settings.enabled,unitPrice,originalUnitPrice:SHOP_PRICE,promotionDiscountPercent:percent,discountEndsAt:discount.endsAt||null}})}
 if(path==='vehicle-draw/purchase'&&request.method==='POST'){
  ...
  const discount=await chiefDiscountState(env),percent=discount.active?Number(discount.percent||25):0,unitPrice=Math.floor(SHOP_PRICE*(100-percent)/100),totalPrice=unitPrice*count;
  await env.DB.batch([
