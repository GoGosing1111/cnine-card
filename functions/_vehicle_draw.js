...
export async function handleVehicleDraw({path,request,env,deps}){const {authenticate,readBody,json}=deps;await ensure(env);
 ...
 if(path==='vehicle-draw/purchase'&&request.method==='POST'){
  ...
  const b=await readBody(request),rawCount=Number(b.count),count=int(rawCount,1,100),requestId=text(b.requestId,120);
  if(!Number.isInteger(rawCount)||rawCount<1||rawCount>100)return json({error:'구매 수량은 1개 이상 100개 이하여야 합니다.'},400);
 ...
 if(path==='vehicle-draw/open'&&request.method==='POST'){
  const user=await authenticate(request,env);if(!user)return json({error:'로그인이 필요합니다.'},401);
  const b=await readBody(request),requestId=text(b.requestId,120),rawCount=Number(b.count??1),count=int(rawCount,1,100);
  if(!requestId)return json({error:'요청 ID가 필요합니다.'},400);
  if(!Number.isInteger(rawCount)||rawCount<1||rawCount>100)return json({error:'차량 뽑기는 1회 이상 100회 이하여야 합니다.'},400);
  const existing=await env.DB.prepare('SELECT status,response_json FROM vehicle_draw_receipts WHERE request_id=? AND user_id=?').bind(requestId,user.id).first();
