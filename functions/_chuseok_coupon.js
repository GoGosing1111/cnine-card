import {ensureChuseok} from './_chuseok.js';
import {CHUSEOK_COIN} from '../js/chuseok-model-v1.js';

export const CHUSEOK_COUPON_MAX=100000;
const label='추석 코인';
class ChuseokCouponError extends Error{
 constructor(message,status=409){super(message);this.status=status}
}
const fail=(message,status)=>{throw new ChuseokCouponError(message,status)};
const validAmount=value=>Number.isSafeInteger(Number(value))&&Number(value)>=1&&Number(value)<=CHUSEOK_COUPON_MAX;

// Keep ordinary coupons unchanged. Chuseok coins use the same inventory consumed by both events.
// Serialize the coupon row as well as the user: different users cannot take the last use twice.
export async function redeemChuseokCoinCoupon({env,user,coupon,body,deps}){
 if(String(coupon?.reward_type||'').toUpperCase()!==CHUSEOK_COIN)return null;
 try{
  await ensureChuseok(env);
  ...
  const result=await env.DB.enqueue(async()=>{
   ...
   try{
    ...
    if(prior){
     ...
     if(prior.reward_type!==CHUSEOK_COIN||!validAmount(prior.reward_amount))fail('쿠폰 지급 기록을 확인해야 합니다.',500);
     ...
     if(current.reward_type!==CHUSEOK_COIN||!validAmount(current.reward_amount))fail('추석 코인 쿠폰 보상 설정을 확인하세요.',400);
     const [item]=await q('SELECT is_active FROM inventory_items WHERE code=$1 FOR SHARE',[CHUSEOK_COIN]);
     if(Number(item?.is_active)!==1)fail('추석 코인 지급이 중지되어 있습니다. 쿠폰은 사용되지 않았습니다.');
     ...
     const inventory=await q(`INSERT INTO cnine_user_inventory(user_id,item_code,quantity,unseen_quantity,created_at,updated_at)
       ...
       RETURNING quantity`,[user.id,CHUSEOK_COIN,rewardAmount]);
     if(inventory.length!==1)fail('추석 코인 지급에 실패했습니다. 쿠폰은 사용되지 않았습니다.',500);
     ...
     const log=await q("INSERT INTO inventory_logs(user_id,item_code,change_amount,balance_after,reason,reference_type,reference_id) VALUES($1,$2,$3,$4,'COUPON','COUPON',$5) RETURNING user_id",[user.id,CHUSEOK_COIN,rewardAmount,inventory[0].quantity,String(current.id)]);
     if(log.length!==1)fail('추석 코인 지급 기록 저장에 실패했습니다.',500);
     const receipt=await q('INSERT INTO coupon_redemptions(coupon_id,user_id,reward_coin,reward_type,reward_amount,operation_key) VALUES($1,$2,0,$3,$4,$5) RETURNING user_id',[current.id,user.id,CHUSEOK_COIN,rewardAmount,operationKey]);
     if(receipt.length!==1)fail('쿠폰 사용 기록 저장에 실패했습니다.',500);
    }
    await q('COMMIT');
    return {ok:true,replayed,rewardType:CHUSEOK_COIN,rewardAmount,rewardLabel:label,rewardCoin:0,
     message:`${label} ${rewardAmount.toLocaleString('ko-KR')}개를 받았습니다. 추석 달빛 잔치에서 사용하세요.`};
 ...
 }catch(error){if(error instanceof ChuseokCouponError)return deps.json({error:error.message},error.status);throw error}
}
