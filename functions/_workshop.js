...
async function synthesizeEquipment(env,user,body){
  ...
  const equipmentId=recipe.input_equipment_id,required=int(recipe.input_quantity,1,20,3),inputs=await env.DB.prepare(`SELECT x.id,i.id equipment_id,i.name,i.slot,i.rarity,i.image_url FROM user_equipment_instances x JOIN character_equipment_items i ON i.id=x.equipment_id LEFT JOIN user_equipment_loadout l ON l.instance_id=x.id WHERE x.user_id=? AND x.equipment_id=? AND l.instance_id IS NULL AND i.is_active=1 AND i.is_public=1 ORDER BY x.id LIMIT ?`).bind(user.id,equipmentId,required).all();
  if((inputs.results||[]).length!==required)throw new Error(`장착하지 않은 동일 장비 ${required}개가 필요합니다.`);
  ...
  const statements=[
    env.DB.prepare(`INSERT INTO ${GUARD_TABLE}(guard_id,user_id,recipe_id,verified) SELECT ?,?,0,CASE WHEN (SELECT COUNT(*) FROM user_equipment_instances x LEFT JOIN user_equipment_loadout l ON l.instance_id=x.id WHERE x.user_id=? AND x.equipment_id=? AND l.instance_id IS NULL AND x.id IN (${marks}))=? THEN 1 ELSE 0 END`).bind(guardId,user.id,user.id,equipmentId,...instanceIds,required),
