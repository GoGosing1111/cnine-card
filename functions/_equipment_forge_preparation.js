// Read-only preparation for the approved Upgrade Lab. No routes or migrations import this yet.
...
export async function readForgePreparationInventory(db, authenticatedUserId, { beforeId = null, limit = 40 } = {}) {
  ...
  const sql = `SELECT x.id AS instance_id,x.equipment_id,x.acquired_at,
      ...
      CASE WHEN l.instance_id IS NULL THEN 0 ELSE 1 END AS equipped
    FROM user_equipment_instances x
    JOIN character_equipment_items i ON i.id=x.equipment_id
    LEFT JOIN user_equipment_loadout l ON l.instance_id=x.id AND l.user_id=x.user_id
    WHERE x.user_id=? AND i.is_active=1 AND i.is_public=1${cursor ? ' AND x.id<?' : ''}
    ORDER BY x.id DESC LIMIT ?`;
  const values = cursor ? [userId, cursor, limit + 1] : [userId, limit + 1];
