/* V1250 CLEAN FRAME + INSTANT EQUIP + ITEM FIT/ALIGNMENT */
(()=>{
  ...
  function mediaFitConfig(context,subtype){
    const key=String(context||'grid-thumb').toLowerCase();
    const normalizedSubtype=String(subtype||'').toUpperCase();
    const base={mode:'cover',padX:.04,padY:.04,boxW:.92,boxH:.92,shiftX:0,shiftY:0,zoom:1};
    const map={
      'slot-weapon':{mode:'cover',padX:.03,padY:.03,boxW:.94,boxH:.94,zoom:1.02},
      'slot-accessory':{mode:'cover',padX:.01,padY:.01,boxW:.98,boxH:.98,zoom:1.08},
      'slot-top':{mode:'cover',padX:.015,padY:.015,boxW:.97,boxH:.97,zoom:1.05,shiftY:-.005},
      'slot-bottom':{mode:'cover',padX:.015,padY:.015,boxW:.97,boxH:.97,zoom:1.04,shiftY:-.004},
      'slot-shoes':{mode:'cover',padX:.015,padY:.015,boxW:.97,boxH:.97,zoom:1.06,shiftY:.01},
      'grid-thumb':{mode:'cover',padX:.04,padY:.04,boxW:.92,boxH:.92,zoom:1.02},
      'drop-thumb':{mode:'contain',padX:.06,padY:.06,boxW:.88,boxH:.88,zoom:1}
    };
    const cfg={...base,...(map[key]||{})};
    if(normalizedSubtype==='RIFLE'){cfg.zoom*=1.02;cfg.shiftX=.005}
    if(normalizedSubtype==='DUAL_DISK'){cfg.zoom*=1.08}
    if(normalizedSubtype==='TOP'){cfg.zoom*=1.03}
    if(normalizedSubtype==='BOTTOM'){cfg.zoom*=1.03}
    if(normalizedSubtype==='SHOES'){cfg.zoom*=1.04;cfg.shiftY+=.012}
    return cfg;
  }
  function itemImage(item,fitContext='grid-thumb'){
    return item?.image?`<div class="trim-fit-v1251" data-fit-context="${esc(fitContext)}" data-item-subtype="${esc(item.subtype||'')}"><img src="${esc(item.image)}" alt="${esc(item.name)}" loading="lazy"></div>`:''
  }
  function getAlphaBounds(img){
    const src=img.currentSrc||img.src||'';
    if(trimBoundsCache.has(src))return trimBoundsCache.get(src);
    ...
    for(let y=0;y<h;y++){
      for(let x=0;x<w;x++){
        ...
        if(alpha>8){if(x<minX)minX=x;if(y<minY)minY=y;if(x>maxX)maxX=x;if(y>maxY)maxY=y}
      }
    }
    const bounds=maxX>=0?{x:Math.max(0,minX-2),y:Math.max(0,minY-2),w:Math.min(w, maxX-minX+5),h:Math.min(h, maxY-minY+5)}:{x:0,y:0,w,h};
    trimBoundsCache.set(src,bounds);
    return bounds;
  }
  function drawTrimmedFit(container){
    const img=container.querySelector('img');
    if(!img||container.dataset.trimReady==='1')return;
    const attempt=()=>{
      if(!img.complete||!img.naturalWidth)return;
      try{
        const bounds=getAlphaBounds(img);
        const cfg=mediaFitConfig(container.dataset.fitContext,container.dataset.itemSubtype);
        const canvas=document.createElement('canvas');
        ...
        const dx=canvas.width*cfg.padX,dy=canvas.height*cfg.padY,dw=canvas.width*cfg.boxW,dh=canvas.height*cfg.boxH;
        let scale=(cfg.mode==='contain'?Math.min(dw/bounds.w,dh/bounds.h):Math.max(dw/bounds.w,dh/bounds.h))*Number(cfg.zoom||1);
        const rw=bounds.w*scale,rh=bounds.h*scale;
        const rx=dx+(dw-rw)/2+canvas.width*(cfg.shiftX||0);
        const ry=dy+(dh-rh)/2+canvas.height*(cfg.shiftY||0);
        ctx.drawImage(img,bounds.x,bounds.y,bounds.w,bounds.h,rx,ry,rw,rh);
        container.appendChild(canvas);
        container.dataset.trimReady='1';
      }catch(error){
        container.dataset.trimReady='1';
        img.style.opacity='1';
      }
    };
    if(img.complete)attempt();else img.addEventListener('load',attempt,{once:true});
  }
  function hydrateTrimFit(root){
    root.querySelectorAll('.trim-fit-v1251').forEach(drawTrimmedFit);
  }
  ...
  function slotOverlay(slot){
    ...
    return `<div class="frame-slot-v1249 slot-${slot.toLowerCase()} ${item?'filled':''} ${item?rarityClass(item.rarity):''} ${item?subtypeClass(item):''}">
      ...
      ${item?`<div class="frame-slot-image-v1249">${itemImage(item, `slot-${slot.toLowerCase()}`)}</div><div class="frame-slot-desc-mask-v1249"><span>${esc(description)}</span></div>`:''}
    ...
    return `<div class="frame-inventory-layer-v1249">
      ...
      <div class="frame-item-grid-v1249">
        ${rows.length?rows.map(row=>{const i=row.item;return `<button type="button" class="frame-item-card-v1249 ${row.equipped?'equipped':''} ${rarityClass(i.rarity)} ${subtypeClass(i)}" ${row.equipped?'disabled':''} data-character-equip="${row.instanceId}" title="${esc(i.name)}">
          ...
          <div class="frame-item-thumb-v1249">${itemImage(i,'grid-thumb')}</div>
  ...
  function render(){
    ...
    root.innerHTML=shellHtml();hydrateTrimFit(root);bind(root);
  }
  ...
  async function load(){
    ...
    catch(error){const root=document.getElementById('characterSystemRoot');if(root)root.innerHTML='<div class="frame-empty-v1249">장비 정보를 불러오지 못했습니다.</div>'}
