...
function renderShell(tab) {
  ...
  const views = { buy: buyView, dex: dexView, evolution:(typeof window.evolutionView==='function'?window.evolutionView:buyView), battle: battleView, pvp: pvpView, magic: magicView, character:(typeof window.characterView==='function'?window.characterView:inventoryView), attendance: attendanceView, dailyquest: dailyQuestView, messages: messagesView, rank: rankView, mineral: mineralExchangeView, inventory: inventoryView };
...
function inventoryItemMarkup(item){
  ...
  const actionLabel=isReroll?'재뽑기':isSupply?'보급 개방':isCube?'큐브 개봉':'아이템 사용';
...
async function openEquipmentSupplyBox(ownedQuantity=0){
  ...
  document.getElementById('supplyOpenConfirm').onclick=async()=>{
    ...
    try{
      ...
      panel.innerHTML=`<div class="supply-result-head"><div><small>SUPPLY RESULT</small><h2>${Number(result.count)}개 개방 완료</h2></div><button type="button" class="icon-close" id="supplyResultClose">×</button></div><div class="supply-result-summary"><span>장비 <b>${rows.filter(row=>row.type==='EQUIPMENT').length}</b></span><span>카드 조각 <b>+${Number(result.shardGained||0).toLocaleString()}</b></span><span>코인 <b>+${Number(result.coinGained||0).toLocaleString()}</b></span></div><div class="supply-result-grid">${rows.map((row,index)=>row.type==='EQUIPMENT'?`<article class="equipment"><div><img src="${escapeHtml(row.item?.image||'')}" alt="${escapeHtml(row.item?.name||'장비')}"></div><small>장비 획득</small><b>${escapeHtml(row.item?.name||'장비')}</b><span>${escapeHtml(row.item?.slotLabel||'')} · PVE +${Number(row.item?.pvePower||0).toLocaleString()} · PVP +${Number(row.item?.pvpPower||0).toLocaleString()}</span></article>`:row.type==='SHARDS'?`<article class="shards"><div>✣</div><small>카드 조각</small><b>+${Number(row.amount||0).toLocaleString()}</b><span>보급상자 ${index+1}번째 결과</span></article>`:`<article class="coins"><div>◇</div><small>코인</small><b>+${Number(row.amount||0).toLocaleString()}</b><span>보급상자 ${index+1}번째 결과</span></article>`).join('')}</div><div class="supply-result-foot"><span>남은 보급상자 <b>${Number(result.remaining||0).toLocaleString()}개</b></span><button type="button" class="btn" id="supplyResultConfirm">인벤토리로 돌아가기</button></div>`;
...
let pvpState={tab:'match',config:null,profile:null,myTitle:null,deck:[],opponents:[],history:[],ranking:[],energy:null,energyTimer:null,serverOffset:0};
