...
const HIGH_ENHANCEMENT_LOCAL_VERSION=3;
...
// V1802: FUR / ZENITH +11~+13. 서버 설정이 오기 전까지 쓰는 대기값이라 enabled 는 false 로 둔다.
...
const TIER_FRAME_GRADES=['FUR','ZENITH'];
...
// FUR/ZENITH 는 11강부터 공용 표 대신 전용 전투력 표를 쓴다. 서버 cardBattlePower 와 같은 규칙.
function clientBreakthroughBonusPercent(grade,lv,settings){
  const key=String(grade||'').trim().toUpperCase(),table=settings?.highBreakthroughBonus?.[key]||HIGH_BREAKTHROUGH_BONUS_FALLBACK[key];
...
const FEATURE_RESOURCE_MANIFEST={
  ...
  prediction:{
    ...
    scripts:['js/coin-prediction-v1.js?v=1793-user-max-100m'],
  ...
  battleV2:{
    ...
    scripts:[
      ...
      'preview/project-v-v3/project-v-pixi-battle.bundle.js?v=56-v1812-pace',
...
function showDetail(id,initialTab='auto') {
  ...
  const level=Number(user.breakthroughs?.[normalizedId]||0),normalizedGrade=String(card.grade||'').trim().toUpperCase(),canBreak=owned&&(gradeOrder[normalizedGrade]||0)>=gradeOrder[breakthroughMinGrade],isMa=normalizedGrade==='MA',isLimited=normalizedGrade==='LIMITED',isZenith=normalizedGrade==='ZENITH',maxLevel=HIGH_BREAKTHROUGH_GRADES.includes(normalizedGrade)?13:10,isMaHigh=HIGH_BREAKTHROUGH_GRADES.includes(normalizedGrade)&&level>=10&&level<13,usesMasterStars=isZenith||isMaHigh,standardRule=user.breakthroughConfig?.[normalizedGrade]?.[level]||{cost:breakthroughCosts[level],rate:breakthroughRates[level]},highConfig=highBreakthroughConfigForGrade(user,normalizedGrade),highRule=highConfig?.steps?.[level-10],rule=isMaHigh?highRule:standardRule,cost=level<maxLevel?Number(rule?.cost||0):null,successRate=level<maxLevel?Number(rule?.rate||0):null,duplicateCost=isMaHigh?Math.max(0,Math.floor(Number(rule?.duplicateCards)||0)):0,heldCardQuantity=Math.max(owned?1:0,Math.floor(Number(user.quantities?.[normalizedId]||0)||0)),duplicateShortage=duplicateCost>0&&heldCardQuantity<duplicateCost+1,pityThreshold=isMaHigh?Math.max(0,Math.floor(Number(rule?.pityThreshold)||0)):0,materialBalance=usesMasterStars?Number(user.masterStars||0):Number(user.cardShards||0),materialName=usesMasterStars?'마스터의 별':'카드 조각',highEnabled=highConfig?.enabled===true;
...
async function breakthroughCard(cardId){
  ...
  const level=Number(user.breakthroughs?.[normalizedCardId]||0),normalizedGrade=String(card?.grade||'').trim().toUpperCase(),isMa=normalizedGrade==='MA',isLimited=normalizedGrade==='LIMITED',isZenith=normalizedGrade==='ZENITH',maxLevel=HIGH_BREAKTHROUGH_GRADES.includes(normalizedGrade)?13:10,isMaHigh=HIGH_BREAKTHROUGH_GRADES.includes(normalizedGrade)&&level>=10,usesMasterStars=isZenith||isMaHigh;
