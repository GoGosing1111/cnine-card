...
function getPowerTypeIcon(card){const grade=String(card?.grade||card?.rarity||'').toUpperCase();if(grade==='FUR')return '';const type=String(card?.powerType||card?.power_type||'').toUpperCase();return type==='NORMAL'?'⚡':type==='HIGH'?'⚡⚡':type==='TOP'?'⚡⚡⚡':'';}
function getPowerTypeClass(card){const type=String(card?.powerType||card?.power_type||'').toUpperCase();return type==='NORMAL'?'power-type-normal':type==='HIGH'?'power-type-advanced':type==='TOP'?'power-type-top':'';}
function powerTypeIndicatorHtml(card,classes=''){const icon=getPowerTypeIcon(card);const typeClass=getPowerTypeClass(card);return icon&&typeClass?`<i class="power-type-indicator ${typeClass} ${classes}" aria-label="전투력 유형">${icon}</i>`:'';}
...
function recentCards(user) {
  ...
  return `<div class="recent-grid">${items.map(item => { const c=cards.find(x=>x.id===item.cardId); if(!c)return''; return `<button class="recent-item" data-card-id="${c.id}"><img src="${c.image}" style="object-position:${c.focusX}% ${c.focusY}%"><span><b>${escapeHtml(c.title)}</b><small>${c.grade}${getPowerTypeIcon(c)?` · ${powerTypeIndicatorHtml(c)}`:''}${item.duplicate?' · 보유중':' · NEW'}</small></span></button>`; }).join('')}</div>`;
}
...
function pvpCardMini(c,user=loadUser()){const level=Number(user?.breakthroughs?.[c.id]||0),limited=c.limitedTotal!==null&&c.limitedTotal!==undefined;return `<div class="pvp-card-mini-full">${cardHtml(c,true,'pvp-card-display',user)}<div class="pvp-card-extra"><b>${escapeHtml(c.grade||c.rarity||'C')}</b><span>${limited?'한정판':'일반'}${getPowerTypeIcon(c)?` · ${powerTypeIndicatorHtml(c)}`:''} · 돌파 ★${level}</span></div></div>`}
