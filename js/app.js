...
let battleState={config:null,monsters:[],selectedMonster:null,deck:[],characterBonus:{equipmentPve:0,equipmentPvp:0,titlePve:0,pve:0,pvp:0},energy:null,energyTimer:null,serverOffset:0,restoreMonsterCursor:false};
...
function renderBattleBuilder(){
  ...
  const deckStatusText=document.getElementById('battleDeckStatusText');if(deckStatusText)deckStatusText.innerHTML=`카드 ${cardPower.toLocaleString()} <i>+</i> 장비 ${Number(bonus.equipmentPve||0).toLocaleString()}${Number(bonus.titlePve||0)>0?` <i>+</i> 칭호 ${Number(bonus.titlePve||0).toLocaleString()}`:''}<em>${isReady?' · 출전 준비 완료':' · 5장 편성 필요'}</em>`;
...
let pvpState={tab:'match',config:null,profile:null,myTitle:null,deck:[],characterBonus:{equipmentPve:0,equipmentPvp:0,titlePve:0,pve:0,pvp:0},opponents:[],history:[],ranking:[],energy:null,energyTimer:null,serverOffset:0};
