...
function decorateMagicRenewal(root,d,cards,equipped){
  ...
  if(altar){altar.classList.add('arcane-summon-stage');altar.insertAdjacentHTML('afterbegin','<div class="arcane-stage-shade"></div><div class="arcane-stage-seal" aria-hidden="true"><i></i><i></i><i></i></div><div class="arcane-stage-status"><span><i></i> ALTAR ONLINE</span><b>ARCANA GATE · 01</b></div>');const foot=altar.querySelector(':scope>small');foot?.insertAdjacentHTML('beforebegin',`<div class="arcane-summon-guide"><span><i>Ⅰ</i> 결정 공명</span><span><i>Ⅱ</i> 카드 소환</span><span><i>Ⅲ</i> 중복 환급</span></div>`)}
...
function renderMagicSystem(){
  ...
  root.querySelector('[data-magic-deck="CAPTAIN"]')?.remove();const altar=root.querySelector('.magic-draw-panel');if(altar){const label=altar.querySelector('.eyebrow'),title=altar.querySelector('h2'),foot=altar.querySelector(':scope>small');if(label)label.textContent='MAGIC CARD SUMMON ALTAR';if(title)title.textContent='아르카나 소환 제단';if(foot)foot.textContent='중복 카드는 등급에 따라 마법 결정으로 환급됩니다.'}const singleDraw=root.querySelector('#magicDrawBtn');if(singleDraw&&d.settings?.drawEnabled){singleDraw.textContent='마법카드 1장 소환';singleDraw.insertAdjacentHTML('afterend',`<button type="button" id="magicDraw10Btn" class="btn magic-draw-ten">마법카드 10장 소환 <small>✦ ${(Number(d.settings?.drawCost||0)*10).toLocaleString()}</small></button>`)}root.querySelectorAll('.magic-card-tile').forEach((tile,index)=>{const card=cards[index],label=tile.querySelector('small');if(card&&label)label.textContent=`${magicRenewedEffectLabel(card.effectType)} · ${Number(card.triggerChance)}% · 최대 ${Number(card.maxActivations)}회`;});
...
async function drawMagicCard(count=1){
  count=count===10?10:1;const unitCost=Number(magicSystemState.settings?.drawCost||0),cost=unitCost*count;if(Number(magicSystemState.magicCrystals||0)<cost)return alert(`마법 결정이 부족합니다. (${cost.toLocaleString()}개 필요)`);if(!confirm(`마법 결정 ${cost.toLocaleString()}개를 사용해 마법카드 ${count}장을 소환할까요?`))return;
