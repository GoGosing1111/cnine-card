...
function patchRaidLiveView(d){
  ...
  (c.minions||[]).forEach((minion,index)=>{const node=stage.querySelector(`[data-raid-minion="${index}"]`);if(!node)return;const pct=Math.max(0,Math.min(100,Number(minion.currentHp||0)/Math.max(1,Number(minion.maxHp||0))*100));node.classList.toggle('spawned',Boolean(minion.spawned));node.classList.toggle('waiting',!minion.spawned);node.classList.toggle('defeated',Boolean(minion.defeated));const label=node.querySelector('small');if(label)label.textContent=minion.spawned?(minion.defeated?'처치 완료':'보스 피해 65% 차단'):'2페이즈 증원';const fill=node.querySelector('i u');if(fill)fill.style.width=`${pct}%`;const hp=node.querySelector('span');if(hp)hp.textContent=`${Number(minion.currentHp||0).toLocaleString()} / ${Number(minion.maxHp||0).toLocaleString()}`});
...
function raidWeeklyBossStrip(data){
  const rows=Array.isArray(data?.weeklyBosses)?data.weeklyBosses:[],today=String(data?.todayBoss?.code||'');if(!rows.length)return'';
  return `<section class="raid-weekly-roster"><header><div><small>WEEKLY BOSS ROTATION</small><h2>요일별 레이드 작전표</h2></div><span>전투력 550만 이상 · 보스별 전용 궁극기 · 쫄몹 2종</span></header><div class="raid-weekly-days">${rows.map(row=>`<article class="${String(row.code)===today?'today':''}" style="--raid-accent:${escapeHtml(row.accent||'#8ca6ff')}"><span>${escapeHtml(row.dayLabel)}요일</span><img src="${escapeHtml(row.battleSprite||row.image||'')}" alt="" loading="${String(row.code)===today?'eager':'lazy'}" decoding="async"><div><b>${escapeHtml(row.name)}</b><small>${Number(row.powerRating||0).toLocaleString()} 전투력</small></div>${String(row.code)===today?'<em>TODAY</em>':''}</article>`).join('')}</div></section>`;
}
function raidMinionMarkup(minions=[]){return `<div class="raid-minion-squad">${minions.map((row,index)=>{const pct=Math.max(0,Math.min(100,Number(row.currentHp||0)/Math.max(1,Number(row.maxHp||0))*100));return `<article data-raid-minion="${index}" class="${row.spawned?'spawned':'waiting'} ${row.defeated?'defeated':''}"><img src="${escapeHtml(row.sprite||'')}" alt=""><div><small>${row.spawned?(row.defeated?'처치 완료':'보스 피해 65% 차단'):'2페이즈 증원'}</small><b>${escapeHtml(row.name||'쫄몹')}</b><i><u style="width:${pct}%"></u></i><span>${Number(row.currentHp||0).toLocaleString()} / ${Number(row.maxHp||0).toLocaleString()}</span></div></article>`}).join('')}</div>`}
...
function renderRaidView(d){
  ...
  const myHpPct=me?Math.max(0,Math.min(100,Number(me.currentHp)/Math.max(1,Number(me.maxHp))*100)):0,minionMarkup=raidMinionMarkup(c.minions||[]),guardPct=Math.round(Number(c.bossDamageReduction||0)*100);
...
function bindView(tab) {
  ...
  void import('./clan-faction-v1.mjs?v=20260922-sessions-prep').then(m=>m.connect({apiRequest,clearApiCache,renderShell,ensureFeatureResources,prepareImmediateBattleV3Entry,ensureBattleSoundButton,battleSfx})).catch(()=>{});
