...
function pvpHealerPenaltyState(){const selected=pvpState.deck.map(id=>cards.find(card=>card.id===id)).filter(Boolean),count=selected.filter(pvpCardIsHealer).length,reduction=count>=5?55:count===4?40:count===3?25:count===2?10:0;return {count,reduction}}
