...
function selectedPveIsApocalypse(){const saved=pveMonsterFilterState(),tab=String(saved.tab||'').toUpperCase();if(tab==='APOCALYPSE')return true;if(PVE_MONSTER_TABS.includes(tab))return false;return pveResolvedMonsterTab(selectedPveMonster())==='APOCALYPSE'}
function activeBattleEnergy(){return selectedPveIsApocalypse()?(battleState.apocalypseEnergy||{mode:'APOCALYPSE',unavailable:true,energy:0,maxEnergy:5,costPerBattle:1,rechargeMinutes:30,nextRechargeAt:null}):battleState.energy}
function applyPveEnergyResponse(data={}){if(String(data.energyKind||'').toUpperCase()==='APOCALYPSE')battleState.apocalypseEnergy=data.energy||battleState.apocalypseEnergy;else battleState.energy=data.energy||battleState.energy;return activeBattleEnergy()}
function renderBattleEnergy(){const apocalypse=selectedPveIsApocalypse(),e=activeBattleEnergy(),count=document.getElementById('battleEnergyCount'),fill=document.getElementById('battleEnergyFill'),timer=document.getElementById('battleEnergyTimer'),label=document.getElementById('battleEnergyLabel'),card=document.getElementById('battleEnergyCard');if(label)label.textContent=apocalypse?'☄ 아포칼립스 행동력':'⚔ 전투 횟수';if(card)card.classList.toggle('apocalypse-energy',apocalypse);if(!e||!count)return;const start=document.getElementById('battleStart');if(e.unavailable){count.textContent='연결 확인 중';if(fill)fill.style.width='0%';if(timer)timer.textContent='아포칼립스 행동력 동기화 대기';if(start){start.disabled=true;start.textContent='아포칼립스 행동력 확인 중'}return}count.textContent=e.unlimited?'∞ 무제한':`${e.energy} / ${e.maxEnergy}`;if(fill)fill.style.width=`${e.unlimited?100:Math.max(0,Math.min(100,e.energy/e.maxEnergy*100))}%`;if(timer){if(e.unlimited)timer.textContent='무제한 적용';else if(e.energy>=e.maxEnergy)timer.textContent=apocalypse?'최대 5회 · 충전 완료':'충전 완료';else if(e.nextRechargeAt){const remain=Date.parse(e.nextRechargeAt)-(Date.now()+battleState.serverOffset);timer.textContent=remain<=0?'충전 갱신 중...':`${apocalypse?'30분 충전 · ':''}다음 충전 ${battleEnergyText(remain)}`;}else timer.textContent='충전 대기';}if(start){const noEnergy=!e.unlimited&&e.energy<e.costPerBattle,autoChecked=document.getElementById('battleAuto')?.checked,violation=deckGradeLimitViolation(battleState.deck,battleState.deckRules);start.disabled=battleState.deck.length!==5||!battleState.selectedMonster||noEnergy||Boolean(violation);start.textContent=violation?`${deckGradeRuleLabel(violation.grade)} 편성 제한 확인`:noEnergy?(apocalypse?'아포칼립스 행동력 부족':'전투 횟수 부족'):autoChecked?'남은 횟수 자동전투':'전투 시작';}}
...
function saveBattleViewSnapshot(data,monsters,deck){
  try{
    writeStartupSnapshot({battleView:{
      ...
      apocalypseEnergy:data?.apocalypseEnergy||null,
      savedAt:Date.now()
    }});
...
function hydrateBattleViewFromSnapshot(){
  try{
    ...
    battleState={...battleState,
      ...
      apocalypseEnergy:cached.apocalypseEnergy||battleState.apocalypseEnergy||null,
