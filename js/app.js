...
const FEATURE_RESOURCE_MANIFEST={
  ...
  battleV2:{
    ...
    scripts:[
      ...
      'preview/project-v-v3/project-v-pixi-battle.bundle.js?v=106-combat-flow&joint=2090&mercenary=2100&projectiles=2106&coup=2115&pveEntry=2119&heeya=2118&suits=2124&flow=2126&zSword=20260918&zDash=2&suitName=20260918&apocalypseLegion=2127&mangisa=20260919&ragniel=20260919',
      'js/battle-v3-live.js?v=3.36.0-combat-flow&furHigh=2114&battleRuntime=2124&heeya=2118&entry=2121&suits=2124&flow=2126&zSword=20260918&zDash=2&suitName=20260918&apocalypseLegion=2127&mangisa=20260919&ragniel=20260919'
    ],
...
function pvpTierGuideHtml(tiers=[],currentTier=null,challenger=null){const road=[...tiers,...(challenger?[challenger]:[])];return `<section class="pvp-tier-guide"><div class="pvp-tier-guide-head"><p class="eyebrow">RANKED TIER ROAD</p><h3>시즌 티어 구간</h3><span>챌린저는 점수와 관계없이 현재 랭킹 1~10위 전용입니다.</span></div><div class="pvp-tier-road">${road.map(t=>`<div class="pvp-tier-road-item ${currentTier&&String(currentTier.id)===String(t.id)?'current':''}">${tierEmblem(t,'small')}<b>${t.id==='challenger'?'랭킹 1~10위':`${Number(t.min||0).toLocaleString()}점+`}</b></div>`).join('')}</div></section>`}
let rankHubMode='pvp';
...
function rankedSeasonRewardHtml(profile,config){
  ...
  return `<div class="pvp-section-head"><div><p class="eyebrow">SEASON REWARD</p><h2>시즌 보상</h2></div></div>
    ...
    <div class="ranked-title-reward ranked-title-reward-wide"><span><b>시즌 최종 1~10위</b> <span class="challenger-title-preview"><span class="challenger-title-name">챌린저</span><span class="challenger-title-stars">★★★★</span></span></span><span><b>그랜드마스터 달성</b> 승부사</span><span><b>마스터 달성</b> 결투가</span><small>챌린저는 최종 순위 기준, 나머지는 시즌 최고 달성 티어 기준 · 다음 시즌 정산까지 활성. 챌린저 보상은 일반 티어 보상을 대체하며 최종 랭킹 보상은 별도로 합산됩니다.</small></div>
    <div class="pvp-tier-rewards">${tiers.map(item=>`<div class="${item.id==='challenger'?'challenger-reward-row':''}"><span>${escapeHtml(item.name)}</span><b>${item.id==='challenger'?'시즌 최종 1~10위':`${Number(item.min).toLocaleString()}점+`}</b><strong>◈ ${Number(item.rewardCoin||0).toLocaleString()} · 조각 ${Number(item.rewardShards||0).toLocaleString()}</strong></div>`).join('')}</div>
