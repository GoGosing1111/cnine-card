...
function renderRaidView(d){
  ...
  if(!c){const scheduleText=!schedule.isOpen&&schedule.nextOpenAt?`<div class="raid-schedule-notice">다음 개방: ${new Date(schedule.nextOpenAt).toLocaleString('ko-KR',{timeZone:'Asia/Seoul'})}</div>`:'';box.innerHTML=`<section class="raid-empty raid-closed-screen"><div class="raid-state-icon">🔒</div><p class="eyebrow">WORLD RAID</p><h2>${escapeHtml(s.title||'월드 레이드')}</h2><p>${schedule.isOpen?'현재 열린 레이드 대기방이 없습니다.<br>CMS → 레이드 관리에서 보스를 선택하고 대기방을 시작하세요.':'현재는 레이드 개방 시간이 아닙니다.'}</p>${scheduleText}</section>`;return;}
  ...
  const battle=c.status==='BATTLE',ended=c.status==='ENDED',lobby=c.status==='LOBBY';
  const resultText=c.result==='CLEAR'?'레이드 클리어':c.result==='FAILED'?'파티 전멸':c.result==='TIMEOUT'?'시간 초과':'전투 종료';
  ...
  const bossHp=`<div class="raid-stage-boss-hud"><div><span>${escapeHtml(c.bossName)}</span><b>${hpPct.toFixed(1)}%</b></div><div class="raid-modern-hp boss"><i style="width:${hpPct}%"></i><em style="width:${Math.min(100,hpPct+4)}%"></em></div><small>${Number(c.currentHp).toLocaleString()} / ${Number(c.maxHp).toLocaleString()}</small></div>`;
  const lobbyScreen=`<section class="raid-lobby-screen"><div class="raid-lobby-boss">${c.bossImage?`<img src="${c.bossImage}" alt="" style="display:block;width:118px!important;height:112px!important;max-width:118px!important;max-height:112px!important;object-fit:contain!important;margin:0 auto!important;">`:'<div class="raid-boss-placeholder">👹</div>'}<div class="raid-lobby-glow"></div></div><p class="eyebrow">RAID LOBBY</p><h2>${escapeHtml(c.bossName)}</h2><div class="raid-lobby-countdown"><span>전투 시작까지</span><b>${String(Math.floor(Math.max(0,sec)/60)).padStart(2,'0')}:${String(Math.max(0,sec)%60).padStart(2,'0')}</b></div><div class="raid-lobby-members"><strong>${c.participantCount}</strong><span>/ ${s.maxParticipants} 참가</span></div><div class="raid-slot-line">${Array.from({length:Math.min(Number(s.maxParticipants||30),12)},(_,i)=>`<i class="${i<c.participantCount?'filled':''}"></i>`).join('')}</div><p>${c.participantCount>=Number(s.minParticipants||1)?'출격 준비 완료 · 파티원을 기다리는 중':'최소 참가 인원을 기다리는 중'}</p><button class="btn raid-lobby-join" id="raidJoin" ${joined||!schedule.canEnter?'disabled':''}>${joined?'참가 완료':!schedule.canEnter?'입장 마감':'레이드 신청'}</button><div class="raid-lobby-deck">${(me?.cards||[]).slice(0,5).map(card=>`<img src="${card.image}" alt="" style="display:block;width:42px!important;height:56px!important;max-width:42px!important;max-height:56px!important;object-fit:cover!important;flex:0 0 42px!important;">`).join('')||'<span>신청하면 저장된 PvE 덱이 표시됩니다.</span>'}</div></section>`;
  const battleScreen=`<section class="raid-battle-stage is-battle ${c.enraged?'is-enraged':''}"><div class="raid-stage-sky"><span></span><span></span><span></span></div><div class="raid-stage-header"><div><small>${c.enraged?'⚠ ENRAGED':'WORLD RAID BATTLE'}</small><b>${c.enraged?'보스 광폭화':'자동 전투 진행 중'}</b></div><div class="raid-stage-timer">${Math.max(0,Math.ceil((Date.parse(c.endsAt)-Date.now())/1000))}s</div></div>${bossHp}<div class="raid-arena"><div class="raid-party-side"><div class="raid-party-aura"></div>${attackCard?`<div class="raid-attacker-card"><img src="${attackCard.image}" alt=""><strong>${escapeHtml(attacker.nickname)}</strong><span>${escapeHtml(attackCard.title)}</span></div>`:'<div class="raid-wait-orb">K.O</div>'}<div class="raid-melee-trail"></div></div><div class="raid-boss-side"><div class="raid-boss-aura"></div><div class="raid-slash-fx"><i></i><i></i><i></i></div>${c.bossImage?`<img class="raid-stage-boss" src="${c.bossImage}" alt="">`:'<div class="raid-stage-boss placeholder">👹</div>'}<div class="raid-hit-flash"></div><div class="raid-floating-damage">${attacker?Math.max(1,Math.floor(Number(attacker.totalPower||0)*Number(s.damageMultiplier||1)/10)).toLocaleString():''}</div></div></div><div class="raid-my-deck-stage">${(me?.cards||[]).slice(0,5).map((card,i)=>{const p=me?Math.max(0,Math.min(100,Number(me.currentHp)/Math.max(1,Number(me.maxHp))*100)):0;return `<div class="raid-stage-card ${me?.isDefeated?'dead':''}" style="--delay:${i*80}ms"><img src="${card.image}" alt=""><div class="raid-card-hp"><i style="width:${p}%"></i></div></div>`}).join('')||'<div class="raid-stage-empty">레이드 미참가</div>'}</div><div class="raid-boss-cast"><span>보스 공격 준비</span><i style="animation-duration:${Math.max(800,Number(s.bossAttackIntervalMs||5000))}ms"></i></div></section>`;
  const clear=c.result==='CLEAR',rewardCoin=Number(s.participationCoin||0)+(clear?Number(s.clearCoin||0):0),rewardShards=clear?Number(s.rewardShards||0):0,claimed=Boolean(Number(me?.rewardClaimed||0));
  const myRank=me?participants.findIndex(x=>Number(x.userId)===Number(me.userId))+1:0,mvp=participants[0];
  const resultScreen=`<section class="raid-result-screen ${clear?'clear':'failed'}"><div class="raid-result-burst"></div><div class="raid-result-crown">${clear?'✦':'×'}</div><p class="eyebrow">RAID RESULT</p><h1>${clear?'RAID CLEAR':'RAID FAILED'}</h1><h2>${escapeHtml(c.bossName)}</h2>${mvp?`<div class="raid-mvp-card"><span>MVP</span><strong>${escapeHtml(mvp.nickname)}</strong><b>${Number(mvp.shownDamage||0).toLocaleString()} DMG</b></div>`:''}<div class="raid-result-summary"><article><span>MY DAMAGE</span><b>${Number(me?.shownDamage||0).toLocaleString()}</b></article><article><span>FINAL RANK</span><b>${myRank?myRank+'위':'-'}</b></article><article class="${me?.isDefeated?'danger':'alive'}"><span>SURVIVAL</span><b>${me?.isDefeated?'K.O':'ALIVE'}</b></article></div><div class="raid-reward-box ${claimed?'is-claimed':''}"><div class="raid-reward-title"><span>RAID REWARD</span><small>${clear?'토벌 성공 보상':'참가 보상'}</small></div><div class="raid-reward-cards"><article class="coin" style="--reward-delay:.12s"><div class="raid-reward-icon">◈</div><small>MINERAL</small><b>+${rewardCoin.toLocaleString()}</b><em>${clear?'참가 + 처치':'참가'}</em></article><article class="shard ${rewardShards<=0?'disabled':''}" style="--reward-delay:.32s"><div class="raid-reward-icon">✦</div><small>CARD SHARD</small><b>+${rewardShards.toLocaleString()}</b><em>${clear?'클리어 보상':'미획득'}</em></article></div><div class="raid-reward-seal">${claimed?'REWARD COMPLETE':'TOUCH TO CLAIM'}</div><button id="raidClaimReward" class="btn raid-claim-btn" ${!me||claimed?'disabled':''}>${!me?'미참가':claimed?'수령 완료':'보상 획득'}</button><small class="raid-reward-note">${claimed?'보상이 계정에 반영되었습니다.':'보상 아이콘이 순서대로 개방됩니다.'}</small></div></section>`;
  const main=lobby?lobbyScreen:battle?battleScreen:resultScreen;
  box.innerHTML=`${main}<section class="raid-live-grid ${lobby?'is-lobby':''}"><div class="raid-party"><div class="panel-title"><div><p class="eyebrow">RAID PARTY</p><h2>${lobby?'참가 대기 명단':'참가자 생존 현황'}</h2></div></div><div class="raid-participant-list">${participants.length?participants.map(x=>{const p=Math.max(0,Math.min(100,Number(x.currentHp)/Math.max(1,Number(x.maxHp))*100));return `<article class="${x.isDefeated?'defeated':''}"><div class="raid-mini-cards">${(x.cards||[]).slice(0,5).map(card=>`<img src="${card.image}" title="${escapeHtml(card.title)}" style="display:block;width:34px!important;height:46px!important;max-width:34px!important;max-height:46px!important;object-fit:cover!important;flex:0 0 34px!important;">`).join('')}</div><span><b>${escapeHtml(x.nickname)}</b><small>전투력 ${Number(x.totalPower).toLocaleString()}</small>${!lobby&&s.showParticipantHp!==false?`<em class="raid-user-hp modern"><i style="width:${p}%"></i></em><small>${x.isDefeated?'전투 불능':`${Math.round(p)}%`}</small>`:''}</span><strong>${lobby?'READY':Number(x.shownDamage||0).toLocaleString()+' DMG'}</strong></article>`}).join(''):'<div class="empty-recent">첫 참가자를 기다리고 있습니다.</div>'}</div></div><aside class="raid-ranking"><div class="panel-title"><div><p class="eyebrow">LIVE DAMAGE</p><h2>딜 체크 미터</h2></div></div>${!lobby&&participants.length?participants.slice(0,Number(s.rankingSize||10)).map((x,i)=>{const max=Math.max(1,Number(participants[0]?.shownDamage||1)),pct=Math.max(2,Number(x.shownDamage||0)/max*100);return `<div class="raid-rank-row ${x.isDefeated?'defeated':''}"><b>${i+1}</b><span>${escapeHtml(x.nickname)}<i style="width:${pct}%"></i></span><strong>${Number(x.shownDamage||0).toLocaleString()}</strong></div>`}).join(''):'<div class="empty-recent">전투 시작 후 실시간으로 표시됩니다.</div>'}<div class="raid-my-damage"><span>내 누적 딜</span><b>${Number(me?.shownDamage||0).toLocaleString()}</b><small>${me&&!lobby?`HP ${Math.round(Number(me.currentHp)/Math.max(1,Number(me.maxHp))*100)}% · ${participants.findIndex(x=>Number(x.userId)===Number(me.userId))+1}위`:me?'출격 대기 중':'레이드 미참가'}</small></div></aside></section>`;
  const join=document.getElementById('raidJoin');if(join&&!join.disabled)join.onclick=joinRaid;
  const claim=document.getElementById('raidClaimReward');if(claim&&!claim.disabled)claim.onclick=()=>claimRaidReward(c.id);
}

async function claimRaidReward(instanceId){const btn=document.getElementById('raidClaimReward'),box=document.querySelector('.raid-reward-box');if(btn){btn.disabled=true;btn.textContent='보상 개방 중...'}try{const d=await apiRequest('raid/claim-reward',{method:'POST',body:JSON.stringify({instanceId})});const user=loadUser();if(d.user){user.coin=d.user.coin;user.cardShards=d.user.cardShards;saveUser(user)}box?.classList.add('claiming');setTimeout(()=>{box?.classList.remove('claiming');box?.classList.add('is-claimed');const seal=box?.querySelector('.raid-reward-seal');if(seal)seal.textContent='REWARD COMPLETE';if(btn)btn.textContent='수령 완료';const note=box?.querySelector('.raid-reward-note');if(note)note.textContent=`미네랄 ${Number(d.coin).toLocaleString()} · 카드 조각 ${Number(d.shards).toLocaleString()} 지급 완료`;},1050)}catch(e){alert(e.message);if(btn){btn.disabled=false;btn.textContent='보상 획득'}}}
