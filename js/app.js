...
const PVE_MONSTER_TABS=['ALL','GENERAL','ELITE','BOSS','EVENT'];
...
function monsterCategoryLabel(v){return ({GENERAL:'일반',ELITE:'정예',BOSS:'보스',EVENT:'이벤트'})[String(v||'GENERAL').toUpperCase()]||'일반'}
function renderPveMonsterBrowser(){
 const root=document.getElementById('battleMonsters');if(!root)return;
 const state={tab:'ALL',sort:'POWER_ASC',query:'',...pveMonsterFilterState()};
 const rows=(battleState.monsters||[]).filter(m=>{const cat=String(m.pveTab||m.category||(m.isBoss?'BOSS':'GENERAL')).toUpperCase();return (state.tab==='ALL'||cat===state.tab)&&String(m.name||'').toLowerCase().includes(String(state.query||'').toLowerCase())}).sort((x,y)=>state.sort==='POWER_DESC'?Number(y.battlePower)-Number(x.battlePower):state.sort==='NAME'?String(x.name).localeCompare(String(y.name),'ko'):Number(x.battlePower)-Number(y.battlePower));
 root.innerHTML=`<div class="pve-monster-browser"><div class="pve-monster-tabs">${PVE_MONSTER_TABS.map(t=>`<button type="button" data-monster-tab="${t}" class="${state.tab===t?'active':''}">${t==='ALL'?'전체':monsterCategoryLabel(t)}</button>`).join('')}</div><div class="pve-monster-tools"><input id="pveMonsterSearch" value="${escapeHtml(state.query||'')}" placeholder="몬스터 이름 검색"><select id="pveMonsterSort"><option value="POWER_ASC" ${state.sort==='POWER_ASC'?'selected':''}>권장 전투력 낮은순</option><option value="POWER_DESC" ${state.sort==='POWER_DESC'?'selected':''}>권장 전투력 높은순</option><option value="NAME" ${state.sort==='NAME'?'selected':''}>이름순</option></select></div><div class="pve-monster-grid">${rows.map(m=>`<button class="monster-choice ${Number(battleState.selectedMonster)===Number(m.id)?'active':''}" data-monster="${m.id}">${m.image?`<img src="${m.image}">`:'<div class="monster-placeholder">👹</div>'}<span><small>${monsterCategoryLabel(m.pveTab||m.category||(m.isBoss?'BOSS':'GENERAL'))}</small><b>${escapeHtml(m.name)}</b><em>전투력 ${Number(m.battlePower).toLocaleString()}</em><strong>승리 보상 ◈ ${Number(m.rewardCoin).toLocaleString()}</strong></span></button>`).join('')||'<div class="empty-recent">조건에 맞는 몬스터가 없습니다.</div>'}</div></div>`;
 root.querySelectorAll('[data-monster-tab]').forEach(b=>b.onclick=()=>{state.tab=b.dataset.monsterTab;savePveMonsterFilterState(state);renderBattleBuilder()});
...
async function startAutoBattle(){
  ...
  battleState.autoRunning=true;battleState.autoTargetBattles=remaining;battleState.autoRemaining=remaining;battleState.autoSummary={battles:0,wins:0,losses:0,totalReward:0,cardRewards:[]};stopBattleEnergyTimer();
...
async function startBattle(){
  ...
  try{
    ...
    if(battleState.autoRunning){
      const summary=battleState.autoSummary||(battleState.autoSummary={battles:0,wins:0,losses:0,totalReward:0,cardRewards:[]});summary.battles++;summary.totalReward+=Number(d.reward||0);if(win)summary.wins++;else summary.losses++;if(d.cardReward)summary.cardRewards.push(d.cardReward);battleState.autoRemaining=Math.max(0,Number(battleState.autoRemaining||0)-1);
