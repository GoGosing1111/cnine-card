...
function renderBattleBuilder(){
  ...
  const groups=viewMode==='deck'?pveDeckGradeGroups(owned):[],deckSet=new Set(battleState.deck),power=battleState.deck.reduce((sum,id)=>{const card=cards.find(item=>item.id===id);return sum+(card?battleCardPower(card,user,battleState.config):0)},0),isReady=battleState.deck.length===5;
  ...
  if(viewMode==='deck'){
    ...
    if(cardRoot)cardRoot.innerHTML=groups.map(group=>`<section class="pve-grade-group grade-${String(group.grade).toLowerCase()}"><div class="pve-grade-title"><b>${escapeHtml(group.grade)}</b><span>${group.cards.length}장</span></div><div class="pve-grade-grid">${group.cards.map(card=>`<button type="button" class="pve-frame-pick ${deckSet.has(card.id)?'selected':''}" data-pick="${card.id}" ${deckSet.has(card.id)?'disabled':''}>${pveDeckCardMini(card,user)}${deckSet.has(card.id)?'<span class="pve-selected-cover"><b>편성됨</b></span>':''}</button>`).join('')}</div></section>`).join('')||'<div class="empty-recent">보유 카드가 없습니다.</div>';
    if(monsterRoot)monsterRoot.innerHTML='';
    document.querySelectorAll('[data-pick]').forEach(button=>button.onclick=()=>{if(battleState.deck.length<5){battleState.deck.push(button.dataset.pick);renderBattleBuilder()}});
