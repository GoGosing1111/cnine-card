...
function renderShell(tab='world') {
  ...
  const user=loadUser();
  if(!user)return renderLogin();
  if(tab!=='world')return openWorldPanel(tab);
...
const WORLD_MAPS={
 town:{name:'씨켓몬 마을',w:1800,h:1250,spawn:{x:860,y:850},grass:{x:1120,y:690,w:560,h:420},exits:[{x:820,y:1180,w:170,h:60,to:'field',tx:800,ty:100}],buildings:[
  ...
  {id:'hall',x:300,y:660,w:350,h:260,label:'모험 안내소',tab:'attendance'}],npcs:[{id:'chulgu',x:720,y:560,label:'철구',sprite:'npcChulgu',dialog:['씨켓몬 마을에 온 걸 환영한다!','강한 덱을 만들고 초원으로 나가봐.']},{id:'jjeomni',x:850,y:545,label:'졈니',sprite:'npcJjeomni',dialog:['카드 상점에는 다양한 카드팩이 있어.','좋은 카드가 나오길 응원할게!']},{id:'jjeomjung',x:980,y:565,label:'점중',sprite:'npcJjeomjung',dialog:['랭킹은 꾸준히 도전해야 올라갈 수 있어.'],tab:'rank'},{id:'yeonji',x:1090,y:545,label:'연지',sprite:'npcYeonji',dialog:['반가워! 오늘도 즐거운 모험 되길 바라.'],tab:'mineral'}]},
...
function openNpcDialog(npc){const box=document.getElementById('worldNpcDialog');if(!box)return;worldState.paused=true;worldState.keys={};const lines=Array.isArray(npc.dialog)&&npc.dialog.length?npc.dialog:['안녕하세요.'];let index=0;const paint=()=>{box.className='world-npc-dialog open';box.innerHTML=`<div class="npc-dialog-name">${escapeHtml(npc.label||'주민')}</div><div class="npc-dialog-text">${escapeHtml(lines[index])}</div><div class="npc-dialog-help">SPACE / 확인 · ESC 닫기</div>`};const advance=()=>{index++;if(index>=lines.length){closeNpcDialog();if(npc.tab)openWorldPanel(npc.tab)}else paint()};worldState.npcAdvance=advance;paint();box.onclick=advance}
function worldAction(){if(worldState.busy||worldState.paused)return;const m=worldMap();if(m.interior){if(Math.hypot(worldState.x-480,worldState.y-360)<115){openWorldPanel(m.feature);return}worldToast('카운터 앞에서 Space 또는 모바일 확인 버튼을 눌러주세요.');return}const nearbyNpc=(m.npcs||[]).map(n=>({n,d:Math.hypot(worldState.x-n.x,worldState.y-n.y)})).filter(x=>x.d<110).sort((a,b)=>a.d-b.d)[0];if(nearbyNpc){openNpcDialog(nearbyNpc.n);return}for(const b of m.buildings||[]){const d=buildingDoor(b),cx=d.x+d.w/2,cy=d.y+d.h/2;if(Math.hypot(worldState.x-cx,worldState.y-cy)<82){worldState.returnX=cx;worldState.returnY=cy+65;changeMap('interior_'+b.id);return}}worldToast(inGrass()?'풀숲에서 인기척이 느껴집니다.':'주변에 상호작용할 대상이 없습니다.')}
