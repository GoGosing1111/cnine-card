...
function renderRaidView(d){
  ...
  const resultText=c.result==='CLEAR'?'레이드 클리어':c.result==='FAILED'?'파티 전멸 · 실패':c.result==='TIMEOUT'?'시간 초과 · 실패':'';
  ...
  const stage=s.showBattleStage!==false?`<section class="raid-battle-stage ${battle?'is-battle':''} ${c.enraged?'is-enraged':''} ${ended?'is-ended':''}">
    <div class="raid-stage-sky"><span></span><span></span><span></span></div>
    <div class="raid-stage-header"><div><small>${c.enraged?'⚠ ENRAGED':'WORLD RAID'}</small><b>${battle?'자동 전투 진행 중':ended?resultText:`${Math.max(0,sec)}초 후 전투 시작`}</b></div><div class="raid-stage-timer">${battle?Math.max(0,Math.ceil((Date.parse(c.endsAt)-Date.now())/1000))+'s':ended?'RESULT':'READY'}</div></div>
    <div class="raid-arena">
      <div class="raid-party-side"><div class="raid-party-aura"></div>${attackCard?`<div class="raid-attacker-card" key="${attackCard.id}-${c.attackTicks}"><img src="${attackCard.image}" alt=""><strong>${escapeHtml(attacker.nickname)}</strong><span>${escapeHtml(attackCard.title)}</span></div>`:`<div class="raid-wait-orb">${battle?'전투 불능':'READY'}</div>`}<div class="raid-projectile"></div></div>
      <div class="raid-boss-side"><div class="raid-boss-aura"></div>${c.bossImage?`<img class="raid-stage-boss" src="${c.bossImage}" alt="">`:'<div class="raid-stage-boss placeholder">👹</div>'}<div class="raid-hit-flash"></div><div class="raid-floating-damage">${battle&&attacker?Math.max(1,Math.floor(Number(attacker.totalPower||0)*Number(s.damageMultiplier||1)/10)).toLocaleString():''}</div></div>
    </div>
    <div class="raid-my-deck-stage">${(me?.cards||[]).slice(0,5).map((card,i)=>`<div class="raid-stage-card ${me?.isDefeated?'dead':''}" style="--delay:${i*80}ms"><img src="${card.image}" alt=""><i style="width:${me?Math.max(0,Math.min(100,Number(me.currentHp)/Math.max(1,Number(me.maxHp))*100)):0}%"></i></div>`).join('')||'<div class="raid-stage-empty">레이드 신청 후 내 덱이 표시됩니다.</div>'}</div>
    ${battle?`<div class="raid-boss-cast"><span>보스 공격</span><i style="animation-duration:${Math.max(800,Number(s.bossAttackIntervalMs||5000))}ms"></i></div>`:''}
  </section>`:'';
  box.innerHTML=`<section class="raid-hero status-${String(c.status).toLowerCase()}"><div class="raid-boss-pane">${c.bossImage?`<img src="${c.bossImage}" alt="">`:'<div class="raid-boss-placeholder">👹</div>'}<div><p class="eyebrow">OWNER TEST RAID</p><h2>${escapeHtml(c.bossName)}</h2><div class="raid-hp-head"><span>BOSS HP</span><b>${Number(c.currentHp).toLocaleString()} / ${Number(c.maxHp).toLocaleString()}</b></div><div class="raid-hp-track"><i style="width:${hpPct}%"></i></div><strong class="raid-status">${c.status==='LOBBY'?`신청 대기 · ${Math.max(0,sec)}초 후 시작`:battle?(c.enraged?'광폭화 전투 진행 중':'전투 진행 중'):resultText}</strong></div></div><div class="raid-join-card"><span>참가 인원</span><b>${c.participantCount} / ${s.maxParticipants}</b><div class="raid-people-bar"><i style="width:${Math.min(100,c.participantCount/Math.max(1,s.maxParticipants)*100)}%"></i></div><button class="btn" id="raidJoin" ${joined||c.status!=='LOBBY'||!schedule.canEnter?'disabled':''}>${joined?'신청 완료':!schedule.canEnter?'입장 시간 종료':c.status==='LOBBY'?'레이드 신청':'신청 마감'}</button><small>저장된 PvE 덱 5장으로 참가합니다.</small></div></section>${stage}<section class="raid-live-grid"><div class="raid-party"><div class="panel-title"><div><p class="eyebrow">RAID PARTY</p><h2>참가자 생존 현황</h2></div></div><div class="raid-participant-list">${participants.length?participants.map(x=>{const p=Math.max(0,Math.min(100,Number(x.currentHp)/Math.max(1,Number(x.maxHp))*100));return `<article class="${x.isDefeated?'defeated':''}"><div class="raid-mini-cards">${(x.cards||[]).slice(0,5).map(card=>`<img src="${card.image}" title="${escapeHtml(card.title)}">`).join('')}</div><span><b>${escapeHtml(x.nickname)}</b><small>전투력 ${Number(x.totalPower).toLocaleString()}</small>${s.showParticipantHp!==false?`<em class="raid-user-hp"><i style="width:${p}%"></i></em><small>${x.isDefeated?'전투 불능':`${Number(x.currentHp).toLocaleString()} / ${Number(x.maxHp).toLocaleString()}`}</small>`:''}</span><strong>${battle||ended?Number(x.shownDamage||0).toLocaleString()+' DMG':'READY'}</strong></article>`}).join(''):'<div class="empty-recent">첫 참가자를 기다리고 있습니다.</div>'}</div></div><aside class="raid-ranking"><div class="panel-title"><div><p class="eyebrow">LIVE DAMAGE</p><h2>딜 체크 미터</h2></div></div>${participants.length?participants.slice(0,Number(s.rankingSize||10)).map((x,i)=>{const max=Math.max(1,Number(participants[0]?.shownDamage||1)),pct=Math.max(2,Number(x.shownDamage||0)/max*100);return `<div class="raid-rank-row ${x.isDefeated?'defeated':''}"><b>${i+1}</b><span>${escapeHtml(x.nickname)}<i style="width:${pct}%"></i></span><strong>${Number(x.shownDamage||0).toLocaleString()}</strong></div>`}).join(''):'<div class="empty-recent">전투 시작 후 표시됩니다.</div>'}<div class="raid-my-damage"><span>내 누적 딜</span><b>${Number(me?.shownDamage||0).toLocaleString()}</b><small>${me?`HP ${Math.round(Number(me.currentHp)/Math.max(1,Number(me.maxHp))*100)}% · ${participants.findIndex(x=>Number(x.userId)===Number(me.userId))+1}위`:'레이드 미참가'}</small></div></aside></section>`;
