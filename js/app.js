...
async function startBattle(){const modal=document.getElementById('modal'),monster=battleState.monsters.find(m=>Number(m.id)===Number(battleState.selectedMonster));modal.className='modal show battle-modal';modal.innerHTML=`<div class="modal-panel battle-stage"><div class="battle-stars"></div><p class="eyebrow">BATTLE START</p><div class="battle-vs"><div class="battle-team">${battleState.deck.map(id=>{const c=cards.find(x=>x.id===id);return `<img src="${c.image}">`}).join('')}</div><strong>VS</strong><div class="battle-enemy">${monster.image?`<img src="${monster.image}">`:'<div>👹</div>'}<b>${escapeHtml(monster.name)}</b></div></div><div class="battle-clash"></div><p id="battleMessage">전투력이 충돌합니다...</p></div>`;try{await new Promise(r=>setTimeout(r,850));modal.querySelector('.battle-stage').classList.add('clashing');const d=await apiRequest('battle/fight',{method:'POST',body:JSON.stringify({monsterId:battleState.selectedMonster,cardIds:battleState.deck})});await new Promise(r=>setTimeout(r,1300));modal.querySelector('.battle-stage').classList.add(d.result==='WIN'?'battle-win':'battle-lose');document.getElementById('battleMessage').innerHTML=d.result==='WIN'?`<strong>VICTORY</strong><span>전투력 ${d.playerPower.toLocaleString()} VS ${d.monsterPower.toLocaleString()}</span><em>보상 ◈ ${d.reward.toLocaleString()}</em>`:`<strong>DEFEAT</strong><span>전투력 ${d.playerPower.toLocaleString()} VS ${d.monsterPower.toLocaleString()}</span><em>돌파로 전투력을 높여보세요.</em>`;saveUser(apiUserToLocal(d.user));setTimeout(()=>{modal.onclick=()=>renderShell('battle')},600);}catch(e){document.getElementById('battleMessage').textContent=e.message;modal.onclick=()=>renderShell('battle')}}
