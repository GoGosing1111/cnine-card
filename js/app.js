...
let burningEventState={enabled:false,generation:0,title:'씨켓몬 버닝이 발동 되었습니다',packDiscountPercent:20,duplicateShardMultiplier:2,battleRewardMultiplier:1.5};
...
function dexView(user) {
  ...
  return `${summaryBar(user)}<section class="dex-cover"><div><p class="eyebrow">MY COLLECTION ALBUM</p><h2>씨켓몬 도감</h2><p>멤버별 앨범을 펼쳐 수집한 카드와 고유 능력을 확인하세요.</p>${uniqueCount?`<button type="button" class="dex-unique-legend" data-scroll-unique="1"><i>◇</i><span><b>고유 능력 카드 ${uniqueCount}장</b><small>카드 이름 오른쪽의 유형 배지를 누르면 능력치 프로필이 바로 열립니다.</small></span></button>`:''}</div><div class="dex-total"><b>${owned.size}</b><span>/ ${cards.length} CARDS</span></div></section><div class="dex-toolbar"><div class="dex-search"><input id="dexSearch" value="${escapeHtml(prefs.search||'')}" placeholder="카드명 또는 멤버 검색"><select id="gradeFilter"><option value="">전체 등급</option>${['FUR','PRESTIGE','LIMITED','MA','SSR','UR','HR','SR','R','U','C'].map(g=>`<option ${prefs.grade===g?'selected':''}>${g}</option>`).join('')}</select><select id="dexSort"><option value="default" ${prefs.sort==='default'?'selected':''}>기본 순서</option><option value="name" ${prefs.sort==='name'?'selected':''}>멤버 이름순</option><option value="owned" ${prefs.sort==='owned'?'selected':''}>보유 카드 많은순</option><option value="rate" ${prefs.sort==='rate'?'selected':''}>수집률 높은순</option><option value="count" ${prefs.sort==='count'?'selected':''}>전체 카드 많은순</option></select></div><button type="button" id="favoriteMemberOnly" class="dex-favorite-filter ${prefs.favoriteOnly?'active':''}" aria-pressed="${prefs.favoriteOnly?'true':'false'}">★ 즐겨찾기 멤버만</button></div><div class="dex-guide"><span>멤버 이름 옆 별을 눌러 즐겨찾기</span><span>${uniqueCount?'유형 배지 선택 시 카드 능력치 확인':'즐겨찾기 멤버는 항상 위에 표시'}</span></div><div id="dexSections">${members.map((item,index)=>dexSection(item,owned,index,prefs)).join('')}</div>`;
}
...
function renderBattleBuilder(){
  ...
  const deckSet=new Set(battleState.deck),power=battleState.deck.reduce((sum,id)=>{const card=cards.find(item=>item.id===id);return sum+(card?battleCardPower(card,user,battleState.config):0)},0),isReady=battleState.deck.length===5;
  ...
  const deckStatusText=document.getElementById('battleDeckStatusText');if(deckStatusText)deckStatusText.textContent=isReady?'5장 편성이 완료되었습니다. 몬스터 토벌을 시작할 수 있습니다.':'아직 5장 편성이 완료되지 않았습니다';
...
function riftRewardHtml(stash={}){return `<div class="rift-stash"><article><i>◈</i><span><small>누적 코인</small><b>${Number(stash.coin||0).toLocaleString()}</b></span></article><article><i>◆</i><span><small>카드 조각</small><b>${Number(stash.shards||0).toLocaleString()}</b></span></article><article><i>◇</i><span><small>균열 결정</small><b>${Number(stash.crystals||0).toLocaleString()}</b></span></article></div>`}
...
function riftMapHtml(run){const max=Number(run.maxStages||7),history=Array.isArray(run.history)?run.history:[];return `<div class="rift-route-map">${Array.from({length:max},(_,stage)=>{const passed=history.filter(x=>Number(x.stage)===stage),current=Number(run.stage)===stage&&run.status==='ACTIVE',done=stage<Number(run.stage)||passed.some(x=>x.result==='WIN'||x.result==='RESOLVED'),boss=stage===3||stage===6;return `<div class="rift-route-column ${current?'current':''} ${done?'done':''} ${boss?'boss':''}"><span class="rift-route-index">${stage+1}</span><div class="rift-route-line"></div>${passed.length?passed.slice(-1).map(x=>{const m=riftTypeMeta(x.type);return `<div class="rift-route-node selected ${x.result==='LOSE'?'lost':''}"><i>${m.icon}</i><b>${escapeHtml(x.name||m.label)}</b><small>${x.result==='WIN'?'승리':x.result==='LOSE'?'패배':'완료'}</small></div>`}).join(''):current?`<div class="rift-route-node current"><i>✦</i><b>현재 구역</b><small>선택 대기</small></div>`:`<div class="rift-route-node locked"><i>${boss?'♛':'·'}</i><b>${boss?(stage===6?'최종 보스':'중간 보스'):'미개방'}</b><small>${stage+1}구역</small></div>`}</div>`}).join('')}</div>`}
function riftChoiceHtml(node){const m=riftTypeMeta(node.type),battle=['BATTLE','ELITE','BOSS','FINAL_BOSS'].includes(node.type);return `<button type="button" class="rift-choice type-${String(node.type).toLowerCase()}" data-rift-node="${escapeHtml(node.id)}"><div class="rift-choice-visual">${node.image?`<img src="${escapeHtml(node.image)}" alt="">`:`<span>${m.icon}</span>`}<em>${m.label}</em></div><div class="rift-choice-copy"><small>STAGE ${Number(node.stage||0)+1}</small><h3>${escapeHtml(node.name||m.label)}</h3><p>${escapeHtml(node.description||(battle?'저장된 원정 덱으로 전투를 진행합니다.':'선택 즉시 효과가 적용됩니다.'))}</p>${battle?`<div><span>전투력 <b>${Number(node.battlePower||0).toLocaleString()}</b></span><span>결정 <b>+${Number(node.rewardPreview?.crystals||0)}</b></span></div>`:''}<strong>${battle?'전투 준비':'선택하기'} <i>›</i></strong></div></button>`}
function renderRiftStart(root,data){const ids=(data.savedDeck||battleState.deck||[]).map(String),deck=ids.map(id=>cards.find(c=>String(c.id)===id)).filter(Boolean),weekly=data.weekly||{},max=Number(data.settings?.maxDifficulty||10);root.innerHTML=`<section class="rift-landing"><div class="rift-space-bg"><i></i><i></i><i></i></div><div class="rift-landing-copy"><p class="eyebrow">DIMENSIONAL RIFT EXPEDITION</p><h2>차원의 균열 원정</h2><p>갈림길을 선택하고, 전투마다 원정 전용 강화를 획득해 최종 보스를 격파하세요.<br>카드 체력과 강화 효과는 원정이 끝날 때까지 유지됩니다.</p><div class="rift-weekly-badges"><span><small>주간 보상</small><b>${Number(weekly.rewardCount||0)} / ${Number(weekly.rewardLimit||3)}회</b></span><span><small>최고 난이도</small><b>${Number(weekly.highestDifficulty||0)}단계</b></span><span><small>보유 균열 결정</small><b>${Number(data.riftCrystals||0).toLocaleString()}</b></span></div></div><div class="rift-core-orb"><span></span><b>RIFT</b><small>WEEK ${escapeHtml(String(data.weekKey||''))}</small></div></section><section class="rift-start-grid"><div class="rift-start-panel"><div class="rift-section-head"><div><p class="eyebrow">EXPEDITION DIFFICULTY</p><h3>원정 난이도 선택</h3><small>난이도가 높을수록 적 전투력과 최종 보상이 증가합니다.</small></div></div><div class="rift-difficulty-grid">${Array.from({length:max},(_,i)=>i+1).map(n=>`<button type="button" data-rift-difficulty="${n}" class="${riftState.difficulty===n?'active':''}"><small>DIFFICULTY</small><b>${n}</b><span>${n<=3?'탐색':n<=6?'심층':n<=9?'극한':'붕괴'}</span></button>`).join('')}</div><div class="rift-start-actions"><button type="button" class="btn rift-start-button" id="riftStartRun" ${deck.length!==5?'disabled':''}>✦ ${riftState.difficulty}단계 원정 시작</button><small>${deck.length===5?'현재 저장된 PvE 덱 5장이 원정에 고정됩니다.':'먼저 PvE 덱 편성에서 카드 5장을 저장하세요.'}</small></div></div><aside class="rift-deck-preview"><div class="rift-section-head"><div><p class="eyebrow">LOCKED EXPEDITION DECK</p><h3>원정 출전 덱</h3></div><b>${deck.length} / 5</b></div><div class="rift-start-deck">${deck.map(c=>`<div>${cardHtml(c,true,'small rift-start-card',loadUser())}</div>`).join('')||'<div class="rift-empty">저장된 PvE 덱이 없습니다.</div>'}</div><div class="rift-rule-list"><span><i>1</i> 원정 시작 후 덱 변경 불가</span><span><i>2</i> 카드 체력은 구역 사이에 유지</span><span><i>3</i> 매 전투 후 강화 1개 선택</span><span><i>4</i> 주간 보상 3회 이후 기록 도전</span></div></aside></section>`;root.querySelectorAll('[data-rift-difficulty]').forEach(b=>b.onclick=()=>{riftState.difficulty=Number(b.dataset.riftDifficulty);renderRiftView(data)});root.querySelector('#riftStartRun')?.addEventListener('click',startRiftRun)}
function renderRiftRun(root,data){const run=data.run,buffs=run.buffs||[],active=run.activeNode,pending=run.pendingBuffChoices||[],failed=run.status==='FAILED',completed=run.status==='COMPLETED_PENDING';root.innerHTML=`<section class="rift-run-hero"><div><p class="eyebrow">DIMENSIONAL RIFT · DIFFICULTY ${run.difficulty}</p><h2>${riftStatusLabel(run.status)}</h2><p>${completed?'최종 보스를 격파했습니다. 누적한 원정 보상을 확정하세요.':failed?'원정 덱이 전투 불능 상태가 되었습니다. 새로운 원정을 준비하세요.':'현재 경로를 선택해 차원의 심층부로 진입하세요.'}</p></div><div class="rift-run-summary"><span><small>진행 구역</small><b>${Math.min(Number(run.stage||0)+1,Number(run.maxStages||7))} / ${Number(run.maxStages||7)}</b></span><span><small>난이도</small><b>${run.difficulty}</b></span><span class="${run.rewardEligible?'eligible':'practice'}"><small>보상 상태</small><b>${run.rewardEligible?'획득 가능':'기록 도전'}</b></span></div></section><section class="rift-map-panel"><div class="rift-section-head"><div><p class="eyebrow">EXPEDITION ROUTE</p><h3>균열 경로</h3></div>${completed?'':'<button type="button" class="rift-abandon" id="riftAbandon">원정 포기</button>'}</div>${riftMapHtml(run)}</section><section class="rift-run-layout"><main class="rift-main-panel">${completed?`<div class="rift-complete-panel"><span class="rift-complete-emblem">♛</span><small>RIFT CONQUERED</small><h3>최종 균열 봉쇄 완료</h3><p>${run.rewardEligible?'이번 주 원정 보상을 수령할 수 있습니다.':'주간 보상 횟수를 모두 사용해 최고 기록만 반영됩니다.'}</p>${riftRewardHtml(run.stash)}<button type="button" class="btn rift-claim-button" id="riftClaim">${run.rewardEligible?'원정 보상 수령':'기록 확정'}</button></div>`:failed?`<div class="rift-failed-panel"><span>☠</span><small>EXPEDITION FAILED</small><h3>원정대 전투 불능</h3><p>진행 중 확보한 임시 보상은 소멸합니다.</p><button type="button" class="btn" id="riftNewRun">새 원정 준비</button></div>`:pending.length?`<div class="rift-buff-select"><div class="rift-section-head"><div><p class="eyebrow">RIFT EVOLUTION</p><h3>원정 강화 선택</h3><small>이번 원정이 끝날 때까지 유지되는 효과입니다.</small></div></div><div class="rift-buff-choice-grid">${pending.map(key=>{const buff=({ATTACK_CORE:{key:'ATTACK_CORE',icon:'⚔',name:'공격 코어',description:'원정 전투력이 15% 증가합니다.'},BARRIER_FIELD:{key:'BARRIER_FIELD',icon:'◆',name:'방벽 전개',description:'전투 후 받는 피해가 25% 감소합니다.'},VELOCITY_LINK:{key:'VELOCITY_LINK',icon:'↯',name:'속도 연계',description:'전투력 10% 증가 · 정예전 추가 8% 증가'},LIFE_PULSE:{key:'LIFE_PULSE',icon:'♥',name:'생명 파동',description:'승리할 때마다 카드 체력을 8 회복합니다.'},BOSS_HUNTER:{key:'BOSS_HUNTER',icon:'♛',name:'보스 사냥꾼',description:'보스 전투력이 25% 증가합니다.'},SECOND_WIND:{key:'SECOND_WIND',icon:'✦',name:'두 번째 숨결',description:'전투 불능 카드 1장을 체력 25로 부활시킵니다.'},CRYSTAL_GREED:{key:'CRYSTAL_GREED',icon:'◇',name:'결정 탐욕',description:'결정 +40% · 받는 피해 +10%'}})[key]||{key,icon:'✦',name:key,description:''};return `<button type="button" data-rift-buff="${key}"><i>${buff.icon}</i><small>EXPEDITION BUFF</small><h4>${escapeHtml(buff.name)}</h4><p>${escapeHtml(buff.description)}</p><strong>선택하기</strong></button>`}).join('')}</div></div>`:active?`<div class="rift-encounter type-${String(active.type).toLowerCase()}"><div class="rift-encounter-visual">${active.image?`<img src="${escapeHtml(active.image)}" alt="">`:'<span>♛</span>'}<div><small>${riftTypeMeta(active.type).label}</small><h3>${escapeHtml(active.name)}</h3><p>요구 전투력 ${Number(active.battlePower||0).toLocaleString()}</p></div></div><div class="rift-power-match"><span><small>현재 생존 전투력</small><b>${Number(run.cards.reduce((s,c)=>s+Number(c.power||0)*Number(c.hp||0)/100,0)).toLocaleString()}</b></span><i>VS</i><span><small>균열 전투력</small><b>${Number(active.battlePower||0).toLocaleString()}</b></span></div><button type="button" class="btn rift-fight-button" id="riftFight">⚔ 균열 전투 시작</button></div>`:`<div class="rift-choice-section"><div class="rift-section-head"><div><p class="eyebrow">CHOOSE YOUR PATH</p><h3>${Number(run.stage||0)+1}구역 경로 선택</h3><small>선택한 경로는 되돌릴 수 없습니다.</small></div></div><div class="rift-choice-grid">${(run.currentChoices||[]).map(riftChoiceHtml).join('')||'<div class="rift-empty">다음 경로를 생성하는 중입니다.</div>'}</div></div>`}</main><aside class="rift-side-panel"><div class="rift-section-head"><div><p class="eyebrow">EXPEDITION PARTY</p><h3>원정대 생존 현황</h3></div></div><div class="rift-deck-status">${(run.cards||[]).map(riftDeckCardHtml).join('')}</div><div class="rift-side-divider"></div><div class="rift-section-head compact"><div><p class="eyebrow">ACTIVE BUFFS</p><h3>원정 강화</h3></div><b>${buffs.length}</b></div><div class="rift-buff-list">${buffs.map(riftBuffChipHtml).join('')||'<span class="rift-no-buff">아직 획득한 강화가 없습니다.</span>'}</div>${riftRewardHtml(run.stash)}</aside></section>`;root.querySelectorAll('[data-rift-node]').forEach(b=>b.onclick=()=>selectRiftNode(b.dataset.riftNode,b));root.querySelectorAll('[data-rift-buff]').forEach(b=>b.onclick=()=>chooseRiftBuff(b.dataset.riftBuff,b));root.querySelector('#riftFight')?.addEventListener('click',fightRift);root.querySelector('#riftClaim')?.addEventListener('click',claimRiftReward);root.querySelector('#riftAbandon')?.addEventListener('click',abandonRift);root.querySelector('#riftNewRun')?.addEventListener('click',async()=>{await abandonRift(true);loadRiftView()})}
...
function riftBattleModal(run,node){const modal=document.getElementById('modal'),alive=(run.cards||[]).filter(c=>Number(c.hp||0)>0);modal.className='modal show';modal.innerHTML=`<div class="modal-panel battle-stage rift-battle-modal intro"><div class="battle-backdrop"></div><div class="battle-fx-layer"></div><div class="rift-modal-head"><span>DIMENSIONAL RIFT</span><b id="riftBattlePhase">ENCOUNTER</b></div><div class="rift-modal-arena"><div class="rift-modal-team">${alive.map(c=>{const card=riftCardModel(c);return `<div>${cardHtml(card,true,'small rift-modal-card',loadUser())}<i style="--hp:${Number(c.hp||0)}%"></i></div>`}).join('')}</div><strong>VS</strong><div class="rift-modal-enemy">${node.image?`<img src="${escapeHtml(node.image)}" alt="">`:'<span>♛</span>'}<b>${escapeHtml(node.name)}</b><small>${Number(node.battlePower||0).toLocaleString()}</small></div></div><div id="riftBattleMessage" class="rift-modal-message"><i></i><b>균열 전투 계산 중...</b></div></div>`;return modal}
async function fightRift(){const run=riftState.data.run,node=run.activeNode;if(!node)return;const button=document.getElementById('riftFight');if(button)button.disabled=true;const modal=riftBattleModal(run,node),phase=modal.querySelector('#riftBattlePhase'),message=modal.querySelector('#riftBattleMessage');await battleSleep(500);phase.textContent='BATTLE';try{const d=await apiRequest('rift/fight',{method:'POST',body:JSON.stringify({runId:run.runId,requestId:riftRequestId('rift-fight')})});modal.querySelector('.rift-battle-modal')?.classList.add(d.result==='WIN'?'victory':'defeat');phase.textContent=d.result==='WIN'?'VICTORY':'DEFEAT';message.innerHTML=`<b>${d.result==='WIN'?'균열 전투 승리':'균열 전투 패배'}</b><span>내 전투력 ${Number(d.playerPower||0).toLocaleString()} · 적 전투력 ${Number(d.monsterPower||0).toLocaleString()} · 체력 피해 ${Number(d.damage||0)}</span>${d.revivedCardId?'<em>두 번째 숨결이 발동했습니다.</em>':''}<button type="button" id="riftBattleContinue">계속하기</button>`;riftState.data.run=d.run;modal.querySelector('#riftBattleContinue').onclick=()=>{modal.className='modal';modal.innerHTML='';renderRiftView(riftState.data)}}catch(e){modal.className='modal';modal.innerHTML='';alert(e.message);if(button)button.disabled=false}}
async function chooseRiftBuff(buffKey,button){if(button)button.disabled=true;try{const d=await apiRequest('rift/buff',{method:'POST',body:JSON.stringify({runId:riftState.data.run.runId,buffKey,requestId:riftRequestId('rift-buff')})});riftState.data.run=d.run;renderRiftView(riftState.data)}catch(e){alert(e.message);if(button)button.disabled=false}}
async function claimRiftReward(){const button=document.getElementById('riftClaim');if(button)button.disabled=true;try{const d=await apiRequest('rift/claim',{method:'POST',body:JSON.stringify({runId:riftState.data.run.runId,requestId:riftRequestId('rift-claim')})});if(d.user)saveUser(apiUserToLocal(d.user));alert(`${d.message}\n코인 ${Number(d.reward?.coin||0).toLocaleString()} · 카드 조각 ${Number(d.reward?.shards||0).toLocaleString()} · 균열 결정 ${Number(d.reward?.crystals||0).toLocaleString()}`);await loadRiftView()}catch(e){alert(e.message);if(button)button.disabled=false}}
async function abandonRift(silent=false){const run=riftState.data?.run;if(!run)return;if(!silent&&!confirm('현재 원정을 포기할까요?\n누적한 임시 보상과 원정 강화가 모두 사라집니다.'))return;try{await apiRequest('rift/abandon',{method:'POST',body:JSON.stringify({runId:run.runId})});riftState.data.run=null;if(!silent)renderRiftView(riftState.data)}catch(e){if(!silent)alert(e.message)}}
...
function renderPvpDeckCardList(list,user=loadUser()){
  ...
  const filter=loadPvpDeckFilter(),deckSet=new Set(pvpState.deck),rows=filterDeckCards(list,filter,user,pvpState.config||battleState.config),groups=pveDeckGradeGroups(rows);
...
async function fightPvp(id){
  ...
  const stage=modal.querySelector('.battle-stage'),phase=document.getElementById('battlePhase'),count=document.getElementById('battleCountdown'),msg=document.getElementById('battleMessage');ensureBattleSoundButton(stage);
  try{
    battleTone(90,.18,'sawtooth',.035);await battleSleep(450);stage.classList.add('cards-enter');phase.textContent='MY TEAM DEPLOY';await battleSleep(700);
    ...
    count.textContent='READY';await battleSleep(650);count.textContent='FIGHT';stage.classList.add('fight');battleTone(440,.18,'square',.075);
    ...
    await battleSleep(950);stage.classList.add(myWin?'battle-win-v863':'battle-lose-v863');phase.textContent=myWin?'PVP VICTORY':'PVP DEFEAT';battleSfx(myWin?'victory':'defeat');
    if(d.cubeReward&&window.showCubeDropAcquisition){try{await window.showCubeDropAcquisition(d.cubeReward)}catch(cubeFxError){console.warn('큐브 획득 연출을 표시하지 못했습니다.',cubeFxError)}}
    ...
    const exitPvpBattle=()=>{modal.onclick=null;modal.className='modal';modal.innerHTML='';pvpState.tab='match';renderShell('pvp')};setTimeout(()=>{modal.onclick=()=>exitPvpBattle();const confirmBtn=document.getElementById('pvpResultConfirm');if(confirmBtn)confirmBtn.onclick=e=>{e.stopPropagation();exitPvpBattle()}},250);
  }catch(e){
