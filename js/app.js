...
async function openPrimeDrawPack(kind,ownedQuantity=null,suppliedConfig=null,autoOpenCount=0){
  ...
  const owned=Math.max(0,Number(ownedQuantity??config.balance??config.ticketQuantity)||0),limit=Math.max(1,Math.min(500,Number(config.maxOpen||500),owned||1)),initial=Math.max(1,Math.min(limit,Number(autoOpenCount)||Math.min(10,limit))),modal=document.getElementById('modal');
  modal.className='modal show supply-open-modal prime-draw-open-modal';
  const options=[1,10,50,limit].filter((value,index,array)=>value<=limit&&array.indexOf(value)===index);
  modal.innerHTML=`<div class="modal-panel supply-open-panel prime-draw-open-panel"><button type="button" class="icon-close" id="primeDrawClose">×</button><div class="supply-open-head"><small>${definition.eyebrow}</small><h2>${escapeHtml(definition.name)} 개봉</h2><p>한 번의 잠금 해제로 최대 ${limit.toLocaleString()}개를 서버에서 일괄 판정합니다.</p></div><div class="supply-open-stage"><span></span><img src="${escapeHtml(config.image||definition.image)}?v=1985-prime-live" alt="${escapeHtml(definition.name)}"></div><div class="supply-open-count"><button type="button" data-prime-count-step="-1">−</button><strong id="primeDrawCount">${initial}</strong><button type="button" data-prime-count-step="1">＋</button><small>보유 ${owned.toLocaleString()}개 · 요청 상한 ${Number(config.maxOpen||500).toLocaleString()}개</small></div><div class="supply-open-quick">${options.map(value=>`<button type="button" data-prime-open-count="${value}" class="${value===initial?'active':''}">${value===limit&&![1,10,50].includes(value)?'최대 ':''}${value.toLocaleString()}개</button>`).join('')}</div><button type="button" class="btn supply-open-confirm" id="primeDrawConfirm" ${owned>0?'':'disabled'}>${owned>0?`${initial}개 밀어서 잠금 해제`:'보유 수량 없음'}</button><small class="supply-open-guide">${kind==='equipment'?'장비 1개 확정':'이동수단 1개 확정'} · 프라임 독립 풀 ${Number(config.pool?.entryCount||0).toLocaleString()}종 · ${escapeHtml(String(config.poolVersion||''))}</small></div>`;
  let selected=initial;
  const close=()=>{try{window.PrimeDrawLiveV1985?.destroy?.()}catch(_){}modal.className='modal';modal.innerHTML=''};
  ...
  document.getElementById('primeDrawConfirm').onclick=async()=>{
    const button=document.getElementById('primeDrawConfirm'),panel=modal.querySelector('.prime-draw-open-panel'),requestId=globalThis.crypto?.randomUUID?.()||`${Date.now()}-${Math.random().toString(36).slice(2)}`;
    button.disabled=true;button.textContent='서버에서 보상 확정 중';panel.classList.add('opening');
    try{
      const result=await apiRequest(definition.openPath,{method:'POST',body:JSON.stringify({count:selected,quantity:selected,itemCode:definition.itemCode,poolVersion:config.poolVersion,requestId})});
      if(kind==='vehicle'){const user=loadUser();user.cardShards=Number(result.cardShards??user.cardShards);saveUser(user)}
      clearApiCache('inventory');clearApiCache('shell/summary');clearApiCache(definition.configPath);
    ...
    }catch(error){panel.classList.remove('opening');button.disabled=false;button.textContent=`${selected.toLocaleString()}개 밀어서 잠금 해제`;showSupplyNotice(error.message||`${definition.name} 개봉에 실패했습니다.`,true)}
  };
