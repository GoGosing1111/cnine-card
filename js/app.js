...
function tierEmblem(tier,size='normal'){const id=escapeHtml(tier?.id||'bronze'),name=escapeHtml(tier?.name||'브론즈'),color=escapeHtml(tier?.color||'#b87333');return `<div class="tier-emblem tier-${id} ${tier?.aura?'has-aura':''} ${size}" style="--tier-color:${color}"><span class="tier-wing left"></span><span class="tier-core"><i></i><b>✦</b></span><span class="tier-wing right"></span><em>${name}</em></div>`}
