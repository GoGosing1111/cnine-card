...
function riftRewardHtml(stash={}){return `<div class="rift-stash"><article><i>◈</i><span><small>누적 코인</small><b>${Number(stash.coin||0).toLocaleString()}</b></span></article><article><i>◆</i><span><small>카드 조각</small><b>${Number(stash.shards||0).toLocaleString()}</b></span></article><article><i>✦</i><span><small>마법 결정</small><b>${Number(stash.crystals||0).toLocaleString()}</b></span></article></div>`}
...
async function claimRiftReward(){const button=document.getElementById('riftClaim');if(button)button.disabled=true;try{const d=await apiRequest('rift/claim',{method:'POST',body:JSON.stringify({runId:riftState.data.run.runId,requestId:riftRequestId('rift-claim')})});if(d.user)saveUser(apiUserToLocal(d.user));alert(`${d.message}\n코인 ${Number(d.reward?.coin||0).toLocaleString()} · 카드 조각 ${Number(d.reward?.shards||0).toLocaleString()} · 마법 결정 ${Number(d.reward?.magicCrystals??d.reward?.crystals??0).toLocaleString()}`);await loadRiftView()}catch(e){alert(e.message);if(button)button.disabled=false}}
