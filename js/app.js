...
function battleView(user){
  ...
  return `${summaryBar(user)}${ownerRaid}<div id="pveHuntView" class="pve-hunt-redesign"><nav class="mobile-pve-tabs" aria-label="모바일 PVE 메뉴"><button type="button" data-pve-tab="deck">덱 요약</button><button type="button" data-pve-tab="cards">덱 편성실</button><button type="button" data-pve-tab="monsters">토벌 목표</button></nav><section class="battle-cover pve-hunt-cover"><div><p class="eyebrow">CNINE PVE HUNT</p><h2>몬스터 토벌 지휘실</h2><p>덱 편성과 토벌 목표를 분리해 한눈에 관리합니다. 덱을 먼저 완성하고, 우측에서 목표를 선택한 뒤 바로 출전하세요.</p></div><div class="battle-cover-side"><div class="battle-energy-card"><div><span>⚔ 전투 횟수</span><b id="battleEnergyCount">- / -</b></div><div class="battle-energy-track"><i id="battleEnergyFill"></i></div><small id="battleEnergyTimer">불러오는 중...</small></div><div class="battle-total pve-total-card"><span>현재 출전 전투력</span><b id="battleDeckPower">0</b><small>저장된 5장 기준</small></div></div></section><section class="pve-command-layout"><div class="pve-command-left"><section class="battle-panel pve-squad-panel pve-mobile-pane" data-pve-pane="deck"><div class="panel-title pve-panel-title"><div><p class="eyebrow">SQUAD LOADOUT</p><h2>PvE 출전 덱</h2><small>출전 카드 5장을 먼저 확인하고 저장하세요.</small></div><div class="pve-deck-actions"><button type="button" class="pve-deck-btn save" id="saveBattleDeck"><span>💾</span> 덱 저장</button><button type="button" class="pve-deck-btn reset" id="clearBattleDeck"><span>↺</span> 덱 리셋</button></div></div><div id="battleDeck" class="battle-deck pve-squad-grid"></div><button type="button" class="pve-mobile-next" data-pve-tab="cards">덱 편성실 열기</button></section><section class="battle-panel pve-builder-panel pve-mobile-pane" data-pve-pane="cards"><div class="pve-mobile-pane-title pve-panel-title"><div><p class="eyebrow">DECK BUILDER</p><h2>덱 편성실</h2><small>보유 카드 중에서 출전 카드 5장을 선택하세요.</small></div><button type="button" data-pve-tab="deck">덱 요약 보기</button></div><div class="pve-builder-helper"><span>카드를 누르면 덱에 추가됩니다.</span><b>중복 선택 불가 · 저장 전까지 자유 편집</b></div><div id="battleCards" class="battle-card-picker pve-builder-list"><div class="empty-recent">전투 정보를 불러오는 중...</div></div></section></div><aside class="battle-panel pve-target-panel pve-mobile-pane" data-pve-pane="monsters"><div class="panel-title pve-panel-title"><div><p class="eyebrow">HUNT TARGET</p><h2>토벌 목표 설정</h2><small>목표 몬스터를 선택하고 전투를 시작하세요.</small></div></div><div id="battleMonsters" class="battle-monsters"></div></aside></section><nav class="mobile-pve-quickbar" aria-label="PVE 빠른 이동"><button type="button" data-pve-tab="deck"><span>🃏</span>덱</button><button type="button" data-pve-tab="cards"><span>🛠</span>편성실</button><button type="button" data-pve-tab="monsters"><span>👹</span>토벌</button></nav></div><div id="pveRaidView" class="pve-raid-view" hidden></div>`;
}
...
function renderBattleBuilder(){const activeMobileTab=getMobilePveTab()||((battleState.deck?.length===5)?'monsters':'deck');const user=loadUser(),owned=cards.filter(c=>ownedIds(user).has(c.id)).sort((a,b)=>battleCardPower(b,user,battleState.config)-battleCardPower(a,user,battleState.config));const deckSet=new Set(battleState.deck),power=battleState.deck.reduce((n,id)=>{const c=cards.find(x=>x.id===id);return n+(c?battleCardPower(c,user,battleState.config):0)},0);document.getElementById('battleDeckPower').textContent=power.toLocaleString();document.getElementById('battleDeck').innerHTML=Array.from({length:5},(_,index)=>{const id=battleState.deck[index],c=cards.find(x=>x.id===id);if(!c)return `<div class="pve-deck-slot-empty"><span>${index+1}</span><b>빈 슬롯</b><small>아래 편성실에서 카드를 추가하세요</small></div>`;return `<button class="pve-deck-slot-card" data-remove="${c.id}"><div class="pve-deck-slot-index">${index+1}</div><div class="pve-deck-slot-body">${deckBuilderListCardHtml(c,user,battleCardPower(c,user,battleState.config),'pve-slot')}</div></button>`}).join('');document.getElementById('battleCards').innerHTML=owned.map(c=>`<button class="pve-builder-row ${deckSet.has(c.id)?'selected':''}" data-pick="${c.id}" ${deckSet.has(c.id)?'disabled':''}><div class="pve-builder-status">${deckSet.has(c.id)?'<b>편성됨</b>':'<span>선택 가능</span>'}</div>${deckBuilderListCardHtml(c,user,battleCardPower(c,user,battleState.config),'pve-pick')}</button>`).join('')||'<div class="empty-recent">보유 카드가 없습니다.</div>';renderPveMonsterBrowser();document.querySelectorAll('[data-pick]').forEach(b=>b.onclick=()=>{if(battleState.deck.length<5){battleState.deck.push(b.dataset.pick);renderBattleBuilder()}});document.querySelectorAll('[data-remove]').forEach(b=>b.onclick=()=>{battleState.deck=battleState.deck.filter(x=>x!==b.dataset.remove);renderBattleBuilder()});const saveDeck=document.getElementById('saveBattleDeck');if(saveDeck){saveDeck.disabled=battleState.deck.length!==5;saveDeck.onclick=saveBattleDeck;}const clearDeck=document.getElementById('clearBattleDeck');if(clearDeck)clearDeck.onclick=resetBattleDeck;const start=document.getElementById('battleStart');const noEnergy=battleState.energy&&!battleState.energy.unlimited&&battleState.energy.energy<battleState.energy.costPerBattle;if(start){start.disabled=battleState.deck.length!==5||!battleState.selectedMonster||noEnergy;start.textContent=noEnergy?'전투 횟수 부족':'전투 시작';start.onclick=startBattle;}renderBattleEnergy();bindMobilePveTabs();setMobilePveTab(activeMobileTab,{scroll:false});if(battleState.restoreMonsterCursor){battleState.restoreMonsterCursor=false;requestAnimationFrame(()=>{const selected=document.querySelector(`#battleMonsters [data-monster="${battleState.selectedMonster}"]`);if(selected&&(!isMobilePve()||activeMobileTab==='monsters')){selected.focus({preventScroll:true});selected.scrollIntoView({behavior:'smooth',block:'center',inline:'nearest'});}});}}
