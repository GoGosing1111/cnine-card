...
else if(pvpState.tab==='deck'){const owned=ownedIds(loadUser()),list=cards.filter(c=>owned.has(c.id));box.innerHTML=`<div class="pvp-section-head"><div><p class="eyebrow">MY PVP DECK</p><h2>PvP 덱 편성</h2></div><span>5장 선택 · 언제든 교체 가능</span></div><div id="pvpDeckSlots" class="pvp-deck-slots"></div><div class="pvp-card-picker">${list.map(c=>`<button class="pvp-pick ${pvpState.deck.includes(c.id)?'selected':''}" data-cid="${c.id}">${pvpCardMini(c,loadUser())}</button>`).join('')}</div><button class="btn" id="savePvpDeck">PvP 덱 저장</button>`;renderPvpDeckSlots();document.querySelectorAll('.pvp-pick').forEach(b=>b.onclick=()=>{const id=b.dataset.cid,i=pvpState.deck.indexOf(id);if(i>=0)pvpState.deck.splice(i,1);else if(pvpState.deck.length<5)pvpState.deck.push(id);else return alert('PvP 덱은 5장까지 편성할 수 있습니다.');renderPvpTab()});document.getElementById('savePvpDeck').onclick=savePvpDeck;}
...
function renderPvpDeckSlots(){const el=document.getElementById('pvpDeckSlots');if(!el)return;el.innerHTML=Array.from({length:5},(_,i)=>{const c=cards.find(x=>x.id===pvpState.deck[i]);return `<div class="pvp-deck-slot ${c?'filled':'empty'}">${c?pvpCardMini(c,loadUser()):`<div class="pvp-empty-slot"><span>${i+1}</span></div>`}</div>`}).join('')}
