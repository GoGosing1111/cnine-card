...
function renderMagicSystem(){
  ...
  root.querySelector('[data-magic-deck="CAPTAIN"]')?.remove();root.querySelectorAll('.magic-card-tile').forEach((tile,index)=>{const card=cards[index],label=tile.querySelector('small');if(card&&label)label.textContent=`${magicRenewedEffectLabel(card.effectType)} · ${Number(card.triggerChance)}% · 최대 ${Number(card.maxActivations)}회`;});
