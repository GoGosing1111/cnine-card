...
function showMagicDrawResults(data){return new Promise(resolve=>{const rows=Array.isArray(data.results)&&data.results.length?data.results:[{card:data.card,duplicate:data.duplicate,refund:data.refund}],overlay=document.createElement('div');overlay.className=`magic-draw-reveal count-${rows.length}`;overlay.innerHTML=`<div class="magic-draw-reveal-backdrop"></div><section><header><small>ARCANE SUMMON RESULT</small><h2>마법카드 ${rows.length}장 소환</h2><p>신규 ${Number(data.newCount??rows.filter(row=>!row.duplicate).length)}장 · 강화 재료 ${Number(data.duplicateCount??rows.filter(row=>row.duplicate).length)}장 · 사용 🪙 ${Number(data.totalCoinCost||0).toLocaleString()} / ✦ ${Number(data.totalCost||0).toLocaleString()}</p></header><div class="magic-draw-reveal-grid">${rows.map((row,index)=>`<article class="rarity-${escapeHtml(row.card?.rarity||'R')} ${row.duplicate?'duplicate':'new'}" style="--reveal-index:${index}">${magicImage(row.card||{})}<span>${row.duplicate?'강화 재료 +1':'NEW'}</span><b>${escapeHtml(row.card?.name||'마법카드')}</b><small>${escapeHtml(row.card?.rarity||'')} ${row.duplicate&&Number(row.refund||0)>0?`· 결정 환급 ✦ ${Number(row.refund||0).toLocaleString()}`:''}</small></article>`).join('')}</div><button type="button">확인</button></section>`;const close=()=>{overlay.classList.add('closing');setTimeout(()=>{overlay.remove();resolve()},180)};overlay.querySelector('button').onclick=close;overlay.querySelector('.magic-draw-reveal-backdrop').onclick=close;document.body.appendChild(overlay);requestAnimationFrame(()=>overlay.classList.add('show'))})}
...
function inventoryItemMarkup(item){
  const owned=Number(item.quantity)>0,kind=String(item.rarity||'normal').toLowerCase(),isCube=item.category==='CUBE',isSupply=item.category==='SUPPLY_BOX'||item.code==='EQUIPMENT_SUPPLY_BOX',isMasterStar=item.code==='MASTER_STAR',isMagicPack=item.code==='MAGIC_CARD_PACK',isReroll=item.category==='REROLL';
