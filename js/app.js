...
function showDetail(id,initialTab='auto') {
  ...
  const level=Number(user.breakthroughs?.[normalizedId]||0),normalizedGrade=String(card.grade||'').trim().toUpperCase(),canBreak=owned&&(gradeOrder[normalizedGrade]||0)>=gradeOrder[breakthroughMinGrade],isMa=normalizedGrade==='MA',isLimited=normalizedGrade==='LIMITED',maxLevel=(isMa||isLimited)?13:10,isMaHigh=(isMa||isLimited)&&level>=10&&level<13,standardRule=user.breakthroughConfig?.[normalizedGrade]?.[level]||{cost:breakthroughCosts[level],rate:breakthroughRates[level]},highConfig=isLimited?(user.limitedHighBreakthrough||LIMITED_HIGH_ENHANCEMENT_FALLBACK):(user.maHighBreakthrough||MA_HIGH_ENHANCEMENT_FALLBACK),highRule=highConfig?.steps?.[level-10],rule=isMaHigh?highRule:standardRule,cost=level<maxLevel?Number(rule?.cost||0):null,successRate=level<maxLevel?Number(rule?.rate||0):null,materialBalance=isMaHigh?Number(user.masterStars||0):Number(user.cardShards||0),materialName=isMaHigh?'마스터의 별':'카드 조각',highEnabled=highConfig?.enabled===true;
...
async function breakthroughCard(cardId){
  ...
  const level=Number(user.breakthroughs?.[normalizedCardId]||0),normalizedGrade=String(card?.grade||'').trim().toUpperCase(),isMa=normalizedGrade==='MA',isLimited=normalizedGrade==='LIMITED',maxLevel=(isMa||isLimited)?13:10,isMaHigh=(isMa||isLimited)&&level>=10;
  if(level>=maxLevel)return alert('이미 최대 강화 단계입니다.');
  const highConfig=isLimited?(user.limitedHighBreakthrough||LIMITED_HIGH_ENHANCEMENT_FALLBACK):(user.maHighBreakthrough||MA_HIGH_ENHANCEMENT_FALLBACK),rule=isMaHigh?highConfig?.steps?.[level-10]:(user.breakthroughConfig?.[normalizedGrade]?.[level]||{cost:breakthroughCosts[level],rate:breakthroughRates[level]}),materialName=isMaHigh?'마스터의 별':'카드 조각',balance=isMaHigh?Number(user.masterStars||0):Number(user.cardShards||0);
