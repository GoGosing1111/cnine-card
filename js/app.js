...
function renderRaidView(d){const box=document.getElementById('pveRaidView');if(!box)return;const c=d.current,s=d.settings||{};if(!c){box.innerHTML=`<section class="raid-empty"><p class="eyebrow">OWNER TEST RAID</p><h2>${escapeHtml(s.title||'월드 레이드')}</h2><p>현재 열린 레이드 대기방이 없습니다.<br>CMS → 레이드 관리에서 보스를 선택하고 대기방을 시작하세요.</p></section>`;return;}const joined=Boolean(d.me),remain=Math.max(0,Date.parse(c.startsAt)-Date.now()),sec=Math.ceil(remain/1000),hpPct=Math.max(0,Math.min(100,Number(c.currentHp)/Math.max(1,Number(c.maxHp))*100));const participants=d.participants||[];box.innerHTML=`<section class="raid-hero status-${String(c.status).toLowerCase()}"><div class="raid-boss-pane">${c.bossImage?`<img src="${c.bossImage}" alt="">`:'<div class="raid-boss-placeholder">👹</div>'}<div><p class="eyebrow">OWNER TEST RAID</p><h2>${escapeHtml(c.bossName)}</h2><div class="raid-hp-head"><span>BOSS HP</span><b>${Number(c.currentHp).toLocaleString()} / ${Number(c.maxHp).toLocaleString()}</b></div><div class="raid-hp-track"><i style="width:${hpPct}%"></i></div><strong class="raid-status">${c.status==='LOBBY'?`신청 대기 · ${Math.max(0,sec)}초 후 시작`:c.status==='BATTLE'?'전투 진행 중':'전투 종료'}</strong></div></div><div class="raid-join-card"><span>참가 인원</span><b>${c.participantCount} / ${s.maxParticipants}</b><div class="raid-people-bar"><i style="width:${Math.min(100,c.participantCount/Math.max(1,s.maxParticipants)*100)}%"></i></div><button class="btn" id="raidJoin" ${joined||c.status!=='LOBBY'?'disabled':''}>${joined?'신청 완료':c.status==='LOBBY'?'레이드 신청':'신청 마감'}</button><small>저장된 PvE 덱 5장으로 참가합니다.</small></div></section><section class="raid-live-grid"><div class="raid-party"><div class="panel-title"><div><p class="eyebrow">RAID PARTY</p><h2>참가자 현황</h2></div></div><div class="raid-participant-list">${participants.length?participants.map(x=>`<article><div class="raid-mini-cards">${(x.cards||[]).slice(0,5).map(card=>`<img src="${card.image}" title="${escapeHtml(card.title)}">`).join('')}</div><span><b>${escapeHtml(x.nickname)}</b><small>전투력 ${Number(x.totalPower).toLocaleString()}</small></span><strong>${c.status==='BATTLE'?Number(x.shownDamage||0).toLocaleString()+' DMG':'READY'}</strong></article>`).join(''):'<div class="empty-recent">첫 참가자를 기다리고 있습니다.</div>'}</div></div><aside class="raid-ranking"><div class="panel-title"><div><p class="eyebrow">LIVE DAMAGE</p><h2>딜 순위</h2></div></div>${participants.length?participants.slice(0,Number(s.rankingSize||10)).map((x,i)=>`<div class="raid-rank-row"><b>${i+1}</b><span>${escapeHtml(x.nickname)}</span><strong>${Number(x.shownDamage||0).toLocaleString()}</strong></div>`).join(''):'<div class="empty-recent">전투 시작 후 표시됩니다.</div>'}<div class="raid-my-damage"><span>내 누적 딜</span><b>${Number((participants.find(x=>Number(x.userId)===Number(loadUser()?.serverUserId))?.shownDamage)||0).toLocaleString()}</b></div></aside></section>`;const join=document.getElementById('raidJoin');if(join&&!join.disabled)join.onclick=joinRaid;}
