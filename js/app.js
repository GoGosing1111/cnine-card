...
function battleTriggerUniqueFx(stage,index,event='attack',enemy=false){
  ...
  fighter.classList.remove('unique-fx-active',`unique-fx-${event}-active`);void fighter.offsetWidth;fighter.classList.add('unique-fx-active',`unique-fx-${event}-active`);
  clearTimeout(fighter._uniqueFxTimer);fighter._uniqueFxTimer=setTimeout(()=>fighter.classList.remove('unique-fx-active',`unique-fx-${event}-active`),type==='hp'?1450:1050);
  return true;
}
...
function uniqueAbilityBadgeHtml(card,classes=''){
  ...
  const top=uniqueAbilityDominant(card);
...
function uniqueAbilityInlineHtml(card,classes=''){
  const x=card?.uniqueAbility;if(!x)return '';
  const dominant=uniqueAbilityDominant(card),stats=uniqueAbilityStats(card).filter(stat=>stat.value!==0).sort((a,b)=>Number(b.value)-Number(a.value)).slice(0,2);
  return `<span class="card-ability-inline ${classes} ${x.ownerTest?'owner-test':''}" data-card-profile="${escapeHtml(String(card.id))}" role="button" tabindex="0"><i>◇</i><b>${x.ownerTest?'TEST':'고유 능력'}</b>${stats.map(stat=>`<em class="${dominant?.key===stat.key?'dominant':''}">${stat.icon} ${stat.value>0?'+':''}${stat.value}%</em>`).join('')}<u>상세</u></span>`;
}
function uniqueAbilityDetailHtml(card){
  ...
  const dominant=uniqueAbilityDominant(card),stat=({key,label,icon,value})=>`<article class="unique-stat unique-${key} ${dominant?.key===key?'dominant':''}"><i>${icon}</i><span>${label}${dominant?.key===key?'<em>대표 발동</em>':''}</span><b>${value>0?'+':''}${value}%</b></article>`;
  return `<section class="card-unique-ability ${x.ownerTest?'owner-test':''}"><div class="unique-ability-head"><div><small>${x.ownerTest?'OWNER TEST · ':''}UNIQUE ABILITY</small><h3>${escapeHtml(x.effectName||'카드 고유 능력')}</h3></div><em>${x.ownerTest?'TEST MODE':'ACTIVE'}</em></div>${x.ownerTest?'<div class="unique-owner-notice"><b>OWNER 테스트 중</b><span>일반 유저에게는 아직 공개·적용되지 않는 능력치입니다.</span></div>':''}<div class="unique-ability-stats">${uniqueAbilityStats(card).map(stat).join('')}</div>${x.effectDescription?`<div class="unique-effect-copy"><small>ABILITY DESCRIPTION</small><p>${escapeHtml(x.effectDescription)}</p></div>`:'<div class="unique-effect-copy muted"><small>ABILITY DESCRIPTION</small><p>등록된 고유 효과 설명이 없습니다.</p></div>'}<div class="unique-scope"><small>적용 콘텐츠</small><b>${escapeHtml(uniqueAbilityScopeText(x))}</b></div><footer><span>공격·방어·속도·HP 수치는 모두 전투 계산에 반영됩니다.</span><b>${dominant?`${dominant.label} ${dominant.value>0?'+':''}${dominant.value}% 대표 이펙트 발동`:'대표 발동 이펙트 없음'}</b></footer></section>`;
}
