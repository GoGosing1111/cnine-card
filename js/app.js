...
function powerTypeStars(card){const type=String(card?.powerType||card?.power_type||'').toUpperCase();return type==='NORMAL'?'⭐':type==='HIGH'?'⭐⭐':type==='TOP'?'⭐⭐⭐':'';}
function powerTypeStarsHtml(card,classes=''){const stars=powerTypeStars(card);return stars?`<i class="power-type-stars ${classes}" aria-label="전투력 유형">${stars}</i>`:'';}
...
function recentCards(user) {
  ...
  return `<div class="recent-grid">${items.map(item => { const c=cards.find(x=>x.id===item.cardId); if(!c)return''; return `<button class="recent-item" data-card-id="${c.id}"><img src="${c.image}" style="object-position:${c.focusX}% ${c.focusY}%"><span><b>${escapeHtml(c.title)}</b><small>${c.grade}${powerTypeStars(c)?` · ${powerTypeStars(c)}`:''}${item.duplicate?' · 보유중':' · NEW'}</small></span></button>`; }).join('')}</div>`;
}
...
function getNextRaidOpenAt(settings={},schedule={},serverNow){
  if(schedule.nextOpenAt&&Number.isFinite(Date.parse(schedule.nextOpenAt)))return schedule.nextOpenAt;
  if(String(settings.scheduleMode||'').toUpperCase()!=='SCHEDULED')return null;
  const openDays=(Array.isArray(settings.openDays)?settings.openDays:[]).map(Number).filter(day=>Number.isInteger(day)&&day>=0&&day<=6);
  const openTime=/^([01]\d|2[0-3]):[0-5]\d$/.test(String(settings.openTime||''))?String(settings.openTime):null;
  if(!openDays.length||!openTime)return null;
  const nowMs=Number.isFinite(Date.parse(serverNow))?Date.parse(serverNow):Date.now();
  const kstNow=new Date(nowMs+9*3600000);
  for(let add=0;add<8;add++){
    const date=new Date(kstNow.getTime()+add*86400000);
    if(!openDays.includes(date.getUTCDay()))continue;
    const ymd=[date.getUTCFullYear(),String(date.getUTCMonth()+1).padStart(2,'0'),String(date.getUTCDate()).padStart(2,'0')].join('-');
    const candidate=Date.parse(`${ymd}T${openTime}:00+09:00`);
...
function formatRaidOpenAt(value){
  ...
  return new Intl.DateTimeFormat('ko-KR',{timeZone:'Asia/Seoul',month:'long',day:'numeric',weekday:'short',hour:'numeric',minute:'2-digit'}).format(new Date(time));
}
...
function renderRaidView(d){
  ...
  if(!c){const bosses=d.availableBosses||[],used=Boolean(d.dailyEntryUsed),nextOpenAt=!schedule.canEnter?getNextRaidOpenAt(s,schedule,d.serverNow):null,nextOpenText=nextOpenAt?formatRaidOpenAt(nextOpenAt):'',statusText=!schedule.canEnter?(schedule.reason==='ENTRY_CLOSED'?'오늘 레이드 입장이 마감되었습니다.':'현재는 레이드 개방 시간이 아닙니다.'):used?'오늘의 레이드 입장 횟수를 이미 사용했습니다.':'현재 개방 가능한 보스를 선택해 직접 레이드를 시작할 수 있습니다.',scheduleText=nextOpenText?`<div class="raid-schedule-notice">다음 개방 <b>${escapeHtml(nextOpenText)}</b> <small>(KST)</small></div>`:'';box.innerHTML=`<section class="raid-empty raid-empty-polished"><p class="eyebrow">USER OPEN RAID</p><h2>${escapeHtml(s.title||'월드 레이드')}</h2><p>${statusText}</p>${scheduleText}${bosses.length?`<div class="raid-open-boss-grid">${bosses.map(b=>`<article class="raid-open-boss">${b.image?`<img src="${escapeHtml(b.image)}" alt="">`:''}<h3>${escapeHtml(b.name)}</h3><p>개방 비용 <b>${Number(b.openCost||0).toLocaleString()} 코인</b></p><p>입장 기회는 성공·실패와 관계없이 하루 1회입니다.</p><button class="btn raidOpenBtn" data-boss-id="${b.id}" ${used||!schedule.canEnter?'disabled':''}>레이드 개방</button></article>`).join('')}</div>`:`<div class="raid-schedule-notice">현재 유저 개방이 허용된 보스가 없습니다.</div>`}</section>`;document.querySelectorAll('.raidOpenBtn').forEach(btn=>btn.onclick=()=>openRaid(Number(btn.dataset.bossId),btn));return;}
...
async function openRaid(bossId,btn){if(!confirm('이 레이드를 개방하면 코인이 차감되고 오늘의 입장 기회 1회를 사용합니다. 성공·실패와 관계없이 환불 및 재도전은 불가능합니다.\n\n레이드를 개방할까요?'))return;const requestId=globalThis.crypto?.randomUUID?.()||`${Date.now()}-${Math.random().toString(36).slice(2)}`;if(btn){btn.disabled=true;btn.textContent='개방 중...'}try{const d=await apiRequest('raid/open',{method:'POST',body:JSON.stringify({bossId,cardIds:battleState.deck,requestId})});alert(`레이드가 개방되었습니다.\n${Number(d.cost||0).toLocaleString()}코인 사용 · 자동 참가 완료`);await loadRaidView()}catch(e){alert(e.message);if(btn){btn.disabled=false;btn.textContent='레이드 개방'}}}
...
function pvpCardMini(c,user=loadUser()){const level=Number(user?.breakthroughs?.[c.id]||0),limited=c.limitedTotal!==null&&c.limitedTotal!==undefined;return `<div class="pvp-card-mini-full">${cardHtml(c,true,'pvp-card-display',user)}<div class="pvp-card-extra"><b>${escapeHtml(c.grade||c.rarity||'C')}</b><span>${limited?'한정판':'일반'}${powerTypeStars(c)?` · ${powerTypeStars(c)}`:''} · 돌파 ★${level}</span></div></div>`}
