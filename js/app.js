...
const RANKED_TIER_ASSET_IDS=new Set(['bronze','silver','gold','platinum','diamond','master','grandmaster']);
function rankedTierAsset(tier,className=''){const raw=String(tier?.id||'bronze').toLowerCase(),id=RANKED_TIER_ASSET_IDS.has(raw)?raw:'bronze',name=escapeHtml(tier?.name||'브론즈');return `<span class="ranked-tier-asset ${className}"><img src="assets/ui/tiers/${id}.png" alt="${name}" loading="lazy" decoding="async"></span>`}
...
async function startRankedMatch(){
  ...
  const button=document.getElementById('rankedMatchStart');pvpState.matching=true;if(button){button.disabled=true;rankedMatchButtonLabel(button,'균형 상대 탐색 중...')}
