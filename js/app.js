...
function syncWorldTouchControls(){
 const touchLike=!!(navigator.maxTouchPoints>0||('ontouchstart' in window)||window.matchMedia?.('(pointer: coarse)').matches||window.matchMedia?.('(hover: none)').matches||Math.min(window.innerWidth||9999,window.visualViewport?.width||9999)<=900);
...
function initWorldView(){syncWorldTouchControls();window.addEventListener('resize',syncWorldTouchControls);window.visualViewport?.addEventListener('resize',syncWorldTouchControls);const saved=JSON.parse(localStorage.getItem('cnine_world_v3')||'null');if(saved&&WORLD_MAPS[saved.map]){worldState.map=saved.map;worldState.x=saved.x;worldState.y=saved.y;worldState.dir=saved.dir||'down'}else{const m=worldMap();worldState.x=m.spawn.x;worldState.y=m.spawn.y}worldState.keys={};worldState.lastTime=0;worldState.busy=false;worldState.paused=false;const c=document.getElementById('worldMap');if(c){c.focus();c.onclick=()=>c.focus()}worldLoadAssets().then(()=>{drawWorld();worldState.raf=requestAnimationFrame(worldLoop)}).catch(()=>worldToast('월드 이미지 로딩에 실패했습니다.'));
 ...
 window.__stopWorld=()=>{cancelAnimationFrame(worldState.raf);saveWorld();window.removeEventListener('keydown',down);window.removeEventListener('keyup',up);window.removeEventListener('resize',syncWorldTouchControls);window.visualViewport?.removeEventListener('resize',syncWorldTouchControls);worldState.keys={}}
}
