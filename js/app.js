...
function renderPvpDeckCardList(list,user=loadUser()){
  ...
  root.querySelectorAll('[data-cid]').forEach(button=>button.onclick=async()=>{if(pvpState.deck.length>=5)return alert('랭크전 덱은 5장까지 편성할 수 있습니다.');pvpState.deck.push(button.dataset.cid);await rerenderPvpDeckPreserveScroll()});
}
