...
function battleView(user){
  ...
  return `${summaryBar(user)}${ownerRaid}<div id="pveHuntView" class="pve-hunt-redesign pve-hunt-v1179"><nav class="mobile-pve-tabs" aria-label="모바일 PVE 메뉴"><button type="button" data-pve-tab="deck">출전 덱</button><button type="button" data-pve-tab="cards">덱 편성실</button><button type="button" data-pve-tab="monsters">몬스터 토벌</button></nav><section class="battle-cover pve-hunt-cover"><div><p class="eyebrow">CNINE PVE HUNT</p><h2>몬스터 토벌 지휘실</h2><p>좌측에서 출전 덱을 완성하고, 중앙 편성실에서 카드를 고른 뒤, 우측 토벌 패널에서 목표를 정해 바로 출전하는 구조입니다.</p></div><div class="battle-cover-side"><div class="battle-energy-card"><div><span>⚔ 전투 횟수</span><b id="battleEnergyCount">- / -</b></div><div class="battle-energy-track"><i id="battleEnergyFill"></i></div><small id="battleEnergyTimer">불러오는 중...</small></div><div class="battle-total pve-total-card"><span>현재 출전 전투력</span><b id="battleDeckPower">0</b><small id="battleDeckStatusText">아직 5장 편성이 완료되지 않았습니다</small></div></div></section><section class="pve-command-layout pve-command-layout-v1179"><section class="battle-panel pve-squad-panel pve-mobile-pane" data-pve-pane="deck"><div class="panel-title pve-panel-title"><div><p class="eyebrow">PVE DEPLOY DECK</p><h2>PvE 출전 덱</h2><small>몬스터 토벌 앞 칸에서 바로 보는 전용 출전 덱입니다.</small></div><div class="pve-deck-actions"><button type="button" class="pve-deck-btn save" id="saveBattleDeck"><span>💾</span> 덱 저장</button><button type="button" class="pve-deck-btn reset" id="clearBattleDeck"><span>↺</span> 덱 리셋</button></div></div><div class="pve-squad-summary"><article><small>편성 수</small><b id="battleDeckCount">0 / 5</b></article><article><small>출전 상태</small><b id="battleDeckReady">편성 중</b></article></div><div id="battleDeck" class="battle-deck pve-squad-grid"></div><button type="button" class="pve-mobile-next" data-pve-tab="cards">덱 편성실 열기</button></section><section class="battle-panel pve-builder-panel pve-mobile-pane" data-pve-pane="cards"><div class="pve-mobile-pane-title pve-panel-title"><div><p class="eyebrow">PVE DECK BUILDER</p><h2>덱 편성실</h2><small>보유 카드에서 출전 카드 5장을 조합하세요.</small></div><button type="button" data-pve-tab="deck">출전 덱 보기</button></div><div class="pve-builder-helper"><span>카드를 누르면 출전 덱에 추가됩니다.</span><b>중복 선택 불가 · 선택된 카드는 자동 잠금</b></div><div id="battleCards" class="battle-card-picker pve-builder-list"><div class="empty-recent">전투 정보를 불러오는 중...</div></div></section><aside class="battle-panel pve-target-panel pve-mobile-pane" data-pve-pane="monsters"><div class="panel-title pve-panel-title"><div><p class="eyebrow">MONSTER HUNT</p><h2>몬스터 토벌</h2><small>공간을 넓혀 목표 몬스터와 토벌 리스트를 별도 구성했습니다.</small></div></div><div id="battleMonsters" class="battle-monsters"></div></aside></section><nav class="mobile-pve-quickbar" aria-label="PVE 빠른 이동"><button type="button" data-pve-tab="deck"><span>🃏</span>출전덱</button><button type="button" data-pve-tab="cards"><span>🛠</span>편성실</button><button type="button" data-pve-tab="monsters"><span>👹</span>토벌</button></nav></div><div id="pveRaidView" class="pve-raid-view" hidden></div>`;
}
...
function renderBattleBuilder(){const activeMobileTab=getMobilePveTab()||((battleState.deck?.length===5)?'monsters':'deck');const user=loadUser(),owned=cards.filter(c=>ownedIds(user).has(c.id)).sort((a,b)=>battleCardPower(b,user,battleState.config)-battleCardPower(a,user,battleState.config));const deckSet=new Set(battleState.deck),power=battleState.deck.reduce((n,id)=>{const c=cards.find(x=>x.id===id);return n+(c?battleCardPower(c,user,battleState.config):0)},0),isReady=battleState.deck.length===5;document.getElementById('battleDeckPower').textContent=power.toLocaleString();const deckCountEl=document.getElementById('battleDeckCount');if(deckCountEl)deckCountEl.textContent=`${battleState.deck.length} / 5`;const deckReadyEl=document.getElementById('battleDeckReady');if(deckReadyEl)deckReadyEl.textContent=isReady?'출전 준비 완료':'편성 중';const deckStatusText=document.getElementById('battleDeckStatusText');if(deckStatusText)deckStatusText.textContent=isReady?'5장 편성이 완료되었습니다. 몬스터 토벌을 시작할 수 있습니다.':'아직 5장 편성이 완료되지 않았습니다';document.getElementById('battleDeck').innerHTML=Array.from({length:5},(_,index)=>{const id=battleState.deck[index],c=cards.find(x=>x.id===id);if(!c)return `<div class="pve-deck-slot-empty"><span>${index+1}</span><b>빈 슬롯</b><small>우측이 아니라 중앙 편성실에서 카드를 추가하세요</small></div>`;return `<button class="pve-deck-slot-card" data-remove="${c.id}"><div class="pve-deck-slot-index">${index+1}</div><div class="pve-deck-slot-body">${deckBuilderListCardHtml(c,user,battleCardPower(c,user,battleState.config),'pve-slot')}</div></button>`}).join('');document.getElementById('battleCards').innerHTML=owned.map(c=>`<button class="pve-builder-row ${deckSet.has(c.id)?'selected':''}" data-pick="${c.id}" ${deckSet.has(c.id)?'disabled':''}><div class="pve-builder-status">${deckSet.has(c.id)?'<b>편성됨</b>':'<span>선택 가능</span>'}</div>${deckBuilderListCardHtml(c,user,battleCardPower(c,user,battleState.config),'pve-pick')}</button>`).join('')||'<div class="empty-recent">보유 카드가 없습니다.</div>';renderPveMonsterBrowser();document.querySelectorAll('[data-pick]').forEach(b=>b.onclick=()=>{if(battleState.deck.length<5){battleState.deck.push(b.dataset.pick);renderBattleBuilder()}});document.querySelectorAll('[data-remove]').forEach(b=>b.onclick=()=>{battleState.deck=battleState.deck.filter(x=>x!==b.dataset.remove);renderBattleBuilder()});const saveDeck=document.getElementById('saveBattleDeck');if(saveDeck){saveDeck.disabled=!isReady;saveDeck.onclick=saveBattleDeck;}const clearDeck=document.getElementById('clearBattleDeck');if(clearDeck)clearDeck.onclick=resetBattleDeck;const start=document.getElementById('battleStart');const noEnergy=battleState.energy&&!battleState.energy.unlimited&&battleState.energy.energy<battleState.energy.costPerBattle;if(start){start.disabled=!isReady||!battleState.selectedMonster||noEnergy;start.textContent=noEnergy?'전투 횟수 부족':'전투 시작';start.onclick=startBattle;}renderBattleEnergy();bindMobilePveTabs();setMobilePveTab(activeMobileTab,{scroll:false});if(battleState.restoreMonsterCursor){battleState.restoreMonsterCursor=false;requestAnimationFrame(()=>{const selected=document.querySelector(`#battleMonsters [data-monster="${battleState.selectedMonster}"]`);if(selected&&(!isMobilePve()||activeMobileTab==='monsters')){selected.focus({preventScroll:true});selected.scrollIntoView({behavior:'smooth',block:'center',inline:'nearest'});}});}}
...
function pvpCardMini(c,user=loadUser(),mode='pvp-pick'){return deckBuilderListCardHtml(c,user,battleCardPower(c,user,pvpState.config||battleState.config),mode)}
...
else if(pvpState.tab==='deck'){const owned=ownedIds(loadUser()),list=cards.filter(c=>owned.has(c.id)).sort((a,b)=>(gradeOrder[b.grade]||0)-(gradeOrder[a.grade]||0)||String(a.name||'').localeCompare(String(b.name||''),'ko')||String(a.title||'').localeCompare(String(b.title||''),'ko'));const grouped=[...new Set(list.map(c=>c.grade))].map(g=>({grade:g,cards:list.filter(c=>c.grade===g)}));box.innerHTML=`<div class="pvp-section-head pvp-deck-head"><div><p class="eyebrow">MY PVP DECK</p><h2>PvP 덱 편성</h2></div><div class="pvp-deck-actions"><span class="pvp-deck-guide">PVE와 동일한 카드 레이아웃 · 5장 선택</span><button class="pvp-reset-badge" id="resetPvpDeck"><i>↺</i> 덱 초기화</button></div></div><div id="pvpDeckSlots" class="pvp-deck-slots unified-pvp-slots"></div><div id="pvpCardPicker" class="pvp-card-picker grouped unified-pvp-picker">${grouped.map(group=>`<section class="pvp-grade-group grade-${String(group.grade).toLowerCase()}"><div class="pvp-grade-title"><b>${escapeHtml(group.grade)}</b><span>${group.cards.length}장</span></div><div class="pvp-grade-grid unified-grade-grid">${group.cards.map(c=>`<button class="pvp-pick unified-deck-card ${pvpState.deck.includes(c.id)?'selected':''}" data-cid="${c.id}">${pvpCardMini(c,loadUser(),'pvp-pick')}</button>`).join('')}</div></section>`).join('')}</div><button class="btn" id="savePvpDeck">PvP 덱 저장</button>`;renderPvpDeckSlots();document.querySelectorAll('.pvp-pick').forEach(b=>b.onclick=async()=>{const id=b.dataset.cid,i=pvpState.deck.indexOf(id);if(i>=0)pvpState.deck.splice(i,1);else if(pvpState.deck.length<5)pvpState.deck.push(id);else return alert('PvP 덱은 5장까지 편성할 수 있습니다.');await rerenderPvpDeckPreserveScroll()});document.getElementById('resetPvpDeck').onclick=async()=>{if(!pvpState.deck.length)return;pvpState.deck=[];await rerenderPvpDeckPreserveScroll()};document.getElementById('savePvpDeck').onclick=savePvpDeck;}
...
function renderPvpDeckSlots(){const el=document.getElementById('pvpDeckSlots');if(!el)return;const user=loadUser();el.innerHTML=Array.from({length:5},(_,i)=>{const c=cards.find(x=>x.id===pvpState.deck[i]);return c?`<button type="button" class="pvp-deck-slot filled unified-deck-card" data-pvp-remove="${c.id}" title="클릭해서 덱에서 제외">${pvpCardMini(c,user,'pvp-slot')}</button>`:`<div class="pvp-deck-slot empty unified-empty-slot"><span>${i+1}</span><b>빈 슬롯</b><small>카드를 선택하세요</small></div>`}).join('');el.querySelectorAll('[data-pvp-remove]').forEach(b=>b.onclick=async()=>{pvpState.deck=pvpState.deck.filter(id=>id!==b.dataset.pvpRemove);await rerenderPvpDeckPreserveScroll()})}
