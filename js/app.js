...
function pvpHealerPenaltyState(){const selected=pvpState.deck.map(id=>cards.find(card=>card.id===id)).filter(Boolean),count=selected.filter(pvpCardIsHealer).length,reduction=count>=5?70:count===4?60:count===3?45:count===2?30:0;return {count,reduction}}
function renderPvpHealerPenalty(){const box=document.getElementById('pvpHealerPenalty');if(!box)return;const state=pvpHealerPenaltyState();box.className=`pvp-healer-penalty ${state.reduction?'active':''}`;box.innerHTML=`<b>힐러 ${state.count}장</b><span>${state.reduction?`PVE·PVP 회복량 ${state.reduction}% 감소 적용`:'힐러 2장부터 PVE·PVP 중복 회복 패널티 적용'}</span><small>2장 -30% · 3장 -45% · 4장 -60% · 5장 -70%</small>`}
