...
function renderPveDeckCardList(owned,user=loadUser()){
  ...
  root.innerHTML=groups.map(group=>`<section class="pve-grade-group grade-${String(group.grade).toLowerCase()}"><div class="pve-grade-title"><b>${escapeHtml(group.grade)}</b><span>${group.cards.length}장</span></div><div class="pve-grade-grid">${group.cards.map(card=>`<button type="button" class="pve-frame-pick ${deckSet.has(card.id)?'selected':''}" data-pick="${card.id}" ${deckSet.has(card.id)?'disabled':''}>${pveDeckCardMini(card,user)}${deckSet.has(card.id)?'<span class="pve-selected-cover"><b>편성됨</b></span>':''}</button>`).join('')}</div></section>`).join('')||'<div class="empty-recent deck-filter-empty"><b>조건에 맞는 카드가 없습니다.</b><span>검색어나 필터를 변경해보세요.</span></div>';
...
function renderBattleBuilder(){
  ...
  if(viewMode==='deck'){
    if(deckRoot)deckRoot.innerHTML=Array.from({length:5},(_,index)=>{const card=cards.find(item=>item.id===battleState.deck[index]);return card?`<button type="button" class="pve-frame-slot filled" data-remove="${card.id}" title="클릭해서 덱에서 제외">${pveDeckCardMini(card,user)}<span class="pve-remove-hint">덱에서 빼기</span></button>`:`<div class="pve-frame-slot empty"><div class="pve-frame-empty"><span>${index+1}</span><b>빈 슬롯</b><small>카드를 선택하세요</small></div></div>`}).join('');
...
function pvpCardMini(c,user=loadUser()){
  ...
  return `<div class="pvp-card-mini-full">${cardHtml(c,true,'pvp-card-display',user)}<div class="pvp-card-extra compact"><b>${escapeHtml(c.grade||c.rarity||'C')}</b><strong>${power.toLocaleString()}</strong></div></div>`;
}
