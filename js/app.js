...
async function loadRaidView(){const box=document.getElementById('pveRaidView');if(!box||box.hidden||document.hidden)return;try{const query=raidState.selectedRoomId?`?instanceId=${Number(raidState.selectedRoomId)}`:'';const d=await apiRequest(`raid/status${query}`,{}, {ttl:0});raidState.data=d;renderRaidView(d);scheduleRaidPoll(d)}catch(e){stopRaidTimer();box.innerHTML=`<section class="raid-empty raid-empty-polished"><h2>월드 레이드 정보를 불러오지 못했습니다</h2><p>${escapeHtml(e.message)}</p><button type="button" class="btn ghost" id="raidStatusRetry">다시 확인</button></section>`;document.getElementById('raidStatusRetry')?.addEventListener('click',loadRaidView);raidState.timer=setTimeout(()=>loadRaidView(),5000)}}
...
function scheduleRaidPoll(data){stopRaidTimer();if(document.hidden)return;const view=document.getElementById('pveRaidView');if(!view||view.hidden)return;const status=String(data?.current?.status||data?.current?.state||'').toUpperCase();const delay=status==='BATTLE'||status==='RUNNING'?1000:status==='LOBBY'?2000:5000;raidState.timer=setTimeout(()=>loadRaidView(),delay)}

function inventoryView(){return `${summaryBar(loadUser())}<section class="inventory-vault"><div class="inventory-hero"><div class="inventory-hero-copy"><h2>인벤토리</h2><p>전투와 이벤트에서 획득한 보상 큐브를 안전하게 보관합니다.</p><div class="inventory-hero-meta"><b id="inventoryOwnedSummary">보관품 확인 중</b></div></div><div class="inventory-vault-mark" aria-hidden="true"><img src="assets/ui/cninelogo.png" alt=""></div></div><div class="inventory-toolbar"><div><button type="button" class="active">전체</button><button type="button" disabled>큐브</button></div><span>아이템 사용 결과는 서버에 즉시 기록됩니다.</span></div><div id="inventoryGrid" class="inventory-grid"><div class="inventory-loading"><i></i><b>보관함 확인 중</b><span>보유 정보를 확인하고 있습니다.</span></div></div></section>`}
