...
function prestigeDeckCount(cardIds=[]){return (cardIds||[]).reduce((count,id)=>count+(String(deckCardById(id)?.grade||deckCardById(id)?.rarity||'').toUpperCase()==='PRESTIGE'?1:0),0)}
function addDeckCardWithPrestigeLimit(deck,cardId,deckName){const card=deckCardById(cardId);if(String(card?.grade||card?.rarity||'').toUpperCase()==='PRESTIGE'&&prestigeDeckCount(deck)>=PRESTIGE_DECK_LIMIT){alert(`${deckName}에는 PRESTIGE 카드를 최대 ${PRESTIGE_DECK_LIMIT}장까지만 편성할 수 있습니다.`);return false}deck.push(String(cardId));return true}
...
function renderPveDeckCardList(owned,user=loadUser()){
  ...
  root.querySelectorAll('[data-pick]').forEach(button=>button.onclick=()=>{if(battleState.deck.length>=5)return alert('PvE 덱은 5장까지 편성할 수 있습니다.');if(addDeckCardWithPrestigeLimit(battleState.deck,button.dataset.pick,'PvE 덱'))renderBattleBuilder()});
}
...
function renderBattleBuilder(){
  ...
  const deckSet=new Set(battleState.deck),power=battleState.deck.reduce((sum,id)=>{const card=cards.find(item=>item.id===id);return sum+(card?battleCardPower(card,user,battleState.config):0)},0),prestigeCount=prestigeDeckCount(battleState.deck),prestigeValid=prestigeCount<=PRESTIGE_DECK_LIMIT,isReady=battleState.deck.length===5&&prestigeValid;
  const powerEl=document.getElementById('battleDeckPower');if(powerEl)powerEl.textContent=power.toLocaleString();
  const deckCountEl=document.getElementById('battleDeckCount');if(deckCountEl)deckCountEl.textContent=`${battleState.deck.length} / 5 · PRESTIGE ${prestigeCount} / ${PRESTIGE_DECK_LIMIT}`;
  const deckReadyEl=document.getElementById('battleDeckReady');if(deckReadyEl)deckReadyEl.textContent=!prestigeValid?'PRESTIGE 제한 초과':isReady?'출전 준비 완료':'편성 중';
  const deckStatusText=document.getElementById('battleDeckStatusText');if(deckStatusText)deckStatusText.textContent=!prestigeValid?`PRESTIGE는 최대 ${PRESTIGE_DECK_LIMIT}장까지만 편성할 수 있습니다.`:isReady?'5장 편성이 완료되었습니다. 몬스터 토벌을 시작할 수 있습니다.':'아직 5장 편성이 완료되지 않았습니다';
...
function renderPvpDeckCardList(list,user=loadUser()){
  ...
  root.querySelectorAll('[data-cid]').forEach(button=>button.onclick=async()=>{if(pvpState.deck.length>=5)return alert('PvP 덱은 5장까지 편성할 수 있습니다.');if(addDeckCardWithPrestigeLimit(pvpState.deck,button.dataset.cid,'PvP 덱'))await rerenderPvpDeckPreserveScroll()});
}
...
async function savePvpDeck(){if(pvpState.deck.length!==5)return alert('보유 카드 5장을 선택하세요.');if(prestigeDeckCount(pvpState.deck)>PRESTIGE_DECK_LIMIT)return alert(`PvP 덱에는 PRESTIGE 카드를 최대 ${PRESTIGE_DECK_LIMIT}장까지만 편성할 수 있습니다.`);try{await apiRequest('pvp/deck',{method:'POST',body:JSON.stringify({cardIds:pvpState.deck})});alert('PvP 덱이 저장되었습니다.')}catch(e){alert(e.message)}}
