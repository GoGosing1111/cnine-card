...
const HIGH_ENHANCEMENT_LOCAL_VERSION=2;
const MA_HIGH_ENHANCEMENT_FALLBACK={enabled:true,steps:[{cost:1,rate:85,retirementShardRefund:3000},{cost:3,rate:50,retirementShardRefund:4000},{cost:5,rate:25,retirementShardRefund:5000}]};
const LIMITED_HIGH_ENHANCEMENT_FALLBACK={enabled:true,steps:[{cost:5,rate:50,retirementShardRefund:3000},{cost:10,rate:30,retirementShardRefund:4000},{cost:15,rate:20,retirementShardRefund:5000}]};
function loadUser() {
  try {
...
async function startBattle(){
  ...
  modal.innerHTML=`<div class="modal-panel battle-stage intro">
    ...
    <div class="battle-topline"><span>${battleState.battleEngine?.active?'SOOPKETMON PVE · BATTLE ENGINE V2 · 1.6X':'SOOP PVE BATTLE'}</span><b id="battlePhase">ENCOUNTER</b></div>
...
function renderMagicSystem(){
  ...
  root.querySelector('[data-magic-deck="CAPTAIN"]')?.remove();const altar=root.querySelector('.magic-draw-panel');if(altar){const label=altar.querySelector('.eyebrow'),title=altar.querySelector('h2'),foot=altar.querySelector(':scope>small'),cost=altar.querySelector('.magic-draw-cost');if(label)label.textContent='MAGIC CARD SUMMON ALTAR';if(title)title.textContent='아르카나 소환 제단';if(foot)foot.textContent='중복 카드는 강화 재료로 보존되며 동일한 마법 결정 환급량이 적용됩니다.';if(cost)cost.innerHTML=`<span>1회 소모</span><b>🪙 ${Number(d.settings?.drawCoinCost||0).toLocaleString()} + ✦ ${Number(d.settings?.drawCost||0).toLocaleString()}</b>`}const singleDraw=root.querySelector('#magicDrawBtn');if(singleDraw&&d.settings?.drawEnabled){singleDraw.textContent='마법카드 1장 소환';singleDraw.insertAdjacentHTML('afterend',`<button type="button" id="magicDraw10Btn" class="btn magic-draw-ten">마법카드 10장 소환 <small>🪙 ${(Number(d.settings?.drawCoinCost||0)*10).toLocaleString()} · ✦ ${(Number(d.settings?.drawCost||0)*10).toLocaleString()}</small></button>`)}root.querySelectorAll('.magic-card-tile').forEach((tile,index)=>{const card=cards[index],label=tile.querySelector('small'),level=Number(card?.enhancementLevel||0),chance=Number(card?.effectiveTriggerChance??card?.triggerChance??0),cost=Number(d.settings?.enhancement?.shardCosts?.[level]||0),rate=Number(d.settings?.enhancement?.successRates?.[level]||0);if(card&&label)label.textContent=`${magicRenewedEffectLabel(card.effectType)} · 발동 ${chance}% · 강화 +${level}`;if(card&&Number(card.quantity)>0){tile.insertAdjacentHTML('beforeend',`<div class="magic-enhance-row"><span><b>+${level}</b>${level<9?`다음 성공률 ${rate}% · 조각 ${cost.toLocaleString()}개`:'최고 강화'}</span><button type="button" data-enhance-magic="${Number(card.id)}" ${level>=9||Number(card.quantity)<2?'disabled':''}>${level>=9?'MAX':Number(card.quantity)<2?'중복 카드 필요':'강화'}</button></div>`)}});
...
function showMagicDrawResults(data){return new Promise(resolve=>{const rows=Array.isArray(data.results)&&data.results.length?data.results:[{card:data.card,duplicate:data.duplicate,refund:data.refund}],overlay=document.createElement('div');overlay.className=`magic-draw-reveal count-${rows.length}`;overlay.innerHTML=`<div class="magic-draw-reveal-backdrop"></div><section><header><small>ARCANE SUMMON RESULT</small><h2>마법카드 ${rows.length}장 소환</h2><p>신규 ${Number(data.newCount??rows.filter(row=>!row.duplicate).length)}장 · 강화 재료 ${Number(data.duplicateCount??rows.filter(row=>row.duplicate).length)}장 · 사용 🪙 ${Number(data.totalCoinCost||0).toLocaleString()} / ✦ ${Number(data.totalCost||0).toLocaleString()}</p></header><div class="magic-draw-reveal-grid">${rows.map((row,index)=>`<article class="magic-enhancement-card ${row.duplicate?'duplicate':'new'}" style="--reveal-index:${index}">${magicImage(row.card||{})}<span>${row.duplicate?'강화 재료 +1':'NEW'}</span><b>${escapeHtml(row.card?.name||'마법카드')}</b><small>강화 +${Number(row.card?.enhancementLevel||0)}${row.duplicate&&Number(row.refund||0)>0?` · 결정 환급 ✦ ${Number(row.refund||0).toLocaleString()}`:''}</small></article>`).join('')}</div><button type="button">확인</button></section>`;const close=()=>{overlay.classList.add('closing');setTimeout(()=>{overlay.remove();resolve()},180)};overlay.querySelector('button').onclick=close;overlay.querySelector('.magic-draw-reveal-backdrop').onclick=close;document.body.appendChild(overlay);requestAnimationFrame(()=>overlay.classList.add('show'))})}
async function drawMagicCard(count=1){
  count=count===10?10:1;const unitCost=Number(magicSystemState.settings?.drawCost||0),unitCoin=Number(magicSystemState.settings?.drawCoinCost||0),cost=unitCost*count,coinCost=unitCoin*count;if(Number(magicSystemState.magicCrystals||0)<cost)return alert(`마법 결정이 부족합니다. (${cost.toLocaleString()}개 필요)`);if(Number(magicSystemState.coin||loadUser()?.coin||0)<coinCost)return alert(`코인이 부족합니다. (${coinCost.toLocaleString()}코인 필요)`);if(!confirm(`코인 ${coinCost.toLocaleString()}과 마법 결정 ${cost.toLocaleString()}개를 사용해 마법카드 ${count}장을 소환할까요?`))return;
  const buttons=[document.getElementById('magicDrawBtn'),document.getElementById('magicDraw10Btn')].filter(Boolean);buttons.forEach(btn=>btn.disabled=true);const btn=count===10?document.getElementById('magicDraw10Btn'):document.getElementById('magicDrawBtn');if(btn)btn.textContent='소환 중...';
  try{const d=await apiRequest('magic/draw',{method:'POST',body:JSON.stringify({requestId:globalThis.crypto?.randomUUID?.()||`${Date.now()}-${Math.random()}`,count})});const current=loadUser();if(current){current.magicCrystals=Number(d.magicCrystals||0);current.coin=Number(d.coin??current.coin);saveUser(current)}await showMagicDrawResults(d);await loadMagicView()}catch(e){alert(e.message);buttons.forEach(button=>button.disabled=false);if(btn)btn.textContent=count===10?'마법카드 10장 소환':'마법카드 1장 소환'}
}
...
function showDetail(id,initialTab='auto') {
  ...
  const level=Number(user.breakthroughs?.[normalizedId]||0),normalizedGrade=String(card.grade||'').trim().toUpperCase(),canBreak=owned&&(gradeOrder[normalizedGrade]||0)>=gradeOrder[breakthroughMinGrade],isMa=normalizedGrade==='MA',isLimited=normalizedGrade==='LIMITED',isZenith=normalizedGrade==='ZENITH',maxLevel=(isMa||isLimited)?13:10,isMaHigh=(isMa||isLimited)&&level>=10&&level<13,usesMasterStars=isZenith||isMaHigh,standardRule=user.breakthroughConfig?.[normalizedGrade]?.[level]||{cost:breakthroughCosts[level],rate:breakthroughRates[level]},highConfig=isLimited?(user.limitedHighBreakthrough||LIMITED_HIGH_ENHANCEMENT_FALLBACK):(user.maHighBreakthrough||MA_HIGH_ENHANCEMENT_FALLBACK),highRule=highConfig?.steps?.[level-10],rule=isMaHigh?highRule:standardRule,cost=level<maxLevel?Number(rule?.cost||0):null,successRate=level<maxLevel?Number(rule?.rate||0):null,materialBalance=usesMasterStars?Number(user.masterStars||0):Number(user.cardShards||0),materialName=usesMasterStars?'마스터의 별':'카드 조각',highEnabled=highConfig?.enabled===true;
  const hasUnique=Boolean(card.uniqueAbility),uniqueType=uniqueAbilityTypeInfo(card),activeTab=hasUnique&&initialTab!=='info'?'ability':'info',basePower=Number(card.basePower||0),rawOwnedQuantity=Number(user.quantities?.[normalizedId]??0),ownedQuantity=Math.max(owned?1:0,Number.isFinite(rawOwnedQuantity)?Math.floor(rawOwnedQuantity):0),furOwned=normalizedGrade==='FUR'&&owned;
  const infoPanel=`<section class="card-profile-panel ${activeTab==='info'?'active':''}" data-profile-panel="info"><div class="card-profile-facts"><article><small>${furOwned?'보유 수량':'보유 상태'}</small><b>${furOwned?`${ownedQuantity.toLocaleString('ko-KR')}장 보유`:owned?'보유 중':'미획득'}</b></article><article><small>등급</small><b>${escapeHtml(card.grade||'?')}</b></article><article><small>기본 전투력</small><b>${basePower>0?basePower.toLocaleString():'-'}</b></article><article><small>전투력 유형</small><b>${escapeHtml(powerTypeIndicator(card)||'기본')}</b></article></div>${owned?`<div class="breakthrough-info"><span>돌파 단계</span><strong>${level>=maxLevel?`★${maxLevel} MAX`:`★${level}`}</strong><small>보유 ${materialName} ${materialBalance.toLocaleString()}개</small>${canBreak?(level<maxLevel?(isMaHigh&&!highEnabled?`<b class="max-breakthrough">${normalizedGrade} 고급 강화 운영 준비 중</b>`:`<button type="button" class="btn breakthrough-btn${materialBalance<cost?' material-shortage':''}" id="breakthroughBtn" data-breakthrough-card="${escapeHtml(normalizedId)}" data-client-shortage="${materialBalance<cost?'1':'0'}">${materialName} ${cost.toLocaleString()}개 · 성공 ${successRate}%<br>★${level+1} 강화</button>`):'<b class="max-breakthrough">LEGEND · 최대 강화</b>'):'<small>SR 등급 이상부터 돌파할 수 있습니다.</small>'}</div>`:'<div class="card-profile-locked-info"><i>LOCKED</i><b>아직 획득하지 않은 카드입니다.</b><span>고유 능력은 미리 확인할 수 있지만 돌파·보유 정보는 획득 후 공개됩니다.</span></div>'}${history?`<p class="obtained-date">최초 획득<br><strong>${new Date(history.at).toLocaleString('ko-KR')}</strong></p>`:''}</section>`;
...
async function breakthroughCard(cardId){
  ...
  const level=Number(user.breakthroughs?.[normalizedCardId]||0),normalizedGrade=String(card?.grade||'').trim().toUpperCase(),isMa=normalizedGrade==='MA',isLimited=normalizedGrade==='LIMITED',isZenith=normalizedGrade==='ZENITH',maxLevel=(isMa||isLimited)?13:10,isMaHigh=(isMa||isLimited)&&level>=10,usesMasterStars=isZenith||isMaHigh;
  if(level>=maxLevel)return alert('이미 최대 강화 단계입니다.');
  const highConfig=isLimited?(user.limitedHighBreakthrough||LIMITED_HIGH_ENHANCEMENT_FALLBACK):(user.maHighBreakthrough||MA_HIGH_ENHANCEMENT_FALLBACK),rule=isMaHigh?highConfig?.steps?.[level-10]:(user.breakthroughConfig?.[normalizedGrade]?.[level]||{cost:breakthroughCosts[level],rate:breakthroughRates[level]}),materialName=usesMasterStars?'마스터의 별':'카드 조각',balance=usesMasterStars?Number(user.masterStars||0):Number(user.cardShards||0);
...
async function openInventoryPack(itemCode,ownedQuantity=0){
  ...
  const meta=reroll?{...reroll,range:`${reroll.grade} 활성 카드`,kind:'reroll'}:{...(cubeItems[itemCode]||cubeItems.PREMIUM_CUBE),kind:'cube'},modal=document.getElementById('modal');
...
async function openEquipmentSupplyBox(ownedQuantity=0,autoOpenCount=0){
  const modal=document.getElementById('modal'),maxCount=Math.max(1,Math.min(10,Number(autoOpenCount)||Number(ownedQuantity)||1));
...
function pvpView(user){return `${summaryBar(user)}<section class="pvp-cover ranked-cover"><div class="pvp-cover-intro"><p class="eyebrow" id="pvpSeasonEyebrow">SOOPKETMON RANKED</p><h2 id="pvpSeasonTitle">랭크전</h2><p id="pvpSeasonDescription">티어와 편성 전투력을 기준으로 상대가 자동 배정됩니다.</p><small id="pvpSeasonStatusLine">상태 불러오는 중</small></div><div class="pvp-me"><div id="pvpMyTierBadge" class="pvp-tier-badge"></div><span id="pvpMyTier">-</span><b id="pvpMyScore">-</b><small id="pvpSeasonTime">시즌 정보 불러오는 중</small></div><div class="battle-energy-card pvp-energy-card"><div class="pvp-energy-head"><span>⚔ 랭크전 횟수</span><b id="pvpEnergyCount">- / -</b></div><div class="battle-energy-track"><i id="pvpEnergyFill"></i></div><small id="pvpEnergyTimer">불러오는 중...</small></div></section><nav class="pvp-tabs"><button data-pvp="match" class="active">랭크전 매칭</button><button data-pvp="deck">덱 편성</button><button data-pvp="history">전투 기록</button><button data-pvp="ranking">시즌 랭킹</button><button data-pvp="reward">시즌 보상</button></nav><section id="pvpContent" class="pvp-content"><div class="empty-recent">랭크전 정보를 불러오는 중...</div></section>`}
...
async function renderPvpTab(){const box=document.getElementById('pvpContent');if(!box)return;box.innerHTML='<div class="empty-recent">불러오는 중...</div>';try{if(pvpState.tab==='match'){const tier=pvpState.profile?.tier||{name:'브론즈'},score=Number(pvpState.profile?.season_score||0),power=pvpState.deck.reduce((sum,id)=>{const card=cards.find(item=>String(item.id)===String(id));return sum+(card?battleCardPower(card,loadUser(),pvpState.battleConfig||battleState.config):0)},0)+Number(pvpState.characterBonus?.pvp||0);box.innerHTML=`<section class="ranked-match-console"><div class="ranked-match-radar" aria-hidden="true"><i></i><i></i><i></i><b>R</b></div><div class="ranked-match-copy"><p class="eyebrow">RANKED AUTO MATCH</p><h2>랭크전 매칭</h2><p>상대 목록은 제공되지 않습니다. 현재 티어·시즌 점수·실제 편성 전투력을 함께 비교해 가장 균형이 맞는 상대 한 명을 서버가 배정합니다.</p><div class="ranked-match-stats"><span><small>현재 티어</small><b>${escapeHtml(tier.name)}</b></span><span><small>시즌 점수</small><b>${score.toLocaleString()}</b></span><span><small>편성 전투력</small><b>${power.toLocaleString()}</b></span></div><div class="ranked-title-reward"><span><b>그랜드마스터</b> 승부사</span><span><b>마스터</b> 결투가</span><small>시즌 종료 후 지급 · 다음 5일 시즌 정산 시 자동 만료</small></div><button type="button" class="btn ranked-match-start pvp-fight" id="rankedMatchStart">랭크전 매칭 시작</button><small class="ranked-match-rule">5일 단위 자동 시즌 · 상대 선택 및 새로고침 불가</small></div></section>`;document.getElementById('rankedMatchStart').onclick=startRankedMatch;renderPvpEnergy();}
...
async function fightPvpV2Live({id,target,mine,pvpPreviewPower,matchToken}){
  ...
  try{
    ...
    if(e.energy)pvpState.energy=e.energy;phase.textContent='RANKED ERROR';msg.innerHTML=`<strong>ERROR</strong><span>${escapeHtml(e.message)}</span><button type="button" class="btn pvp-result-confirm" id="pvpErrorConfirm">랭크전 화면으로 돌아가기</button>`;msg.classList.add('is-visible');modal.onclick=close;const b=document.getElementById('pvpErrorConfirm');if(b)b.onclick=event=>{event.stopPropagation();close()};
  }
}
...
async function startRankedMatch(){
  ...
  const button=document.getElementById('rankedMatchStart');pvpState.matching=true;if(button){button.disabled=true;button.textContent='균형 상대 탐색 중...'}
  try{const matched=await apiRequest('pvp/match',{method:'POST',body:JSON.stringify({requestId:globalThis.crypto?.randomUUID?.()||`${Date.now()}-${Math.random()}`})});pvpState.opponents=[matched.opponent];await fightPvp(Number(matched.opponent.id),matched.token)}catch(error){alert(error.message);if(error.code==='PVP_SEASON_SETTLING')setTimeout(()=>loadPvpView(),Number(error.retryAfterMs||1500))}finally{pvpState.matching=false;if(button&&document.body.contains(button)){button.disabled=false;button.textContent='랭크전 매칭 시작'}}
}
