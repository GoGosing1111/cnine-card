...
function battleCardPower(card,user,settings){const grade=String(card?.grade||card?.rarity||'').toUpperCase(),base=Number(card.basePower||settings?.powerByGrade?.[grade]||0),lv=Number(user.breakthroughs?.[card.id]||0),pct=Number(settings?.breakthroughBonus?.[lv]||0);return Math.floor(base*(1+pct/100));}
const PRESTIGE_DECK_LIMIT=2;
const FUR_DECK_LIMIT=1;
function deckCardById(cardId){return cards.find(card=>String(card.id)===String(cardId))||null}
function deckGradeCount(cardIds=[],grade=''){const target=String(grade||'').toUpperCase();return (cardIds||[]).reduce((count,id)=>count+(String(deckCardById(id)?.grade||deckCardById(id)?.rarity||'').toUpperCase()===target?1:0),0)}
function prestigeDeckCount(cardIds=[]){return deckGradeCount(cardIds,'PRESTIGE')}
function furDeckCount(cardIds=[]){return deckGradeCount(cardIds,'FUR')}
function addDeckCardWithGradeLimits(deck,cardId,deckName){const card=deckCardById(cardId),grade=String(card?.grade||card?.rarity||'').toUpperCase();if(grade==='PRESTIGE'&&prestigeDeckCount(deck)>=PRESTIGE_DECK_LIMIT){alert(`${deckName}에는 PRESTIGE 카드를 최대 ${PRESTIGE_DECK_LIMIT}장까지만 편성할 수 있습니다.`);return false}if(grade==='FUR'&&furDeckCount(deck)>=FUR_DECK_LIMIT){alert(`${deckName}에는 FUR 카드를 최대 ${FUR_DECK_LIMIT}장까지만 편성할 수 있습니다.`);return false}deck.push(String(cardId));return true}
...
function renderPveDeckCardList(owned,user=loadUser()){
  ...
  root.querySelectorAll('[data-pick]').forEach(button=>button.onclick=()=>{if(battleState.deck.length>=5)return alert('PvE 덱은 5장까지 편성할 수 있습니다.');if(addDeckCardWithGradeLimits(battleState.deck,button.dataset.pick,'PvE 덱'))renderBattleBuilder()});
}
...
function renderBattleBuilder(){
  ...
  const deckSet=new Set(battleState.deck),power=battleState.deck.reduce((sum,id)=>{const card=cards.find(item=>item.id===id);return sum+(card?battleCardPower(card,user,battleState.config):0)},0),prestigeCount=prestigeDeckCount(battleState.deck),furCount=furDeckCount(battleState.deck),prestigeValid=prestigeCount<=PRESTIGE_DECK_LIMIT,furValid=furCount<=FUR_DECK_LIMIT,isReady=battleState.deck.length===5&&prestigeValid&&furValid;
  const powerEl=document.getElementById('battleDeckPower');if(powerEl)powerEl.textContent=power.toLocaleString();
  const deckCountEl=document.getElementById('battleDeckCount');if(deckCountEl)deckCountEl.textContent=`${battleState.deck.length} / 5 · FUR ${furCount} / ${FUR_DECK_LIMIT} · PRESTIGE ${prestigeCount} / ${PRESTIGE_DECK_LIMIT}`;
  const deckReadyEl=document.getElementById('battleDeckReady');if(deckReadyEl)deckReadyEl.textContent=!furValid?'FUR 제한 초과':!prestigeValid?'PRESTIGE 제한 초과':isReady?'출전 준비 완료':'편성 중';
  const deckStatusText=document.getElementById('battleDeckStatusText');if(deckStatusText)deckStatusText.textContent=!furValid?`FUR는 최대 ${FUR_DECK_LIMIT}장까지만 편성할 수 있습니다.`:!prestigeValid?`PRESTIGE는 최대 ${PRESTIGE_DECK_LIMIT}장까지만 편성할 수 있습니다.`:isReady?'5장 편성이 완료되었습니다. 몬스터 토벌을 시작할 수 있습니다.':'아직 5장 편성이 완료되지 않았습니다';
...
async function saveBattleDeck(){
  ...
  if(furDeckCount(battleState.deck)>FUR_DECK_LIMIT)return alert(`PvE 덱에는 FUR 카드를 최대 ${FUR_DECK_LIMIT}장까지만 편성할 수 있습니다.`);
  if(prestigeDeckCount(battleState.deck)>PRESTIGE_DECK_LIMIT)return alert(`PvE 덱에는 PRESTIGE 카드를 최대 ${PRESTIGE_DECK_LIMIT}장까지만 편성할 수 있습니다.`);
...
function renderPvpDeckCardList(list,user=loadUser()){
  ...
  root.querySelectorAll('[data-cid]').forEach(button=>button.onclick=async()=>{if(pvpState.deck.length>=5)return alert('PvP 덱은 5장까지 편성할 수 있습니다.');if(addDeckCardWithGradeLimits(pvpState.deck,button.dataset.cid,'PvP 덱'))await rerenderPvpDeckPreserveScroll()});
}
...
async function savePvpDeck(){if(pvpState.deck.length!==5)return alert('보유 카드 5장을 선택하세요.');if(furDeckCount(pvpState.deck)>FUR_DECK_LIMIT)return alert(`PvP 덱에는 FUR 카드를 최대 ${FUR_DECK_LIMIT}장까지만 편성할 수 있습니다.`);if(prestigeDeckCount(pvpState.deck)>PRESTIGE_DECK_LIMIT)return alert(`PvP 덱에는 PRESTIGE 카드를 최대 ${PRESTIGE_DECK_LIMIT}장까지만 편성할 수 있습니다.`);try{await apiRequest('pvp/deck',{method:'POST',body:JSON.stringify({cardIds:pvpState.deck})});alert('PvP 덱이 저장되었습니다.')}catch(e){alert(e.message)}}
