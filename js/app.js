...
async function openWagoVerification(){
  ...
  try{
    ...
    panel.innerHTML=`<div class="verify-card"><h3>와고 닉네임 2단계 인증</h3>${v?`<div class="verify-status status-${String(v.status).toLowerCase()}"><b>${verified?'인증 완료':escapeHtml(v.status)}</b><span>${escapeHtml(v.wago_nickname)} · 회원번호 ${escapeHtml(v.wago_member_no)}</span>${v.verification_code&&!verified?`<code>${escapeHtml(v.verification_code)}</code>`:''}</div>`:''}${verified?'<div class="verify-guide"><p>댓글 주소와 회원번호 프로필 주소 검증이 완료된 계정입니다.</p></div>':`<div class="verify-form"><input id="verifyWagoName" placeholder="와고 닉네임" value="${escapeHtml(v?.wago_nickname||'')}"><input id="verifyMemberNo" inputmode="numeric" placeholder="와고 회원번호 (예: 697197)" value="${escapeHtml(v?.wago_member_no||'')}"><button class="btn" id="issueVerifyCode">인증코드 발급</button></div><div class="verify-guide"><p>발급된 코드를 지정 인증 게시글에 댓글로 작성한 뒤, 댓글 주소와 본인의 회원번호 프로필 주소를 모두 입력하세요.</p>${s.postUrl?`<a class="btn secondary" href="${escapeHtml(s.postUrl)}" target="_blank" rel="noopener">와고 인증 게시글 열기</a>`:'<b>현재 CMS에 인증 게시글 주소가 설정되지 않았습니다.</b>'}<label><span>인증 댓글 주소</span><input id="verifyCommentUrl" type="url" placeholder="https://ygosu.com/board/..." value="${escapeHtml(v?.comment_url||'')}"></label><label><span>회원번호 프로필 주소</span><input id="verifyProfileUrl" type="url" placeholder="https://ygosu.com/minilog/?member=697197&m2=profile" value="${escapeHtml(v?.profile_url||'')}"></label><button class="btn" id="checkVerifyComment">자동 인증 확인</button></div>`}</div>`;
    if(verified)return;
    document.getElementById('issueVerifyCode').onclick=async()=>{try{const r=await apiRequest('wago-verification/request',{method:'POST',body:JSON.stringify({wagoNickname:document.getElementById('verifyWagoName').value,wagoMemberNo:document.getElementById('verifyMemberNo').value})});alert(`인증코드: ${r.verificationCode}\n${r.expiresMinutes}분 안에 지정 게시글 댓글로 작성하세요.`);openWagoVerification()}catch(e){alert(e.message)}};
    document.getElementById('checkVerifyComment').onclick=async()=>{const commentUrl=document.getElementById('verifyCommentUrl')?.value.trim(),profileUrl=document.getElementById('verifyProfileUrl')?.value.trim();if(!commentUrl)return alert('인증코드를 작성한 댓글 주소를 입력하세요.');if(!profileUrl)return alert('회원번호 프로필 주소를 입력하세요.');try{const r=await apiRequest('wago-verification/check',{method:'POST',body:JSON.stringify({commentUrl,profileUrl})});alert(r.message||'자동 인증이 완료되었습니다.');openWagoVerification()}catch(e){alert(e.message)}};
