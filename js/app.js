...
function battleCardPower(card,user,settings){const grade=String(card?.grade||card?.rarity||'').trim().toUpperCase(),gradePower=Number(settings?.powerByGrade?.[grade]),savedPower=Number(card?.basePower??card?.base_power),base=Number.isFinite(savedPower)&&savedPower>0?savedPower:(Number.isFinite(gradePower)?Math.max(0,gradePower):0),lv=Number(user.breakthroughs?.[card.id]||0),pct=Number(settings?.breakthroughBonus?.[lv]||0);return Math.floor(base*(1+pct/100));}
...
async function breakthroughCard(cardId){
  ...
  if(isMaHigh&&user.maHighBreakthrough?.enabled!==true)return alert(`${normalizedGrade} +11~+13 강화가 아직 운영 준비 중입니다.`);
