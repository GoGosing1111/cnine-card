...
function uniqueAbilityDetailHtml(card){
  ...
  const stat=(label,value)=>`<span><small>${label}</small><b>${Number(value||0)>0?'+':''}${Number(value||0)}%</b></span>`;
  return `<section class="card-unique-ability ${x.ownerTest?'owner-test':''}"><div class="unique-ability-head"><div><small>${x.ownerTest?'OWNER TEST · ':''}UNIQUE ABILITY</small><h3>${escapeHtml(x.effectName||'카드 고유 능력')}</h3></div><em>${x.ownerTest?'TEST':'ACTIVE'}</em></div><div class="unique-ability-stats">${stat('공격',x.attackPercent)}${stat('방어',x.defensePercent)}${stat('HP',x.hpPercent)}${stat('속도',x.speedPercent)}</div>${x.effectDescription?`<p>${escapeHtml(x.effectDescription)}</p>`:''}<footer><span>${x.scopes?.pve?'PVE ':''}${x.scopes?.pvp?'PVP ':''}${x.scopes?.captain?'대장전':''}</span><b>발동형 효과는 준비 중</b></footer></section>`;
}
...
const STARTUP_SNAPSHOT_KEY='cnine_startup_snapshot_v1145';
...
async function refreshStartupCatalog(runId,cardTask,packTask){
  ...
  if(cardResult.ok&&Array.isArray(cardResult.value?.cards)&&cardResult.value.cards.length){
  ...
  }else if(!cards.length)console.warn('카드 데이터 백그라운드 갱신 실패:',cardResult.error);
...
async function init(){
  ...
  try{
    ...
    if(!hasCatalogSnapshot){
      ...
      cards=cardResult.value.cards.map(normalizeClientCard);
      const cachePatch={cards:cardResult.value.cards};
