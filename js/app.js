...
async function startAutoBattle(){
  ...
  battleState.autoRunning=true;stopBattleEnergyTimer();
  const modal=document.getElementById('modal');modal.className='modal show';modal.innerHTML='<div class="modal-panel battle-auto-progress"><div class="battle-auto-spinner"></div><h2>자동전투 진행 중</h2><p>전투와 보상을 처리하고 있습니다.</p><strong>완료될 때까지 잠시 기다려 주세요.</strong></div>';
  try{
    const d=await apiRequest('battle/auto',{method:'POST',body:JSON.stringify({requestId:crypto.randomUUID(),monsterId:battleState.selectedMonster,cardIds:battleState.deck})});
    ...
    modal.innerHTML=`<div class="modal-panel battle-auto-result"><p class="eyebrow">AUTO BATTLE COMPLETE</p><h2>자동전투 완료</h2><div class="battle-auto-stats"><span>진행 <b>${Number(d.battles||0)}회</b></span><span>승리 <b>${Number(d.wins||0)}회</b></span><span>패배 <b>${Number(d.losses||0)}회</b></span><span>획득 코인 <b>◈ ${Number(d.totalReward||0).toLocaleString()}</b></span></div>${d.cardRewards?.length?`<div class="battle-auto-drops"><b>카드 획득 ${d.cardRewards.length}장</b>${d.cardRewards.map(x=>`<span>${escapeHtml(x.card.grade)} ${escapeHtml(x.card.title)}${x.duplicate?` · 조각 +${Number(x.shardGained||0)}`:' · 신규'}</span>`).join('')}</div>`:''}<button class="btn" id="battleAutoDone">확인</button></div>`;
    document.getElementById('battleAutoDone').onclick=()=>renderShell('battle');
  }catch(e){modal.innerHTML=`<div class="modal-panel battle-auto-result"><h2>자동전투 중단</h2><p>${escapeHtml(e.message)}</p><button class="btn" id="battleAutoDone">확인</button></div>`;document.getElementById('battleAutoDone').onclick=()=>renderShell('battle')}
  finally{battleState.autoRunning=false}
}
...
function inventoryView(){return `${summaryBar(loadUser())}<section class="inventory-vault"><div class="inventory-hero"><div class="inventory-hero-copy"><p class="eyebrow">CNINE REWARD VAULT</p><h2>인벤토리</h2><p>전투와 이벤트에서 획득한 보상 큐브를 안전하게 보관합니다.</p><div class="inventory-hero-meta"><span><i></i> SERVER VERIFIED</span><b id="inventoryOwnedSummary">보관품 확인 중</b></div></div><div class="inventory-vault-mark" aria-hidden="true"><span></span><img src="assets/ui/cninelogo.png" alt=""><b>CN VAULT</b><em>REWARD STORAGE</em></div></div><div class="inventory-toolbar"><div><button type="button" class="active">전체</button><button type="button" disabled>큐브</button><button type="button" disabled>입장권</button><button type="button" disabled>이벤트</button></div><span>아이템 사용 결과는 서버에 즉시 기록됩니다.</span></div><div id="inventoryGrid" class="inventory-grid"><div class="inventory-loading"><i></i><b>보관함 잠금 해제 중</b><span>서버 보유 정보를 확인하고 있습니다.</span></div></div></section>`}
