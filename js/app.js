...
let raidState={timer:null,data:null,resultRevealed:new Set(),selectedRoomId:0};
...
function renderRaidView(d){
  ...
  if(c.status==='LOBBY'){
    ...
    if(!joined&&raidState.selectedRoomId){const screen=box.querySelector('.raid-lobby-screen');screen?.insertAdjacentHTML('afterbegin','<button type="button" class="btn ghost" id="raidRoomBack">← 방 목록</button>');const back=document.getElementById('raidRoomBack');if(back)back.onclick=()=>{raidState.selectedRoomId=0;loadRaidView()};}
