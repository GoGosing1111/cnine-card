...
function renderRaidView(d){
  ...
  const resultScreen=`<section class="raid-result-screen ${clear?'clear':'failed'}"><div class="raid-result-burst"></div><p class="eyebrow">RAID RESULT</p><h1>${clear?'RAID CLEAR':'RAID FAILED'}</h1><h2>${escapeHtml(c.bossName)}</h2><div class="raid-result-summary"><article><span>내 누적 피해</span><b>${Number(me?.shownDamage||0).toLocaleString()}</b></article><article><span>최종 순위</span><b>${me?participants.findIndex(x=>Number(x.userId)===Number(me.userId))+1:'-'}위</b></article><article><span>생존 상태</span><b>${me?.isDefeated?'전투 불능':'생존'}</b></article></div><div class="raid-reward-box"><span>토벌 보상</span><div class="raid-reward-cards"><article style="--reward-delay:.15s"><em>🪙</em><b>${rewardCoin.toLocaleString()} 코인</b><small>${clear?'참가 + 처치 보상':'참가 보상'}</small></article><article style="--reward-delay:.35s"><em>✦</em><b>${rewardShards.toLocaleString()} 조각</b><small>${clear?'클리어 보상':'실패 시 미지급'}</small></article></div><button id="raidClaimReward" class="btn raid-claim-btn" ${!me||claimed?'disabled':''}>${!me?'미참가':claimed?'보상 수령 완료':'보상 받기'}</button><small class="raid-reward-note">${claimed?'보상이 계정에 지급되었습니다.':'버튼을 누르면 보상이 순서대로 지급됩니다.'}</small></div></section>`;
...
async function claimRaidReward(instanceId){const btn=document.getElementById('raidClaimReward');if(btn){btn.disabled=true;btn.textContent='보상 정산 중...'}try{const d=await apiRequest('raid/claim-reward',{method:'POST',body:JSON.stringify({instanceId})});const user=loadUser();if(d.user){user.coin=d.user.coin;user.cardShards=d.user.cardShards;saveUser(user)}document.querySelector('.raid-reward-box')?.classList.add('is-claimed');setTimeout(()=>{alert(`레이드 보상 지급 완료!\n코인 ${Number(d.coin).toLocaleString()}\n카드 조각 ${Number(d.shards).toLocaleString()}`);loadRaidView()},900)}catch(e){alert(e.message);if(btn){btn.disabled=false;btn.textContent='보상 받기'}}}
