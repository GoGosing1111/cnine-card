...
async function playBossBattleUltimate(stage,phase,ult){
  ...
  overlay.innerHTML=`
    ...
    <div class="boss-ultimate-overlay-media">
      ${mediaSrc?(isVideo?`<video src="${escapeHtml(mediaSrc)}" ${soundSrc?'muted':''} playsinline webkit-playsinline preload="auto" disablepictureinpicture></video>`:`<img src="${escapeHtml(mediaSrc)}" alt="${escapeHtml(ult.name||'BOSS ULTIMATE')}">`):'<div class="boss-ultimate-overlay-fallback">BOSS ULTIMATE</div>'}
    </div>
  ...
  await new Promise(resolve=>{
    ...
    if(video){
      ...
      const mobilePlayback=matchMedia('(max-width: 800px), (pointer: coarse)').matches;
      video.defaultMuted=mobilePlayback||Boolean(soundSrc)||!battleSoundEnabled()||volume<=0;
      ...
      if(play&&typeof play.catch==='function')play.catch(()=>{video.defaultMuted=true;video.muted=true;video.load();const retry=video.play();if(retry&&typeof retry.catch==='function')retry.catch(()=>overlay.classList.add('media-failed'))});
    }
...
let pvpState={tab:'match',config:null,battleConfig:null,profile:null,myTitle:null,deck:[],characterBonus:{equipmentPve:0,equipmentPvp:0,titlePve:0,pve:0,pvp:0},battleEngine:{active:false,version:'LEGACY',mode:'LEGACY'},opponents:[],history:[],ranking:[],energy:null,energyTimer:null,serverOffset:0};
...
function renderPvpDeckTab(box){
  ...
  const cardPower=pvpState.deck.reduce((sum,id)=>{const card=cards.find(item=>item.id===id);return sum+(card?battleCardPower(card,user,pvpState.battleConfig||battleState.config):0)},0),equipmentPvp=Number(pvpState.characterBonus?.pvp||0),totalPower=cardPower+equipmentPvp;
