(() => {
  ...
  function renderPveMonsterBrowserV2() {
    ...
    const myPower = cardPower + Number(bonus.pve || 0), requiredPower = Math.max(0, Number(selected?.recommendedPower ?? selected?.battlePower ?? 0)), powerDelta = myPower - requiredPower, powerDeltaPercent = requiredPower > 0 ? powerDelta / requiredPower * 100 : 0, powerReady = requiredPower <= 0 || powerDelta >= 0, noEnergy = battleState.energy && !battleState.energy.unlimited && battleState.energy.energy < battleState.energy.costPerBattle, ruleViolation = pveDeckLimitViolation();
