(() => {
  ...
  function tacticMarkup(run,settings){
    ...
    return `<section class="escort-tactic-panel"><header><div><small>ROUTE COMMAND / TACTICAL BUFFER</small><h3>다음 구간 전술 선택</h3></div><p>두 전술 중 하나만 적용됩니다. 효과 범위와 유지 시간을 확인하십시오.</p></header><div class="escort-v1833-tactic-grid" style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;margin-top:22px">${choices.map((tactic,index)=>`<button type="button" class="escort-v1833-tactic-card is-${esc(String(tactic.key||'').toLowerCase())}" data-escort-action="tactic" data-tactic="${esc(tactic.key)}" data-escort-primary="${index===0?'1':'0'}" aria-label="${esc(`${tactic.name} 선택`)}" style="display:grid;grid-template-columns:34px 132px minmax(0,1fr) 18px;grid-template-rows:auto;align-items:center;gap:14px;width:100%;min-width:0;min-height:166px;box-sizing:border-box;padding:15px 17px;text-align:left;cursor:pointer;writing-mode:horizontal-tb;"><span class="escort-v1833-tactic-number" style="grid-column:1;align-self:start;white-space:nowrap;writing-mode:horizontal-tb;">0${index+1}</span><div class="escort-v1833-tactic-icon" style="grid-column:2;position:relative;display:grid;place-items:center;width:100%;min-width:0;aspect-ratio:1;"><img src="${esc(imageUrl(TACTIC_ICONS[String(tactic.key||'').toUpperCase()]||tactic.icon))}?v=1835" alt="" width="256" height="256" decoding="async" style="display:block;width:100%;height:100%;object-fit:contain;"></div><div class="escort-v1833-tactic-copy" style="grid-column:3;display:block;min-width:0;writing-mode:horizontal-tb;word-break:keep-all;overflow-wrap:anywhere;"><div class="escort-v1833-tactic-meta"><small>${esc(tactic.type)}</small><em>${esc(tactic.duration||'다음 1구간')}</em></div><b>${esc(tactic.name)}</b><p>${esc(tactic.description)}</p><u>전술 적용</u></div><i class="escort-v1833-tactic-arrow" aria-hidden="true" style="grid-column:4;"></i></button>`).join('')}</div></section>`;
  }
