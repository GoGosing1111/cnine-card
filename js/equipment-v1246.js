/* V1246 INSTANT EQUIP + ITEM FIT/ALIGNMENT */
(()=>{
  ...
  function showNotice(message,error=false){
    document.querySelector('.frame-action-toast-v1246')?.remove();
    const toast=document.createElement('div');toast.className=`frame-action-toast-v1246${error?' error':''}`;toast.textContent=message;document.body.appendChild(toast);
  ...
  function slotOverlay(slot){
    ...
    return `<div class="frame-slot-v1246 slot-${slot.toLowerCase()} ${item?'filled':''} ${item?rarityClass(item.rarity):''} ${item?subtypeClass(item):''}">
      <button type="button" class="frame-slot-hit-v1246" data-character-filter="${slot}" aria-label="${esc(slotLabels[slot])}"></button>
      ${item?`<div class="frame-slot-image-v1246">${itemImage(item)}</div><div class="frame-slot-desc-mask-v1246"><span>${esc(description)}</span></div>`:''}
      ${item?`<button type="button" class="frame-slot-remove-v1246" data-character-unequip="${slot}" aria-label="해제">×</button>`:''}
    </div>`;
  }
  function equipmentLayer(){
    const b=state.data?.bonuses||{},title=currentTitle();
    return `<div class="frame-equipment-layer-v1246">
      <div class="frame-nickname-v1246" data-dynamic-nickname>${esc(nicknameText())}</div>
      <div class="frame-title-v1246 ${titleClass(title?.stylePreset)}" data-dynamic-title>[${esc(titleText(title))}]</div>
      ${slotOrder.map(slotOverlay).join('')}
      <div class="frame-stat-box-v1246 pve" data-dynamic-pve><small>PVE</small><b>+${num(b.pve)}</b></div>
      <div class="frame-stat-box-v1246 pvp" data-dynamic-pvp><small>PVP</small><b>+${num(b.pvp)}</b></div>
      ${inventoryLayer()}
    </div>`;
  }
  function inventoryLayer(){
    ...
    return `<div class="frame-inventory-layer-v1246">
      <div class="frame-inventory-head-v1246"><b>보유 장비</b><span>${all.length}</span></div>
      <div class="frame-filter-row-v1246">
      ...
      <div class="frame-item-grid-v1246">
        ${rows.length?rows.map(row=>{const i=row.item;return `<button type="button" class="frame-item-card-v1246 ${row.equipped?'equipped':''} ${rarityClass(i.rarity)} ${subtypeClass(i)}" ${row.equipped?'disabled':''} data-character-equip="${row.instanceId}" title="${esc(i.name)}">
          <i class="frame-item-rarity-v1246">${esc(rarityLabels[normRarity(i.rarity)])}</i>
          <div class="frame-item-thumb-v1246">${itemImage(i)}</div>
          <div class="frame-item-info-v1246">
            <strong>${esc(i.name)}</strong>
            <span class="frame-item-type-v1246">${esc(slotLabels[i.slot]||i.slot)} · ${esc(subtypeLabels[i.subtype]||i.subtype)}</span>
            <div class="frame-item-power-v1246"><i>PVE +${num(i.pvePower)}</i><i class="pvp">PVP +${num(i.pvpPower)}</i></div>
          </div>
        </button>`}).join(''):'<div class="frame-empty-v1246">보유 장비<br>없음</div>'}
      </div>
    </div>`;
  }
  ...
  function titleLayer(){
    ...
    return `<div class="frame-title-layer-v1246">
      <div class="frame-nickname-v1246" data-dynamic-nickname>${esc(nicknameText())}</div>
      <div class="frame-title-current-v1246 ${titleClass(equippedTitle?.stylePreset)}">
      ...
      <div class="frame-title-list-v1246">
        ${rows.length?rows.map(row=>{
          ...
          return `<article class="frame-title-card-v1246 ${row.owned?'owned':'locked'} ${row.equipped?'equipped':''} ${titleClass(row.stylePreset)}"><strong>[${esc(row.badgeText||row.name)}]</strong><small>${esc(row.owned?(unlockLabels[row.unlockType]||row.unlockType):titleRequirement(row))}</small><b>PVE +${num(row.pvePower)}</b>${action}</article>`;
        }).join(''):'<div class="frame-empty-v1246">등록된 칭호 없음</div>'}
      </div>
    </div>`;
  }
  function shellHtml(){
    return `<div class="image-frame-ui-v1246">
      <div class="frame-background-v1246"></div>
      <button type="button" class="frame-tab-hit-v1246 equipment ${state.tab==='equipment'?'active':''}" data-character-tab="equipment" aria-label="장비"></button>
      <button type="button" class="frame-tab-hit-v1246 title ${state.tab==='title'?'active':''}" data-character-tab="title" aria-label="칭호"></button>
  ...
  function render(){
    ...
    if(state.loading&&!state.data){root.innerHTML='<div class="frame-loading-v1246"><span></span><b>불러오는 중...</b></div>';return}
    if(!state.data){root.innerHTML='<div class="frame-empty-v1246">장비 정보를 불러오지 못했습니다.</div>';return}
  ...
  async function load(){
    ...
    catch(error){const root=document.getElementById('characterSystemRoot');if(root)root.innerHTML='<div class="frame-empty-v1246">장비 정보를 불러오지 못했습니다.</div>'}
    finally{state.loading=false}
  }
  window.characterView=()=>'<section id="characterSystemRoot" class="character-system-root-v1246"><div class="frame-loading-v1246"><span></span><b>불러오는 중...</b></div></section>';
  ...
  window.showEquipmentDropReward=async reward=>{
    ...
    const toast=document.createElement('div');toast.id='equipmentDropToast';toast.className='frame-drop-toast-v1246';
    toast.innerHTML=`<div class="frame-drop-backdrop-v1246"></div><div class="frame-drop-card-v1246 ${rarityClass(item.rarity)}"><h3>장비 획득</h3><div class="frame-drop-item-v1246 ${subtypeClass(item)}">${itemImage(item)}</div><strong>${esc(item.name)}</strong><small>${esc(rarityLabels[normRarity(item.rarity)])} · PVE ${num(item.pvePower)} · PVP ${num(item.pvpPower)}</small><button type="button">확인</button></div>`;
