(() => {
  ...
  function view() {
    return `<section class="cp2-shell">
      ...
      <div class="cp2-stage">
        <header class="cp2-stage-head"><div><small class="cp2-eyebrow">PREDICTION OPERATIONS</small><h2 id="cpStageTitle">오늘의 승부를 선택하십시오</h2><p id="cpStageCopy">경기 목록과 선택 경기 정보를 분리해 많은 경기도 빠르게 탐색할 수 있습니다. 항목 선택부터 금액 확정까지 한 화면에서 완료됩니다.</p></div><div class="cp2-ruleline" id="cpRuleLine"><span>10% FEE</span><span>100,000 MIN</span><span>2,000,000,000 MAX</span></div></header>
  ...
  function render() {
    ...
    if (ruleLine) ruleLine.innerHTML = `<span>10% FEE</span><span>100,000 MIN</span><span>${ownerUnlimited ? 'OWNER UNLIMITED' : '2,000,000,000 MAX'}</span>`;
    ...
    submit?.addEventListener('click', async clickEvent => {
      ...
      if (!Number.isSafeInteger(amount) || amount < MIN_BET || amount + current > max) {
        return alert(unlimited
          ...
          : `최소 10만 코인이며 이벤트 누적 최대는 20억 코인입니다.\n현재 참여: ${fmt(current)}코인`);
      }
