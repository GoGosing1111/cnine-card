(()=>{
  ...
  function card(e){const open=e.status==='OPEN'&&state?.settings?.enabled!==false,mine=e.myBet,selected=Number(mine?.option_id||0),result=Number(e.result_option_id||0),max=Number(e.max_bet||3000000),remaining=Math.max(0,max-Number(mine?.amount||0)),remain=e.closes_at?Math.max(0,Date.parse(e.closes_at)-Date.now()):0,image=String(e.image_url||'').replace(/^\/+/,''),canAdd=remaining>=100000;return `<article class="cp-event ${open?'is-open':''}" data-cp-event="${e.id}" data-cp-current="${Number(mine?.amount||0)}">${image?`<div class="cp-event-cover"><img src="${esc(image)}" alt="" onerror="this.parentElement.remove()"></div>`:''}<div class="cp-event-head"><div><small>${eventStatus(e)} · ${fmt(e.participant_count)}명 참여</small><h2>${esc(e.title)}</h2><p>${esc(e.description||'결과를 예측하고 게임 코인으로 참여하세요.')}</p></div><div class="cp-clock" data-cp-end="${esc(e.closes_at||'')}"><b>${open?formatTime(remain):eventStatus(e)}</b><span>총 풀 ${fmt(e.total_pool)} COIN</span></div></div><div class="cp-options">${(e.options||[]).map((o,i)=>`<div class="cp-option-block"><button class="cp-option ${selected===Number(o.id)?'selected':''} ${result===Number(o.id)?'is-winner':''}" data-cp-option="${o.id}" ${open&&(!mine||selected===Number(o.id))?'':'disabled'}><i>${String(i+1).padStart(2,'0')}</i><span><b>${esc(o.label)}</b><small>${fmt(o.bet_count)}명 · ${fmt(o.total_bet)}코인${result===Number(o.id)?' · 적중 결과':''}</small></span><em>${predicted(e,o)}</em></button>${bettorLedger(e,o)}</div>`).join('')}</div>${mine?`<div class="cp-my-bet"><span>MY PREDICTION</span><b>${esc((e.options||[]).find(o=>Number(o.id)===selected)?.label||'')} · 누적 ${fmt(mine.amount)}코인</b><em>${mine.status==='SETTLED'?`수령 ${fmt(mine.payout)}코인`:mine.status==='REFUNDED'?`환불 ${fmt(mine.payout)}코인`:canAdd?`추가 가능 ${fmt(remaining)}코인`:'최대 참여 완료'}</em></div>`:''}${open&&canAdd?`<div class="cp-entry"><div><button data-cp-quick="100000">10만</button><button data-cp-quick="500000">50만</button><button data-cp-quick="1000000">100만</button><button data-cp-quick="${remaining}">잔여 전액</button></div><input type="number" min="100000" max="${remaining}" step="100000" placeholder="${mine?'추가 참여':'참여'} 코인 입력"><button class="cp-submit" data-cp-submit="${e.id}" disabled>${mine?'추가 참여':'예측 확정'}</button></div>`:''}</article>`}
