...
function ensureStyle(){if(document.getElementById('quest-hub-style'))return;const link=document.createElement('link');link.id='quest-hub-style';link.rel='stylesheet';link.href='/css/quest-hub-20260924.css?v=2';document.head.append(link)}
export async function mountQuestHub({root,api,onUser=()=>{},navigate=()=>{}}){
 ...
 function render(){
  ...
  root.innerHTML=`<section class="qh-hub" aria-label="일일·주간 퀘스트">
   ...
   <div class="qh-workspace"><aside class="qh-list"><div class="qh-list-head"><b>${weekly?'이번 주 미션':'오늘의 미션'}</b><span>수령 ${done} / ${list.length}</span></div>
    ...
    <article class="qh-detail" aria-label="선택한 퀘스트"><div class="qh-detail-top"><span>${icon(quest.id)} ${weekly?'주간':'일일'} 미션</span><b class="qh-status ${quest.claimed?'is-done':''}">${state(quest)}</b></div>
     ...
     <div class="qh-week" aria-label="${weekly?'월요일부터 일요일 주간 기록':'이번 주 달력 · 일일은 오늘 기록만 표시'}">${data.period.allDays.map((date,i)=>{const today=date===data.period.today,day=weekly?quest.days?.find(d=>d.date===date):today?{count:quest.count}:null;return `<div class="${today?'is-today':''} ${date>data.period.today?'is-future':''}"><small>${['월','화','수','목','금','토','일'][i]}</small><b>${date.slice(-2)}</b><span>${quest.id==='POST'?(day?number(day.count)+'개':'—'):(today?'오늘':'·')}</span></div>`}).join('')}</div>
     <p class="qh-record-note">${weekly?'월요일~일요일 누적 기록 · 퀘스트마다 주 1회 수령':'오늘 작성한 글만 집계 · 매일 00시 초기화 · 하루 1회 수령'}</p>
