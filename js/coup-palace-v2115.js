(function () {
  ...
  function hp(side, r) {
    ...
    return `<div class="coup-faction ${side.toLowerCase()}">${crest(side)}<div class="coup-faction-info"><div><strong>${sideName(side)}</strong><span>${recruiting ? state.mine?.side === side ? '내 진영' : '' : members + '명' + (state.mine?.side === side ? ' · 내 진영' : '')}</span></div><div class="coup-hp" role="meter" aria-label="${sideName(side)} 진영 체력" aria-valuemin="0" aria-valuemax="${r?.maxHp || 1}" aria-valuenow="${value || 0}"><i style="width:${amount}%"></i></div><small>${r ? `${num(value)} <em>/ ${num(r.maxHp)}</em>` : '참가 모집 대기'}</small></div></div>`;
  }
