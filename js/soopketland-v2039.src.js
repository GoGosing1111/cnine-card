...
function bindControls(s){
  ...
  host.querySelector('[data-sl-grant]')?.addEventListener('submit',async event=>{
    ...
    if(!confirm(`${label}\n이용권 ${quantity}개 · 당첨 쿠폰당 ${couponUses}명\n코인 최고 당첨 기준 쿠폰당 ${num(couponUses*Number(s.data.prizes.find(p=>p.key==='COIN')?.max||300)*100000000)}코인이 지급될 수 있습니다.\n이대로 지급할까요?`))return;
