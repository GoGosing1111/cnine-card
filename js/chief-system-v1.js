(()=>{
  ...
  function markup(){
    ...
    return `<section class="chief-main-card${c.isChief?' is-chief':''}" data-chief-status="ACTIVE"><div class="chief-art">${chiefArt('대의회에서 족장이 선출되는 장면',c.avatar)}<i></i></div><div class="chief-copy"><small>THE ELECTED CHIEF · PLAY DK 투표 선출</small><h2><em>${chiefTitle}</em> ${esc(c.nickname)}</h2><p>숲켓몬 대의회의 뜻을 받들어 7일간 부족을 이끕니다.</p><div class="chief-term"><span><b>${remaining(c.remainingMs)}</b> 남음</span><time>${new Date(c.endsAt).toLocaleString('ko-KR')}까지</time></div></div>${c.isChief?`<div class="chief-console"><header><span>족장 권한</span><small>버닝 운영은 OWNER 전용 CMS에서 관리됩니다</small></header><div class="chief-power-grid">${powerButton('TOWER_RESET','무한의 탑 초기화',`임기 중 ${towerResetCount}/${towerResetLimit}회`,towerResetCount>=towerResetLimit)}</div></div>`:`<div class="chief-public-powers"><span>임기 ${towerResetLimit}회 탑 초기화</span></div>`}</section>`;
  }
  function mount(){
  ...
  async function activate(type,button){
    const names={TOWER_RESET:'모든 유저의 무한의 탑 진행도 초기화'};
