(()=>{
  ...
  function markup(){
    ...
    const c=state?.chief;if(runtimeStatus==='VACANT'||!c?.active)return `<section class="chief-main-card vacant" data-chief-status="VACANT"><div><small>FOREST COUNCIL</small><h2>족장 선출 대기</h2><p>와이고수 투표 결과에 따라 CMS에서 차기 족장을 임명합니다.</p></div></section>`;
    const u=c.usage||{},l=c.limits||{};
    const burningToday=Number(u.burningToday||0),burningLimit=Math.max(1,Number(l.burningUsesPerDay||2));
    ...
    return `<section class="chief-main-card${c.isChief?' is-chief':''}" data-chief-status="ACTIVE"><div class="chief-art">${chiefArt('대의회에서 족장이 선출되는 장면')}<i></i></div><div class="chief-copy"><small>THE ELECTED CHIEF · 와이고수 투표 선출</small><h2><em>${chiefTitle}</em> ${esc(c.nickname)}</h2><p>숲켓몬 대의회의 뜻을 받들어 7일간 부족을 이끕니다.</p><div class="chief-term"><span><b>${remaining(c.remainingMs)}</b> 남음</span><time>${new Date(c.endsAt).toLocaleString('ko-KR')}까지</time></div></div>${c.isChief?`<div class="chief-console"><header><span>족장 권한</span><small>모든 제한은 서버에서 검증됩니다</small></header><div class="chief-power-grid">${powerButton('HYPER','하이퍼 버닝','오늘 1시간',Number(u.hyperToday||0)>=1)}${powerButton('BURNING','숲켓몬 버닝',`오늘 ${burningToday}/${burningLimit}회 · 3시간`,burningToday>=burningLimit)}${powerButton('TOWER_RESET','무한의 탑 초기화',`임기 중 ${towerResetCount}/${towerResetLimit}회`,towerResetCount>=towerResetLimit)}</div></div>`:`<div class="chief-public-powers"><span>매일 버닝 ${burningLimit}회</span><span>임기 ${towerResetLimit}회 탑 초기화</span></div>`}</section>`;
  }
  function mount(){
