...
globalThis.PveV3Runtime={async tryOpen(content){
  if(!destinations.has(content)||(await feature).enabled!==true)return false;
  if(content==='cow-room'){await cowPortalPrompt.recover();return true;}
  location.assign(`/pve-v3/?content=${encodeURIComponent(content)}`);return true;
}};
feature.then(value=>{
  if(value.enabled!==true)return;
  let available=0;
  const update=()=>{const button=document.querySelector('[data-v3-cow-entry]');if(button){button.hidden=available<1;button.textContent=`카우방 · 포탈 ${available}개`;}};
  ...
  const add=()=>{
    ...
    button.onclick=()=>void globalThis.PveV3Runtime.tryOpen('cow-room');nav.append(button);update();
    const mercenary=document.createElement('a');mercenary.className='pve-mode-btn';mercenary.href='/mercenary-hangar/';mercenary.textContent='용병 지휘소';nav.append(mercenary);
  };
