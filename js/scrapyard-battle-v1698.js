(()=>{
  ...
  window.playScrapyardBattleV1698=async(result,helpers={})=>{
    ...
    for(let wi=0;wi<waves.length&&!skip;wi++){
      ...
      await sleep(wave.boss?760:480);if(!alive())return;monster.classList.add('enter');stage.classList.add('cards-ready');await sleep(wave.boss?820:520);if(!alive())return;
      ...
      await sleep(wave.boss?880:520);if(!alive())return;stage.classList.remove('wave-clear');monster.classList.remove('enter');
    }
    if(alive()&&typeof showResult==='function')showResult(modal,result);
  };
})();
