(() => {
  ...
  function inject() {
    ...
    b.innerHTML = "<span>몬스터 공성전</span><small>6H</small>";
  ...
  function phaseTrack() {
    ...
    return `<nav class="ms-phase-track">${phases.map((p, i) => `<div class="${i < current ? "done" : i === current ? "active" : ""}"><i>${i < current ? "✓" : i + 1}</i><span>${esc(p.name)}</span><small>${i === 0 ? "0~1H" : i === 1 ? "1~2H" : i === 2 ? "2~4H" : i === 3 ? "4~5H" : "5~6H"}</small></div>`).join("")}</nav>`;
  }
  function ranking() {
  ...
  function reward() {
    const r = data.reward;
    if (!r) return "";
    return `<section class="ms-reward"><div><small>SIEGE SETTLEMENT</small><h3>공성전 보상 도착</h3><span>코인 ${fmt(r.coin)} · 카드 조각 ${fmt(r.shards)}</span></div><button data-ms-claim>보상 수령</button></section>`;
  }
  function render() {
    ...
    try {
      ...
      if (
        ...
        if (msg) {
          ...
          msg.innerHTML = `<small>${out.result === "WIN" ? "FRONTLINE VICTORY" : "DEFEAT · LINE DAMAGE"}</small><strong>${out.result === "WIN" ? "승리" : "패배"}</strong><b>전선 HP -${fmt(out.damagePercent)}%</b><span>${fmt(out.damage)} 피해 · 적 전투력 ${fmt(out.monsterPower)}</span>${out.result === "WIN" ? `<div class="siege-v2-win-reward"><small>1판 승리 보상</small><b>코인 ${fmt(out.winReward?.coin)} · 카드 조각 ${fmt(out.winReward?.shards)}</b></div>` : ""}<button type="button" data-siege-battle-confirm>공성전으로 돌아가기</button>`;
          renderer.showResult();
  ...
  async function claim() {
    ...
    try {
      ...
      alert(`코인 ${fmt(out.coin)} · 카드 조각 ${fmt(out.shards)} 수령 완료`);
      await load();
    } catch (e) {
      alert(e.message);
    } finally {
