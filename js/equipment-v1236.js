/* V1236 CHARACTER EQUIPMENT + TITLE UI */
(()=>{
  ...
  function titleRibbon(title,{small=false}={}){
    ...
    return `<div class="character-title-ribbon-v1236 ${small?'small':''} ${title?titleStyleClass(title.stylePreset):'title-style-default'} ${title?'owned':'empty'}"><span>[${esc(name)}]</span></div>`;
  }
  function itemIcon(item,slot,{large=false}={}){
    ...
    return `<div class="equip-icon-v1236 ${large?'large':''} ${item?rarityClass(item.rarity):'empty'}">${body}${item?`<b>${esc(rarityLabels[normRarity(item.rarity)])}</b>`:''}</div>`;
  }
  function heroHtml(){
    const d=state.data,u=user(),title=currentTitle();
    return `<section class="equip-hero-v1236"><div class="equip-hero-name-v1236"><small>SOOP EQUIPMENT</small><h2>${esc(u?.nickname||'플레이어')}</h2>${titleRibbon(title,{small:true})}</div><div class="equip-hero-stats-v1236"><span><small>PVE</small><b>+${num(d?.bonuses?.pve)}</b></span><span><small>PVP</small><b>+${num(d?.bonuses?.pvp)}</b></span><span><small>칭호</small><b>+${num(d?.bonuses?.titlePve)}</b></span></div></section>`;
  }
  function slotNode(slotId){
    ...
    return `<div class="equip-slot-node-v1236 ${item?'filled':''} ${item?rarityClass(item.rarity):''} ${state.slot===slotId?'focused':''}"><button type="button" class="equip-slot-hit-v1236" data-character-filter="${slotId}"></button><div class="equip-slot-label-v1236">${esc(slot.label)}</div>${itemIcon(item,slotId)}<div class="equip-slot-name-v1236">${esc(item?.name||'비어 있음')}</div>${item?`<button type="button" class="equip-slot-remove-v1236" data-character-unequip="${slotId}">×</button>`:''}</div>`;
  }
  function stageHtml(){
    const title=currentTitle();
    return `<section class="equip-stage-shell-v1236"><div class="equip-stage-board-v1236"><div class="equip-stage-column-v1236 left">${stageLayout.left.map(slotNode).join('')}</div><div class="equip-stage-center-v1236"><div class="equip-stage-frame-v1236"><div class="equip-stage-title-v1236">${titleRibbon(title)}</div><div class="equip-character-panel-v1236"><div class="equip-character-aura-v1236"></div><img src="assets/ui/character-avatar-modern-v1236.png" alt="캐릭터" class="equip-character-art-v1236" loading="lazy"></div><div class="equip-stage-bottomstats-v1236"><span><small>장비 PVE</small><b>+${num(state.data?.bonuses?.equipmentPve)}</b></span><span><small>장비 PVP</small><b>+${num(state.data?.bonuses?.equipmentPvp)}</b></span></div></div></div><div class="equip-stage-column-v1236 right">${stageLayout.right.map(slotNode).join('')}</div></div><div class="equip-loadout-strip-v1236">${(state.data?.slots||[]).map(slot=>{const row=equippedFor(slot.id),item=row?.item;return `<div class="equip-loadout-cell-v1236 ${item?'filled':''}"><small>${esc(slot.label)}</small><b>${esc(item?.name||'-')}</b></div>`}).join('')}</div></section>`;
  }
  function inventoryHtml(){
    ...
    return `<section class="equip-panel-v1236"><div class="equip-panel-head-v1236"><h3>장비</h3><div class="equip-filter-row-v1236"><button class="${filter==='ALL'?'active':''}" data-character-filter="ALL">전체</button>${(state.data?.slots||[]).map(slot=>`<button class="${filter===slot.id?'active':''}" data-character-filter="${slot.id}">${esc(slot.label)}</button>`).join('')}</div></div><div class="equip-grid-v1236">${list.length?list.map(row=>{const i=row.item;return `<article class="equip-item-tile-v1236 ${row.equipped?'equipped':''} ${rarityClass(i.rarity)}"><div class="equip-item-top-v1236">${itemIcon(i,i.slot)}<button type="button" ${row.equipped?'disabled':''} data-character-equip="${row.instanceId}">${row.equipped?'장착':'착용'}</button></div><div class="equip-item-meta-v1236"><strong>${esc(i.name)}</strong><small>${esc(i.slotLabel)} · ${esc(subtypeLabels[i.subtype]||i.subtype)}</small><div class="equip-chip-row-v1236"><i>${esc(rarityLabels[normRarity(i.rarity)]||i.rarity)}</i><i>PVE ${num(i.pvePower)}</i><i>PVP ${num(i.pvpPower)}</i></div></div></article>`}).join(''):'<div class="character-empty-list-v1236">보유 장비 없음</div>'}</div></section>`;
  }
  ...
  function titlesHtml(){
    ...
    const body=titles.length?titles.map(t=>{
      const icon=t.image?`<div class="equip-title-icon-v1236"><img src="${esc(t.image)}" alt=""></div>`:'<div class="equip-title-icon-v1236"><span>♛</span></div>';
      ...
      return `<article class="equip-title-tile-v1236 ${t.owned?'owned':'locked'} ${t.equipped?'equipped':''}"><div class="equip-title-head-v1236">${titleRibbon(t,{small:true})}${icon}</div><div class="equip-chip-row-v1236"><i>${esc(titleStyleLabels[normTitlePreset(t.stylePreset)]||'기본')}</i><i>PVE ${num(t.pvePower)}</i><i>${esc(t.owned?(unlockLabels[t.unlockType]||t.unlockType):titleCondition(t))}</i></div>${action}</article>`;
    }).join(''):'<div class="character-empty-list-v1236">등록된 칭호 없음</div>';
    return `<section class="equip-panel-v1236"><div class="equip-panel-head-v1236"><h3>칭호</h3><div class="equip-title-count-v1236">${ownedCount}/${titles.length}</div></div><div class="equip-title-grid-v1236">${body}</div></section>`;
  }
  function render(){
    const root=document.getElementById('characterSystemRoot');if(!root)return;
    if(state.loading&&!state.data){root.innerHTML='<div class="character-loading-v1236"><span></span><b>불러오는 중...</b></div>';return}
    if(!state.data){root.innerHTML='<div class="character-empty-list-v1236">장비 정보를 불러오지 못했습니다.</div>';return}
    root.innerHTML=heroHtml()+stageHtml()+inventoryHtml()+titlesHtml();
    root.querySelectorAll('[data-character-filter]').forEach(button=>button.onclick=()=>{state.slot=button.dataset.characterFilter;render();document.querySelector('.equip-grid-v1236')?.scrollIntoView({behavior:'smooth',block:'center'})});
  ...
  async function load(){
    ...
    try{state.data=await request('character/loadout');render()}catch(error){const root=document.getElementById('characterSystemRoot');if(root)root.innerHTML=`<div class="character-empty-list-v1236"><b>장비 정보를 불러오지 못했습니다.</b><p>${esc(error.message)}</p><button type="button" id="characterRetry">다시 불러오기</button></div>`;document.getElementById('characterRetry')?.addEventListener('click',load)}finally{state.loading=false}
  }
  window.characterView=function(characterUser){return `${typeof summaryBar==='function'?summaryBar(characterUser):''}<section id="characterSystemRoot" class="character-system-root-v1236"><div class="character-loading-v1236"><span></span><b>불러오는 중...</b></div></section>`};
  ...
  window.showEquipmentDropReward=async function(reward){
    ...
    toast.className=`equipment-drop-toast-v1236 ${rarityClass(item.rarity)}`;
    toast.innerHTML=`<div class="equipment-drop-backdrop-v1236"></div><div class="equipment-drop-toast-card-v1236"><div class="equipment-drop-title-v1236">장비 획득</div>${itemIcon(item,item.slot,{large:true})}<strong>${esc(item.name)}</strong><div class="equip-chip-row-v1236 centered"><i>${esc(item.slotLabel)}</i><i>${esc(subtypeLabels[item.subtype]||item.subtype)}</i><i>${esc(rarityLabels[normRarity(item.rarity)]||item.rarity)}</i></div><div class="equip-chip-row-v1236 centered"><i>총 ${num(item.totalPower)}</i><i>PVE ${num(item.pvePower)}</i><i>PVP ${num(item.pvpPower)}</i></div><button type="button">확인</button></div>`;
