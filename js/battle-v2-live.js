(() => {
  ...
  function createRenderer({ stage, phase, msg, modal, data, mode, monster, playUltimateCinematics = true }) {
    ...
    async function eventPlay(event) {
      ...
      if (event.type === 'PVE_ULTIMATE') { const target=fighterState(event.targetId);focusTarget(event.targetId);if (playUltimateCinematics && data.activatedUltimate && mode==='PVE') { const ult={...data.activatedUltimate,playbackRate:PLAYBACK_SPEED,durationMs:Math.max(500,Math.round(Number(data.activatedUltimate.durationMs||3000)/PLAYBACK_SPEED))};try{await playBattleUltimate(root,ult,event.damage||data.ultimateDamage)}catch{}}updateCard(event.targetId,{hp:event.targetHpAfter,shield:event.targetShieldAfter});const node=fighterNode(event.targetId);impactFx(fighterNode(state.activeAId),node,'attack','ULTIMATE');burstFx(node,'attack',true);damageNumber(node,`-${number(Number(event.damage||0)+Number(event.absorbed||0))}`,'critical');setMessage('ULTIMATE','ULTIMATE HIT',target?.title||'MONSTER');await sleep(Math.max(420, base));return; }
