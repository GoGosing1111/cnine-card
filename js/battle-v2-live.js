  ...
  function createRenderer({ stage, phase, msg, modal, data, mode, monster, playUltimateCinematics = true }) {
    ...
    function startMagicWebGL(canvas,targetNode,kind,effectType){
      ...
      const draw=now=>{if(dead)return;const dpr=resize(),rect=canvas.getBoundingClientRect(),tr=targetNode?.getBoundingClientRect?.(),margin=Math.min(canvas.width,canvas.height)*.16,rawX=((tr?tr.left+tr.width/2:rect.left+rect.width*.7)-rect.left)*dpr,rawY=(rect.bottom-(tr?tr.top+tr.height/2:rect.top+rect.height*.45))*dpr,x=Math.max(margin,Math.min(canvas.width-margin,rawX)),y=Math.max(margin,Math.min(canvas.height-margin,rawY)),u=Math.min(1,(now-started)/1050);gl.viewport(0,0,canvas.width,canvas.height);gl.clearColor(0,0,0,0);gl.clear(gl.COLOR_BUFFER_BIT);gl.enable(gl.BLEND);gl.blendFunc(gl.ONE,gl.ONE_MINUS_SRC_ALPHA);
        ...
        gl.useProgram(particles);gl.bindBuffer(gl.ARRAY_BUFFER,cloud);const ps=gl.getAttribLocation(particles,'s');gl.enableVertexAttribArray(ps);gl.vertexAttribPointer(ps,3,gl.FLOAT,false,0,0);gl.uniform2f(gl.getUniformLocation(particles,'r'),canvas.width,canvas.height);gl.uniform2f(gl.getUniformLocation(particles,'t'),x,y);gl.uniform1f(gl.getUniformLocation(particles,'u'),u);gl.uniform1f(gl.getUniformLocation(particles,'m'),mode);gl.uniform3fv(gl.getUniformLocation(particles,'c'),color);gl.drawArrays(gl.POINTS,0,220);if(u<1)frame=requestAnimationFrame(draw);else stop();};frame=requestAnimationFrame(draw);
      return stop;
    }

    async function playMagicCardSummon(event,kind){
      const effectType=String(event.effectType||'').toUpperCase(),resource=MAGIC_EFFECT_RESOURCES[effectType],effect=effectType.toLowerCase().replace(/_/g,'-'),level=Math.max(0,Math.min(9,Number(event.magicEnhancementLevel||0))),signature=Boolean(resource),overlay=document.createElement('div');
      kind=resource?.kind||kind;
      overlay.className=`magic-card-cinematic magic-kind-${kind} magic-effect-${effect} magic-enhance-${level} ${signature?'magic-signature-resource':'magic-generic-resource'}`;
      overlay.innerHTML=`<div class="magic-cinematic-dim"></div>${signature?'':`<canvas class="magic-gl-layer" aria-hidden="true"></canvas><div class="magic-cinematic-runes"><i></i><i></i><i></i></div><div class="magic-target-seal"><i>ᚱ</i><i>ᛉ</i><i>ᚨ</i><i>ᛟ</i><b></b></div>`}<div class="magic-unique-stage" aria-hidden="true">${'<i></i>'.repeat(12)}<b></b></div><div class="magic-cinematic-card${event.magicImageUrl?' has-card-art':''}"><div class="magic-cinematic-frame"><div class="magic-cinematic-art">${event.magicImageUrl?`<img src="${assetUrl(event.magicImageUrl)}" alt="${esc(event.magicName||'마법카드')}" onerror="this.closest('.magic-cinematic-card')?.classList.remove('has-card-art');this.remove()">`:'<b>✦</b>'}</div><small>강화 +${level} · MAGIC CARD</small><strong>${esc(event.magicName||event.magicCode||'마법카드')}</strong></div></div>${signature?'':`<div class="magic-cinematic-release"><i></i><i></i><i></i><b>${esc(event.magicName||'MAGIC')}</b></div>`}`;
    ...
    async function eventPlay(event) {
      ...
      if (event.type === 'BOSS_ULTIMATE') { const apocalypse=Boolean(data.bossUltimate?.apocalypseExclusive||data.bossUltimateState?.apocalypseExclusive||data.difficulty?.isApocalypse||data.monster?.apocalypse||String(data.difficulty?.difficulty||data.monster?.difficulty||'').toUpperCase()==='APOCALYPSE');if(!apocalypse&&playUltimateCinematics&&data.bossUltimate&&mode==='PVE'){const ult={...data.bossUltimate,playbackRate:PLAYBACK_SPEED,durationMs:Math.max(500,Math.round(Number(data.bossUltimate.durationMs||2400)/PLAYBACK_SPEED))};try{await playBossBattleUltimate(root,preservedPhase,ult)}catch{}}for(const hit of event.hits||[])updateCard(hit.targetId,{hp:hit.targetHpAfter,shield:hit.targetShieldAfter});setMessage(apocalypse?'APOCALYPSE SKILL':'BOSS ULTIMATE',data.bossUltimate?.name||'광역 공격',`${(event.hits||[]).length}명 타격`);await sleep(Math.max(440, base));return; }
      if (event.type === 'MONSTER_MULTI_ATTACK_READY') { focusTarget(event.actorId);setMessage('APOCALYPSE ASSAULT',`${Number(event.attackCount||2)}연속 공격`,event.label||'몬스터가 연속 행동을 준비합니다.');await sleep(base*.55);return; }
    ...
    return {
      ...
      async previewMagicEffect(card={}) {
        ...
        try{
          ...
          await playMagicCardSummon({actorId:actor.id,targetId:target.id,effectType,magicName:card.name||resource.label,magicCode:card.code||effectType,magicImageUrl:card.imageUrl||card.image_url||'',magicEnhancementLevel:Number(card.enhancementLevel||0)},resource.kind);
