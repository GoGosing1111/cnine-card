(() => {
  ...
  function createRenderer({ stage, phase, msg, modal, data, mode, monster, playUltimateCinematics = true }) {
    ...
    async function playMagicCardSummon(event,kind){
      ...
      overlay.innerHTML=`<div class="magic-cinematic-dim"></div><div class="magic-cinematic-runes"><i></i><i></i><i></i></div><div class="magic-cinematic-card"><div class="magic-cinematic-frame"><div class="magic-cinematic-art">${event.magicImageUrl?`<img src="${assetUrl(event.magicImageUrl)}" alt="${esc(event.magicName||'마법카드')}" onerror="this.remove()">`:'<b>✦</b>'}</div><small>${esc(rarity)} · MAGIC CARD</small><strong>${esc(event.magicName||event.magicCode||'마법카드')}</strong></div></div><div class="magic-cinematic-release"><i></i><i></i><i></i><b>${esc(event.magicName||'MAGIC')}</b></div>`;
      root.appendChild(overlay);void overlay.offsetWidth;overlay.classList.add('is-playing');setMessage('MAGIC CARD',event.magicName||event.magicCode||'마법카드','마력이 전장에 전개됩니다.');
      await sleep(1120);overlay.classList.add('is-releasing');await sleep(430);overlay.remove();
    }
    ...
    async function eventPlay(event) {
      ...
      if(event.type==='MAGIC_CARD'){
        ...
        if(event.damage||event.absorbed){impactFx(actorNode,targetNode,kind,'MAGIC');burstFx(targetNode,kind,true);damageNumber(targetNode,`-${number(Number(event.damage||0)+Number(event.absorbed||0))}`,'critical');}
