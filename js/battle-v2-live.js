(() => {
  ...
  function createRenderer({ stage, phase, msg, modal, data, mode, monster, playUltimateCinematics = true }) {
    ...
    function startMagicWebGL(canvas,targetNode,kind,effectType){
      ...
      const draw=now=>{if(dead)return;const dpr=resize(),rect=canvas.getBoundingClientRect(),tr=targetNode?.getBoundingClientRect?.(),x=((tr?tr.left+tr.width/2:rect.left+rect.width*.7)-rect.left)*dpr,y=(rect.bottom-(tr?tr.top+tr.height/2:rect.top+rect.height*.45))*dpr,u=Math.min(1,(now-started)/1050);gl.viewport(0,0,canvas.width,canvas.height);gl.clearColor(0,0,0,0);gl.clear(gl.COLOR_BUFFER_BIT);gl.enable(gl.BLEND);gl.blendFunc(gl.ONE,gl.ONE_MINUS_SRC_ALPHA);
    ...
    async function playMagicCardSummon(event,kind){
      ...
      overlay.innerHTML=`<div class="magic-cinematic-dim"></div><canvas class="magic-gl-layer" aria-hidden="true"></canvas><div class="magic-cinematic-runes"><i></i><i></i><i></i></div><div class="magic-target-seal"><i>ᚱ</i><i>ᛉ</i><i>ᚨ</i><i>ᛟ</i><b></b></div><div class="magic-cinematic-card"><div class="magic-cinematic-frame"><div class="magic-cinematic-art">${event.magicImageUrl?`<img src="${assetUrl(event.magicImageUrl)}" alt="${esc(event.magicName||'마법카드')}" onerror="this.remove()">`:'<b>✦</b>'}</div><small>${esc(rarity)} · MAGIC CARD</small><strong>${esc(event.magicName||event.magicCode||'마법카드')}</strong></div></div><div class="magic-cinematic-release"><i></i><i></i><i></i><b>${esc(event.magicName||'MAGIC')}</b></div>`;
      ...
      await sleep(1120);overlay.classList.add('is-releasing');const targetNode=fighterNode(event.targetId||event.actorId),stopGL=startMagicWebGL(overlay.querySelector('.magic-gl-layer'),targetNode,kind,event.effectType),targetPoint=pointFor(targetNode);overlay.style.setProperty('--magic-target-x',`${targetPoint.x}px`);overlay.style.setProperty('--magic-target-y',`${targetPoint.y}px`);await sleep(1900);stopGL();overlay.remove();
    }
