(()=>{
  ...
  async function startFight(fromAuto=false){
    ...
    try{
      ...
      if(d.bossUltimate){const ult=d.bossUltimate;stage.dataset.bossUltimateTheme=String(ult.theme||'CRIMSON').toLowerCase();stage.classList.add('boss-ultimate-cast');phase.textContent=ult.warningText||'BOSS ULTIMATE';const banner=document.createElement('div');banner.className='boss-ultimate-banner';banner.innerHTML=`<small>${esc(ult.warningText||'BOSS ULTIMATE')}</small><strong>${esc(ult.name||'ULTIMATE')}</strong>${ult.description?`<span>${esc(ult.description)}</span>`:''}`;stage.appendChild(banner);const src=String(ult.mediaUrl||'').trim()?(typeof normalizeUltimateMediaPath==='function'?normalizeUltimateMediaPath(ult.mediaUrl):String(ult.mediaUrl||'')):'';let media=null;if(src){media=document.createElement('div');media.className='boss-ultimate-resource';const video=/\.(mp4|webm)(?:[?#].*)?$/i.test(src);media.innerHTML=video?`<video src="${esc(src)}" autoplay ${ult.soundUrl?'muted':''} playsinline></video>`:`<img src="${esc(src)}" alt="">`;stage.appendChild(media);media.querySelector('video')?.play?.().catch(()=>{});}let audio=null;const snd=String(ult.soundUrl||'').trim()?(typeof normalizeUltimateMediaPath==='function'?normalizeUltimateMediaPath(ult.soundUrl):String(ult.soundUrl||'')):'';if(snd&&typeof battleSoundEnabled==='function'&&battleSoundEnabled()){audio=new Audio(snd);audio.volume=.9;audio.play().catch(()=>{});}if(ult.zoom)stage.classList.add('boss-ultimate-zoom');if(ult.shake)stage.classList.add('boss-ultimate-shake');if(navigator.vibrate)navigator.vibrate([140,55,190,60,120]);await battleSleep(Math.max(700,Math.min(5000,Number(ult.durationMs||2400)*.45)));teamHp=Math.max(0,teamHp-Math.max(12,Math.min(55,Number(ult.damagePercent||15))));battleSetHp(stage,'team',teamHp);stage.querySelectorAll('.battle-card-fighter').forEach(el=>el.classList.add('boss-ultimate-hit'));battleBurst(stage,'30%','43%',52);battleDamage(stage,ult.penalty?`-${Math.floor(Number(ult.penalty)).toLocaleString()}`:'ULTIMATE HIT','player',true);phase.textContent=`${ult.name||'BOSS ULTIMATE'} · HIT`;await battleSleep(Math.max(650,Math.min(3500,Number(ult.durationMs||2400)*.35)));if(audio){audio.pause();audio.currentTime=0;}media?.remove();banner.remove();stage.classList.remove('boss-ultimate-cast','boss-ultimate-zoom','boss-ultimate-shake');stage.querySelectorAll('.battle-card-fighter').forEach(el=>el.classList.remove('boss-ultimate-hit'));}
