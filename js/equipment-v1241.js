/* V1241 IMAGE-FRAME EQUIPMENT + TITLE UI */
(()=>{
  ...
  function slotOverlay(slot){
    ...
    const description=item?(subtypeLabels[item.subtype]||item.name):slotDefaults[slot];
    return `<div class="frame-slot-v1241 slot-${slot.toLowerCase()} ${item?'filled':''} ${item?rarityClass(item.rarity):''}">
      <button type="button" class="frame-slot-hit-v1241" data-character-filter="${slot}" aria-label="${esc(slotLabels[slot])}"></button>
      ${item?`<div class="frame-slot-image-v1241">${itemImage(item)}</div>`:''}
      <div class="frame-slot-desc-mask-v1241"><span>${esc(description)}</span></div>
      ${item?`<button type="button" class="frame-slot-remove-v1241" data-character-unequip="${slot}" aria-label="해제">×</button>`:''}
    </div>`;
  }
  function equipmentLayer(){
    const b=state.data?.bonuses||{},title=currentTitle();
    return `<div class="frame-equipment-layer-v1241">
      <div class="frame-nickname-v1241" data-dynamic-nickname>${esc(nicknameText())}</div>
      <div class="frame-title-v1241 ${titleClass(title?.stylePreset)}" data-dynamic-title>[${esc(titleText(title))}]</div>
      ${slotOrder.map(slotOverlay).join('')}
      <div class="frame-stat-box-v1241 pve" data-dynamic-pve><small>PVE</small><b>+${num(b.pve)}</b></div>
      <div class="frame-stat-box-v1241 pvp" data-dynamic-pvp><small>PVP</small><b>+${num(b.pvp)}</b></div>
      ${inventoryLayer()}
    </div>`;
  }
  function inventoryLayer(){
    ...
    return `<div class="frame-inventory-layer-v1241">
      <div class="frame-inventory-head-v1241"><b>보유 장비</b><span>${all.length}</span></div>
      <div class="frame-filter-row-v1241">
      ...
      <div class="frame-item-grid-v1241">
        ${rows.length?rows.map(row=>{const i=row.item;return `<button type="button" class="frame-item-card-v1241 ${row.equipped?'equipped':''} ${rarityClass(i.rarity)}" ${row.equipped?'disabled':''} data-character-equip="${row.instanceId}">
          <div class="frame-item-thumb-v1241">${itemImage(i)}</div>
          <strong>${esc(i.name)}</strong>
          <small>${esc(rarityLabels[normRarity(i.rarity)])} · PVE ${num(i.pvePower)} · PVP ${num(i.pvpPower)}</small>
        </button>`}).join(''):'<div class="frame-empty-v1241">보유 장비<br>없음</div>'}
      </div>
    </div>`;
  }
  ...
  function titleLayer(){
    ...
    return `<div class="frame-title-layer-v1241">
      <div class="frame-nickname-v1241" data-dynamic-nickname>${esc(nicknameText())}</div>
      <div class="frame-title-current-v1241 ${titleClass(equippedTitle?.stylePreset)}">
      ...
      <div class="frame-title-list-v1241">
        ${rows.length?rows.map(row=>{
          ...
          return `<article class="frame-title-card-v1241 ${row.owned?'owned':'locked'} ${row.equipped?'equipped':''} ${titleClass(row.stylePreset)}"><strong>[${esc(row.badgeText||row.name)}]</strong><small>${esc(row.owned?(unlockLabels[row.unlockType]||row.unlockType):titleRequirement(row))}</small><b>PVE +${num(row.pvePower)}</b>${action}</article>`;
        }).join(''):'<div class="frame-empty-v1241">등록된 칭호 없음</div>'}
      </div>
    </div>`;
  }
  function shellHtml(){
    return `<div class="image-frame-ui-v1241">
      <div class="frame-background-v1241"></div>
      <button type="button" class="frame-tab-hit-v1241 equipment ${state.tab==='equipment'?'active':''}" data-character-tab="equipment" aria-label="장비"></button>
      <button type="button" class="frame-tab-hit-v1241 title ${state.tab==='title'?'active':''}" data-character-tab="title" aria-label="칭호"></button>
  ...
  function render(){
    ...
    if(state.loading&&!state.data){root.innerHTML='<div class="frame-loading-v1241"><span></span><b>불러오는 중...</b></div>';return}
    if(!state.data){root.innerHTML='<div class="frame-empty-v1241">장비 정보를 불러오지 못했습니다.</div>';return}
  ...
  async function load(){
    ...
    catch(error){const root=document.getElementById('characterSystemRoot');if(root)root.innerHTML='<div class="frame-empty-v1241">장비 정보를 불러오지 못했습니다.</div>'}
    finally{state.loading=false}
  }
  window.characterView=()=>'<section id="characterSystemRoot" class="character-system-root-v1241"><div class="frame-loading-v1241"><span></span><b>불러오는 중...</b></div></section>';
  ...
  window.showEquipmentDropReward=async reward=>{
    ...
    const toast=document.createElement('div');toast.id='equipmentDropToast';toast.className='frame-drop-toast-v1241';
    toast.innerHTML=`<div class="frame-drop-backdrop-v1241"></div><div class="frame-drop-card-v1241 ${rarityClass(item.rarity)}"><h3>장비 획득</h3><div class="frame-drop-item-v1241">${itemImage(item)}</div><strong>${esc(item.name)}</strong><small>${esc(rarityLabels[normRarity(item.rarity)])} · PVE ${num(item.pvePower)} · PVP ${num(item.pvpPower)}</small><button type="button">확인</button></div>`;
