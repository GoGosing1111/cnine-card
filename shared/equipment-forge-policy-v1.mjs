...
export function forgeRuntimeDraft(){return {revision:0,version:'forge-runtime-draft-20260913',mode:'OFF',approved:false,quoteSeconds:120,
 steps:Array.from({length:10},(_,i)=>({level:i,successPpm:null,maintainPpm:null,destroyPpm:null,coinCost:null,itemCode:null,itemQuantity:null,protectionQuantity:null})),
...
export function validateForgePolicy(raw){
 ...
 const steps=raw.steps.map((r,i)=>{if(r.level!==i)fail('강화 단계가 중복되거나 순서가 잘못됐습니다.');const n={level:i,successPpm:int(r.successPpm,100000,1000000,'성공률 (최소 10%)',true),maintainPpm:int(r.maintainPpm,0,1000000,'유지율',true),destroyPpm:int(r.destroyPpm,0,1000000,'파괴율',true),coinCost:int(r.coinCost,0,1e12,'코인 비용',true),itemCode:code(r.itemCode),itemQuantity:int(r.itemQuantity,1,1e8,'재료 수량',true),protectionQuantity:int(r.protectionQuantity,1,10000,'보호권 수량',true)};
 if([n.successPpm,n.maintainPpm,n.destroyPpm].some(x=>x!==null)&&[n.successPpm,n.maintainPpm,n.destroyPpm].reduce((s,n)=>s+(n??0),0)!==1000000)fail('단계별 확률 합계는 100%여야 합니다.');if((n.itemCode===null)!==(n.itemQuantity===null))fail('강화 재료와 수량을 함께 입력하세요.');return n;});
 ...
 const approvedSources=sources.map((s,i)=>{if(s.content!==FORGE_PROTECTION_SOURCES[i]||typeof s.enabled!=='boolean')fail('보호권 획득처를 확인하세요.');const next={content:s.content,enabled:s.enabled,chancePpm:int(s.chancePpm,1,1000000,'보호권 획득 확률',!s.enabled),quantity:int(s.quantity,1,100,'보호권 획득량',!s.enabled)};if(s.enabled&&!p.itemCode)fail('보호권 코드를 먼저 설정하세요.');return next;});
