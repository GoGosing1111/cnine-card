...
export const MERCENARY_GUARD_BASIC_SCALE=.5;
export const isMercenaryGuardSkill=skill=>skill?.mechanic==='INTERCEPT_ONE_HIT';
export const MERCENARY_GUARD_SUMMARY='자신을 제외한 최저 HP 비율 아군을 현재 행동에서 호위하며 50% 위력의 기본 공격을 함께 수행합니다. 보호 대상의 행동을 기준으로 유지하며, 유효한 호위가 남아 있으면 중복 시전하지 않고 기본 공격합니다.';
export function mercenaryGuardSkillText(skill){
 ...
 return {...skill,trigger:'자원과 재사용 대기 조건을 충족하면 자신을 제외한 최저 HP 비율 아군을 즉시 호위합니다.',
  effect:'호위 설치와 50% 위력의 기본 공격을 함께 수행합니다. 유지 시간은 보호 대상의 행동으로 계산합니다. 첫 단일 기본 공격 또는 단일 용병 공격 스킬의 피해 일부를 대신 받습니다. 호위는 한 번만 작동하며 보호자가 실제로 받은 피해만 이전됩니다. 유효한 호위가 남아 있으면 기본 공격합니다.',
  ...
  procRule:'호위 설치와 50% 위력의 기본 공격을 한 행동에서 수행합니다. 설치 행동의 공격은 자원을 회복하지 않습니다. 비용과 재사용 대기는 설치할 때 한 번 적용하며, 피해 이전은 추가 공격·자원 회복·연쇄 호위를 만들지 않습니다.'};
}
