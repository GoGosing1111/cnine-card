...
export function expandMercenarySkillCatalog(document, defaults, catalog) {
  ...
  if(catalog.cards.some(c=>c.code==='V-048')&&!document?.mercenaries?.some(c=>c.code==='V-048')){
    ...
    const previous=expandMercenarySkillCatalog(document,defaults,previousCatalog);
    return validateMercenaryCms({...previous,
      mercenaries:[...previous.mercenaries,structuredClone(defaults.mercenaries.find(c=>c.code==='V-048'))],
      skills:[...previous.skills,structuredClone(defaults.skills.find(s=>s.id==='MS-048'))],
      assignments:[...previous.assignments,structuredClone(defaults.assignments.find(c=>c.code==='V-048'))]},catalog);
  }
  if(catalog.cards.some(c=>c.code==='V-047')&&!document?.mercenaries?.some(c=>c.code==='V-047')){
    const previousCatalog={...catalog,cards:catalog.cards.filter(c=>c.code!=='V-047'),skills:catalog.skills.filter(s=>s.id!=='MS-047')};
    const previous=expandMercenarySkillCatalog(document,defaults,previousCatalog);
    return validateMercenaryCms({...previous,
      mercenaries:[...previous.mercenaries,structuredClone(defaults.mercenaries.find(c=>c.code==='V-047'))],
      skills:[...previous.skills,structuredClone(defaults.skills.find(s=>s.id==='MS-047'))],
      assignments:[...previous.assignments,structuredClone(defaults.assignments.find(c=>c.code==='V-047'))]},catalog);
  }
  ...
  if(catalog.cards.some(c=>c.code===MANGISA_CODE)&&!document?.mercenaries?.some(c=>c.code===MANGISA_CODE)){
    ...
    const previous=expandMercenarySkillCatalog(document,defaults,previousCatalog);
    return validateMercenaryCms({...previous,
      mercenaries:[...previous.mercenaries,structuredClone(defaults.mercenaries.find(c=>c.code===MANGISA_CODE))],
      skills:[...previous.skills,structuredClone(defaults.skills.find(s=>s.id===MANGISA_SKILL_ID))],
      assignments:[...previous.assignments,structuredClone(defaults.assignments.find(c=>c.code===MANGISA_CODE))]},catalog);
  }
  if(catalog.cards.some(c=>c.code===HEEYA_CODE)&&!document?.mercenaries?.some(c=>c.code===HEEYA_CODE)){
    const previousCatalog={...catalog,cards:catalog.cards.filter(c=>c.code!==HEEYA_CODE),skills:catalog.skills.filter(s=>s.id!==HEEYA_SKILL_ID)};
    const previous=expandMercenarySkillCatalog(document,defaults,previousCatalog);
    return validateMercenaryCms({...previous,
      mercenaries:[...previous.mercenaries,structuredClone(defaults.mercenaries.find(c=>c.code===HEEYA_CODE))],
      skills:[...previous.skills,structuredClone(defaults.skills.find(s=>s.id===HEEYA_SKILL_ID))],
      assignments:[...previous.assignments,structuredClone(defaults.assignments.find(c=>c.code===HEEYA_CODE))]},catalog);
  }
