...
export const COUP_REBEL_SKILLS = Object.freeze(COUP_CHIEF_SKILLS.filter(s => s.code !== 'NUCLEAR').map(s => ({ ...s,
  label: s.code === 'ARTILLERY' ? '현재 전선 · 족장팀' : '반란군 전체',
  detail: s.code === 'ARTILLERY' ? '현재 전선 족장팀의 최대 HP 30%만큼 피해를 줍니다. HP가 소진되면 기존 전선 돌파 규칙이 적용됩니다.' : '참가한 반란군 전원의 행동력을 50으로 채웁니다. 10 이상에서는 자연 회복이 멈추며, 출격마다 1씩 소모합니다.'
})));
export function coupRebelCommanderId(roundId, settings) {
