import {createBerkanSkill} from './mercenary-berkan-v1.mjs';
import {createSniperOrikkungSkill} from './mercenary-sniper-orikkung-v1.mjs';
import {createNurseSkill} from './mercenary-nurse-healers-v1.mjs';
...
import {createBikiniJoeunSkill} from './mercenary-bikini-joeun-v1.mjs';
import {createRagnielSkill} from './mercenary-ragniel-v1.mjs';
...
export const MERCENARY_SKILLS = [
  ...
  definition('MS-021', '종언의 사건지평선', 'VANGUARD', 'FRONT_GROUP', 'RIFT_MARK_DETONATION',
    ...
    '두 타격 사이 균열을 정화하거나 시전자를 제압한다. 표적이 먼저 쓰러지면 남은 몫은 다른 전열 적으로 옮겨 터진다. 과부하로 다음 행동이 늦어지지는 않는다.',
  ...
  definition('MS-003', '황금 구명선', 'GUARDIAN', 'ALLY_LOW_HP', 'INTERCEPT_ONE_HIT',
    'HP 비율이 가장 낮은 아군에게 단일 직접 공격이 예고되면 호위한다.',
    '첫 타격의 피해 일부를 시전자가 대신 받는다. 피해 총량은 늘지 않으며 한 번 막으면 연결이 끊어진다.',
  ...
  definition('MS-004', '루비 최후통첩', 'SNIPER', 'BACK_THREAT', 'LOCKED_THREAT_SHOT',
  ...
  definition('MS-006', '흑철 해체', 'VANGUARD', 'FRONT_ENEMY', 'BREAK_ARMOR_WINDOW',
  ...
  definition('MS-005', '탄착 교정', 'MARKSMAN', 'FRONT_ENEMY', 'SAME_TARGET_CALIBRATION',
  ...
  definition('MS-015', '독사 그림자', 'ASSASSIN', 'BACK_THREAT', 'INFILTRATE_DELAYED_VENOM',
  ...
  definition('MS-016', '황혼 봉인', 'CONTROLLER', 'FRONT_GROUP', 'FRONT_OFFENSE_VEIL',
  ...
  definition('MS-018', '생명선 봉합', 'SUPPORT', 'ALLY_LOW_HP', 'CLEANSE_THEN_MEND',
  ...
  definition('MS-023', '푸른 응수', 'VANGUARD', 'FRONT_ENEMY', 'MELEE_PARRY_RIPOSTE',
  ...
  definition('MS-025', '백색 정적', 'SNIPER', 'BACK_THREAT', 'UNDISTURBED_FIRST_SHOT',
  ...
  definition('MS-027', '폐쇄 사선', 'CONTROLLER', 'FRONT_GROUP', 'ADVANCE_SUPPRESSION',
    '적 전열의 접근 경로에 기관총 사격 구역을 만든다.',
    '전열 두 명까지 진입 동작을 지연시킨다. 범위 사격의 전체 예산을 나누며 적 수만큼 단일 피해를 복제하지 않는다.',
    '후열 사격과 고정 위치 기술은 억제 대상이 아니다. 준비 중 시전자를 제압하면 사선이 열리지 않는다.',
  ...
  definition('MS-028', '연화 결계', 'SUPPORT', 'ALLY_FRONT', 'FRONT_SHARED_BARRIER',
  ...
  definition('MS-032', '마지막 약실', 'ASSASSIN', 'LOW_HP_ENEMY', 'FINISHER_WITH_RELOAD',
    '살아 있는 적 중 HP 비율이 가장 낮은 대상을 고른다.',
    '명중 시점에 마무리 기준을 만족하면 강화탄을 쓴다. 처치하지 못해도 다음 행동이 늦어지지 않는다.',
    '타격 직전 회복·보호막으로 마무리를 막는다. 즉사 기술이 아니며 처치해도 스킬을 초기화하지 않는다.',
    '보스는 처형 대상이 아니다. 낮은 HP 기준의 제한된 추가 피해만 허용하고 최대 HP 비례 피해는 금지한다.',
    art('last-chamber', 'FINISH', .8, [1.2], 3.0, '#bca2ee'), ['약화 표적 선택', '마지막 탄 발사', '타격 시 HP 조건 검사', '즉시 재정비']),
  definition('MS-038', '업무 종료', 'CONTROLLER', 'FRONT_ENEMY', 'INTERRUPT_WINDUP',
  ...
  definition('MS-042', '현행범 체포', 'CONTROLLER', 'FRONT_ENEMY', 'REPEAT_OFFENDER_RESTRAINT',
  ...
  definition('MS-043', '퇴근길 앙코르', 'MARKSMAN', 'FRONT_ENEMY', 'TWO_BEAT_FOLLOWUP',
    '적 전열 우선 대상에게 박자가 다른 권총 두 발을 준비한다.',
    '첫 탄 뒤 짧은 빈틈을 두고 같은 적에게 둘째 탄을 쏜다. 첫 탄이 빗나가거나 대상이 사라지면 둘째 탄을 취소한다.',
    '첫 탄 회피·타격 사이 제압으로 후속타를 끊는다. 남은 탄을 다른 적에게 넘기지 않으며 종료 뒤 행동이 늦어지지 않는다.',
  ...
  createSniperOrikkungSkill(definition,art),
  createNurseSkill(definition,art),
  createBerkanSkill(definition,art),
];
...
const reviewKeys = ['id', 'name', 'review', 'note'];
const legacyRootKeys = ['format', 'version', 'revision', 'rosterVersion', 'status', 'runtimeEnabled', 'skills'];

export function createSkillDraft() {
  return {format: 'PROJECT_V_MERCENARY_SKILL_DRAFT_V1', version: SKILL_VERSION, revision: 1,
    catalogVersion: SKILL_CATALOG_VERSION, status: 'DRAFT', runtimeEnabled: false,
    skills: MERCENARY_SKILLS.map(skill => ({id: skill.id, name: skill.name, review: 'PENDING', note: ''}))};
}
export function validateSkillDraft(draft) {
  const fail = message => {throw new Error(message)};
  if (!exact(draft, ['format', 'version', 'revision', 'catalogVersion', 'status', 'runtimeEnabled', 'skills']) ||
      draft.format !== 'PROJECT_V_MERCENARY_SKILL_DRAFT_V1' || draft.version !== SKILL_VERSION ||
      draft.catalogVersion !== SKILL_CATALOG_VERSION || draft.status !== 'DRAFT' || draft.runtimeEnabled !== false ||
      !Number.isSafeInteger(draft.revision) || draft.revision < 1 || draft.revision >= Number.MAX_SAFE_INTEGER)
    fail('스킬 목록 형식·버전·운영 미연결 상태를 확인하세요.');
  ...
  for (const row of draft.skills) {
    if (!exact(row, reviewKeys)) fail('스킬 목록에는 용병 배정이나 허용되지 않은 설정을 넣을 수 없습니다.');
    if (!MERCENARY_SKILLS.some(skill => skill.id === row.id) || seen.has(row.id)) fail('스킬이 누락·중복되었거나 목록과 다릅니다.');
    seen.add(row.id);
    if (typeof row.name !== 'string' || !row.name.trim() || row.name.length > 40 ||
        typeof row.note !== 'string' || row.note.length > 800 || !['PENDING', 'REVISE', 'REVIEWED'].includes(row.review))
      fail('이름·검토 상태·메모 형식이 올바르지 않습니다.');
  }
  return copy(draft);
}
export function parseSkillDraft(text) {
  ...
  if (legacy) {
    const expected = MERCENARY_SKILLS.filter(skill => !['MS-044','MS-045','MS-046','MS-047','MS-048','MS-049','MS-050','MS-051','MS-055'].includes(skill.id) && !S_SKILL_IDS.includes(skill.id) && (draft.version !== 1 || skill.id !== 'MS-021'));
    if (!Array.isArray(draft.skills) || draft.skills.length !== expected.length ||
        ...
        draft.skills.some(row => !exact(row, ['id', 'code', 'name', 'review', 'note']) ||
          !expected.some(skill => skill.id === row.id) || row.code !== `V-${row.id.slice(3)}`))
      throw new Error('기존 스킬 검토본의 형식이 올바르지 않습니다. 원본을 보존해 주세요.');
    // Legacy codes were assistant proposals, not the user's assignments. Keep
    // every review edit and revision, but never migrate those codes as ownership.
  ...
  if (draft.version === SKILL_VERSION && draft.catalogVersion === 1) {
    const legacySkills = MERCENARY_SKILLS.filter(s => !['MS-044','MS-045','MS-046','MS-047','MS-048','MS-049','MS-050','MS-051','MS-055'].includes(s.id) && !S_SKILL_IDS.includes(s.id));
  ...
  const priorIds=MERCENARY_SKILLS.filter(s=>!['MS-044','MS-045','MS-046','MS-047','MS-048','MS-049','MS-050','MS-051','MS-055'].includes(s.id)).map(s=>s.id);
  ...
  const beforeMangisa=MERCENARY_SKILLS.filter(s=>!['MS-045','MS-046','MS-047','MS-048','MS-049','MS-050','MS-051','MS-055'].includes(s.id)).map(s=>s.id);
  ...
  const beforeRagniel=MERCENARY_SKILLS.filter(s=>!['MS-046','MS-047','MS-048','MS-049','MS-050','MS-051','MS-055'].includes(s.id)).map(s=>s.id);
  ...
  const beforeJoeun=MERCENARY_SKILLS.filter(s=>!['MS-047','MS-048','MS-049','MS-050','MS-051','MS-055'].includes(s.id)).map(s=>s.id);
  ...
  const beforeHeukwol=MERCENARY_SKILLS.filter(s=>!['MS-048','MS-049','MS-050','MS-051','MS-055'].includes(s.id)).map(s=>s.id);
  ...
  const beforeCryvern=MERCENARY_SKILLS.filter(s=>!['MS-049','MS-050','MS-051','MS-055'].includes(s.id)).map(s=>s.id);
  ...
  const beforeOrikkung=MERCENARY_SKILLS.filter(s=>!['MS-050','MS-051','MS-055'].includes(s.id)).map(s=>s.id);
  if(Array.isArray(draft.skills)&&draft.skills.length===beforeOrikkung.length&&new Set(draft.skills.map(s=>s?.id)).size===beforeOrikkung.length&&draft.skills.every(s=>beforeOrikkung.includes(s?.id)))
    draft={...draft,skills:[...draft.skills,createSkillDraft().skills.find(s=>s.id==='MS-050')]};
  const beforeNurses=MERCENARY_SKILLS.filter(s=>!['MS-051','MS-055'].includes(s.id)).map(s=>s.id);
  if(Array.isArray(draft.skills)&&draft.skills.length===beforeNurses.length&&new Set(draft.skills.map(s=>s?.id)).size===beforeNurses.length&&draft.skills.every(s=>beforeNurses.includes(s?.id)))
    draft={...draft,skills:[...draft.skills,createSkillDraft().skills.find(s=>s.id==='MS-051')]};
  const beforeBerkan=MERCENARY_SKILLS.filter(s=>s.id!=='MS-055').map(s=>s.id);
  if(Array.isArray(draft.skills)&&draft.skills.length===beforeBerkan.length&&new Set(draft.skills.map(s=>s?.id)).size===beforeBerkan.length&&draft.skills.every(s=>beforeBerkan.includes(s?.id)))
    draft={...draft,skills:[...draft.skills,createSkillDraft().skills.find(s=>s.id==='MS-055')]};
