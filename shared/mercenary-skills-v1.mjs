...
export function parseSkillDraft(text) {
  ...
  if (legacy) {
    const expected = MERCENARY_SKILLS.filter(skill => !['MS-044','MS-045','MS-046','MS-047'].includes(skill.id) && !S_SKILL_IDS.includes(skill.id) && (draft.version !== 1 || skill.id !== 'MS-021'));
  ...
  if (draft.version === SKILL_VERSION && draft.catalogVersion === 1) {
    const legacySkills = MERCENARY_SKILLS.filter(s => !['MS-044','MS-045','MS-046','MS-047'].includes(s.id) && !S_SKILL_IDS.includes(s.id));
  ...
  const priorIds=MERCENARY_SKILLS.filter(s=>!['MS-044','MS-045','MS-046','MS-047'].includes(s.id)).map(s=>s.id);
  ...
  const beforeMangisa=MERCENARY_SKILLS.filter(s=>!['MS-045','MS-046','MS-047'].includes(s.id)).map(s=>s.id);
  ...
  const beforeRagniel=MERCENARY_SKILLS.filter(s=>!['MS-046','MS-047'].includes(s.id)).map(s=>s.id);
  ...
  const beforeJoeun=MERCENARY_SKILLS.filter(s=>s.id!=='MS-047').map(s=>s.id);
