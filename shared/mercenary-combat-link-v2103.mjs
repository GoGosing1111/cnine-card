...
export const MERCENARY_COMBAT_LINK=Object.freeze({
 version:2103,regularActionsPerTurn:3,
 ranks:Object.freeze(Object.fromEntries([
  ['C',35,60],['B',45,70],['A',55,80],['S',65,90],['SS',85,120],['SSS',115,150],
 ].map(([rank,attackPercent,shieldPercent])=>[rank,Object.freeze({attackPercent,shieldPercent})])))
});

export function mercenaryCombatLinkText(rank){
 const row=MERCENARY_COMBAT_LINK.ranks[rank];if(!row)return '';
 return `아군 일반 카드 5명의 평균 공격력 ${row.attackPercent}%를 최소 공격력으로 사용합니다. 전투 시작 시 평균 최대 체력 ${row.shieldPercent}%의 전용 방벽을 얻습니다. 아군 카드 ${MERCENARY_COMBAT_LINK.regularActionsPerTurn}회 행동마다 추가 행동하며, 자연 행동 시 주기를 다시 셉니다. 기본 공격과 피해·회복·보호 스킬 모두 연계 공격력을 사용합니다.`;
}
...
export function applyMercenaryCombatLink(teams){
 for(const team of teams){
  ...
  for(const m of team.filter(c=>c.isMercenary&&c.statMode==='RANK_FIXED')){
   ...
   const attackFloor=Math.round(averageAttack*rule.attackPercent/100);
   const openingShield=Math.round(averageHp*rule.shieldPercent/100*Math.max(0,Math.min(1,m.hp/m.maxHp)));
   if(!Number.isSafeInteger(attackFloor)||!Number.isSafeInteger(openingShield))throw Error('INVALID_MERCENARY_COMBAT_LINK');
   m.mercenaryLink={version:MERCENARY_COMBAT_LINK.version,attackFloor,openingShield};
