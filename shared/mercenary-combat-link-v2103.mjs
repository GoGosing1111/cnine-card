...
export const MERCENARY_COMBAT_LINK=Object.freeze({
 version:2106,regularActionsPerTurn:1,
...
export function mercenaryCombatLinkText(rank,position){
 ...
 const shieldPercent=row.shieldPercent+(position==='FRONT'?MERCENARY_COMBAT_LINK.frontRowShieldBonusPercent:0);
 return `아군 일반 카드 5명의 평균 공격력 ${row.attackPercent}%를 최소 공격력, 평균 최대 체력 ${row.hpPercent}%를 최소 최대 체력으로 사용합니다. 전투 시작 시 평균 최대 체력 ${shieldPercent}%의 전용 방벽을 얻습니다. 아군 카드 ${MERCENARY_COMBAT_LINK.regularActionsPerTurn}회 행동마다 추가 행동합니다. 기본 공격과 피해·회복·보호 스킬 모두 연계 공격력을 사용합니다.`;
}
...
export function applyMercenaryCombatLink(teams){
 for(const team of teams){
  ...
  for(const m of team.filter(c=>c.isMercenary&&c.statMode==='RANK_FIXED')){
   ...
   const hpFloor=Math.round(averageHp*rule.hpPercent/100);
   const shieldPercent=rule.shieldPercent+(m.position==='FRONT'?MERCENARY_COMBAT_LINK.frontRowShieldBonusPercent:0);
   const openingShield=Math.round(averageHp*shieldPercent/100*healthRatio);
