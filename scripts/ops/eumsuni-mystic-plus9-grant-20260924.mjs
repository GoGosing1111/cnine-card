...
export const OPERATION_KEY='ops:eumsuni-mystic-plus9-grant:20260924:v1';
export const TARGET=Object.freeze({id:65,nickname:'음순이'});
export const EQUIPMENT=Object.freeze([
 {id:32,code:'EQ_1787156640727',name:'미스틱 레깅스',slot:'BOTTOM'},
 {id:34,code:'EQ_1787156691265',name:'미스틱 슈트',slot:'TOP'},
 {id:35,code:'EQ_1787156718021',name:'미스틱 듀얼디스크',slot:'ACCESSORY'}
]);
...
export async function grantEumsuniMysticPlus9(client){
 ...
 try{
  ...
  const forged=await q(`INSERT INTO equipment_forge_states_v1(instance_id,user_id,level,revision)
   SELECT id,$2,9,1 FROM unnest($1::bigint[]) AS id RETURNING instance_id`,[instanceIds,TARGET.id]);
  assert.equal(forged.length,3,'Partial enhancement insert');
  ...
  for(const [index,row] of granted.entries()){
   ...
   assert.equal(Number(row.level),9);assert.equal(Number(row.revision),1);
  }
  ...
  for(let n=0;n<3;n++)assert.equal(BigInt(countsAfter[n].quantity),BigInt(countsBefore[n].quantity)+1n,'Equipment count trigger mismatch');
  ...
  const receipt={operationKey:OPERATION_KEY,status:'COMPLETED',actor:'SYSTEM_OPS',user:TARGET,quantityPerType:1,enhancementLevel:9,
   items:granted.map((row,n)=>({...EQUIPMENT[n],instanceId:String(row.id),level:9,quantity:1})),countsBefore,countsAfter,balancesPreserved:true,loadoutPreserved:true,completedAt:now};
  const [audit]=await q(`INSERT INTO admin_logs(admin_id,action_type,target_type,target_id,before_data,after_data)
   VALUES($1,'OPS_EQUIPMENT_PLUS9_GRANT','USER',$2,$3,$4) RETURNING id`,[owner.id,String(TARGET.id),JSON.stringify({operationKey:OPERATION_KEY,counts:countsBefore,loadout:loadoutBefore}),JSON.stringify({...receipt,reason:'사용자 지시: 음순이 계정에 미스틱 디스크 상의 하의 9강으로 지급'})]);
