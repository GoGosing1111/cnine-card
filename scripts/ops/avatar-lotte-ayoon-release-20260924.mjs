...
import catalog from '../../preview/avatar-lotte-ayoon-v1/catalog.json' with {type:'json'};
import {clanAdminTransaction} from '../../functions/_clan_inactivity_cleanup.js';

export const LOTTE_AVATAR=catalog;
export const LOTTE_GRANT_KEY='ops:avatar-lotte-ayoon-clan-season-grant:20260924:v1';
export const LOTTE_EXPECTED_EFFECTS=Object.freeze([
...
export async function releaseLotteAyoon(db,{expectedSeasonId,expectedClanId,expectedRecipientIds,expectedSeasonEndsAt}){
  check(Number.isSafeInteger(expectedSeasonId)&&expectedSeasonId>0,'A verified season is required');
  check(Number.isSafeInteger(expectedClanId)&&expectedClanId>0,'A verified Lotte clan is required');
  ...
  check(Number.isFinite(Date.parse(expectedSeasonEndsAt)),'A verified season end is required');
  ...
  return clanAdminTransaction(db,async q=>{
    ...
    await q("INSERT INTO app_meta(key,value,updated_at) VALUES($1,'{\"status\":\"PENDING\"}',sqlite_now()) ON CONFLICT(key) DO NOTHING",[LOTTE_GRANT_KEY]);
    const [saved]=await q('SELECT value FROM app_meta WHERE key=$1 FOR UPDATE',[LOTTE_GRANT_KEY]);
    ...
    check(pack(sourceEffects)===pack(LOTTE_EXPECTED_EFFECTS),'T1 Joeun options changed; refresh the reviewed release');
    ...
    check(ids.includes(5209)&&members.some(m=>Number(m.user_id)===5209&&m.nickname==='김아윤'),'Kim Ayoon must be admitted before the grant');
    ...
    const times={acquired_at:clock.acquired_at,expires_at:season.expires_at};
    const granted=await q(`INSERT INTO avatar_user_ownership_v1(user_id,avatar_code,source_type,source_ref,acquired_at,expires_at)
      ...
      [expectedSeasonId,expectedClanId,catalog.code,LOTTE_GRANT_KEY,times.acquired_at,times.expires_at]);
    ...
    check(ownership.length===ids.length&&ownership.every((r,i)=>Number(r.user_id)===ids[i]&&r.source_ref===LOTTE_GRANT_KEY&&r.acquired_at===times.acquired_at&&r.expires_at===times.expires_at),'Ownership verification failed');
    ...
    const result={ok:true,status:'COMPLETED',avatar:{code:catalog.code,serial:catalog.serial,name:catalog.name},clan:{id:expectedClanId,name:'롯데'},seasonId:expectedSeasonId,
      sourceAvatar:{code:source.code,version:Number(source.version)},effects,granted:ownership.length,seasonEndsAt:season.ends_at,acquiredAt:times.acquired_at,expiresAt:times.expires_at,
    ...
    const [audit]=await q(`INSERT INTO admin_logs(admin_id,action_type,target_type,target_id,before_data,after_data) VALUES($1,'OPS_AVATAR_CLAN_SEASON_GRANT','AVATAR',$2,$3,$4) RETURNING id`,[owner.id,catalog.code,pack({operationKey:LOTTE_GRANT_KEY,registered:false,ownership:[]}),pack({...result,operationKey:LOTTE_GRANT_KEY,actor:'SYSTEM_OPS',authorization:'롯데 아바타로 등록 롯데 아윤 / 아바타 T1조은 하고 동일 옵션 / 롯데 전원 남은 시즌 기간동안 기간제 지급'})]);
    check(audit,'Avatar grant audit missing');result.adminLogId=String(audit.id);
    await q('UPDATE app_meta SET value=$2,updated_at=sqlite_now() WHERE key=$1',[LOTTE_GRANT_KEY,pack({status:'COMPLETED',result})]);
    return result;
  });
}
