...
export const CAMPAIGN_KEY='chuseok-verified-1500eok-20260925-v1';
export const OPERATION_KEY='ops:chuseok-verified-1500eok:20260925:v1';
export const AMOUNT=150_000_000_000;
export const TITLE='추석 떡값';
...
export async function verifyChuseokGift(client,receipt,{unclaimed=false}={}){
 assert.equal(receipt.status,'COMPLETED');assert.equal(receipt.campaignKey,CAMPAIGN_KEY);
 assert.equal(receipt.rewardType,'COIN');assert.equal(receipt.rewardAmount,AMOUNT);assert.equal(receipt.title,TITLE);assert.equal(receipt.body,TITLE);
...
export async function sendChuseokGift(client,{expectedRecipientHash,dryRun=false}={}){
 ...
 try{
  ...
  const messages=await client.query(`INSERT INTO user_messages(user_id,sender_type,title,body,message_type,campaign_key)
   SELECT id,'ADMIN',$2,$2,'COIN_REWARD',$3 FROM unnest($1::bigint[]) AS id ORDER BY id RETURNING id`,[plan.recipients.map(row=>row.id),TITLE,CAMPAIGN_KEY]);
  ...
  const rewards=await client.query(`INSERT INTO user_message_rewards(message_id,user_id,reward_type,reward_amount)
   SELECT id,user_id,'COIN',$2::bigint FROM user_messages WHERE campaign_key=$1 ORDER BY user_id RETURNING id`,[CAMPAIGN_KEY,AMOUNT]);
  ...
  const receipt={...details,status:'COMPLETED',delivery:'MESSAGE',actor:'SYSTEM_OPS',
   authorization:'2차인증 유저 전체 1500억 지급해 추석 떡값',
   eligibility:'All ACTIVE secondary-verified accounts, all roles',completedAt:new Date().toISOString()};
  const verification=await verifyChuseokGift(client,receipt,{unclaimed:true});
