...
import catalog from '../../preview/avatar-fm-orikkung-v1/catalog.json' with {type:'json'};
import {clanAdminTransaction} from '../../functions/_clan_inactivity_cleanup.js';

export const FM_AVATAR=catalog;
export const FM_GRANT_KEY='avatar_fm_orikkung_clan_grant_20260917_v1';
...
export async function releaseFmOrikkung(db,{expectedSeasonId,expectedClanId,expectedRecipientIds}){
  check(Number.isSafeInteger(expectedSeasonId)&&expectedSeasonId>0,'A verified season is required');
  check(Number.isSafeInteger(expectedClanId)&&expectedClanId>0,'A verified FM clan is required');
  ...
  return clanAdminTransaction(db,async q=>{
    ...
    check(pack(sourceEffects)===pack(FM_EXPECTED_EFFECTS),'T1 Joeun options changed; refresh the reviewed release');
    ...
    const [clan]=await q("SELECT id,name,is_active FROM clan_organizations WHERE name='FM' FOR SHARE");
    check(Number(clan?.id)===expectedClanId&&Number(clan.is_active)===1,'FM clan changed');
    ...
    check(pack(members.map(m=>Number(m.user_id)))===pack(ids)&&members.every(m=>m.status==='ACTIVE'),'FM recipients changed; refresh the roster');
    ...
    const granted=await q(`INSERT INTO avatar_user_ownership_v1(user_id,avatar_code,source_type,source_ref,acquired_at,expires_at)
      ...
      [expectedSeasonId,expectedClanId,catalog.code,FM_GRANT_KEY,times.acquired_at,times.expires_at]);
    ...
    check(ownership.length===ids.length&&ownership.every((r,i)=>Number(r.user_id)===ids[i]&&r.source_ref===FM_GRANT_KEY&&r.acquired_at===times.acquired_at&&r.expires_at===times.expires_at),'Ownership verification failed');
    ...
    const result={ok:true,status:'COMPLETED',avatar:{code:catalog.code,serial:catalog.serial,name:catalog.name},clan:{id:expectedClanId,name:'FM'},seasonId:expectedSeasonId,
    ...
    await q('UPDATE app_meta SET value=$2,updated_at=sqlite_now() WHERE key=$1',[FM_GRANT_KEY,pack({status:'COMPLETED',result})]);
    return result;
  });
}
