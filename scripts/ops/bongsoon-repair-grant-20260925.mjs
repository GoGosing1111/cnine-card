...
export const OPERATION_KEY='ops:bongsoon-repair-coupon:20260925:v1';
...
const REASON='사용자 지시: 나무늘봉순 리페어권 1장 지급';
...
export async function grantBongsoonRepairCoupon(client){
 ...
 try{
  ...
  if(saved){
   ...
   assert.equal(receipt.user.id,TARGET.id);assert.equal(receipt.itemCode,ITEM_CODE);assert.equal(receipt.amount,1);
  ...
  for(const value of Object.values(before))assert.ok(BigInt(value)>=0n&&BigInt(value)<BigInt(Number.MAX_SAFE_INTEGER),'Invalid inventory balance');
  ...
  const inventory=await q(`INSERT INTO cnine_user_inventory(user_id,item_code,quantity,unseen_quantity,created_at,updated_at)
   ...
   ON CONFLICT(user_id,item_code) DO UPDATE SET quantity=cnine_user_inventory.quantity+1,
    unseen_quantity=cnine_user_inventory.unseen_quantity+1,updated_at=excluded.updated_at
  ...
  for(const key of Object.keys(before))assert.equal(BigInt(after[key]),BigInt(before[key])+1n,'Incorrect inventory delta');
  const ledger=await q(`INSERT INTO inventory_logs(user_id,item_code,change_amount,balance_after,reason,reference_type,reference_id,admin_id,created_at)
   VALUES($1,$2,1,$3,$4,'SYSTEM_GRANT',$5,$6,$7) RETURNING id`,[TARGET.id,ITEM_CODE,after.quantity,REASON,OPERATION_KEY,owner.id,now]);
  ...
  const receipt={status:'COMPLETED',operationKey:OPERATION_KEY,actor:'SYSTEM_OPS',user:TARGET,itemCode:ITEM_CODE,itemName:ITEM_NAME,amount:1,
