...
for(const local of [f,scenarios.recruit])for(const step of ['create','recruit'])await local.call('admin/ranked-duo/'+step,{method:'POST',body:step==='create'?{config:local.config}:{}});
for(const user of [2,3,4,5,6,7])await f.call('ranked-duo/join',{user,method:'POST'});
f.advance(72*3600000);await f.call('admin/ranked-duo/pair',{method:'POST'});
for(let i=0;i<8;i++){const r=await f.call('admin/ranked-duo/pair-step',{method:'POST'});if(r.data.done)break;}
await f.call('admin/ranked-duo/start',{method:'POST'});
...
http.createServer(async(req,res)=>{
 try{
  ...
  }else if(!pathname.startsWith('ranked-duo/')&&!pathname.startsWith('admin/ranked-duo/')){result={status:404,data:{error:'Local QA route only'}};}else{let body='';for await(const chunk of req)body+=chunk;result=await (scenarios[req.headers['x-duo-qa-scenario']]||f).call(pathname+url.search,{user:pathname.startsWith('admin/')?1:2,method:req.method,body:body?JSON.parse(body):{}});}
