...
import {createDuoBattleV2,createPvpBattleV2} from '../functions/_battle_v2_preview.js';
...
const player=()=>mountRankedDuo({root,userId:2,api,navigate:()=>{},ensureBattle:()=>ProjectVBattleV3Live.ensureRuntime()});document.getElementById('qa-player').onclick=player;document.getElementById('qa-cms').onclick=()=>mountDuoCms(root);
...
for(const id of ['qa-formation','qa-legacy'])document.getElementById(id).onclick=async()=>{renderer?.destroy();const data=await api(id==='qa-legacy'?'qa/legacy':'qa/formation'),modal=document.getElementById('modal'),live=ProjectVBattleV3Live.prepareLoading({modal,mode:'PVP',playerName:'참가자 2 + 참가자 5',opponentName:'참가자 3 + 참가자 4'});renderer=await ProjectVBattleV3Live.createRenderer({...live,modal,data,mode:'PVP'});await ProjectVPixiBattle.restoreDeployedFormation();const close=document.createElement('button');close.textContent='진형 검수 닫기';close.className='duo-battle-exit';close.onclick=()=>{renderer.destroy();modal.className='modal';};live.stage.append(close);document.getElementById('qa-diagnostics').textContent=JSON.stringify(engine.viewportGeometry());};
...
http.createServer(async(req,res)=>{
 try{
  ...
  if(url.pathname.startsWith('/api/')){const pathname=url.pathname.slice(5);let result;if(pathname.startsWith('qa/')){
   ...
   result={status:200,data:{battleV2:pathname==='qa/legacy'?createPvpBattleV2({attackerCards:cards,defenderCards:cards,attackerMercenary:merc,defenderMercenary:merc}):createDuoBattleV2({attackerSquads:[squad(2),squad(5)],defenderSquads:[squad(3),squad(4)],seed:42})}};
  }else{let body='';for await(const chunk of req)body+=chunk;result=await f.call(pathname,{user:pathname.startsWith('admin/')?1:2,method:req.method,body:body?JSON.parse(body):{}});}
