// Local-only CMS verification against the real routes and an isolated SQLite DB.
...
await f.p('DELETE FROM character_equipment_items WHERE id=900').run();
for(const [id,name] of [[32,'미스틱 레깅스'],[33,'미스틱 슈즈'],[34,'미스틱 슈트'],[35,'미스틱 듀얼디스크'],[44,'엠퍼러 슈트']]){
 await f.p("INSERT INTO character_equipment_items(id,code,name,rarity,image_url) VALUES(?,?,?,'MYTHIC','assets/items/pig-coin-v1.png')",id,'EQ_QA_'+id,name).run();
}
...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(url.pathname==='/mobile')return send(res,200,'<!doctype html><html><meta charset="utf-8"><body style="margin:0;background:#080d16"><iframe title="390px 모바일 CMS" src="/cms" style="width:390px;height:850px;border:0"></iframe></body></html>');
 if(url.pathname==='/cms')return send(res,200,`<!doctype html><html lang="ko"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="stylesheet" href="/admin/admin-v945.css"><link rel="stylesheet" href="/admin/loot-shop-admin-v1.css"><style>body{display:block;padding:16px;background:#0c111b;color:#fff}main{margin:auto}nav{display:flex;gap:20px;margin-bottom:16px}a{color:#eed295}</style><nav><a href="#mystic">미스틱 상품</a><a href="#save">저장 버튼</a><a href="/mobile">390px 모바일</a></nav><main class="loot-admin" id="root"></main><script type="module">import {mountLootShopCms} from '/admin/loot-shop-admin-v1.mjs?v=5';localStorage.setItem('cnine_admin_token','local-account-7');await mountLootShopCms(document.querySelector('#root'));const anchors=()=>{document.querySelector('[data-product="3"]').id='mystic';document.querySelector('footer').id='save'};anchors();new MutationObserver(anchors).observe(document.querySelector('form'),{childList:true});</script></html>`);
