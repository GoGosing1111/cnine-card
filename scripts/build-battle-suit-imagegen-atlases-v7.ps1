...
# V7 changes only Battle Suit 03 + M200: restore the original helmeted
# ImageGen source verbatim. The approved M4 row and every other V6 row stay
# byte-for-byte sourced from their prior assets.
...
[BattleSuitAtlasV6]::BuildPreservedTopGeneratedBottom(
    ...
    (Join-Path $sources 'battle-suit-03-m200-helmeted-imagegen-authored-v7.png'),
    (Join-Path $animations 'battle-suit-03-m4a1-m200-horizontal-fire-atlas-v7.png')
)

Get-ChildItem -LiteralPath $animations -Filter '*horizontal-fire-atlas-v*.png' |
    Where-Object { $_.Name -match 'atlas-v[67]\.png$' } |
    Sort-Object Name |
