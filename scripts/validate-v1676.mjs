...
assert('폐차장 완주 후 부품 1회 판정',scrapyard.includes('if(battle.success)drop=')&&!scrapyard.includes('rollsMultiplier:battle.wavesCleared'));
assert('원정 영수증 재생',scrapyard.includes("prior?.status==='COMPLETED'")&&scrapyard.includes('replayed:true'));
assert('동일 장비 정확히 3개',workshop.includes('LIMIT 3')&&workshop.includes('length!==required'));
...
assert('신화 합성 중복 결과 허용',!workshop.includes('MYTHIC_EQUIPMENT_DUPLICATE')&&workshop.includes("source_type,source_id,request_id"));
...
assert('밀어서 결과 확인',ui.includes('role="slider"')&&(ui.includes('pct>=92')||ui.includes('pct>=94'))&&ui.includes("classList.add('revealed'"));
