...
async function updateQuote(){
 ...
 const item=visibleItems().find(r=>r.selectionId===selected);if(localStorage.getItem(pendingKey())||!item||!(mode==='enhance'?data?.canEnhance:data?.canRestore))return;
 ...
 try{const q=await api('quote',undefined,{requestId:crypto.randomUUID(),kind:mode.toUpperCase(),...(mode==='enhance'?{instanceId:item.instanceId,useProtection:$('protection-toggle').checked}:{recordId:item.recordId})});if(stamp!==quoteGeneration)return;quote=q;
  if(mode==='enhance'){$('success-rate').textContent=`${q.cost.successPpm/10000}%`;$('rate-level').textContent=`+${q.item.level} → +${q.item.level+1}`;$('power-after').textContent=number(forgePower(q.item.basePower.total,q.item.level+1).total);
   ...
   document.querySelector('.material-heading span').textContent='이번 시도 비용';document.querySelector('.success-label p').textContent='현재 장비 단계의 확정된 견적입니다.';document.querySelector('.material-list').textContent=`${number(q.cost.coinCost)} 코인${q.cost.itemCode?` · ${q.cost.itemName||q.cost.itemCode} ${number(q.cost.itemQuantity)}개`:''}`;
  ...
  }else{document.querySelector('.restore-coupon').hidden=!q.cost.itemCode;document.querySelector('.restore-coupon b').textContent=q.cost.itemName||'복구 재료';document.querySelector('.restore-coupon div span').textContent=`${q.cost.itemQuantity||0}개 사용`;document.querySelector('.restore-snapshot').textContent=`${q.item.name} · +${q.cost.levelMode==='PREVIOUS'?q.item.level:0} 복구`;document.querySelector('.restore-policy').textContent=`${number(q.cost.coinCost)} 코인${q.cost.itemCode?` · ${q.cost.itemName||q.cost.itemCode} ${q.cost.itemQuantity}개`:''} · ${q.cost.expiresHours?`${q.cost.expiresHours}시간 이내`:'기간 제한 없음'}`;}
...
async function execute(){
 ...
 try{const receipt=await api(pending.kind==='ENHANCE'?'enhance':'restore',undefined,{requestId:pending.requestId,quoteId:pending.quoteId});
  ...
  localStorage.removeItem(pendingKey());if(fx){await fx.sound.enable(localStorage.getItem('cnine_battle_sound')!=='OFF');await fx.play(receipt.outcome==='RESTORED'?'restore':receipt.outcome.toLowerCase());}
