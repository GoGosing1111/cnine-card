...
const browser=await chromium.launch({headless:true,...(process.env.QA_CHROMIUM?{executablePath:process.env.QA_CHROMIUM}:{channel:'chrome'})});
const checks=[];
try{for(const [width,height] of [[1440,1000],[1024,900],[390,844],[360,740]]){
  ...
  await page.locator('[data-tab="economy"]').click();assert.equal(await page.locator('[data-power-rank="SS"]').textContent(),'120,000');assert.equal(await page.locator('[data-power-rank="SSS"]').textContent(),'180,000');await page.locator('[data-power-rank="SS"]').scrollIntoViewIfNeeded();await page.screenshot({path:path.join(out,`power-${width}.png`),fullPage:false});assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));assert.equal(await page.locator('[data-field^="settings.rankGrowth."]').count(),0);assert.match(await page.locator('.mc-editor').innerText(),/동일 용병 중복 카드 \+ 마스터의 별/);await page.locator('[data-field="settings.growthNotes"]').fill('동일 용병 중복 카드 + 마스터의 별 / 차후 공개');await page.locator('[data-save]').click();await page.getByRole('status').filter({hasText:'저장 완료'}).waitFor();
  assert.deepEqual(d.settings.rankGrowth,seed.document.settings.rankGrowth);assert.equal(d.settings.growthNotes,'동일 용병 중복 카드 + 마스터의 별 / 차후 공개');
