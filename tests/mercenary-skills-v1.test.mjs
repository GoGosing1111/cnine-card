...
test('both legacy reviews preserve names, notes and revisions while removing proposed ownership',()=>{
  for(const version of [1,2]){
    const old={format:'PROJECT_V_MERCENARY_SKILL_DRAFT_V1',version,rosterVersion:version===1?10:11,revision:7,status:'DRAFT',runtimeEnabled:false,
      skills:createSkillDraft().skills.filter(s=>!['MS-044','MS-045','MS-046','MS-047','MS-048','MS-049','MS-050'].includes(s.id)&&!S_SKILL_IDS.includes(s.id)&&(version===2||s.id!=='MS-021')).map(s=>({...s,code:`V-${s.id.slice(3)}`}))};
    ...
    assert.equal(migrated.revision,7);assert.equal(migrated.skills.length,33);assert.equal(migrated.version,3);
    assert.equal(migrated.rosterVersion,undefined);assert.equal(migrated.catalogVersion,2);assert.deepEqual(old,before);
    assert.deepEqual(migrated.skills.filter(s=>!['MS-044','MS-045','MS-046','MS-047','MS-048','MS-049','MS-050'].includes(s.id)&&!S_SKILL_IDS.includes(s.id)&&(version===2||s.id!=='MS-021')),old.skills.map(({code,...review})=>review));
...
test('33 independent skills cover seven effect categories without a mercenary or rank owner',()=>{
  assert.equal(skills.length,33);assert.equal(new Set(skills.map(s=>s.role)).size,7);
  for(const key of ['id','mechanic'])assert.equal(new Set(skills.map(s=>s[key])).size,33);
  assert.equal(new Set(skills.map(s=>s.visual.asset)).size,33);
  assert.equal(new Set(skills.map(s=>s.visual.motion)).size,33);
...
test('assignment drafts start with all 50 mercenaries unassigned, including SSS Omega',()=>{
  ...
  assert.equal(draft.assignments.length,50);assert.ok(draft.assignments.every(row=>row.skillIds.length===0));
...
test('thirty-three individually authored sequences retain 528 original frames and clean gutters',()=>{
  const manifest=read('preview/project-v-mercenary-system-v1/skill-assets-v2/manifest.json');
  assert.equal(manifest.images.length,33);assert.equal(manifest.frameCount,528);assert.equal(manifest.runtimeEnabled,false);
  ...
  assert.equal(ids.size,33);assert.ok(hashes.size>=390,'All substantive frames are independently authored; a final empty extinction frame can be shared.');
});
test('skill review stays outside production battle routes, source-art roster and five-card contract',()=>{
  ...
  assert.ok(roster.cards.every(c=>['V-021','V-046','V-049'].includes(c.code)?c.rank==='SSS':['V-044','V-045','V-047','V-048','V-050'].includes(c.code)?c.rank==='SS':c.rank===null));assert.equal(roster.formationRule.regularCardSlots,5);
});
