...
test('both legacy reviews preserve names, notes and revisions while removing proposed ownership',()=>{
  for(const version of [1,2]){
    const old={format:'PROJECT_V_MERCENARY_SKILL_DRAFT_V1',version,rosterVersion:version===1?10:11,revision:7,status:'DRAFT',runtimeEnabled:false,
      skills:createSkillDraft().skills.filter(s=>s.id!=='MS-044'&&!S_SKILL_IDS.includes(s.id)&&(version===2||s.id!=='MS-021')).map(s=>({...s,code:`V-${s.id.slice(3)}`}))};
    ...
    assert.equal(migrated.revision,7);assert.equal(migrated.skills.length,27);assert.equal(migrated.version,3);
    assert.equal(migrated.rosterVersion,undefined);assert.equal(migrated.catalogVersion,2);assert.deepEqual(old,before);
    assert.deepEqual(migrated.skills.filter(s=>s.id!=='MS-044'&&!S_SKILL_IDS.includes(s.id)&&(version===2||s.id!=='MS-021')),old.skills.map(({code,...review})=>review));
...
test('26 independent skills cover seven effect categories without a mercenary or rank owner',()=>{
  assert.equal(skills.length,27);assert.equal(new Set(skills.map(s=>s.role)).size,7);
  for(const key of ['id','mechanic'])assert.equal(new Set(skills.map(s=>s[key])).size,27);
  assert.equal(new Set(skills.map(s=>s.visual.asset)).size,27);
  assert.equal(new Set(skills.map(s=>s.visual.motion)).size,27);
...
test('assignment drafts start with all 43 mercenaries unassigned, including SSS Omega',()=>{
  ...
  assert.equal(draft.assignments.length,44);assert.ok(draft.assignments.every(row=>row.skillIds.length===0));
...
test('twenty-six individually authored sequences retain 416 original frames and clean gutters',()=>{
  ...
  assert.equal(manifest.images.length,27);assert.equal(manifest.frameCount,432);assert.equal(manifest.runtimeEnabled,false);
  ...
  assert.equal(ids.size,27);assert.ok(hashes.size>=390,'All substantive frames are independently authored; a final empty extinction frame can be shared.');
});
test('skill review stays outside production battle routes, source-art roster and five-card contract',()=>{
  ...
  assert.ok(roster.cards.every(c=>c.code==='V-021'?c.rank==='SSS':c.code==='V-044'?c.rank==='SS':c.rank===null));assert.equal(roster.formationRule.regularCardSlots,5);
});
