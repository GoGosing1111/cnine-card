...
const liveRoster=[
  ...
  ['CN-25F931CE393D474E','디임','딤우스','assets/cards/976467971.jpg'],
  ...
  ['CN-21B307E7279A4F3D','나무늘봉순','나무늘봉순','assets/cards/new-card-20260919-01.png']
];
...
assert.equal(manifest.rosterSnapshot?.expectedCount,liveRoster.length);
...
assert.equal(manifest.visualApprovalRecord?.approvedCount,29);
...
assert.deepEqual(
  ...
  liveRoster,
  'manifest가 잠근 운영 ZENITH 로스터와 일치하지 않습니다.'
);
...
for(const [index,entry] of manifest.characters.entries()){
  ...
  }else if(entry.cardId==='CN-21B307E7279A4F3D'){
    assert.equal(entry.qa.liveConnectionAuthorized,true,`${label}: 사용자 제작·연결 지시가 없습니다.`);
    assert.equal(entry.qa.authorizationSource,'USER_CREATE_AND_CONNECT_20260919');
...
const responsiveBattleMap=await readFile(new URL('js/responsive-battle-sprites-v1815.js',rootUrl),'utf8');
assert.match(previewHtml,/v=9-orikkung-heeya/,'프리뷰 cache key가 전체 로스터 검수 버전이 아닙니다.');
assert.match(previewHtml,/TECHNICAL PASS 29 \/ 29 · VISUAL APPROVED/);
assert.match(responsiveBattleMap,/zenith-cn-25f931ce393d474e-sd-v1\.png":"\/assets\/responsive\/project-v\/zenith\/zenith-cn-25f931ce393d474e-sd-v1-768\.webp/,'딤우스 SD 반응형 전투 매핑이 누락됐습니다.');
assert.match(responsiveBattleMap,/zenith-cn-b75be0dc1af749eb-sd-v1\.png":"\/assets\/responsive\/project-v\/zenith\/zenith-cn-b75be0dc1af749eb-sd-v1-768\.webp/,'히나 SD 반응형 전투 매핑이 누락됐습니다.');
assert.match(responsiveBattleMap,/zenith-cn-fc5b6212b541498c-sd-v1\.png":"\/assets\/responsive\/project-v\/zenith\/zenith-cn-fc5b6212b541498c-sd-v1-768\.webp/,'요닝 SD 반응형 전투 매핑이 누락됐습니다.');
assert.match(responsiveBattleMap,/zenith-cn-b5718bf375ca42c8-sd-v1\.png":"\/assets\/responsive\/project-v\/zenith\/zenith-cn-b5718bf375ca42c8-sd-v1-768\.webp/,'오리꿍 SD 반응형 전투 매핑이 누락됐습니다.');
assert.match(responsiveBattleMap,/zenith-cn-91a4fc0601d14931-sd-v1\.png":"\/assets\/responsive\/project-v\/zenith\/zenith-cn-91a4fc0601d14931-sd-v1-768\.webp/,'하이희야 SD 반응형 전투 매핑이 누락됐습니다.');
...
console.log('project-v ZENITH SD assets v1: 31 TECHNICAL_PASS, 29 prior visual approvals + Ayoon/Bongsoon direct connection authorization, battle-only contract OK');
