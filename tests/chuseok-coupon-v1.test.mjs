...
import {ensureChuseok,chuseokState} from '../functions/_chuseok.js';
import {redeemChuseokCoinCoupon,CHUSEOK_COUPON_MAX} from '../functions/_chuseok_coupon.js';
import {CHUSEOK_COIN} from '../js/chuseok-model-v1.js';

const read=path=>readFileSync(new URL('../'+path,import.meta.url),'utf8');
const api=read('functions/api/[[path]].js'),cms=read('admin/admin-v1276.js'),wishCms=read('admin/chuseok-v1.js');
...
const creationRoute=new AsyncFunction('path','request','env','requirePermission','readBody','releaseDeletedCouponCode','ensureChuseok','json',specs+permanent+compatible);
...
async function fixture(){
 ...
 async function create(body={},path='admin/coupon-create-permanent-v3'){
  const request=new Request('https://example.test/api/'+path,{method:'POST',body:JSON.stringify({code:'PINGDU-QA',rewardType:CHUSEOK_COIN,rewardAmount:3,maxUses:10,...body})});
  return creationRoute(path,request,env,async()=>permitted?{id:99,role:'OWNER'}:null,r=>r.json(),async()=>{},ensureChuseok,json);
 }
 async function redeem({userId=1,key='WISH_QA_REQUEST_1',code='PINGDU-QA',extra={}}={}){
  ...
  return redeemChuseokCoinCoupon({env,user:{id:userId},coupon,body:{operationKey:key,...extra},deps});
 }
...
test('Chuseok coin is a coupon-only reward and keeps all existing coin/item limits',()=>{
 const spec=couponRewardSpec(' chuseok_coin ');assert.equal(spec.label,'추석 코인');assert.equal(spec.inventory,true);assert.equal(spec.max,CHUSEOK_COUPON_MAX);
 assert.equal(verifiedMessageRewardSpec(CHUSEOK_COIN),null);assert.equal(couponRewardSpec('COIN').max,10000000000);
 ...
 assert.equal(meta[CHUSEOK_COIN].max,spec.max);assert.equal(meta[CHUSEOK_COIN].label,spec.label);
});
...
test('both real CMS create routes register ticket, preserve settings, and never issue items on creation',async()=>{
 const f=await fixture();try{
  for(const [i,path] of ['admin/coupon-create-permanent-v3','admin/coupons','admin/coupons-v2'].entries()){
   ...
   assert.equal(coupon.reward_type,CHUSEOK_COIN);assert.equal(Number(coupon.reward_amount),3);assert.equal(Number(coupon.reward_coin),0);assert.equal(coupon.starts_at,null);assert.equal(coupon.ends_at,null);
  }
  assert.equal((await f.row('SELECT code FROM inventory_items')).code,CHUSEOK_COIN);await noRedemption(f);
  assert.equal(Number((await f.row('SELECT COUNT(*) n FROM app_meta')).n),0);
  assert.equal((await chuseokState(f.env,1)).events.songpyeon.phase,'UNCONFIGURED');
...
test('redemption credits the event inventory immediately, with one receipt/log and no coin change',async()=>{
 const f=await fixture();try{
  ...
  const result=await response.json();assert.equal(result.rewardType,CHUSEOK_COIN);assert.equal(result.rewardAmount,3);assert.equal(result.rewardCoin,0);assert.equal(result.user.coin,5000000000);assert.match(result.message,/추석 달빛 잔치/);
  const inventory=await f.row('SELECT * FROM cnine_user_inventory');assert.equal(Number(inventory.user_id),1);assert.equal(inventory.item_code,CHUSEOK_COIN);assert.equal(Number(inventory.quantity),3);assert.equal(Number(inventory.unseen_quantity),3);
  const state=await chuseokState(f.env,1);assert.equal(state.chuseokCoins,3);assert.equal(state.events.songpyeon.phase,'UNCONFIGURED');assert.equal(state.coin,'5000000000');
...
test('existing inventory accumulates and parallel last-use requests cannot exceed the global cap',async()=>{
 const f=await fixture();try{
  await f.create({maxUses:1});await f.pg.exec("INSERT INTO cnine_user_inventory(user_id,item_code,quantity,unseen_quantity) VALUES(1,'CHUSEOK_COIN',9,4)");
...
test('disabled/deleted/exhausted/misconfigured coupons, disabled tickets and inactive users grant nothing',async()=>{
 const f=await fixture();try{
  ...
  assert.equal(await redeemChuseokCoinCoupon({coupon:{reward_type:'COIN'}}),null);
 }finally{await f.close()}
});

test('CMS exposes the Chuseok coin coupon shortcut and the dedicated event settings',()=>{
 assert.match(read('admin/index.html'),/<option value="CHUSEOK_COIN">추석 코인/);
 assert.match(wishCms,/select.value='CHUSEOK_COIN'/);
 assert.match(wishCms,/추석 코인 쿠폰 만들기/);
 assert.match(read('admin/index.html'),/chuseok-v1.js/);
});

test('bulk coupon list renders Chuseok coin amount rather than zero coins',()=>{
 ...
 Function('document',labels+`const couponsCache=[{id:1,code:'PINGDU-QA',reward_type:'CHUSEOK_COIN',reward_amount:5,reward_coin:0}];
 ...
 assert.match(root.innerHTML,/<small>추석 코인<\/small><strong>5<\/strong>/);assert(!root.innerHTML.includes('보상 코인'));
});
