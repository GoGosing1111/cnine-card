...
import {SniperOrikkungSkillFX} from '../preview/mercenary-sniper-orikkung-v1/source/SniperOrikkungSkillFX.js';
import {MERCENARY_CMS_SEED as seed} from '../functions/_mercenary_cms_seed.js';
import {expandMercenarySkillCatalog} from '../shared/mercenary-cms-model-v1.mjs';
import {SNIPER_ORIKKUNG_BALANCE,sniperOrikkungPoseAt,sniperOrikkungVisualPlan} from '../shared/mercenary-sniper-orikkung-v1.mjs';
...
const art=seed.catalog.cards.find(c=>c.code==='V-050'),skill=seed.document.skills.find(s=>s.id==='MS-050');
const snapshot={code:art.code,name:art.name,rank:'SS',role:'SNIPER',position:'REAR',level:1,basePower:120000,stats:{hp:100000,attack:1000,defense:100,speed:100},skills:[skill],combat,sourceArt:art.sourceArt,battleSprite:art.battleSprite};
test('previous complete CMS adds only Sniper Orikkung while preserving live edits and assignments',()=>{
 const old=structuredClone(seed.document);old.mercenaries=old.mercenaries.filter(c=>c.code!=='V-050');old.assignments=old.assignments.filter(c=>c.code!=='V-050');old.skills=old.skills.filter(s=>s.id!=='MS-050');
 ...
 assert.deepEqual(old,before);assert.deepEqual(next.settings,old.settings);assert.deepEqual(next.skills.at(-1).balance,SNIPER_ORIKKUNG_BALANCE);assert.deepEqual(next.assignments.at(-1).skillIds,['MS-050']);
...
test('approved card art stays separate; native six poses and sixteen impact frames have safe alpha borders',async()=>{
 const base=new URL('../',import.meta.url),m=JSON.parse(fs.readFileSync(new URL('preview/mercenary-sniper-orikkung-v1/manifest.json',base)));
 ...
 for(const [type,count]of [['motion',6],['impact',16]]){
  ...
  for(const f of frames){const {data,info}=await sharp(new URL('preview/mercenary-sniper-orikkung-v1/'+f.file,base).pathname.replace(/^\/([A-Z]:)/,'$1')).ensureAlpha().raw().toBuffer({resolveWithObject:true});let filled=0;
   ...
   assert.ok(filled>20);if(type==='motion'){assert.ok(f.muzzle.x>f.footAnchor.x);assert.ok(f.muzzle.y<f.footAnchor.y);assert.ok(f.bodyPixels>350);}
  }
 }
...
test('both attacks sample real recoil poses; visual plans never own damage',()=>{
 const poses=new Set(Array.from({length:190},(_,i)=>sniperOrikkungPoseAt(i/100)));assert.deepEqual([...poses].sort(),[0,1,2,3,4,5]);
 assert.equal(sniperOrikkungPoseAt(.20,{basic:true}),1);assert.equal(sniperOrikkungPoseAt(.32,{basic:true}),2);assert.equal(sniperOrikkungPoseAt(.85,{basic:true}),5);
 for(const basic of [false,true])for(const dodge of [false,true]){const p=sniperOrikkungVisualPlan({basic,dodge});assert.equal(p.budget,0);assert.equal(p.damageAuthority,'SERVER_ONLY');assert.equal(p.events.filter(e=>e.kind==='SHOT').length,1);assert.equal(p.events.filter(e=>e.kind==='HIT').length,dodge?0:1);assert.ok(p.events.every(e=>!e.ratio));}
});
test('Pixi pose playback preserves scale, HP and anchors, cancels cleanly and keeps shared atlas textures',()=>{
 const manifest=JSON.parse(fs.readFileSync(new URL('../preview/mercenary-sniper-orikkung-v1/manifest.json',import.meta.url)));
 ...
 for(const basic of [false,true]){
  ...
  const fx=new SniperOrikkungSkillFX(engine,actor,[target],assets,manifest,sniperOrikkungVisualPlan({basic}),()=>{},{authoritative:true});
  for(const time of basic?[.20,.32,.5,.7,.85]:[.3,.56,.9,1.26,1.6]){fx.seek(time);assert.notEqual(actor.fullBodySprite.texture,before.texture);assert.equal(actor.fullBodySprite.height,before.height);assert.equal(actor.hp,77);assert.equal(target.hp,77);}
...
test('one heavy sniper shot spends one server hit budget, one cost and one cooldown, respecting evasion and absent targets',()=>{
 for(const outcome of ['hit','dodge','lost']){
  ...
  assert.equal(events.filter(e=>e.type==='MERCENARY_HIT').length,outcome==='lost'?0:1);if(outcome!=='lost')assert.equal(enemy.hp,outcome==='hit'?92800:100000);
  assert.equal(runtime.state(actor).energy,outcome==='lost'?100:75);assert.equal(runtime.state(actor).cooldown.get('MS-050'),outcome==='lost'?undefined:3);assert.equal(runtime.state(actor).pending,null);
 }
});
for(const postgres of [false,true])test(`${postgres?'PostgreSQL':'SQLite'} SS acquisition is repeat-safe and deploys the assigned skill in a separate slot`,async t=>{
 ...
 const pools=mercenaryGradePools(f.document.mercenaries,seed.catalog.cards.map(c=>c.code)),index=pools.SS.indexOf('V-050');assert.ok(index>=0);
 ...
 const before=await f.coin(),request={requestId:crypto.randomUUID(),count:1};const result=await openMercenaryCards(f.env,f.user,request,{randomInt:n=>n===1000000?0:ticket});assert.equal(result.draws[0].mercenaryCode,'V-050');
 ...
 await saveMercenaryLoadout(f.env,f.user,{requestId:crypto.randomUUID(),mercenaryCode:'V-050',revision:0});const deployed=await loadMercenaryBattleSnapshot(f.env,f.user);
 assert.equal(deployed.rank,'SS');assert.equal(deployed.basePower,120000);assert.equal(deployed.sourceArt,art.sourceArt);assert.equal(deployed.battleSprite,art.battleSprite);assert.deepEqual(deployed.skills.map(s=>s.id),['MS-050']);
});
test('PVE and both PVP sides receive five regular cards plus SS Orikkung and immediate skill events',()=>{
 ...
 for(const data of [createPveBattleV2({cards,mercenary:merc,monster:{id:1,name:'검수',battle_power:1000000},seed:17}),createPvpBattleV2({attackerCards:cards,defenderCards:cards,attackerMercenary:merc,defenderMercenary:merc,seed:12})]){
  assert.equal(data.teams.A.cards.length,5);assert.equal(data.teams.A.mercenaries.length,1);assert.equal(data.teams.A.mercenaries[0].cardId,'V-050');assert.ok(data.result.timeline.some(e=>e.skillId==='MS-050'&&e.type==='MERCENARY_HIT'));
 }
});
