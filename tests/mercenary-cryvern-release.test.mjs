...
const weights={'V-021':8991,'V-046':999,'V-049':10,'V-055':10};
test('approved Cryvern uses a new code; Heukwol and every previous character survive',()=>{
 ...
 assert.deepEqual(sss,['V-021','V-046','V-049','V-055']);
});
test('exactly 10 of 10010 SSS tickets select Cryvern, while the others stay 9:1',()=>{
 ...
 assert.deepEqual(odds.map(c=>c.withinRankPercent),[8991,999,10,10].map(w=>w/10010*100));
 assert.equal(odds.at(-1).percent,10/(10000*10010));
 assert.equal(mercenaryCardChances(50,sss,policy.cardRules).at(-1).percent,500/(10000*10010));
 const counts=Object.fromEntries(sss.map(c=>[c,0]));
 const rankTicket=policy.outcomes.slice(0,5).reduce((sum,r)=>sum+r.chancePpm,0);
 for(let ticket=0;ticket<10010;ticket++){
...
test('deployment-to-CMS-save gap uses 0.1%, never default weight 1; saved settings remain authoritative',()=>{
 ...
 const effective=cryvernSelectionWeights(sss.filter(c=>c!=='V-055'),old);
 assert.deepEqual(effective,{'V-021':8991,'V-046':999,'V-049':10,'V-044':3});assert.deepEqual(old,before);
 assert.deepEqual(mercenaryCardChances(1,sss,{cardWeights:old}).map(c=>c.weight),[8991,999,10,10]);
...
test('SS-to-SSS fusion shares the same rare within-rank selection without changing promotion odds',()=>{
 ...
 assert.equal(pickFusionResult({rank:'SS',pools,rules,randomInt:max=>max===10000?9999:9989}).mercenaryCode,'V-046');
 assert.equal(pickFusionResult({rank:'SS',pools,rules,randomInt:max=>max===10000?9999:9990}).mercenaryCode,CRYVERN_CODE);
});
