...
test('approved Hi Heeya artwork is promoted once into the SS gameplay catalog without rewriting existing CMS values',()=>{
  ...
  assert.equal(result.cards.length,50);assert.equal(result.revision,54);assert.equal(result.artReleaseVersion,'20260919-ragniel-live-v1');
  ...
  assert.equal(result.cards[0].name,'운영 저장 이름');assert.deepEqual(saved,before);assert.equal(seed.catalog.cards.length,50);
