...
test('approved Hi Heeya artwork is published as SS without changing saved CMS or draw eligibility',()=>{
  ...
  assert.equal(result.cards.length,44);assert.equal(result.revision,54);assert.equal(result.artReleaseVersion,'20260915-hi-heeya-v1');
  assert.equal(heeya.name,'하이희야');assert.equal(heeya.rank,'SS');assert.equal(heeya.artOnly,true);
  assert.equal(heeya.basePower,null);assert.equal(heeya.battleSprite,null);assert.deepEqual(heeya.skills,[]);
  assert.equal(result.cards[0].name,'운영 저장 이름');assert.deepEqual(saved,before);assert.equal(seed.catalog.cards.length,43);
  ...
  const invalid=structuredClone(result);invalid.cards.at(-1).basePower=120000;assert.throws(()=>validateCatalog(invalid));
});
