...
assert.match(serverSynthesis, /ORDER BY x\.id LIMIT \?`\)\.bind\(user\.id,equipmentId,required\)/);
assert.match(serverSynthesis, /if\(\(inputs\.results\|\|\[\]\)\.length!==required\)/);
assert.match(serverSynthesis, /x\.id IN \(\$\{marks\}\)\)=\? THEN 1 ELSE 0 END`\)\.bind\(guardId,user\.id,user\.id,equipmentId,\.\.\.instanceIds,required\)/);
