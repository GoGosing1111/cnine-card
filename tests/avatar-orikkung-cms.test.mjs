...
const marker='safe_runtime_upgrade_orikkung_zenith_avatar_v1';
const coldModule=()=>import(`../functions/_avatar.js?orikkung-qa=${crypto.randomUUID()}`);

test('Orikkung CMS registration executes atomically against PostgreSQL without releasing or granting it',async t=>{
  const pg=new PGlite();
  ...
  try{
    ...
    await t.test('admin list includes all three real art paths, but user catalog and purchases remain gated',async()=>{
      ...
      const item=response.body.avatars.find(a=>a.code==='ORIKKUNG_ZENITH');
      assert.equal(item.serial,'A-15');assert.equal(item.name,'오리꿍');
      ...
      assert.equal(item.lobbyImage,'preview/avatar-orikkung-v1/assets/avatar-orikkung-lobby-v1-1024.webp');
      assert.equal(item.lobbyMobileImage,'preview/avatar-orikkung-v1/assets/avatar-orikkung-lobby-v1-640.webp');
      assert.equal(item.equipmentImage,'preview/avatar-orikkung-v1/assets/avatar-orikkung-equipment-v1-640.webp');
      ...
      assert.ok(!catalog.body.avatars.some(a=>a.code==='ORIKKUNG_ZENITH'));
      assert.equal((await call('avatar/equip',{avatarCode:'ORIKKUNG_ZENITH'})).status,404);
      assert.equal((await call('avatar/purchase',{avatarCode:'ORIKKUNG_ZENITH',requestId:'orikkung-qa-purchase'})).status,404);
      ...
      legacy=await all("SELECT * FROM avatar_catalog_v1 WHERE code<>'ORIKKUNG_ZENITH' ORDER BY code");
    });

    await t.test('CMS edits are saved normally and survive a cold-start seed replay',async()=>{
      const saved=await call('admin/avatars',{action:'SAVE_AVATAR',code:'ORIKKUNG_ZENITH',version:1,
      ...
      const before=await one("SELECT * FROM avatar_catalog_v1 WHERE code='ORIKKUNG_ZENITH'");
      const effects=await all("SELECT * FROM avatar_effect_options_v1 WHERE avatar_code='ORIKKUNG_ZENITH'");
      await pg.query('DELETE FROM app_meta WHERE key=$1',[marker]);
      api=await coldModule();await api.ensureAvatarFoundation(env);
      assert.deepEqual(await one("SELECT * FROM avatar_catalog_v1 WHERE code='ORIKKUNG_ZENITH'"),before);
      assert.deepEqual(await all("SELECT * FROM avatar_effect_options_v1 WHERE avatar_code='ORIKKUNG_ZENITH'"),effects);
      assert.deepEqual(await all("SELECT * FROM avatar_catalog_v1 WHERE code<>'ORIKKUNG_ZENITH' ORDER BY code"),legacy);
      assert.equal(Number((await one("SELECT COUNT(*) n FROM avatar_catalog_v1 WHERE code='ORIKKUNG_ZENITH'")).n),1);
      assert.equal(logs.length,1);
    });

    await t.test('marker failure rolls back insertion and permits a clean retry',async()=>{
      await pg.exec("DELETE FROM avatar_effect_options_v1 WHERE avatar_code='ORIKKUNG_ZENITH'; DELETE FROM avatar_catalog_v1 WHERE code='ORIKKUNG_ZENITH';");
      await pg.query('DELETE FROM app_meta WHERE key=$1',[marker]);
      api=await coldModule();failMarker=true;
      await assert.rejects(api.ensureAvatarFoundation(env),/QA marker write failed/);
      assert.equal(Number((await one("SELECT COUNT(*) n FROM avatar_catalog_v1 WHERE code='ORIKKUNG_ZENITH'")).n),0);
