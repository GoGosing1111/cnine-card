...
test('ON survives a store rerender; OFF and back navigation refresh the same status and buttons',async()=>{
 ...
 const settle=()=>new Promise(resolve=>setImmediate(resolve));await settle();assert.equal(replaceable.buttons[0].disabled,false);assert.match(replaceable.statuses[0].textContent,/가중치/);
