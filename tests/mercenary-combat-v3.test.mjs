...
// v2119: 1단계가 시전 행동에서 바로 터지고, 정화된 몫은 사라지며, 재장전 지연은 없다.
test('two-stage rift keeps the original total budget, permits cleanse, and never delays the next basic',()=>{const h=harness('RIFT_MARK_DETONATION'),extra={...h.b,id:'B:OTHER',slot:1};h.teams.B.push(extra);h.turn(h.a);assert.equal(h.b.hp,9750);assert.equal(extra.hp,9750);h.runtime.cleanse(h.b);h.turn(h.a);assert.equal(h.b.hp,9750);assert.equal(extra.hp,9500);assert.equal(h.runtime.state(h.a).reload,undefined);h.a.gauge=50;h.turn(h.a);assert.equal(h.a.gauge,50);});
// v2119: 준비 행동이 사라져 "준비 중 피격"이라는 창이 없어졌다. 이 메커니즘은 어떤 용병에도
// 배정돼 있지 않으며, 지금은 시전 행동에서 바로 발사되므로 항상 집중 상태로 나간다.
test('undisturbed first shot now fires focused in its casting action and is still once per battle',()=>{const h=harness('UNDISTURBED_FIRST_SHOT');h.runtime.onDamage(h.a,{hpDamage:0,absorbed:1});h.turn(h.a);assert.equal(h.events.find(e=>e.type==='MERCENARY_FOCUS').focused,true);h.a.actions=100;h.turn(h.a);assert.equal(h.events.filter(e=>e.type==='MERCENARY_WINDUP').length,1);});
test('interception conserves incoming damage and affects only one direct hit',()=>{const h=harness('INTERCEPT_ONE_HIT');h.turn(h.a);h.turn(h.a);const amount=h.runtime.beforeBasicDamage(h.b,h.c,100);assert.equal(amount,30);assert.equal(h.a.hp,9930);assert.equal(h.runtime.beforeBasicDamage(h.b,h.c,100),100);});
test('interception records the protecting actor and original damage owner separately',()=>{const h=harness('INTERCEPT_ONE_HIT');h.turn(h.a);h.turn(h.a);h.runtime.beforeBasicDamage(h.b,h.c,100);const e=h.events.find(e=>e.type==='MERCENARY_INTERCEPT');assert.equal(e.actorId,h.a.id);assert.equal(e.sourceAttackerId,h.b.id);assert.equal(h.b.damageDealt,70);assert.equal(h.a.damageDealt,0);});
...
test('area targets share the same authored stage and repeats cannot recharge or add offender marks',()=>{const h=harness('RIFT_MARK_DETONATION');h.teams.B.push({...h.b,id:'B:extra',slot:1});h.turn(h.a);assert.deepEqual(h.events.filter(e=>e.type==='MERCENARY_HIT').map(e=>e.skillPhaseIndex),[0,0]);h.turn(h.a);assert.deepEqual(h.events.filter(e=>e.type==='MERCENARY_HIT').map(e=>e.skillPhaseIndex),[0,0,1,1]);const r=harness('REPEAT_OFFENDER_RESTRAINT');r.runtime.afterBasic(r.b,r.c,true,{additional:true});assert.equal(r.runtime.debuffs.get(r.b.id)?.offender,undefined);});
...
// v2119: 정화는 시전 행동에서 즉시 일어나고(행동을 소모하지 않는다), 회복은 다음 행동에서 따로 온다.
test('cleanse removes poison in the casting action before the separately timed heal and never revives a lost target',()=>{const h=harness('CLEANSE_THEN_MEND');h.runtime.debuffs.set(h.c.id,{poison:{damage:99}});h.turn(h.a);assert.equal(h.runtime.debuffs.get(h.c.id).poison,undefined);assert.equal(h.c.hp,100);h.turn(h.a);assert.equal(h.c.hp,1100);const g=harness('CLEANSE_THEN_MEND');g.runtime.debuffs.set(g.c.id,{poison:{damage:99}});g.turn(g.a);g.c.hp=0;g.c.alive=false;g.turn(g.a);assert.equal(g.c.hp,0);assert.ok(!g.events.some(e=>e.type==='MERCENARY_HEAL'));});
...
// v2119: 발도는 시전 행동에서 끝나고 재장전 지연이 없다.
test('moon draw uses a bounded missing-health multiplier and never owes a recovery turn',()=>{
 const h=harness('WOUNDED_MOON_DRAW');h.b.hp=5000;h.turn(h.a);assert.equal(h.b.hp,3750);assert.equal(h.runtime.state(h.a).reload,undefined);
 const g=harness('WOUNDED_MOON_DRAW');g.b.hp=1;g.turn(g.a);assert.equal(g.b.hp,0);assert.equal(g.runtime.state(g.a).reload,undefined);
});
