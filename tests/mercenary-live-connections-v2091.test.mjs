...
for(const postgres of [false,true]){
 ...
 test(`${label}: S6 and SS8 CMS assignments reach owned cards, separate loadout and real PVE snapshots`,async t=>{
  const f=await mercenaryFixture(t,{postgres}),d=structuredClone(f.document);for(const c of d.mercenaries)c.rank=plan.targets.find(r=>r.code===c.code)?.rank||(['V-021','V-046'].includes(c.code)?'SSS':['V-044','V-045','V-047','V-048'].includes(c.code)?'SS':'C');
