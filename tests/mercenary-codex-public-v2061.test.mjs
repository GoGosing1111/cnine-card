...
test('codex keeps public catalog browsing while account writes are limited to the existing loadout route', () => {
  ...
  assert.ok(roster.cards.filter(c=>!['V-044','V-045','V-047','V-048'].includes(c.code)).every(card => ['V-021','V-046'].includes(card.code)? card.rank === 'SSS' && card.rankStatus === 'USER_ASSIGNED_RANK' : card.rank === null && card.rankStatus === 'PENDING_USER_ASSIGNMENT'));
