...
test('public information remains read-only with separate local favorites and no guessed ranks', () => {
  ...
  assert.ok(roster.cards.filter(c=>!['V-044','V-045'].includes(c.code)).every(card => ['V-021','V-046'].includes(card.code)? card.rank === 'SSS' && card.rankStatus === 'USER_ASSIGNED_RANK' : card.rank === null && card.rankStatus === 'PENDING_USER_ASSIGNMENT'));
