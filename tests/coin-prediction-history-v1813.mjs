...
assert.match(server, /const USER_MAX_BET_PER_EVENT=2000000000;/);
...
assert.match(server, /이벤트당 누적 최대 2,000,000,000코인/);
...
assert.match(userUi, /const USER_MAX_BET_PER_EVENT = 2000000000;/);
assert.match(userUi, /경기당 최대 20억/);
assert.match(userUi, /이벤트 누적 최대는 20억 코인/);
...
assert.match(adminUi, /이벤트당 최대<\/span><b>2,000,000,000/);
...
assert.match(app, /coin-prediction-v2033\.js\?v=2045-poll-perf-prediction-2b/);
...
assert.match(adminIndex, /coin-prediction-admin-v1\.js\?v=2033-matchday-prediction-2b/);
