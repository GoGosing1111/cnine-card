...
test('all six ranks without an assigned skill still perform basic attacks at real deck power',()=>{
 for(const [rank,power]of Object.entries(powerStandard.basePowerByRank)){
  ...
  for(const key of ['attack','defense','speed'])assert.equal(actual.stats[key],fixed[key],rank+' base '+key+' stays fixed before opening buffs');
  assert.ok(actual.attack>=actual.stats.attack,rank+' publishes its applied opening attack buff');
