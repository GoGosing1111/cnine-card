...
test('operation board ships desktop and mobile game UI with cache-busted assets',async()=>{
  ...
  assert.match(index,/app\.js\?v=2021-core-protocol-user-test/);
  assert.match(index,/soopketmon-v21-exact-shell-adapter\.js\?v=21\.19\.0-refresh-home-sticky/);
  assert.match(worker,/soop-card-shell-v2021-core-protocol-user-test/);
  assert.match(shell,/const VERSION = '21\.19\.0'/);
});
