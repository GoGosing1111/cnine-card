...
test('operation board ships desktop and mobile game UI with cache-busted assets',async()=>{
  ...
  assert.match(index,/app\.js\?v=2061-mercenary-codex/);
  assert.match(index,/soopketmon-v21-exact-shell-adapter\.js\?v=21\.26\.0-mercenary-codex/);
  assert.match(worker,/soop-card-shell-v2061-mercenary-codex/);
  assert.match(shell,/const VERSION = '21\.26\.0-mercenary-codex'/);
});
