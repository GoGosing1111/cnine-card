...
test('일반 공개 OFF 상태에서 OWNER와 지정 닉네임 세 명만 개봉할 수 있다', () => {
  assert.deepEqual([...SUPERSTAR_PACK_EARLY_ACCESS_NICKNAMES], ['조은', '강구열', '진짜디임']);
...
test('클라이언트는 일반팩을 제거하고 슈퍼스타팩을 우측 끝에 배치한다', () => {
  ...
  assert.match(source, /const SUPERSTAR_PACK_EARLY_ACCESS_NICKNAMES=new Set\(\['조은','강구열','진짜디임'\]\);/);
...
test('팩 원본·반응형 리소스와 전용 스타일이 배포 엔트리에 연결된다', () => {
  ...
  assert.match(index, /app\.js\?v=1921-inventory-reroll-route/);
  assert.match(serviceWorker, /soop-card-shell-v1916-territory-100-attack-reward/);
