...
const inventoryRoute=new AsyncFunction('deps',`const {env,request,authenticate,json,ensureForgeProtectionCatalog,FORGE_PROTECTION_ITEM}=deps;
...
for(const postgres of [false,true]){
  ...
  test(`${label}: actual inventory route returns owned protection artwork and quantity but no direct-use action`,async t=>{
    ...
    const deps={env:f.env,request:new Request('https://qa.test/api/inventory'),authenticate:async()=>f.user,json:(body,status=200)=>({body,status}),ensureForgeProtectionCatalog,FORGE_PROTECTION_ITEM};
