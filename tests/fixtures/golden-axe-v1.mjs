...
export async function fixture(){
 ...
 return {pg,env,doc,row,configure,body,state:(id=1)=>goldenAxeState(env,id),draw:(body,id=1,n=0)=>drawGoldenAxe(env,id,body,{randomInt:max=>n%max}),use:body=>useGoldenAxeItem(env,1,body),fault:value=>{fault=value;},zero:value=>{zero=value;},expire:()=>{expire=true;clockReads=0;},clock:value=>{clock=value;},close:()=>pg.close()};
}
