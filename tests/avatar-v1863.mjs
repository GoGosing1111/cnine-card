...
test('avatar foundation seeds eighteen hidden unsold records including Hi Heeya, Cheon and Orikkung without overwriting settings', async () => {
  ...
  const legacySeedStatements=seedStatements.filter(statement=>!['DIMWOOS_ESPORTS_ACE','TERRAN_EMPRESS_JOEUN','HI_HEEYA','CHEON','ORIKKUNG_ZENITH','SAENGBYUWANG','HANBOK_DIIM','T1_JOEUN'].some(code=>statement.values.includes(code)));
  assert.equal(seedStatements.length,18);
  ...
  const cheonSeed=seedStatements.find(statement=>statement.values.includes('CHEON'));
  assert.ok(cheonSeed.values.includes('체온'));
  assert.ok(cheonSeed.values.includes('A-14'));
  assert.ok(cheonSeed.values.includes('preview/avatar-cheon-v1/assets/avatar-cheon-equipment-v1-640.webp'));
  assert.ok(prepared.some(statement=>statement.values.includes('safe_runtime_upgrade_v2068_cheon_avatar_v1')));
...
test('live avatar route is gated and wired through both V21 routers', async () => {
  ...
  assert.match(index,/app\.js\?v=2108-shared-navigation/);
