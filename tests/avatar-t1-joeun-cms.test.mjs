...
const marker='safe_runtime_upgrade_t1_joeun_avatar_v1';
const coldModule=()=>import(`../functions/_avatar.js?t1-joeun-qa=${crypto.randomUUID()}`);

test('T1 Joeun CMS registration executes atomically against PostgreSQL without releasing or granting it',async t=>{
  const pg=new PGlite();
  ...
  try{
    ...
    await t.test('admin list includes all three real art paths, but user catalog and purchases remain gated',async()=>{
      ...
      const item=response.body.avatars.find(a=>a.code==='T1_JOEUN');
      assert.equal(item.serial,'A-18');assert.equal(item.name,'T1 조은');
      ...
      assert.equal(item.lobbyImage,'preview/avatar-t1-joeun-v1/assets/avatar-t1-joeun-lobby-v1-1024.webp');
      assert.equal(item.lobbyMobileImage,'preview/avatar-t1-joeun-v1/assets/avatar-t1-joeun-lobby-v1-640.webp');
      assert.equal(item.equipmentImage,'preview/avatar-t1-joeun-v1/assets/avatar-t1-joeun-equipment-v1-640.webp');
      ...
      assert.ok(!catalog.body.avatars.some(a=>a.code==='T1_JOEUN'));
      assert.equal((await call('avatar/equip',{avatarCode:'T1_JOEUN'})).status,404);
      assert.equal((await call('avatar/purchase',{avatarCode:'T1_JOEUN',requestId:'t1-joeun-qa-purchase'})).status,404);
      ...
      legacy=await all("SELECT * FROM avatar_catalog_v1 WHERE code<>'T1_JOEUN' ORDER BY code");
    });

    await t.test('CMS edits are saved normally and survive a cold-start seed replay',async()=>{
      const saved=await call('admin/avatars',{action:'SAVE_AVATAR',code:'T1_JOEUN',version:1,
      ...
      const before=await one("SELECT * FROM avatar_catalog_v1 WHERE code='T1_JOEUN'");
      const effects=await all("SELECT * FROM avatar_effect_options_v1 WHERE avatar_code='T1_JOEUN'");
      await pg.query('DELETE FROM app_meta WHERE key=$1',[marker]);
      api=await coldModule();await api.ensureAvatarFoundation(env);
      assert.deepEqual(await one("SELECT * FROM avatar_catalog_v1 WHERE code='T1_JOEUN'"),before);
      assert.deepEqual(await all("SELECT * FROM avatar_effect_options_v1 WHERE avatar_code='T1_JOEUN'"),effects);
      assert.deepEqual(await all("SELECT * FROM avatar_catalog_v1 WHERE code<>'T1_JOEUN' ORDER BY code"),legacy);
      assert.equal(Number((await one("SELECT COUNT(*) n FROM avatar_catalog_v1 WHERE code='T1_JOEUN'")).n),1);
      assert.equal(logs.length,1);
    });

    await t.test('marker failure rolls back insertion and permits a clean retry',async()=>{
      await pg.exec("DELETE FROM avatar_effect_options_v1 WHERE avatar_code='T1_JOEUN'; DELETE FROM avatar_catalog_v1 WHERE code='T1_JOEUN';");
      await pg.query('DELETE FROM app_meta WHERE key=$1',[marker]);
      api=await coldModule();failMarker=true;
      await assert.rejects(api.ensureAvatarFoundation(env),/QA marker write failed/);
      assert.equal(Number((await one("SELECT COUNT(*) n FROM avatar_catalog_v1 WHERE code='T1_JOEUN'")).n),0);
    ...
    await t.test('a later CMS release and grant exposes the real art through the equipped-avatar contract',async()=>{
      ...
      const grant=await api.grantAvatarOwnership(env,{userId:1,avatarCode:'T1_JOEUN',sourceType:'EVENT',sourceRef:'qa-only'});
      assert.equal(grant.granted,true);
      const equipped=await call('avatar/equip',{avatarCode:'T1_JOEUN'});
      assert.equal(equipped.status,200,JSON.stringify(equipped.body));
      const avatar=await api.equippedAvatarEffect(env,1);
      assert.equal(avatar.name,'T1 조은');
      assert.equal(avatar.equipmentImage,'preview/avatar-t1-joeun-v1/assets/avatar-t1-joeun-equipment-v1-640.webp');
      assert.equal(avatar.lobbyImage,'preview/avatar-t1-joeun-v1/assets/avatar-t1-joeun-lobby-v1-1024.webp');
      assert.equal(avatar.lobbyMobileImage,'preview/avatar-t1-joeun-v1/assets/avatar-t1-joeun-lobby-v1-640.webp');
    });
