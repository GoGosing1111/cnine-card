...
test('43 approved roster codes each have a distinct, complete proposed assignment', () => {
  ...
  assert.equal(new Set(seed.assignments.map(entry => entry.specialty)).size,47);
  assert.equal(roster.cards.every(card => ['V-021','V-046'].includes(card.code)? card.rank === 'SSS' : ['V-044','V-045','V-047'].includes(card.code) ? card.rank === 'SS' : card.rank === null), true);
  assert.deepEqual(summarizePositions(seed), {
    total:47,
    positions: { FRONT: 21, MIDDLE: 18, REAR: 8 },
    roles: { GUARDIAN: 4, VANGUARD: 11, ASSASSIN: 5, MARKSMAN: 14, SNIPER: 4, CONTROLLER: 6, SUPPORT: 3 }
  });
});
