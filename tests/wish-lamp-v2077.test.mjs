...
test('retired wish lamp cannot charge, grant, recreate tickets or be turned back on by cached clients',async()=>{
 ...
 assert.equal(await handleWishLamp({path:'events/golden-axe/state',env,deps}),null);
});
test('live menu, CMS and old page links retire wish lamp and point at the new event',()=>{
 ...
 assert(!read('functions/_wish_lamp.js').includes('INSERT INTO'));assert.match(read('functions/api/[[path]].js'),/i.code<>'PINGDU_WISH_TICKET'/);
});
