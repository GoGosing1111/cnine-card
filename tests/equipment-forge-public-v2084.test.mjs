...
test('approved power is identical in status and CMS while save preserves the execution lock',async()=>{
  const f=await fixture();try{
    ...
    const saved=await f.call('admin',{method:'PATCH',body:settings()});assert.deepEqual(saved.body.powerStandard,admin.body.powerStandard);assert.equal(saved.body.executionReady,true);
...
test('read-only account inventory separates duplicates and includes every armor slot without hidden or foreign items',async()=>{
  const f=await fixture();try{const before=await f.rows('SELECT * FROM user_equipment_loadout');const r=await f.call();assert.equal(r.status,200);assert.deepEqual(r.body.items.map(x=>x.instanceId),['13','12','11','14','16','15']);assert.equal(r.body.wallet.coins,'5000000000');assert.deepEqual(r.body.items.filter(x=>x.equipped).map(x=>x.instanceId),['13']);assert.ok(r.body.items.every(x=>x.enhancement===null));
    const page=await f.call('state',{query:'?group=armor&limit=2'});assert.deepEqual(page.body.items.map(x=>x.instanceId),['13','12']);assert.equal(page.body.nextCursor,'12');const next=await f.call('state',{query:'?group=armor&limit=2&beforeId=12'});assert.deepEqual(next.body.items.map(x=>x.instanceId),['14','15']);assert.equal(next.body.nextCursor,null);
...
test('CMS cannot enable without an approved release document and a manual DB ON fails closed',async()=>{
  const f=await fixture();try{assert.equal(FORGE_EXECUTION_IMPLEMENTED,true);assert.equal((await f.call('admin',{method:'PATCH',body:settings(0,{executionMode:'ON'})})).status,423);const forced={schemaVersion:1,revision:1,publicVisible:true,executionMode:'ON',notice:'forced'};await f.pg.query('INSERT INTO app_meta(key,value) VALUES($1,$2)',[FORGE_SETTINGS_KEY,JSON.stringify(forced)]);assert.equal((await f.call('enhance',{method:'POST'})).status,503);assert.equal((await f.call('status')).body.canEnhance,false);assert.equal((await f.rows('SELECT count(*) AS count FROM user_equipment_instances'))[0].count,10);}finally{await f.close();}
});
