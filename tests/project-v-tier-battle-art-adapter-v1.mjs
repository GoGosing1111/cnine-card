...
assert.equal(manifests.SUPERSTAR.characters.length,8);
const all=[...manifests.FUR.characters,...manifests.PRESTIGE.characters,...manifests.SUPERSTAR.characters];
assert.equal(new Set(all.map(row=>row.cardId)).size,50);
...
await adapter.ready();
assert.equal(adapter.getMappedCardIds().length,50);
