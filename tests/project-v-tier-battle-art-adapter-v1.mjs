...
const all=[...manifests.FUR.characters,...manifests.PRESTIGE.characters];
assert.equal(new Set(all.map(row=>row.cardId)).size,42);
...
await adapter.ready();
assert.equal(adapter.getMappedCardIds().length,42);
...
console.log('project-v FUR/PRESTIGE battle art adapter: OK');
