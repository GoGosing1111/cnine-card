...
for(const postgres of [false,true])test(`${postgres?'PostgreSQL':'SQLite'}: 20일 라운드 전적을 무효화하고 21일 재경기 및 이후 일정 순연을 원자 적용한다`,async t=>{
  ...
  for(const [clan,score,wins,losses] of [[1,3,1,0],[2,0,0,1],[3,3,1,0],[4,0,0,1]])await f.p('INSERT INTO clan_season_teams VALUES(7,?,?,?,?,CURRENT_TIMESTAMP)',clan,score,wins,losses).run();
  for(let user=1;user<=4;user++)await f.p('INSERT INTO clan_members VALUES(7,?,?,?, ?,?,CURRENT_TIMESTAMP)',user,user,user===1?1:0,user===2?1:0,user===1?3:0).run();
  ...
  const replay=await f.p('SELECT * FROM clan_wars WHERE id=10').first();assert.equal(replay.status,'SCHEDULED');assert.equal(replay.starts_at,'2026-09-21T12:00:00.000Z');assert.equal(replay.score_a,0);assert.equal(replay.winner_clan_id,null);
  ...
  const team=await f.p('SELECT score,wins,losses FROM clan_season_teams WHERE clan_id=1').first();assert.deepEqual([team.score,team.wins,team.losses],[0,0,0]);
  const member=await f.p('SELECT battle_wins,battle_losses,contribution_score FROM clan_members WHERE user_id=1').first();assert.deepEqual([member.battle_wins,member.battle_losses,member.contribution_score],[0,0,0]);
