...
for(const postgres of [false,true])test(`${postgres?'PostgreSQL':'SQLite'}: 월요일 재경기는 화목토일 정규 일정과 대진을 보존한다`,async t=>{
  const f=await fixture(t,postgres);await f.p("INSERT INTO app_meta VALUES('clan_settings_v1',?,CURRENT_TIMESTAMP)",JSON.stringify({seasonWinScore:3,seasonLossScore:0,scheduleEnabled:true,warOpenTime:'21:00',openDays:[0,2,4,6]})).run();
  await f.p("INSERT INTO clan_seasons VALUES(7,'ACTIVE','2026-09-27T13:00:00.000Z',CURRENT_TIMESTAMP)").run();
  ...
  for(const [index,day] of [22,24,26,27].entries())for(const [pair,a,b] of [[0,1,3],[1,2,4]]){
    await f.p("INSERT INTO clan_wars VALUES(?,7,?,?,?,'SCHEDULED',0,0,0,?,?,NULL,CURRENT_TIMESTAMP)",12+index*2+pair,2+index,a,b,`2026-09-${day}T12:00:00.000Z`,`2026-09-${day}T13:00:00.000Z`).run();
  }
  const regularBefore=(await f.p('SELECT * FROM clan_wars WHERE id>=12 ORDER BY id').all()).results;
  ...
  const result=await ensureClanRematch20260920(f.env);assert.equal(result.status,'COMPLETED',JSON.stringify(result));assert.equal(result.roundNo,1);assert.equal(result.voidedBattles,1);assert.equal(result.retainedParticipationCoin,12);assert.equal(result.retainedPigCoin,90);assert.equal(result.shiftedRounds,0);
  ...
  assert.deepEqual((await f.p('SELECT * FROM clan_wars WHERE id>=12 ORDER BY id').all()).results,regularBefore);
  assert.equal((await f.p('SELECT ends_at FROM clan_seasons WHERE id=7').first()).ends_at,'2026-09-27T13:00:00.000Z');
...
for(const postgres of [false,true])test(`${postgres?'PostgreSQL':'SQLite'}: 재경기 시간 충돌은 정규경기를 임의 순연하지 않고 차단한다`,async t=>{
  const f=await fixture(t,postgres);
  await f.p("INSERT INTO clan_seasons VALUES(7,'ACTIVE','2026-09-27T13:00:00.000Z',CURRENT_TIMESTAMP)").run();
  await f.p("INSERT INTO clan_wars VALUES(10,7,1,1,2,'COMPLETED',3,1,0,'2026-09-20T12:00:00.000Z','2026-09-20T13:00:00.000Z',1,CURRENT_TIMESTAMP)").run();
  await f.p("INSERT INTO clan_wars VALUES(12,7,2,1,3,'SCHEDULED',0,0,0,'2026-09-21T12:30:00.000Z','2026-09-21T13:30:00.000Z',NULL,CURRENT_TIMESTAMP)").run();
  const before=(await f.p('SELECT * FROM clan_wars ORDER BY id').all()).results;
  const result=await ensureClanRematch20260920(f.env);
  assert.equal(result.status,'BLOCKED');assert.equal(result.reason,'REPLAY_WINDOW_CONFLICT_12');
  assert.deepEqual((await f.p('SELECT * FROM clan_wars ORDER BY id').all()).results,before);
});

for(const postgres of [false,true])test(`${postgres?'PostgreSQL':'SQLite'}: 기존 재경기 완료 영수증은 재경기 결과와 복구한 정규 일정을 다시 변경하지 않는다`,async t=>{
  const f=await fixture(t,postgres);
  await f.p('INSERT INTO app_meta VALUES(?,?,CURRENT_TIMESTAMP)',CLAN_REMATCH_20260920_KEY,JSON.stringify({status:'COMPLETED',seasonId:7,roundNo:1,warIds:[10],shiftedRounds:4})).run();
  await f.p("INSERT INTO clan_wars VALUES(10,7,1,1,2,'COMPLETED',30,15,20,'2026-09-21T12:00:00.000Z','2026-09-21T13:00:00.000Z',1,CURRENT_TIMESTAMP)").run();
  await f.p("INSERT INTO clan_wars VALUES(12,7,2,1,3,'SCHEDULED',0,0,0,'2026-09-22T12:00:00.000Z','2026-09-22T13:00:00.000Z',NULL,CURRENT_TIMESTAMP)").run();
  const before=(await f.p('SELECT * FROM clan_wars ORDER BY id').all()).results;
  const result=await ensureClanRematch20260920(f.env);assert.equal(result.status,'COMPLETED');
  assert.deepEqual((await f.p('SELECT * FROM clan_wars ORDER BY id').all()).results,before);
});
