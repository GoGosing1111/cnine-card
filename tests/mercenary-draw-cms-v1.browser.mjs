...
try{
  for(const [width,height] of [[1440,1000],[1024,900],[390,844],[360,740]]){
    ...
    await pg.exec('CREATE TABLE app_meta(key TEXT PRIMARY KEY,value TEXT)');
    ...
    await page.route(origin+'/api/admin/mercenaries/opening',route=>route.fulfill({json:{mode:'OFF',ready:true,rankCounts:{SSS:2},drawRevision:1,cmsRevision:1,blockers:[]}}));
    ...
    assert.match(await page.locator('.md-card-rules').innerText(),/용병별 가중치 적용/);
    ...
    assert.match(await page.locator('[data-draw-card-chance="SSS"]').innerText(),/카드당 0\.00005% · 등급 내 1\/2/);
    await page.locator('[data-draw-weight="V-021"]').fill('9');
    await page.locator('[data-draw-weight="V-046"]').fill('1');
    assert.match(await page.locator('[data-draw-weight-rate="V-021"]').innerText(),/등급 내 90% · 전체 개봉 0\.00009%/);
    assert.match(await page.locator('[data-draw-weight-rate="V-046"]').innerText(),/등급 내 10% · 전체 개봉 0\.00001%/);
    ...
    await page.locator('[data-draw-weight="V-021"]').scrollIntoViewIfNeeded();await page.screenshot({path:path.join(out,'weights-'+width+'.png')});
    ...
    await page.locator('[data-draw-reason]').fill('기기별 확률 수량 저장 검증');await page.locator('[data-draw-save]').click();await page.locator('[data-draw-message]').filter({hasText:'운영 확률·수량 저장 완료'}).waitFor();
    ...
    assert.equal(await page.locator('[data-draw-weight="V-021"]').inputValue(),'9');assert.equal(await page.locator('[data-draw-weight="V-046"]').inputValue(),'1');
    ...
    assert.deepEqual(exported.policy.cardRules,{sameRankSelection:'WEIGHTED',ownershipWeighting:'NONE',duplicateHandling:'COUNT_EXTRA_COPIES',cardWeights:{'V-021':9,'V-046':1}});
    ...
    await page.locator('[data-draw-save]').click();await page.locator('[data-draw-message]').filter({hasText:'운영 확률·수량 저장 완료'}).waitFor();assert.equal(writes,2);
