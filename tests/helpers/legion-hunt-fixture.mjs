...
export async function legionFixture({postgres=false}={}){
  ...
  const env={DB},clock={now:1000},owner={id:1,role:'OWNER'},player={id:2,role:'USER'};
  let user=owner;
  ...
  return {env,DB,clock,owner,player,deps,call,configure,setUser:u=>user=u,queries,resetQueries:()=>{queries.length=0;},fail:v=>failure=v,loseReply:()=>lostReply=true,close:async()=>{if(pg)await pg.close();else sql.close();}};
}
