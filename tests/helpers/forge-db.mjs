...
export async function forgeFixture(t,options){
 ...
 for(const s of policy.steps)Object.assign(s,{successPpm:500000,maintainPpm:250000,destroyPpm:250000,coinCost:100,itemCode:'FORGE_TEST_MATERIAL',itemQuantity:2,protectionQuantity:1});await f.setting(FORGE_RUNTIME_KEY,policy);
 for(const code of ['FORGE_TEST_MATERIAL','FORGE_TEST_PROTECTION']){await f.p('INSERT INTO inventory_items(code,name,rarity,image_url) VALUES(?,?,?,?)',code,code,'SPECIAL','/test.png').run();await f.p('INSERT INTO cnine_user_inventory(user_id,item_code,quantity) VALUES(7,?,100)',code).run();}
