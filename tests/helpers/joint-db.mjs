...
export async function jointFixture(t,{postgres=false,filename,productionEquipmentRequests=false}={}){
  invalidateUnifiedDropPoolCache();__idleDungeonTest.resetCaches();let DB,pg,failAt='';
  const schema=productionEquipmentRequests?jointFixtureSchema.map(s=>s.startsWith('CREATE TABLE user_equipment_instances(')?s.replace('request_id TEXT UNIQUE','request_id TEXT'):s):jointFixtureSchema;
  ...
  }else{DB=new JointSQLiteDB(filename);DB.sql.exec(schema.join(';'));}
  ...
  for(let i=0;i<6;i++)await discoverCowPortalReady(env,{id:7},{battleMode:'PVE',sourceType:'HUNT',sourceRef:`fixture-portal-${i}`,result:'WIN'},{randomInt:()=>0});
