...
test('reads the canonical 42-card preview roster and never inherits historic ranks', () => {
  ...
  assert.deepEqual(summarize(roster.cards), { total: 42, sourceReady: 42, spriteReady: 37, rankPending: 42, positions: { 전위: 20, 중거리: 13, 후열: 9 } });
});
...
test('combined position/role/search filters do not mutate or omit roster records', () => {
  ...
  assert.equal(filterCards(roster.cards).length, 42);
...
test('responsive WebP derivatives are complete, traceable, transparent for SD and keep every source hash', async () => {
  ...
  assert.equal(media.entries.length, 121);
  let listBytes = 0;
  const seen = new Set();
