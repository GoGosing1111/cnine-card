...
test('reads the canonical 55-card preview roster and never inherits historic ranks', () => {
  ...
  assert.deepEqual(summarize(roster.cards), { total:55, sourceReady:55, spriteReady:55, rankPending: 42, positions: { 전위: 23, 중거리: 17, 후열: 15 } });
});
...
test('combined position/role/search filters do not mutate or omit roster records', () => {
  ...
  assert.equal(filterCards(roster.cards).length,55);
  assert.deepEqual(filterCards(roster.cards, { position: '후열', role: '저격' }).map(card => card.code), ['V-004', 'V-008', 'V-025', 'V-036', 'V-050']);
  ...
  assert.equal(filterCards(roster.cards, { sort: 'newest' })[0].code, 'V-055');
...
test('responsive WebP derivatives are complete, traceable, transparent for SD and keep every source hash', async () => {
  ...
  assert.equal(media.entries.length, 165);
  let listBytes = 0;
  const seen = new Set();
