...
test('reads the canonical 41-card preview roster and never inherits historic ranks', () => {
  ...
  assert.deepEqual(summarize(roster.cards), { total: 41, sourceReady: 41, spriteReady: 37, rankPending: 41, positions: { 전위: 20, 중거리: 12, 후열: 9 } });
});
...
test('combined position/role/search filters do not mutate or omit roster records', () => {
  ...
  assert.equal(filterCards(roster.cards).length, 41);
...
test('responsive WebP derivatives are complete, traceable, transparent for SD and keep every source hash', async () => {
  ...
  assert.equal(media.entries.length, 119);
  let listBytes = 0;
  const seen = new Set();
