...
const settings=cleanChuseokSettings();
const endpoints={
 'shell/summary':{avatarFeature:{visible:true},alchemyFeature:{visible:false}},
 'events/chuseok/feature':{visible:true,events:settings.events},
 'events/chuseok/state':{...settings,userId:4242,coin:'1000000000',chuseokCoins:0,events:Object.fromEntries(Object.entries(settings.events).map(([key,s])=>[key,{...s,phase:'UNCONFIGURED',dailyUsed:0}])),serverNow:new Date().toISOString(),history:[]},
...
try{
 for(const viewport of [{width:1440,height:1000},{width:1024,height:768},{width:390,height:844},{width:320,height:740}]){
  ...
  for(const [url,route,ready] of [['/equipment-forge/','equipmentForge','#inventory-note'],['/mercenary-codex/','mercenaryDex','.roster-row'],['/events/chuseok/','chuseok','#ckStatus']]){
