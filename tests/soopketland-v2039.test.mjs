...
async function fixture(){
  ...
  for(const code of ['MASTER_STAR','BLACK_MIRACLE_PACK','HIGH_GRADE_REROLL_TICKET','STARLIGHT_ARMOR_CORE'])sqlite.prepare('INSERT INTO inventory_items(code,is_active) VALUES(?,1)').run(code);
...
test('previously issued ZENITH/FUR coupons grant public active cards and preserve enhancement; empty pools preserve coupons',async()=>{
  const f=await fixture(),code=issuedCoupon(f,'ZENITH_RANDOM_CARD',1,2);
  ...
  const fur=issuedCoupon(f,'FUR_RANDOM_CARD');assert.equal((await f.redeem(fur)).body.card.id,'f1');
});
test('previously issued Hyper tickets retain server-wide 15x/60min activation and idempotency',async()=>{
  ...
  const oldId=crypto.randomUUID(),oldResult={ok:true,requestId:oldId,prize:{key:HYPER_TICKET,amount:1},code:null,delivery:'STREAMER_INVENTORY',couponUses:0};
  f.sqlite.prepare('INSERT INTO soopketland_rolls VALUES(?,?,?,?,?)').run(oldId,2,'old-lot',JSON.stringify(oldResult),'2026-09-12T12:00:00Z');
  assert.deepEqual((await f.spin(oldId)).body,{...oldResult,replayed:true});assert.equal(f.qty(HYPER_TICKET),1);
...
test('all stopped, finite-edition and globally excluded random cards preserve unused coupons',async()=>{
  const f=await fixture(),code=issuedCoupon(f,'FUR_RANDOM_CARD',1,2);
...
test('client integration is shared, recovers pending requests, and never renders a self-redeem action for event codes',()=>{
  ...
  assert.match(app,/data-copy-event-coupon/);assert.match(app,/activateLandHyperTicket/);assert.match(app,/soopketland:\{/);assert.match(index,/js\/app\.js\?v=2091-live-connections/);
...
function issuedCoupon(f,key,amount=1,uses=1){
  const code='SLD-'+crypto.randomUUID().replaceAll('-','').slice(0,24).toUpperCase();
  f.sqlite.prepare('INSERT INTO soopketland_coupons(code,issuer_id,request_id,reward_json,max_uses,created_at) VALUES(?,2,?,?,?,?)').run(code,crypto.randomUUID(),JSON.stringify({key,label:key,amount}),uses,'2026-09-12T12:00:00Z');
  return code;
}
...
test('viewer coupon delivers a guaranteed ticket; normal viewer opens once, including duplicate card, without coins',async()=>{
 ...
 assert.equal(f.qty(SUPERSTAR_TICKET,20),0);assert.equal((await f.call('superstar/open',{requestId})).body.replayed,true);
 const card=f.sqlite.prepare("SELECT * FROM user_cards WHERE user_id=20 AND card_id='ss1'").get();assert.equal(card.quantity,3);assert.equal(card.breakthrough_level,13);
 assert.equal(f.sqlite.prepare('SELECT coin FROM users WHERE id=20').get().coin,0);
 assert.equal((await f.call('superstar/open',{requestId:crypto.randomUUID()})).status,409);
});

test('guaranteed ticket empty pool and failed card insert preserve inventory; concurrent open spends only once',async()=>{
 const f=await fixture();f.sqlite.prepare('INSERT INTO cnine_user_inventory VALUES(?,?,1,1,NULL,NULL)').run(20,SUPERSTAR_TICKET);f.current.id=20;
 assert.equal((await f.call('superstar/open',{requestId:crypto.randomUUID()})).body.code,'LAND_POOL_EMPTY');assert.equal(f.qty(SUPERSTAR_TICKET,20),1);
 f.sqlite.exec("INSERT INTO cards VALUES('ss1','슈퍼스타 테스트','SUPERSTAR','ss.png',1,1,'PUBLIC',NULL)");
 f.sqlite.exec("CREATE TRIGGER superstar_fault BEFORE INSERT ON user_cards BEGIN SELECT RAISE(ABORT,'card offline'); END");
 await assert.rejects(()=>f.call('superstar/open',{requestId:crypto.randomUUID()}),/card offline/);assert.equal(f.qty(SUPERSTAR_TICKET,20),1);
 f.sqlite.exec('DROP TRIGGER superstar_fault');
 const out=await Promise.all([f.call('superstar/open',{requestId:crypto.randomUUID()}),f.call('superstar/open',{requestId:crypto.randomUUID()})]);
 assert.equal(out.filter(r=>r.status===200).length,1);assert.equal(f.qty(SUPERSTAR_TICKET,20),0);
...
test('previously issued maximum random card coupons deliver all copies, preserve enhancement, and replay cannot add copies',async()=>{
  for(const [key,amount,id] of [['FUR_RANDOM_CARD',5,'f1'],['ZENITH_RANDOM_CARD',3,'z1']]){
    const f=await fixture(),code=issuedCoupon(f,key,amount);
...
test('each card is independently sampled and a later card insert failure rolls back the whole coupon',async t=>{
  ...
  const code=issuedCoupon(f,'FUR_RANDOM_CARD',5);
...
test('Iyejun coupon uses the fixed ID, grants one unenhanced copy, and is excluded only from land random cards',async()=>{
  const f=await fixture();addIyejun(f);await f.force('COIN');await f.grant(1,2);f.current.id=2;const roll=(await f.spin()).body;
  f.sqlite.prepare('UPDATE soopketland_coupons SET reward_json=? WHERE code=?').run(JSON.stringify({key:LAND_IYEJUN_PRIZE,label:'이예준 카드',amount:1}),roll.code);
  ...
  assert.equal((await f.redeem(issuedCoupon(f,'FUR_RANDOM_CARD'),21)).body.code,'LAND_POOL_EMPTY');
});
...
test('client formats named cards as 장, uses server coin cap, and loads the new versioned bundle',()=>{
  ...
  assert.match(app,/soopketland-v2039\.bundle\.js\?v=2091-live-connections/);
});
...
test('seven-prize live settings lose ZENITH/FUR rows without changing any remaining weight; OWNER cannot add them back',async()=>{
  ...
  assert.equal((await f.call('settings',{requestId:crypto.randomUUID(),weights:expected})).status,200);
...
test('PostgreSQL credits all expanded maximum rewards once, including 50억 BIGINT coin and audit balances',async()=>{
  ...
  try{
    ...
    // Copy the isolated fixture using the live BIGINT column widths, then run the real handler and adapter.
    for(const {name,sql} of f.sqlite.prepare("SELECT name,sql FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'").all()){
      assert.match(name,/^[a-z_]+$/);await pg.exec(sql.replaceAll('INTEGER','BIGINT'));
      for(const row of f.sqlite.prepare(`SELECT * FROM ${name}`).all()){
        const fields=Object.keys(row);await pg.query(`INSERT INTO ${name}(${fields.join(',')}) VALUES(${fields.map((_,i)=>'$'+(i+1)).join(',')})`,Object.values(row));
      }
    }
    ...
    for(const {key,amount,code} of coupons){
      const args={env:{DB},user:{id:20,status:'ACTIVE'},body:{code,operationKey:crypto.randomUUID()},deps:f.deps};
      const first=await redeemLandCoupon(args);assert.equal(first.status,200,JSON.stringify(first));assert.equal(first.body.rewardAmount,amount);
      assert.equal((await redeemLandCoupon(args)).body.replayed,true);
