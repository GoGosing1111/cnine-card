...
test('canonical registration contains every mercenary, independent skill and original/SD reference',()=>{
  ...
  assert.equal(seed.catalog.cards.length,50);assert.equal(seed.document.skills.length,33);assert.equal(seed.catalog.effects.frameCount,528);
  ...
  assert.ok(seed.document.assignments.every(r=>['V-044','V-045','V-046','V-047','V-048','V-049','V-050'].includes(r.code)?r.skillIds.join()==='MS-'+r.code.slice(2):r.skillIds.length===0));
...
test('PostgreSQL registers once, persists complete configuration and never changes wallet or formation',async()=>{
  const f=await fixture();try{
    ...
    assert.deepEqual(read.body.document,d);assert.equal(read.body.audit.length,2);assert.equal(read.body.catalog.cards.length,50);
...
test('API and CMS navigation are connected without gameplay imports',()=>{
  ...
  assert.match(read('admin/index.html'),/mercenary-admin-v1\.js\?v=20260924-cryvern/);
