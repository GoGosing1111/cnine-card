...
test('canonical registration contains every mercenary, independent skill and original/SD reference',()=>{
  ...
  assert.equal(seed.catalog.cards.length,49);assert.equal(seed.document.skills.length,32);assert.equal(seed.catalog.effects.frameCount,512);
  ...
  assert.ok(seed.document.assignments.every(r=>['V-044','V-045','V-046','V-047','V-048','V-049'].includes(r.code)?r.skillIds.join()==='MS-'+r.code.slice(2):r.skillIds.length===0));
