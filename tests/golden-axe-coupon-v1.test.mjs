...
import {ensureGoldenAxe,goldenAxeState} from '../functions/_golden_axe.js';
import {redeemOldAxeCoupon,AXE_COUPON_MAX} from '../functions/_golden_axe_coupon.js';
import {OLD_AXE} from '../js/golden-axe-model-v1.js';

const read=path=>readFileSync(new URL('../'+path,import.meta.url),'utf8');
const api=read('functions/api/[[path]].js'),cms=read('admin/admin-v1276.js'),wishCms=read('admin/golden-axe-v1.js');
...
const creationRoute=new AsyncFunction('path','request','env','requirePermission','readBody','releaseDeletedCouponCode','ensureGoldenAxe','json',specs+permanent+compatible);
...
async function fixture(){
 ...
 async function create(body={},path='admin/coupon-create-permanent-v3'){
  const request=new Request('https://example.test/api/'+path,{method:'POST',body:JSON.stringify({code:'PINGDU-QA',rewardType:OLD_AXE,rewardAmount:3,maxUses:10,...body})});
  return creationRoute(path,request,env,async()=>permitted?{id:99,role:'OWNER'}:null,r=>r.json(),async()=>{},ensureGoldenAxe,json);
 }
 async function redeem({userId=1,key='WISH_QA_REQUEST_1',code='PINGDU-QA',extra={}}={}){
  ...
  return redeemOldAxeCoupon({env,user:{id:userId},coupon,body:{operationKey:key,...extra},deps});
 }
...
test('both real CMS create routes register ticket, preserve settings, and never issue items on creation',async()=>{
 const f=await fixture();try{
  for(const [i,path] of ['admin/coupon-create-permanent-v3','admin/coupons','admin/coupons-v2'].entries()){
   ...
   assert.equal(coupon.reward_type,OLD_AXE);assert.equal(Number(coupon.reward_amount),3);assert.equal(Number(coupon.reward_coin),0);assert.equal(coupon.starts_at,null);assert.equal(coupon.ends_at,null);
  }
  assert.equal((await f.row('SELECT code FROM inventory_items')).code,OLD_AXE);await noRedemption(f);
  assert.equal(Number((await f.row('SELECT COUNT(*) n FROM app_meta')).n),0);
  assert.equal((await goldenAxeState(f.env,1)).phase,'HIDDEN');
...
test('redemption credits the event inventory immediately, with one receipt/log and no coin change',async()=>{
 const f=await fixture();try{
  ...
  const result=await response.json();assert.equal(result.rewardType,OLD_AXE);assert.equal(result.rewardAmount,3);assert.equal(result.rewardCoin,0);assert.equal(result.user.coin,5000000000);assert.match(result.message,/금도끼 은도끼/);
  const inventory=await f.row('SELECT * FROM cnine_user_inventory');assert.equal(Number(inventory.user_id),1);assert.equal(inventory.item_code,OLD_AXE);assert.equal(Number(inventory.quantity),3);assert.equal(Number(inventory.unseen_quantity),3);
  const state=await goldenAxeState(f.env,1);assert.equal(state.axes,3);assert.equal(state.phase,'HIDDEN');assert.equal(state.coin,'5000000000');
...
test('existing inventory accumulates and parallel last-use requests cannot exceed the global cap',async()=>{
 const f=await fixture();try{
  await f.create({maxUses:1});await f.pg.exec("INSERT INTO cnine_user_inventory(user_id,item_code,quantity,unseen_quantity) VALUES(1,'PINGDU_OLD_AXE',9,4)");
...
test('disabled/deleted/exhausted/misconfigured coupons, disabled tickets and inactive users grant nothing',async()=>{
 const f=await fixture();try{
  ...
  assert.equal(await redeemOldAxeCoupon({coupon:{reward_type:'COIN'}}),null);
 }finally{await f.close()}
});

test('CMS exposes the old axe coupon shortcut and the dedicated event settings',()=>{
 assert.match(read('admin/index.html'),/<option value="PINGDU_OLD_AXE">낡은도끼/);
 assert.match(wishCms,/select.value='PINGDU_OLD_AXE'/);
 assert.match(wishCms,/amount.value='1'/);
 assert.match(wishCms,/낡은도끼 쿠폰 만들기/);
 assert.match(read('admin/index.html'),/golden-axe-v1.js/);
});

test('bulk coupon list renders old axe amount rather than zero coins',()=>{
 ...
 Function('document',labels+`const couponsCache=[{id:1,code:'PINGDU-QA',reward_type:'PINGDU_OLD_AXE',reward_amount:5,reward_coin:0}];
 ...
 assert.match(root.innerHTML,/<small>낡은도끼<\/small><strong>5<\/strong>/);assert(!root.innerHTML.includes('보상 코인'));
});
