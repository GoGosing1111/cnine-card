...
import {
  ...
  ensureTargetedAvatarGrantSaitamaV1
} from '../functions/_targeted_avatar_grant_saitama_v1.js';
...
test('활성 싸이타마 계정에 테란여제 조은을 영구 지급하고 재실행하지 않는다',async()=>{
  ...
  const audit=DB.db.prepare("SELECT admin_id,target_id FROM admin_logs WHERE action_type='SYSTEM_AVATAR_GRANT_SAITAMA_V1'").get();
  ...
  const replay=await ensureTargetedAvatarGrantSaitamaV1({DB});
  assert.equal(replay.replayed,true);
  assert.equal(DB.db.prepare("SELECT COUNT(*) count FROM admin_logs WHERE action_type='SYSTEM_AVATAR_GRANT_SAITAMA_V1'").get().count,1);
});

test('기간제 소유권은 영구로 전환하고 기존 영구 소유권은 덮어쓰지 않는다',async()=>{
  const temporary=fixture({ownership:'TEMPORARY'});
  const upgraded=await ensureTargetedAvatarGrantSaitamaV1({DB:temporary});
  ...
  const preserved=await ensureTargetedAvatarGrantSaitamaV1({DB:permanent});
...
test('싸이타마 계정이 비활성이거나 두 개면 지급을 중단한다',async()=>{
  const inactive=fixture({status:'BANNED'});
  await assert.rejects(()=>ensureTargetedAvatarGrantSaitamaV1({DB:inactive}),error=>!String(error.message).includes(TARGETED_AVATAR_GRANT_NICKNAME));
  ...
  await assert.rejects(()=>ensureTargetedAvatarGrantSaitamaV1({DB:duplicate}),/정확히 한 개/);
...
test('사전 조회 후 계정이 변경되면 소유권·로그·RUNNING 마커를 모두 롤백한다',async()=>{
  ...
  await assert.rejects(()=>ensureTargetedAvatarGrantSaitamaV1({DB}));
...
test('라이브 health는 싸이타마 지급을 실행하고 계정 식별자는 응답에서 제외한다',()=>{
  const api=readFileSync(new URL('../functions/api/[[path]].js',import.meta.url),'utf8');
  assert.match(api,/ensureTargetedAvatarGrantSaitamaV1\(env\)/);
  assert.match(api,/avatarCode:saitamaAvatarGrant\.avatarCode\|\|null,permanent:Boolean\(saitamaAvatarGrant\.permanent\)/);
  assert.doesNotMatch(api,/targetedAvatarGrantSaitamaV1=saitamaAvatarGrant;/);
});
...
test('완료 마커가 있어도 현재 영구 보유를 재조회하며 회수된 아바타를 재지급하지 않는다',async()=>{
  ...
  assert.equal((await ensureTargetedAvatarGrantSaitamaV1({DB})).ownershipVerified,true);
  DB.db.prepare('DELETE FROM avatar_user_ownership_v1 WHERE user_id=?').run(88);
  const after=await ensureTargetedAvatarGrantSaitamaV1({DB});
...
test('운영 PostgreSQL 호환 계층에서 원자 지급·재조회·지급 0행 롤백을 검증한다',async()=>{
  ...
  try{
    await pg.exec(`
      ...
      INSERT INTO users VALUES(1,'운영자','OWNER','ACTIVE'),(88,'싸이타마','USER','ACTIVE');
    ...
    await pg.exec(`DELETE FROM avatar_user_ownership_v1; DELETE FROM admin_logs; DELETE FROM app_meta; DELETE FROM targeted_avatar_grant_saitama_v1_verifications;
    ...
    await assert.rejects(()=>ensureTargetedAvatarGrantSaitamaV1({DB}));
    for(const table of ['avatar_user_ownership_v1','admin_logs','app_meta','targeted_avatar_grant_saitama_v1_verifications'])
