...
test('SUT 원본은 보존하고 슈트 코어 1·2·3을 투명 RGBA PNG로 등록한다',()=>{
  assert.deepEqual(BATTLE_SUIT_CORE_CODES,['SUIT_CORE_1','SUIT_CORE_2','SUIT_CORE_3','SUIT_CORE_4','SUIT_CORE_5','SUIT_CORE_6']);
  assert.deepEqual(BATTLE_SUIT_CORE_CATALOG.map(item=>item.name),['슈트 코어 1','슈트 코어 2','슈트 코어 3','슈트 코어 4','슈트 코어 5','슈트 코어 6']);
...
test('슈트 코어와 기존 차량 부품은 MATERIAL 카테고리로 안전하게 업서트된다',async()=>{
  ...
  assert.equal(batched.filter(row=>/INSERT INTO inventory_items/.test(row.sql)).length,6);
