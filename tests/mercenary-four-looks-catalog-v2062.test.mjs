...
test('catalog release uses current CMS data and preserves all 50 separate art and SD resources', () => {
  ...
  assert.equal(published.filter(card=>!card.artOnly).length,54);
  assert.equal(published.filter(card=>card.sourceArt && card.battleSprite && card.sourceArt!==card.battleSprite).length,54);
