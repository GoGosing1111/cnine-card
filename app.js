...
function renderLoading() {
  app.innerHTML = `<div class="loading-screen"><div class="loading-orbit"></div><img src="assets/ui/cninelogo.png" class="loading-logo" alt="SOOP"><strong>씨켓몬 카드뽑기</strong><div class="loading-bar"><i></i></div></div>`;
}
...
function renderShell(tab) {
  ...
  document.getElementById('soopHallOfFame')?.remove();
  const shellHeader=document.querySelector('.header');
  if(shellHeader) shellHeader.insertAdjacentHTML('beforebegin',hallOfFameHtml());
...
async function loadHallOfFameFresh(){const hall=document.getElementById('soopHallOfFame');if(!hall)return;if(!API_MODE)return;try{clearApiCache('hall-of-fame');const data=await apiRequest('hall-of-fame',{}, {ttl:0,replaceInflight:true});if(data.enabled===false){hall.hidden=true;return}hall.hidden=false;const title=document.getElementById('soopHallTitle'),name=document.getElementById('soopHallName');if(title)title.textContent=String(data.title||'숲카라 명예의 전당');if(name)name.textContent=String(data.name||'🏆 남수단 🏆')}catch{}}
window.addEventListener('storage',event=>{if(event.key==='cnine_hall_of_fame_refresh'){clearApiCache('hall-of-fame');loadHallOfFameFresh()}});
...
function recentCards(user) {
  ...
  return `<div class="recent-grid">${items.map(item => { const c=cards.find(x=>x.id===item.cardId); if(!c)return''; return `<button class="recent-item" data-card-id="${c.id}"><img src="${c.image}" style="object-position:${c.focusX}% ${c.focusY}%"><span><b>${escapeHtml(c.title)}</b><small>${c.grade}${powerTypeIndicator(c)?` · ${powerTypeIndicator(c)}`:''}${item.duplicate?' · 보유중':' · NEW'}</small></span></button>`; }).join('')}</div>`;
}
...
async function startAutoBattle(){
  ...
  battleState.autoRunning=true;battleState.autoSummary={battles:0,wins:0,losses:0,totalReward:0,cardRewards:[]};stopBattleEnergyTimer();
...
async function startBattle(){
  ...
  try{
    ...
    if(battleState.autoRunning){
      const summary=battleState.autoSummary||(battleState.autoSummary={battles:0,wins:0,losses:0,totalReward:0,cardRewards:[]});summary.battles++;summary.totalReward+=Number(d.reward||0);if(win)summary.wins++;else summary.losses++;if(d.cardReward)summary.cardRewards.push(d.cardReward);
      const remaining=Math.floor(Number(battleState.energy?.energy||0)/Math.max(1,Number(battleState.energy?.costPerBattle||1)));
...
let raidState={timer:null,data:null,resultRevealed:new Set(),selectedRoomId:0,lastSoundTick:-1,lastSoundInstance:0};
...
function renderRaidView(d){
  ...
  if(ended){
    ...
    if(!raidState.resultRevealed.has(Number(c.id))){const outcome=c.result==='CLEAR'?'clear':c.result==='FAILED'?'failed':'timeout',outcomeTitle=outcome==='clear'?'레이드 클리어':outcome==='failed'?'레이드 실패':'시간 초과',outcomeText=outcome==='clear'?'보스 처치에 성공했습니다.':outcome==='failed'?'참가 인원이 모두 전투 불능 상태가 되었습니다.':'제한 시간 안에 보스를 처치하지 못했습니다.',outcomeIcon=outcome==='clear'?'✓':outcome==='failed'?'✕':'⌛';box.innerHTML=`<section class="raid-outcome-screen ${outcome}"><div class="raid-outcome-backdrop"></div><div class="raid-outcome-mark">${outcomeIcon}</div><p class="eyebrow">WORLD RAID RESULT</p><span class="raid-outcome-label">${c.result==='CLEAR'?'RAID CLEAR':c.result==='FAILED'?'RAID FAILED':'TIME OUT'}</span><h1>${outcomeTitle}</h1><h2>${escapeHtml(c.bossName)}</h2><p>${outcomeText}</p><div class="raid-outcome-hp"><span>보스 최종 HP</span><b>${Number(c.currentHp||0).toLocaleString()} / ${Number(c.maxHp||0).toLocaleString()}</b></div><button type="button" class="btn raid-result-continue" id="raidResultContinue">결과 상세 확인</button><small>버튼을 누르기 전에는 다음 화면으로 넘어가지 않습니다.</small></section>`;const next=document.getElementById('raidResultContinue');if(next)next.onclick=()=>{raidState.resultRevealed.add(Number(c.id));renderRaidView(d)};stopRaidTimer();return;}
