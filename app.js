...
let raidState={timer:null,data:null,lastAttackTicks:-1,settledInstanceId:Number(sessionStorage.getItem('cnineRaidSettledInstance')||localStorage.getItem('cnineRaidSettledInstance')||0)};
...
function renderRaidView(d){
  ...
  if(c.status==='ENDED'&&(raidState.settledInstanceId===Number(c.id)||Number(d.me?.rewardClaimed||0)===1)){raidState.settledInstanceId=Number(c.id);sessionStorage.setItem('cnineRaidSettledInstance',String(c.id));localStorage.setItem('cnineRaidSettledInstance',String(c.id));box.innerHTML=`<section class="raid-empty raid-empty-polished"><p class="eyebrow">WORLD RAID</p><h2>${schedule.isOpen?'현재 열린 레이드가 없습니다':'현재 레이드를 이용할 수 없습니다'}</h2><p>${schedule.isOpen?'새로운 레이드 대기방이 열리면 참가할 수 있습니다.':'레이드 개방 시간에 다시 이용해 주세요.'}</p></section>`;stopRaidTimer();return;}
  ...
  const ended=c.status==='ENDED',resultText=c.result==='CLEAR'?'RAID CLEAR':c.result==='FAILED'?'RAID FAILED':'TIME OUT';
  if(c.status==='LOBBY'){
    const countPct=Math.min(100,Number(c.participantCount||0)/Math.max(1,Number(s.maxParticipants||10))*100);
    const minReady=Number(c.participantCount||0)>=Number(s.minParticipants||1);
    const deck=(me?.cards||[]).slice(0,5);
    box.innerHTML=`<section class="raid-lobby-hero">
      <div class="raid-lobby-main">
        <div class="raid-lobby-portrait">${c.bossImage?`<img src="${c.bossImage}" alt="">`:'<div class="raid-boss-placeholder">👹</div>'}<span>RAID BOSS</span></div>
        <div class="raid-lobby-copy"><p class="eyebrow">OWNER TEST RAID</p><h2>${escapeHtml(c.bossName)}</h2><div class="raid-lobby-hp-label"><span>BOSS HP</span><b>${Number(c.maxHp).toLocaleString()}</b></div><div class="raid-lobby-hp"><i style="width:100%"></i></div><div class="raid-lobby-status ${minReady?'ready':''}">${minReady?'출격 준비 완료':'최소 참가 인원을 기다리는 중'}</div></div>
      </div>
      <aside class="raid-lobby-entry"><span>참가 인원</span><b>${Number(c.participantCount||0)} <em>/ ${Number(s.maxParticipants||10)}</em></b><div class="raid-lobby-count-track"><i style="width:${countPct}%"></i></div><div class="raid-lobby-time"><span>전투 시작까지</span><strong>${String(Math.floor(Math.max(0,sec)/60)).padStart(2,'0')}:${String(Math.max(0,sec)%60).padStart(2,'0')}</strong></div><button class="btn raid-lobby-join" id="raidJoin" ${joined||!schedule.canEnter?'disabled':''}>${joined?'신청 완료':!schedule.canEnter?'입장 마감':'레이드 신청'}</button><small>저장된 PvE 덱 5장으로 참가합니다.</small></aside>
      <div class="raid-lobby-bottom"><div><p class="eyebrow">MY RAID DECK</p><div class="raid-lobby-deck">${deck.map(card=>`<figure><img src="${card.image}" title="${escapeHtml(card.title)}"><figcaption>${escapeHtml(card.title)}</figcaption></figure>`).join('')||'<span>레이드 신청 후 참가 덱이 표시됩니다.</span>'}</div></div><div><p class="eyebrow">PARTY MEMBERS</p><div class="raid-lobby-members">${participants.map((x,i)=>`<article><b>${i+1}</b><div class="raid-lobby-member-cards">${(x.cards||[]).slice(0,5).map(card=>`<img src="${card.image}" alt="">`).join('')}</div><span><strong>${escapeHtml(x.nickname)}</strong><small>전투력 ${Number(x.totalPower).toLocaleString()}</small></span></article>`).join('')||'<p>첫 참가자를 기다리는 중...</p>'}</div></div></div>
    </section>`;
    const join=document.getElementById('raidJoin');if(join&&!join.disabled)join.onclick=joinRaid;return;
  }
  if(ended){
    const rank=me?participants.findIndex(x=>Number(x.userId)===Number(me.userId))+1:0,clear=c.result==='CLEAR';
    box.innerHTML=`<section class="raid-result-screen ${clear?'clear':'fail'}"><div class="raid-result-glow"></div><p class="eyebrow">RAID RESULT</p><h1>${resultText}</h1><h2>${escapeHtml(c.bossName)}</h2><div class="raid-result-stats"><div><span>MY DAMAGE</span><b>${Number(me?.shownDamage||0).toLocaleString()}</b></div><div><span>FINAL RANK</span><b>${rank?rank+'위':'-'}</b></div><div><span>SURVIVAL</span><b>${me?(me.isDefeated?'K.O':'생존'):'-'}</b></div></div><div class="raid-reward-stage"><article><div class="raid-reward-icon">◇</div><span>코인</span><b>+${Number(s.participationCoin||0)+(clear?Number(s.clearCoin||0):0)}</b></article><article><div class="raid-reward-icon">✦</div><span>카드 조각</span><b>+${clear?Number(s.rewardShards||0):0}</b></article></div>${me?`<button class="btn raid-claim-btn" id="raidClaim" ${Number(me.rewardClaimed||0)===1?'disabled':''}>${Number(me.rewardClaimed||0)===1?'보상 수령 완료':'보상 받기'}</button>`:'<p class="raid-result-note">레이드 참가 기록이 없습니다.</p>'}</section>`;
  ...
  const bossHit=raidState.lastAttackTicks>=0&&Number(c.attackTicks||0)>raidState.lastAttackTicks;raidState.lastAttackTicks=Number(c.attackTicks||0);
  const myHpPct=me?Math.max(0,Math.min(100,Number(me.currentHp)/Math.max(1,Number(me.maxHp))*100)):0;
  box.innerHTML=`<section class="raid-battle-stage is-battle ${c.enraged?'is-enraged':''} ${bossHit?'raid-party-hit':''}"><div class="raid-stage-sky"><span></span><span></span><span></span></div><div class="raid-stage-header"><div><small>${c.enraged?'⚠ ENRAGED':'WORLD RAID'}</small><b>자동 전투 진행 중</b></div><div class="raid-stage-timer">${Math.max(0,Math.ceil((Date.parse(c.endsAt)-Date.now())/1000))}s</div></div><div class="raid-stage-boss-hud"><div><span>${escapeHtml(c.bossName)}</span><b>${Number(c.currentHp).toLocaleString()} / ${Number(c.maxHp).toLocaleString()}</b></div><div class="raid-stage-boss-track"><i style="width:${hpPct}%"></i><em style="width:${hpPct}%"></em></div></div><div class="raid-arena"><div class="raid-party-side"><div class="raid-party-aura"></div>${attackCard?`<div class="raid-attacker-card"><img src="${attackCard.image}" alt=""><strong>${escapeHtml(attacker.nickname)}</strong><span>${escapeHtml(attackCard.title)}</span></div>`:'<div class="raid-wait-orb">READY</div>'}</div><div class="raid-boss-side"><div class="raid-boss-aura"></div>${c.bossImage?`<img class="raid-stage-boss" src="${c.bossImage}" alt="">`:'<div class="raid-stage-boss placeholder">👹</div>'}<div class="raid-slash-effect"></div><div class="raid-hit-flash"></div><div class="raid-floating-damage">${attacker?Math.max(1,Math.floor(Number(attacker.totalPower||0)*Number(s.damageMultiplier||1)/10)).toLocaleString():''}</div></div></div><div class="raid-boss-impact"><div class="raid-boss-strike-wave"></div><span>⚠ BOSS ATTACK</span></div>${c.enraged?`<div class="raid-enrage-warning"><strong>광폭화</strong><span>공격 속도와 피해량 증가</span></div>`:''}<div class="raid-my-deck-stage">${(me?.cards||[]).slice(0,5).map((card,i)=>`<div class="raid-stage-card ${me?.isDefeated?'dead':''}" style="--delay:${i*80}ms"><div class="raid-card-frame"><img src="${card.image}" alt=""><div class="raid-card-damage">-${bossHit?Math.max(1,Math.floor(Number(s.bossAttackPower||850))):0}</div><div class="raid-card-status"><span>HP</span><b>${Math.round(myHpPct)}%</b></div><div class="raid-card-hp ${myHpPct<=25?'danger':''}"><em style="width:${myHpPct}%"></em><i style="width:${myHpPct}%"></i><u></u></div><small>${me?.isDefeated?'K.O':'전투 중'}</small></div></div>`).join('')||'<div class="raid-stage-empty">레이드 미참가</div>'}</div><div class="raid-boss-cast"><span>보스 공격</span><i style="animation-duration:${Math.max(800,Number(s.bossAttackIntervalMs||5000))}ms"></i></div></section><section class="raid-live-grid"><div class="raid-party"><div class="panel-title"><div><p class="eyebrow">RAID PARTY</p><h2>생존 현황</h2></div></div><div class="raid-participant-list">${participants.map(x=>{const p=Math.max(0,Math.min(100,Number(x.currentHp)/Math.max(1,Number(x.maxHp))*100));return `<article class="${x.isDefeated?'defeated':''}"><div class="raid-mini-cards">${(x.cards||[]).slice(0,5).map(card=>`<img src="${card.image}">`).join('')}</div><span><b>${escapeHtml(x.nickname)}</b><small>전투력 ${Number(x.totalPower).toLocaleString()}</small><em class="raid-user-hp"><i style="width:${p}%"></i></em><small>${x.isDefeated?'전투 불능':`${Math.round(p)}%`}</small></span><strong>${Number(x.shownDamage||0).toLocaleString()} DMG</strong></article>`}).join('')}</div></div><aside class="raid-ranking"><div class="panel-title"><div><p class="eyebrow">LIVE DAMAGE</p><h2>딜 체크 미터</h2></div></div>${participants.slice(0,Number(s.rankingSize||10)).map((x,i)=>{const max=Math.max(1,Number(participants[0]?.shownDamage||1)),pct=Math.max(2,Number(x.shownDamage||0)/max*100);return `<div class="raid-rank-row ${x.isDefeated?'defeated':''}"><b>${i+1}</b><span>${escapeHtml(x.nickname)}<i style="width:${pct}%"></i></span><strong>${Number(x.shownDamage||0).toLocaleString()}</strong></div>`}).join('')}<div class="raid-my-damage"><span>내 누적 딜</span><b>${Number(me?.shownDamage||0).toLocaleString()}</b><small>${me?`HP ${Math.round(myHpPct)}%`:'레이드 미참가'}</small></div></aside></section>`;
  if(bossHit)setTimeout(()=>document.querySelector('.raid-battle-stage')?.classList.remove('raid-party-hit'),900);
}
async function claimRaidReward(){const btn=document.getElementById('raidClaim');if(btn)btn.disabled=true;try{const requestedInstanceId=Number(raidState.data?.current?.id||0),d=await apiRequest('raid/claim',{method:'POST'});if(d.user)saveUser(d.user);const instanceId=Number(d.instanceId||requestedInstanceId||0);raidState.settledInstanceId=instanceId;sessionStorage.setItem('cnineRaidSettledInstance',String(instanceId));localStorage.setItem('cnineRaidSettledInstance',String(instanceId));raidState.data=null;stopRaidTimer();const box=document.getElementById('pveRaidView');if(box)box.innerHTML=`<section class="raid-empty raid-empty-polished"><p class="eyebrow">REWARD COMPLETE</p><h2>보상 정산 완료</h2><p>보상이 계정에 지급되었습니다.</p><button class="btn" id="raidReturnHunt">몬스터 토벌로 돌아가기</button></section>`;document.getElementById('raidReturnHunt')?.addEventListener('click',()=>switchPveMode('hunt'));}catch(e){if(btn)btn.disabled=false;alert(e.message)}}
