...
async function openWagoVerification(){const panel=document.getElementById('wagoVerifyPanel');if(!panel)return;panel.hidden=false;const d=await apiRequest('wago-verification/status'),v=d.verification,s=d.settings;panel.innerHTML=`<div class="verify-card"><h3>와고 닉네임 2단계 인증</h3>${v?`<div class="verify-status status-${String(v.status).toLowerCase()}"><b>${escapeHtml(v.status)}</b><span>${escapeHtml(v.wago_nickname)} · 회원번호 ${escapeHtml(v.wago_member_no)}</span>${v.verification_code&&v.status!=='VERIFIED'?`<code>${escapeHtml(v.verification_code)}</code>`:''}</div>`:''}<div class="verify-form"><input id="verifyWagoName" placeholder="와고 닉네임" value="${escapeHtml(v?.wago_nickname||'')}"><input id="verifyMemberNo" inputmode="numeric" placeholder="와고 회원번호" value="${escapeHtml(v?.wago_member_no||'')}"><button class="btn" id="issueVerifyCode">인증코드 발급</button></div><div class="verify-guide"><p>발급된 코드를 지정 인증 게시글 댓글에 그대로 작성하세요.</p>${s.postUrl?`<a class="btn secondary" href="${escapeHtml(s.postUrl)}" target="_blank" rel="noopener">와고 인증 게시글 열기</a>`:'<b>현재 CMS에 인증 게시글 주소가 설정되지 않았습니다.</b>'}<button class="btn" id="checkVerifyComment">댓글 인증 확인</button></div></div>`;document.getElementById('issueVerifyCode').onclick=async()=>{try{const r=await apiRequest('wago-verification/request',{method:'POST',body:JSON.stringify({wagoNickname:document.getElementById('verifyWagoName').value,wagoMemberNo:document.getElementById('verifyMemberNo').value})});alert(`인증코드: ${r.verificationCode}\n${r.expiresMinutes}분 안에 인증 게시글 댓글로 작성하세요.`);openWagoVerification()}catch(e){alert(e.message)}};document.getElementById('checkVerifyComment').onclick=async()=>{try{const r=await apiRequest('wago-verification/check',{method:'POST'});alert(r.message||'인증 확인 완료');openWagoVerification()}catch(e){alert(e.message)}}}
