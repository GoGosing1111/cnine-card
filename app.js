...
async function loadRecentHighGradeFeed(){
  ...
  if(!track)return;
  if(!API_MODE){track.innerHTML='<span class="high-grade-empty">서버 연결 시 SSR 이상 획득 소식이 표시됩니다.</span>';return;}
  try{
...
async function startAutoBattle(){
  ...
  if(battleState.energy?.unlimited)return alert('무제한 계정에서는 남은 횟수 자동사냥을 사용할 수 없습니다.');
  if(!remaining)return alert('남은 전투 횟수가 없습니다.');
  if(!confirm(`선택한 몬스터를 남은 ${remaining}회 자동사냥할까요?`))return;
  battleState.autoRunning=true;stopBattleEnergyTimer();
  const modal=document.getElementById('modal');modal.className='modal show';modal.innerHTML='<div class="modal-panel battle-auto-progress"><div class="battle-auto-spinner"></div><h2>자동사냥 진행 중</h2><p>전투와 보상을 처리하고 있습니다.</p><strong>창을 닫거나 새로고침하지 마세요.</strong></div>';
  try{
