...
async function refresh(){const s=await send({type:'CNINE_STATUS'});status.textContent=s.connected?'관리자 인증 연결됨':'관리자 인증 필요';status.className=s.connected?'ok':'no';detail.textContent=s.connected?`저장 시각: ${s.adminTokenSavedAt||'-'}\n와고 닉네임 클릭 시 지급창이 열립니다.`:'CMS에 OWNER/ADMIN 계정으로 로그인한 뒤 이 창을 다시 여세요.';}
