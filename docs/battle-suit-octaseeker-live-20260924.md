...
- 범위 검사 **105/105 통과** (약 13초), Worker 문법/배포 컴파일 통과. 기존 일반 사격·로켓·헬기, PVP 제외, 보유/중복 장착, SQLite/PostgreSQL CMS 미편입과 기존 거래 원자성까지 확인했다.
- 커밋 `6bf28c49`을 `origin/main`에 반영하고 지정 `npm run deploy:production -- --scoped` 1회 성공.
- Pages: `https://49361924.cnine-card.pages.dev` / 운영 도메인 `https://cnine-card.pages.dev`.
- Clan Worker: `f5d7766b-8c30-4141-85a8-ce5973839e33`. 운영 Hyperdrive 쿼리 캐시 OFF, Pages/Worker 바인딩 일치 확인.
- 배포 후 운영 도메인에서 공용 칩 카탈로그·V3 래퍼·실제 메인 번들·새 아이콘 4개 해시가 후보와 일치했다. 공개 프리뷰의 `승인 완료 · 17초 · 헬기폭격 피해 ×2` 표시를 확인했다.
- 계정 지급/획득처/확률/보상 풀 수정 없음. 운영 DB는 기존 첫 카탈로그 조회의 멱등 시드 경로로 신규 칩만 등록한다.
- 이 완료 기록은 문서 전용 후속 커밋이며 게임을 재배포하지 않는다.
