...
후속 사용자 확정: **한 식탁에 최대 4명**, 시작 전 3초, 경기 90초, 24입 먼저 완료한 순서로 순위. **스페이스·식탁 클릭/터치 한 번에 한 입**이며 길게 눌러도 자동으로 먹지 않는다.
매 700ms 이상 간격으로 서버가 한 입을 인정한다. 난이도 상향 요청에 따라 신문 읽기 1.0~2.8초 미만 → **0.65초 시선 예고** → 감시 1.1~2.8초 미만을 서버 난수로 정한다.
감시 전환 직후 고정 **100ms** 전송 여유만 적용하며 클라이언트 시각·완료·진행도는 받지 않는다. 제한 시간까지 미완주했다는 이유만으로 사망 처리하지 않는다.
페이지 숨김·포인터 취소·통신 실패 시 추가 식사 요청을 보내지 않는다. 빠른 연타/쿨다운 입력은 예약하지 않는다. 진행 중 화면 닫기를 막고 재접속 시 기존 참가 기록을 복구한다.

4인 식탁 후속판의 입력·관전 동기화, 자산, 프리뷰와 범위 배포 기록은 `preview/prison-death-game-v2/README.md`를 따른다. 아래 최초 출시의 100명/홀드 입력 검수 기록은 당시 상태이며 후속판으로 대체한다.
