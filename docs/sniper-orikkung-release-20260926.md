...
- 소스 `b50c6f8033dd5de843c1d297c307eaa0b844e021` 배포 완료: `972287d4-4bc9-4bec-b6ad-5cfe8e3d98be`, https://972287d4.cnine-card.pages.dev.
- 전투/등록 관련 258개 검사 중 초기 실패 5개를 고친 인접 재검사 33개 통과. Worker 컴파일, 출시 상태, Hyperdrive 캐시 OFF 확인.
- 운영 공개 도감에서 V-050 / SS / REAR / SNIPER / MS-050 준비 완료 확인. 원화·SD·모션/탄착 아틀라스·전투 번들·래퍼·메인 로더 8종의 운영 SHA-256이 배포 파일과 일치.
- 2026-09-26 03:52:33 KST 운영 설정 원자적 저장 완료: CMS 57→58, 추첨 정책 17→18, 운영 감사 ID 35994.
- 작업 영수증 `ops:sniper-orikkung-release:20260926:v1`, CMS/추첨 감사 요청 각각 `sniper-orikkung-release-20260926-cms-v1`, `sniper-orikkung-release-20260926-draw-v1`.
- 저장 후 재조회: SS 가중치 41/4100 = 1%, 전체 0.0005%; 스킬 7.2배/2턴/25 및 배정 일치. 기존 SSS 가중치 V-021 8991 / V-046 50 / V-049 10 등 사용자 저장값 보존.
- 배포 후 도감에서 단발 전용 스킬의 설명에 공통 연사 문구가 섞인 것을 발견해 `rangedMercenarySkillText`의 MS-050 전용 설명만 수정했다. 전투·추첨·DB 값 및 생성 자산은 동일하다.
- 문구 보완 배포 기준은 위 `b50c6f8033dd5de843c1d297c307eaa0b844e021`이며 검사 대상은 `tests/mercenary-codex-live-v2098.test.mjs` 하나로 제한한다. 운영 도감 API 설명만 재확인한다.
- 문구 보완 배포 완료: `60acb717dd501240937faad54b4c2f36a9e288fe` → `058a3107-9d31-4302-a83a-8bc03d6463ae`, https://058a3107.cnine-card.pages.dev. 관련 도감 검사 3개 통과.
- 2026-09-26 03:54:55 KST 최종 운영 도감 재조회에서 CMS 58, SS, 후열 저격, MS-050 준비 완료와 단발 전용 설명 확인. 원화·SD·전투 코드·확률은 첫 출시 검증 결과와 동일.
- 최종 공개 검증 기록: [sniper-orikkung-production-check-20260926.json](sniper-orikkung-production-check-20260926.json). 본 완료 기록만의 커밋은 정책에 따라 운영 재배포하지 않는다.

## 최초 배포 검사와 인접 회귀

- 최초 지정 배포 검사는 258개 중 253개 통과 후 중단했다. 운영 배포/DB 저장은 실행되지 않았다.
- 실패 5개는 과거 테스트의 균등 SS 선택 인덱스(2개), 흑월 당시 로스터 끝 항목 가정(1개), S/SS 배정 시험의 신규 확정 SS 누락(2개)이었다.
- 실제 지급 오류가 아니라 신규 용병·희귀 가중치에 맞지 않는 테스트 입력이었다. 기존 지급 기대값은 유지한 채 실제 가중치 구간으로 선택하고, 흑월 당시 범위와 신규 SS 배정 계획을 명시했다. 게임 코드는 수정하지 않았다.
- 재배포 검사는 실패 파일 3개 `mercenary-fusion-transactions.test.mjs`, `mercenary-heukwol-live.test.mjs`, `mercenary-s-skills-v3.test.mjs`만 다시 선택한다. 공용 메인 로더 회귀·Worker 컴파일·출시/캐시 가드는 지정 배포 도구가 유지한다. 앞서 통과한 253개를 별도 사전 전체 검사로 반복하지 않는다.
