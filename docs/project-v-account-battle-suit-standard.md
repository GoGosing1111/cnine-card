# PROJECT V 계정 배틀슈트 기준

## 적용 범위

- 배틀슈트는 기존 장비 5부위와 분리된 `BATTLE_SUIT` 전용 슬롯이다.
- 초기 적용 범위는 PVE 전투다. PVP, 랭크전, 영토전, 대장전, 클랜전과 공성 전장에서는 수치와 V3 외형을 모두 사용하지 않는다.
- 운영 전투력은 1번 100,000, 2번 200,000, 3번 300,000이며 CMS에서도 PVE 전용 수치로 관리한다.
- 기본 시드 배틀슈트는 장비 보급상자 풀에 자동 편입하지 않는다.

## 전투력 계약

```text
equipmentPve = BATTLE_SUIT를 제외한 장착 장비 PVE 합계
battleSuitPve = 장착 BATTLE_SUIT의 PVE 전투력
pve = equipmentPve + battleSuitPve + garagePve + titlePve

equipmentPvp = BATTLE_SUIT를 제외한 장착 장비 PVP 합계
pvp = equipmentPvp + garagePvp + titlePvp
```

- 배틀슈트는 `total_power = pve_power`, `pvp_power = 0`으로 저장·공개한다.
- CMS 입력과 DB의 과거 값에 관계없이 배틀슈트의 공개 PVP 전투력과 PVP 합산값은 항상 0이다.
- 서버는 `battleSuitPve`를 기존 카드 5장에 분배하지 않는다. 배틀슈트는 별도 지원 액터로 몬스터 HP·방벽에 독립 피해를 적용하고 `damageBreakdown.battleSuit`에 실제 적용량을 기록한다.
- 2026-09-07 V2063: 일반 사격과 스킬칩 피해를 직전 라이브 대비 각각 3배로 상향한다. 기존 정수 반올림·최소 피해·아포칼립스 덱 게이트 이후 적용하는 누적 최종 배율은 4에서 12로 변경한다. 아포칼립스 관통 피해도 3배이며, 장비 전투력·카드 피해·사격 주기는 바꾸지 않는다. 잔여 HP/방벽보다 큰 피해는 기존처럼 실제 적용량으로 제한한다.
- 같은 날 후속 일반공격 롤백: 위 V2063의 **일반 사격 3배만 취소**한다. 일반 사격과 해당 사격의 아포칼립스 관통은 누적 배율 4로 복원하고, 스킬칩의 일반 성분·관통 성분은 별도 누적 배율 12로 유지한다. 로켓런처·헬기폭격 피해와 발동 주기는 상향된 상태 그대로이며 장비 전투력·일반 카드·PVP는 변경하지 않는다.

## V3 렌더링 계약

- 계정 캐릭터는 기존 아군 카드 5명의 `allies`, `characters`, 카드 도크와 진형 배열에 넣지 않는 보조 유닛이다.
- 위치는 전열 왼쪽 고정이며 이동하지 않는다. 대기 모션과 총기 반동·총구 섬광·탄도 연출만 사용한다.
- 승인된 배틀슈트 3종 × 총기 6종은 정확한 DB 무기 래스터를 손·팔 가림과 함께 합성한 4프레임 authored atlas를 사용한다. 배틀슈트 교체는 전신 외형을, 기존 `WEAPON` 장착 교체는 atlas의 총기 외형과 자세를 함께 바꾼다. 2026-09-04 추가된 동방무기상 금룡 돌격소총·금룡 대물저격총도 세 외형 모두 전용 atlas를 사용한다.
- 승인 조합 atlas가 로드되지 않거나 아직 등록되지 않은 외형은 정지 배틀슈트 본체와 명시적인 투명 무기 컷아웃의 별도 스프라이트 계층으로 복구한다. 사각형 장비 카드 이미지는 이 복구 경로에서도 사용하지 않는다.
- 배틀슈트 사격 루프는 배치 완료 직후 시작해 전투 종료까지 계속되며 카드 공격 턴·스킬·행동력 게이지와 무관하다. 서버의 배틀슈트 독립 피해 이벤트는 다음 연속 사격의 탄착에 결합하되 발사 주기를 시작하거나 멈추지 않는다.
- 배틀슈트는 HP와 피격 대상이 없는 타깃 불가 지원 액터다. 서버 판정은 총기별 독립 시간 주기(`INDEPENDENT_TIME_CADENCE`)를 사용하고 카드/몬스터 속도 게이지와 전투 행동 횟수를 소비하지 않는다.
- 승인된 총기 코드 6종은 투명 전투 컷아웃에 매핑한다. 그 외 무기는 명시적인 `battleSprite` 또는 `appearanceUrl`이 있을 때만 표시하며, 사각형 DB 카드 이미지는 전투 스프라이트로 사용하지 않는다.
- animated 자산의 단일 기준은 `assets/ui/project-v/account-battle-suits/manifest-v2.json`이다. `manifest-v1.json`은 정지 컷아웃 호환 기준으로만 유지한다.

## 2026-09-08 신규 H-BODY 외형 제작 고정 기준

- 승인 기준은 `preview/battle-suit-prestige-v1/`의 백색 판금·검은 내피·주황 반응로·금색 관절·트윈 안테나 한 종이다. 날개형 두 번째 시안은 취소됐으며 재사용하지 않는다. 포스터가 아닌 실사용 V3 리소스를 제작한다.
- **원본 무기 PNG 확보 → 몸 대비 무기 길이 결정 → 총 전체의 균일 배율·반전·조준각 설정 → 고정 녹색 실루엣을 기준으로 몸·어깨·양손 자세 보정 → 원본 PNG와 전경 손가락 합성 → 진짜 RGBA·아틀라스·V3 사격 검수** 순서를 고정한다.
- 총기의 개머리판·리시버·조준기·탄창·총열 디자인은 원본 래스터를 유지한다. AI로 재해석하거나 총열만 늘리거나, 옛 손 위치에 맞추기 위해 저격총을 짧게 축소하지 않는다. 총구는 전방 수평, 개머리판은 어깨, 주 손은 실제 손잡이·방아쇠, 받치는 손은 탄창 앞 총열덮개에 맞춘다.
- 이미지 생성은 고정된 녹색 무기 실루엣 주변 몸·팔·손 수선에 사용한다. 배경 제거·프록시 제거·무기 합성·아틀라스 구성은 사용자 승인된 프로젝트 스크립트로 처리한다. 원본·프롬프트·합성 설정·해시는 보존한다.
- 이번 1280×1536 몸 기준 승인값: SKS 전체 폭 840px, 좌우 반전 후 전체 +6° 회전, 실제 총열축 약 0.073°; 금룡 대물저격총 폭 980px, 회전 0°; 후속 확대된 금룡 돌격소총 V7은 폭 900px, 회전 0°. 각각 몸 높이 대비 약 60%·70%·64%이며 다른 몸이나 무기의 공통 축소 규칙으로 일반화하지 않는다.
- M200 승인 일체형 원본 `helios-m200-authored-v2.png`의 SHA-256은 `DF2775D3A0D4E2BC4DB47A10D0C0CA92196A628FD084D7BFDD45DB1359B4D732`다. 이 승인본은 재생성하지 않고 배경 제거 및 전신 균일 리사이즈만 적용한다. SKS V6·금룡 대물저격총 V6도 후속 변경 지시 없이 바꾸지 않는다.
- 출력은 384×512 RGBA 전신, 4열×2행 PNG 아틀라스, 발바닥 피벗·실제 총구 기준점이다. 현재 런타임과 같이 네 프레임에는 승인된 정지 조준 자세를 반복하며 총구 섬광·탄도·탄착은 V3 런타임에서 처리한다. 네 종류의 관절 동작을 그렸다고 표현하지 않는다.
- 후속 사용자 지시로 H-BODY 장비·슈트 코어 4 재료 등록과 라이브 V3 연결이 승인됐다. 금룡 돌격소총은 원본 PNG의 몸 대비 크기를 키우고 파지를 다시 맞춘 뒤 연결한다. 별도 지시 없는 전투력·제작 비용·확률 및 자동 획득 경로는 임의로 확정하지 않는다.

### 다음 작업 시작 전 필수 확인

1. 이 문서, `preview/battle-suit-prestige-v1/README.md`, `preview/battle-suit-prestige-v1/exact-weapon-fit.mjs`를 먼저 읽는다. 이미지 생성이 필요한 경우 같은 프리뷰의 `PROMPTS.md`에서 승인된 몸·손 보정 프롬프트도 확인한다.
2. 요청한 무기와 바꿀 항목을 먼저 특정한다. 승인된 다른 무기·슈트는 수정하지 않는다. 공통 빌더를 실행해야 해도 아래 비대상 승인본의 해시가 유지되는지 전후 비교한다.
3. 원본 총 PNG·무기별 균일 배율·반전·각도·총구·파지점·전경 손가락 가림을 재사용한다. 무기 디자인 재생성, 총열만 늘리기, 비균일 변형, 기존 손에 맞춘 축소, 위로 기울어진 총구는 이전 반려 사유다.
4. 원본·프롬프트·합성 설정을 보존하고 실제 알파, 전신 비율, 어깨와 양손 접점, 수평 조준, 총구와 발바닥 기준점 및 실제 V3 사격을 검수한다. 요청한 변경 이외의 전투 수치·획득 경로는 바꾸지 않는다.
5. 스킬 이펙트를 함께 작업할 때는 `docs/project-v-skill-effects-standard.md`의 **PixiJS 렌더링 + GSAP 타임라인** 및 사용 기록 규칙을 적용한다.

2026-09-08 최종 승인 전신 PNG 잠금값(각 경로는 `preview/battle-suit-prestige-v1/assets/sprites/` 기준):

| 무기 | 승인 파일 | SHA-256 |
| --- | --- | --- |
| M200 | `helios-3.png` | `92B9FB66C5C2160E05168C03617DBC31B8D377BCE726C6A1E00C8F7732278EAC` |
| SKS V6 | `helios-4.png` | `463D446D2C9DF31F236DBC06718EFF6A28927E7DF238CA8174AA8051AB77B082` |
| 금룡 돌격소총 V7 | `helios-5.png` | `B885B6ADB9D37F281A35A2BA1CDAC071EBDDEA271F179135601EA34D86795A6E` |
| 금룡 대물저격총 V6 | `helios-6.png` | `B68271FBB54155A393F82378E57620F640C09AA2FDB2D700A083F7FCDEF6EEEA` |

후속 변경 승인을 받은 파일만 새 승인 기록을 추가한다. 기존 승인 이력·원본을 지우지 않는다.

## 사격음 검수 계약

- 신규 총기 사운드는 실제 녹음·폴리 기반 원음만 사용하고, action/notice·ballistic impact·acoustic tail의 3계층으로 분리한다.
- strongest impact는 authored `fire` 프레임 콜백의 ±20ms 이내에 맞춘다. 원음 URL, 라이선스, 자산 ID, 해시, 파형 측정값과 런타임 가공 방식을 프리뷰 매니페스트에 남긴다.
- 2026-09-02 사용자 연결 승인 이후 동일한 녹음·믹스 매니페스트를 라이브 PVE와 독립 QC 화면이 함께 사용한다. PVP·영토전 등 경쟁 모드에는 배틀슈트와 사격음을 연결하지 않는다.
- 동방무기상 2종은 새 합성음을 만들지 않고 승인된 실총 녹음을 물리 등급별로 재사용한다. 금룡 돌격소총은 M4A1 AR 대역, 금룡 대물저격총은 Tac-50 고구경 대역이며 프록시 사실을 오디오 매니페스트에 표시한다.
- 라이브 전투음 설정이 OFF이면 신규 사격 예약을 음소거하고 진행 중인 배틀슈트 사운드 계층도 즉시 중지한다. 모바일은 실제 전투 시작/전투음 클릭의 trusted gesture 또는 공용 전투 AudioContext를 사용한다.

## API 계약

- `character/loadout`과 `userEquipmentBonuses()`는 `equippedBattleSuit`, `equippedWeapon`, `battleSuitPve`를 제공한다.
- `equippedBattleSuit`에는 인스턴스 ID, 장비 코드, 이미지, PVE 전투력과 `pvpPower: 0`을 포함한다.
- V3는 최상위 `equippedBattleSuit`/`equippedWeapon`를 우선하고, 호환을 위해 `characterBonus` 내부 값을 보조 경로로 읽는다.
- 토벌·탑·봉인전·호송·레이드는 V3 진입 payload에 같은 장착 메타데이터와 로그인 계정명을 전달한다.
- PVE 외 모드에서는 클라이언트 값과 관계없이 모드 게이트가 계정 유닛을 생성하지 않는다.

## 2026-09-16 S-BODY · Z-BODY 운영 연결 승인

- 사용자 명칭은 **S-BODY**, **Z-BODY**이며 계열 순서는 **H-BODY → S-BODY → Z-BODY**다.
- `라이브 배포해 전체 승인`으로 H-BODY 방식의 무기 착용 6종씩, 총 12조합과 공용 V3 연결을 승인했다. `슈터코어 5 6 만들고`에 따라 **S-BODY는 슈트 코어 5**, **Z-BODY는 슈트 코어 6**을 사용한다.
- 최종 원화·색감·머리 외형 잠금은 `preview/battle-suit-sz-v1/approval.json`과 `manifest.json`을 따른다. 과거 Z 후보를 최종 외형 대신 사용하지 않는다.
- 운영 확장은 `assets/ui/project-v/account-battle-suits/sz-body-v2124.json`이다. 승인 전투 PNG 12개와 아틀라스 6개를 바이트 그대로 복사했으며 기존 H-BODY 리소스는 고정한다.
- 신규 바디 역시 PVE 전용 지원 유닛이다. 기본 카드 5장과 분리하고 PVP 전투력은 0이다. 미정 전투력·제작 비용·확률·획득 경로를 자동 배정하지 않는다. 신규 장비는 전투력 0으로 등록하고 이후 CMS 설정을 보존한다.
- 코어 5·6은 별도 이미지가 있는 `MATERIAL` 재료다. 재료 등록으로 지급·상점·드롭·블랙미라클·제작식이 자동 추가되지 않는다.
- 재현·검증·운영 절차는 `docs/battle-suit-sz-body-v2124.md`를 따른다.

## 2026-09-18 Z-BODY 대검 교체 승인

- 사용자 승인으로 Z만 대검 외형, 돌진 검격, 검 꽂기 시전, V3 대형 뇌검 연출로 교체한다. 기존 제자리 총기 규칙의 Z 전용 예외이며 H/S는 보존한다.
- 후속 지시에 따라 몸체 크기는 H/S 헬멧~발바닥 높이에 맞춘다. 모션 전체에 같은 균일 배율을 쓰고 광역기 크기는 유지한다.
- 서버 피해와 대상·스킬칩·전투력·획득 정책을 보존한다. 승인 기록과 운영 연결은 `docs/z-body-sword-live-20260918.md`를 따른다.
