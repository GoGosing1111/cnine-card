...
function combatLinkReference(rank){const rule=data?.combatLink?.ranks?.[rank];return rule?fields('전력 연계 · 전투 적용 기준',`아군 카드 ${data.combatLink.regularActionsPerTurn}회 행동마다 용병이 추가 행동합니다. 자연 행동 시 주기를 다시 셉니다.`, `<p class="mc-field mc-wide">기본 공격과 피해·회복·보호 스킬의 최소 공격력: 아군 일반 카드 5명의 평균 공격력 ${rule.attackPercent}%. 전투 시작 전용 방벽: 평균 최대 체력 ${rule.shieldPercent}%. 기존 카드 방벽을 나누어 가져오지 않으며, 피해를 받으면 소진됩니다.</p>`):'';}
