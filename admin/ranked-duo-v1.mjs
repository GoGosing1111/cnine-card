...
const labels={DRAFT:'초안',RECRUITING:'모집 중',PAIRING:'전력 평가 중',PUBLISHING:'팀 공개 중',READY:'대전 준비',ACTIVE:'진행 중',CLOSED:'종료'};
...
export async function mountDuoCms(root){
 ...
 function draw(){
  const c=data.config,s=data.season,phase=s?.status,editable=!s||['DRAFT','RECRUITING','READY','CLOSED'].includes(phase);
 ...
 form.onclick=async event=>{
  ...
  let body={};if(action==='recruit'&&data.season.status!=='DRAFT'){const value=prompt('추가모집 시간 (기존 팀 유지, 1~720시간)','72');if(value===null)return;body.hours=Number(value);}
  lock(true);try{
 ...
 try{await load();status.textContent='행동력과 날짜를 직접 설정하세요. 미설정 상태에서는 대전을 시작할 수 없습니다.';}catch(error){status.textContent=error.message;}
}
function install(){
