(() => {
  ...
  function install() {
    ...
    if (!section) {
      ...
      section.hidden = true;
      section.innerHTML = `<div class="sectionIntro"><div><small>ALCHEMY REACTOR CONTROL</small><h2>연금술 <span class="buildBadge">v4</span></h2><p>LIMITED 이상 카드·장비·이동수단·특수 재화의 자동 보정 최종 등장확률을 관리합니다.</p></div><button type="button" class="ghost" id="alchemyAdminReload">새로고침</button></div><div id="alchemyAdminRoot" class="alchemy-admin-root">불러오는 중...</div>`;
  ...
  function render() {
    ...
    const settings = state.settings || {}, rules = settings.requirements || {}, scoring = state.scoring || {}, bonuses = scoring.cardGradeBonus || {}, gradeFactors = scoring.cardRewardGradeFactor || {}, bounds = scoring.equipmentPowerBounds || {}, probability = probabilityModel();
  ...
  function bind() {
    ...
    $('#alchemyRewardSync').onclick = event => { if (confirm('공개 LIMITED·PRESTIGE·FUR·ZENITH 카드를 등급·전투력 자동 확률로 다시 동기화하시겠습니까? 직접 만든 보상 행은 유지됩니다.')) void mutate('SYNC_REWARDS', {}, event.currentTarget, 'LIMITED 이상 카드 보상 풀을 자동 동기화했습니다.'); };
