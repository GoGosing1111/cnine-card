(() => {
  ...
  function install() {
    ...
    if (!section) {
      ...
      section.hidden = true;
      section.innerHTML = `<div class="sectionIntro"><div><small>ALCHEMY REACTOR CONTROL · ZENITH ERA</small><h2>연금술 <span class="buildBadge">v5</span></h2><p>단계별 카드 등급 총확률과 장비·이동수단·특수 재화의 최종 등장확률을 관리합니다.</p></div><button type="button" class="ghost" id="alchemyAdminReload">새로고침</button></div><div id="alchemyAdminRoot" class="alchemy-admin-root">불러오는 중...</div>`;
  ...
  function probabilityRows(rows) {
    return rows.map(row => {
      ...
      const gradeFactor = Number(row.gradeFactor ?? 1);
      const rawWeight = Number(row.rawEffectiveWeight ?? row.effectiveWeight ?? row.weight ?? 0), divisor = Math.max(1, Number(row.densityDivisor || 1)), densityScale = Number(row.densityScale ?? state?.scoring?.cardRewardDensityScale ?? 1);
      const weightFormula = row.type === 'CARD' ? `등급 × ${gradeFactor.toFixed(2)} · 강도 × ${Number(row.autoFactor || 0).toFixed(4)} = ${rawWeight.toFixed(4)} · 카드풀 × ${densityScale.toFixed(2)} ÷ ${divisor}장 = ${effectiveWeight(row).toFixed(4)}` : `강도 × ${Number(row.autoFactor || 0).toFixed(4)} = ${effectiveWeight(row).toFixed(4)}`;
      return `<tr data-reward-id="${esc(row.rewardId)}" class="probability-${level}"><td><b>${esc((state.settings?.tiers || []).find(tier => tier.code === row.tierCode)?.name || row.tierCode)}</b><small>${esc(row.rewardId)}</small></td><td>${esc(TYPE_LABEL[row.type] || row.type)}<small>${esc(row.rarity || '-')}</small></td><td><b>${esc(row.name)}</b><small>${esc(row.id)} × ${Number(row.quantity || 1).toLocaleString()}</small></td><td><b>${esc(strength)}</b><small>보상 강도 지수 ${Number(row.strengthPercent || 0).toFixed(1)} / 100</small></td><td>${Number(row.manualWeight ?? row.weight ?? 0).toLocaleString()}<small>${weightFormula}</small></td><td><div class="alchemy-final-probability"><strong>${formatProbability(probability)}<i>%</i></strong><span><i style="width:${Math.max(0, Math.min(100, probability))}%"></i></span><small>${oneInLabel(probability)} · ${row.valid ? (row.active ? 'ACTIVE' : 'OFF') : 'INVALID'}</small></div></td><td><button type="button" data-reward-edit>편집</button><button type="button" class="danger" data-reward-delete>삭제</button></td></tr>`;
  ...
  function render() {
    ...
    const settings = state.settings || {}, rules = settings.requirements || {}, scoring = state.scoring || {}, bonuses = scoring.cardGradeBonus || {}, gradeFactors = scoring.cardRewardGradeFactor || {}, tierGrades = scoring.cardRewardTiers || {}, bounds = scoring.equipmentPowerBounds || {}, probability = probabilityModel();
    root.innerHTML = `<section class="alchemy-admin-gate mode-${esc(String(settings.mode || 'OFF').toLowerCase())}"><div><small>LIVE RELEASE GATE</small><h3>현재 공개 상태 <b>${esc(settings.mode || 'OFF')}</b></h3><p>OWNER TEST에서는 전체메뉴와 API가 OWNER 계정에만 열립니다.</p></div><label><span>공개 단계</span><select id="alchemyMode">${modeOptions(settings.mode || 'OFF')}</select></label><label><span>최소 슬롯</span><input id="alchemyMinSlots" type="number" min="3" max="5" value="${Number(rules.minSlots || 3)}"></label><label><span>최대 슬롯</span><input id="alchemyMaxSlots" type="number" min="3" max="5" value="${Number(rules.maxSlots || 5)}"></label><label><span>안정도 보장</span><input id="alchemyStabilityMax" type="number" min="1" max="100" value="${Number(settings.stabilityMax || 10)}"></label><button type="button" id="alchemySettingsSave">운영 설정 저장</button></section>
      <div class="alchemy-admin-warning"><b>ZENITH ERA 카드풀</b><span>각성은 LIMITED, 과부하는 PRESTIGE·FUR, 최종 금단은 FUR·ZENITH만 등장합니다. 금단에서 LIMITED·PRESTIGE는 제외되며, 같은 등급의 카드 수가 많아져도 그 등급의 총확률은 늘어나지 않습니다. MA·SUPERSTAR는 카드 보상에서 제외됩니다.</span></div>
      <section class="alchemy-admin-columns"><article class="panel alchemy-admin-formula"><header class="alchemy-admin-heading"><div><small>MATERIAL QUALITY CURVE</small><h3>재료 가치 공식</h3></div><b>SERVER AUTHORITATIVE</b></header><p>장비는 실제 전투력 ${Number(bounds.min || 0).toLocaleString()} ~ ${Number(bounds.max || 0).toLocaleString()} 범위를 로그 정규화해 재료당 ${Number(scoring.equipmentScoreRange?.min || 25)} ~ ${Number(scoring.equipmentScoreRange?.max || 250)}점으로 환산합니다.</p><div class="alchemy-admin-bonus-grid">${['LIMITED','PRESTIGE','FUR','ZENITH'].map(grade => `<span><small>${esc(grade)} 재료</small><b>+${Number(bonuses[grade] || 0).toLocaleString()}</b></span>`).join('')}</div><div class="alchemy-admin-bonus-grid">${['LIMITED','PRESTIGE','FUR','ZENITH'].map(grade => `<span><small>${esc(grade)} 보상</small><b>×${Number(gradeFactors[grade] || 0).toFixed(2)}</b></span>`).join('')}</div><div class="alchemy-admin-tier-grid">${(settings.tiers || []).map(tier => `<span style="--tier-color:${esc(tier.color)}"><small>${esc(tier.name)}</small><b>${Number(tier.minValue || 0).toLocaleString()}+</b></span>`).join('')}</div><div class="alchemy-admin-bonus-grid">${['LIMITED','PRESTIGE','FUR','ZENITH'].map(grade => `<span><small>${esc(grade)} 등장 단계</small><b>${esc((tierGrades[grade] || []).join(' · ') || '제외')}</b></span>`).join('')}</div><p class="alchemy-admin-formula-note">카드 보상은 먼저 단계별 등급 총확률을 만든 뒤 같은 등급 카드끼리 나눕니다. 카드 수가 많은 LIMITED가 확률을 잠식하지 않으며, 최종 금단 단계는 FUR·ZENITH 시대 기준입니다. CMS 가중치는 같은 등급 안의 개별 카드 비중을 조절합니다.</p></article>
      <article class="panel"><header class="alchemy-admin-heading"><div><small>REWARD POOL</small><h3>보상·가중치 편집</h3></div><button type="button" class="ghost" id="alchemyRewardSync">단계별 카드풀 자동 동기화</button></header><div class="alchemy-admin-reward-form"><label><span>보상 ID</span><input id="alchemyRewardId" placeholder="비우면 자동 생성"></label><label><span>유형</span><select id="alchemyRewardType">${Object.keys(TYPE_LABEL).map(type => option(type, TYPE_LABEL[type], 'ITEM')).join('')}</select></label><label class="wide"><span>대상</span><select id="alchemyRewardRef">${catalogOptions('ITEM')}</select></label><label><span>단계</span><select id="alchemyRewardTier">${tierOptions(settings.tiers?.[0]?.code)}</select></label><label><span>수량</span><input id="alchemyRewardQuantity" type="number" min="1" max="100000" value="1"></label><label><span>CMS 기준 가중치</span><input id="alchemyRewardWeight" type="number" min="0.001" step="0.001" value="10"></label><label><span>정렬</span><input id="alchemyRewardSort" type="number" value="0"></label><label class="alchemy-admin-check"><input id="alchemyRewardActive" type="checkbox" checked><span>활성</span></label><button type="button" id="alchemyRewardSave">보상 저장</button><button type="button" class="ghost" id="alchemyRewardReset">새 보상</button></div></article></section>
  ...
  function bind() {
    ...
    $('#alchemyRewardSync').onclick = event => { if (confirm('각성 LIMITED / 과부하 PRESTIGE·FUR / 금단 FUR·ZENITH 기준으로 카드풀을 다시 동기화하시겠습니까? 직접 만든 보상 행은 유지됩니다.')) void mutate('SYNC_REWARDS', {}, event.currentTarget, 'ZENITH ERA 단계별 카드 보상 풀을 자동 동기화했습니다.'); };
