...
export async function mountPveV3Cms(host){
  ...
  const note=document.createElement('p');note.textContent='무한의탑은 기존 층별 보상·진행 정책으로 운영하며 재등반은 보류합니다. 카우방은 아래 운영 설정이 적용됩니다.';
  const status=document.createElement('p');status.setAttribute('role','status');
  const tabs=document.createElement('div'),form=document.createElement('form'),buttons=document.createElement('div'),reload=document.createElement('button'),save=document.createElement('button');reload.type='button';reload.textContent='다시 불러오기';save.type='submit';save.textContent='운영 설정 저장';buttons.append(reload,save);let selected='TOWER';
  const live=document.createElement('nav');live.style.cssText='display:flex;gap:12px;flex-wrap:wrap;margin:12px 0';for(const [label,url] of [['무한의탑 실게임','/?screen=battle&pve=tower'],['카우방 실게임','/pve-v3/?content=cow-room'],['기존 탑 CMS','/admin/#tower']]){const a=document.createElement('a');a.textContent=label+' ↗';a.href=url;a.target='_blank';a.rel='noopener';live.append(a);}host.append(title,note,live,tabs,form,status);let original;
  function draw(){form.replaceChildren();if(!data)return;original=selected==='TOWER'?data.tower:data.cow;const version=document.createElement('p');version.textContent=selected==='TOWER'?'기존 정책으로 운영 · 재등반 보류':`저장 버전 r${original.revision} · 카우방 ${original.mode==='ON'&&original.approved?'ON':original.mode}`;form.append(version);
    if(selected==='TOWER'){const description=document.createElement('p');description.textContent='기존 CMS의 층·몬스터·클리어 보상과 계정 진행도를 그대로 사용합니다. 최고층 완료 후 자동 초기화하지 않습니다. 재등반·반복 보상·160층 초안은 운영에 적용하지 않습니다.';form.append(description);return;}
    ...
    for(const [section,values] of Object.entries(sections)){const fieldset=document.createElement('fieldset'),legend=document.createElement('legend');legend.textContent=section==='config'?'전투 규칙':'입장과 보상';fieldset.append(legend);
      for(const [key,value] of Object.entries(values)){if(!Object.hasOwn(names,key))continue;const label=document.createElement('label'),name=document.createElement('span');name.textContent=names[key];let input;
        if(key==='mode'){input=document.createElement('select');for(const mode of data.release.publicContent?.COW_ROOM?.enabled?['OFF','TEST','ON']:['OFF','TEST']){const option=document.createElement('option');option.value=mode;option.textContent={OFF:'OFF · 입장 중지',TEST:'TEST · OWNER 검수',ON:'ON · 전체 유저 입장'}[mode];input.append(option);}}
