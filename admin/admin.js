...
function render(){
  ...
  $('#cards').innerHTML=list.map(c=>{const a=Number(c.is_active)!==0;return `<article class="item ${a?'':'inactive'}" data-id="${esc(c.id)}">
...
async function hardDelete(c){if(!confirm(`정말 '${c.title}' 카드를 완전 삭제할까요?\n보유 기록과 뽑기 기록도 함께 삭제됩니다.`))return;try{await api('admin/cards',{method:'DELETE',body:JSON.stringify({id:c.id})});all=all.filter(x=>x.id!==c.id);render();alert('완전 삭제 완료')}catch(e){alert(e.message)}}
