(()=>{
    ...
    root.innerHTML=`
      ...
      <section class="panel magicSettingsPanel">
        ...
        <div class="magicSettingsGrid">
          ...
          <label class="wide"><span>강화 단계별 실제 발동률 (0강~+9)</span><input id="magicEnhanceTriggerRates" value="${h((cfg.enhancement?.triggerRates||[]).join(','))}" placeholder="0,5,10,15,20,25,30,35,40,50"><small>카드 등급과 무관하며 이 수치가 그대로 전투 발동률이 됩니다.</small></label>
  ...
  function magicCardEditor(){
    const x=magicAdmin.editingMagic||{rarity:'R',effectType:'OPENING_ATTACK',triggerType:'BATTLE_START',effectValue:10,triggerChance:0,maxActivations:1,drawWeight:1,scopes:{pve:true,pvp:true},isActive:true,sortOrder:0};
