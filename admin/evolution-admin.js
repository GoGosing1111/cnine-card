(()=>{
  ...
  const isZenith=type=>String(type||'')==='LIMITED_TO_ZENITH';
  const typeLabel=type=>isZenith(type)?'LIMITED +13 → ZENITH':isPrestige(type)?'MA +13 → PRESTIGE':'SSR +10 → MA';

  function logMeta(row){
    if(isZenith(row.evolution_type)){
      return `${num(row.master_star_cost)}개 · ${num(row.coin_cost)}코인 · 성공률 ${num(row.success_rate)}% · ${num(row.attempt_no)}회차${row.is_pity?' · 천장':''} · 원본 카드 ${row.source_consumed?'소모':'유지'}`;
    }
  ...
  async function load(force=false){
    ...
    loadPromise=(async()=>{try{
      ...
      document.getElementById('evolutionPrestigeMediaUrl').value=String(settings.prestigeSuccessMediaUrl||'');
      document.getElementById('evolutionPrestigeSoundUrl').value=String(settings.prestigeSuccessSoundUrl||'');
  ...
  function mount(){
    ...
    panel.innerHTML=`<div class="maintenanceHead"><div><small>STANDARD + PRESTIGE EVOLUTION</small><h2>카드 진화 설정</h2><p>SSR → MA와 MA +13 → PRESTIGE의 성공 확률·천장을 각각 관리합니다. PRESTIGE 진화 재료는 마스터의 별입니다.</p></div><span id="evolutionCmsState" class="statusPill off">불러오는 중</span></div><div class="evolutionCmsRouteGrid"><section><header><small>STANDARD EVOLUTION</small><h3>SSR +10 → MA</h3><p>실패 시 코인과 카드조각만 소모되며 원본 카드는 유지됩니다.</p></header><div class="formgrid evolutionCmsLegacyGrid"><label class="field"><span>1회 코인 비용</span><input id="evolutionCoinCost" type="number" min="0" step="1"></label><label class="field"><span>1회 카드조각 비용</span><input id="evolutionShardCost" type="number" min="0" step="1"></label><label class="field"><span>기본 성공 확률 (%)</span><input id="evolutionSuccessRate" type="number" min="0" max="100" step="0.1"></label><label class="field"><span>천장 회차</span><input id="evolutionPityAttempts" type="number" min="1" max="100" step="1"><small>현재 운영 기준 10회</small></label></div></section><section class="prestige"><header><small>PRESTIGE ASCENSION</small><h3>MA +13 → PRESTIGE</h3><p>실패 시 마스터의 별만 소모되고 MA +13 카드는 유지됩니다. 성공 시에만 원본 카드가 소모됩니다.</p></header><div class="formgrid evolutionCmsPrestigeGrid"><label class="field"><span>마스터의 별 소모량</span><input id="evolutionPrestigeStarCost" type="number" min="1" max="9999" step="1"><small>PRESTIGE 진화 1회당 소모 수량</small></label><label class="field"><span>기본 성공 확률 (%)</span><input id="evolutionPrestigeSuccessRate" type="number" min="0" max="100" step="0.1"><small>천장 회차 전까지 적용됩니다.</small></label><label class="field"><span>확정 천장 회차</span><input id="evolutionPrestigePityAttempts" type="number" min="1" max="100" step="1"><small>해당 회차에는 성공 확정</small></label><label class="field"><span>성공 연출 영상 경로</span><input id="evolutionPrestigeMediaUrl" placeholder="assets/effects/prestige-evolution.mp4"><small>MP4 / WebM / GIF 경로를 입력하면 성공 시 전체화면으로 재생됩니다.</small></label><label class="field"><span>성공 연출 사운드 경로</span><input id="evolutionPrestigeSoundUrl" placeholder="assets/effects/prestige-evolution.mp3"><small>별도 사운드를 사용할 경우 입력하세요.</small></label><label class="field"><span>연출 재생 시간 (ms)</span><input id="evolutionPrestigeDurationMs" type="number" min="800" max="30000" step="100"><small>영상 종료 전에도 이 시간이 지나면 다음 결과 화면으로 이동합니다.</small></label><label class="field"><span>연출 볼륨 (%)</span><input id="evolutionPrestigeVolumePercent" type="number" min="0" max="100" step="1"><small>사운드 파일과 영상 음량에 공통으로 적용됩니다.</small></label></div></section></div><div class="formgrid evolutionCmsGlobalGrid"><label class="field"><span>진화 운영</span><select id="evolutionEnabled"><option value="ON">ON · 사용</option><option value="OFF">OFF · 중지</option></select></label></div><div class="evolutionCmsNotice"><b>처리 기준</b><span>SSR 진화는 기존 10회 천장 누적을 유지합니다. PRESTIGE 진화는 매 시도 마스터의 별을 차감하며, 실패 시 MA +13 카드는 유지됩니다. 천장 성공 시를 포함해 성공한 경우에만 원본 카드 소모와 결과 지급이 서버에서 함께 처리됩니다. 성공 연출 영상/사운드를 입력하면 유저 화면에서 궁극기처럼 꽉 차는 전체화면 연출이 재생됩니다.</span></div><button id="saveEvolutionCms">진화 설정 저장</button><div class="maintenanceHead evolutionLogHead"><div><h3>최근 진화 기록</h3><p>최근 50건의 경로별 재료 사용과 성공·실패 기록입니다.</p></div><button type="button" class="ghost" id="refreshEvolutionLogs">새로고침</button></div><div id="evolutionCmsLogs"></div>`;
    panel.querySelector('.maintenanceHead small').textContent='STANDARD · PRESTIGE · ZENITH EVOLUTION';
    panel.querySelector('.maintenanceHead p').textContent='기존 진화 경로와 LIMITED +13 → ZENITH 고정 정책을 함께 확인합니다.';
    panel.querySelector('.evolutionCmsRouteGrid').insertAdjacentHTML('beforeend',`<section class="zenith"><header><small>ZENITH ASCENSION</small><h3>LIMITED +13 → ZENITH</h3><p>활성화된 공개 ZENITH 카드 중 1장으로 진화합니다. 실패 시 원본은 유지되고 재료만 소모됩니다.</p></header><div class="evolutionCmsFixedPolicy"><span><small>성공 확률</small><b>25%</b></span><span><small>마스터의 별</small><b>30개</b></span><span><small>코인</small><b>5,000,000</b></span><span><small>천장</small><b>7회</b></span></div><p class="muted">7번째 도전 확정 성공 · ZENITH 덱 편성은 서버에서 최대 2장으로 제한됩니다.</p></section>`);
    ...
    document.getElementById('saveEvolutionCms').onclick=async()=>{
      const payload={
        ...
        prestigeSuccessMediaUrl:document.getElementById('evolutionPrestigeMediaUrl').value.trim(),
        prestigeSuccessSoundUrl:document.getElementById('evolutionPrestigeSoundUrl').value.trim(),
        prestigeSuccessDurationMs:Number(document.getElementById('evolutionPrestigeDurationMs').value),
        prestigeSuccessVolumePercent:Number(document.getElementById('evolutionPrestigeVolumePercent').value)
      };
      ...
      if(payload.prestigeSuccessDurationMs<800||payload.prestigeSuccessDurationMs>30000)return alert('PRESTIGE 연출 재생 시간은 800~30000ms 사이여야 합니다.');
      if(payload.prestigeSuccessVolumePercent<0||payload.prestigeSuccessVolumePercent>100)return alert('PRESTIGE 연출 볼륨은 0~100 사이여야 합니다.');
      try{
