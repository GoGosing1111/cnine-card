(()=>{
  ...
  function logMeta(row){
    if(isZenith(row.evolution_type)){
      return `${num(row.master_star_cost)}개 · ${num(row.coin_cost)}코인 · 성공률 ${num(row.success_rate)}% · ${num(row.attempt_no)}회차 · 원본 카드 ${row.source_consumed?'소모':'유지'}`;
    }
