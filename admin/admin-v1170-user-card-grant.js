(()=>{
  ...
  function ensureGrantBlock(){
    ...
    anchor.insertAdjacentHTML('beforebegin',`<div class="actionBlock userCardGrantBlock" id="userCardGrantBlock">
      <div class="userCardGrantHead"><div><h3>카드 개별 수동 지급</h3><p>FUR·ZENITH·SUPERSTAR·MA는 +13까지 지급 가능 · LIMITED 제외 · 공개 활성 카드만 지급 · 기존 보유 카드는 동일 강화 수치일 때만 수량이 증가합니다.</p></div><span>+13 READY</span></div>
  ...
  async function loadGrantCards(){
    ...
    try{
      ...
      status.textContent=`FUR·ZENITH·SUPERSTAR·MA +13 지원 · LIMITED 제외 · 지급 가능 카드 ${Number(data.cards?.length||0)}개`;
