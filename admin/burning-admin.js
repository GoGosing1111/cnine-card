(()=>{
  ...
  function setState(prefix,settings={},activeMode='NONE'){
    ...
    $(`#${prefix}Discount`).value=Number(settings.packDiscountPercent??(hyper?30:20));
    $(`#${prefix}RewardMultiplier`).value=Number(settings.battleRewardMultiplier??(hyper?2.5:1.5));
    if(hyper)$(`#${prefix}EquipmentDiscount`).value=Number(settings.equipmentBoxDiscountPercent??20);
  ...
  function validate(settings){
    ...
    if(settings.packDiscountPercent<0||settings.packDiscountPercent>90||settings.equipmentBoxDiscountPercent<0||settings.equipmentBoxDiscountPercent>90)return '할인율은 0~90%로 입력하세요.';
    return '';
  }
