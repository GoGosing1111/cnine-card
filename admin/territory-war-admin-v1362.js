(()=>{
  ...
  function mount(){
    ...
    root.innerHTML='';const box=document.createElement('section');box.id='territoryWarAdminV1402';box.className='panel';box.innerHTML=`
      ...
      <section class="panel" style="margin-top:14px;border:1px solid #2d7185;background:linear-gradient(135deg,#071e29,#091019);overflow:hidden"><div style="display:grid;grid-template-columns:minmax(220px,36%) 1fr;gap:22px;align-items:center"><img src="../assets/ui/territory-war/truce-v1.png" alt="두 진영이 전투를 멈춘 임시 휴전 작전" style="width:100%;aspect-ratio:3/2;object-fit:cover;border-radius:14px;border:1px solid #397b91"><div><small style="color:#71d8ee;letter-spacing:.16em;font-weight:900">ADMINISTRATIVE CEASEFIRE</small><h3 style="font-size:28px;margin:7px 0">임시 휴전</h3><p id="tw3TruceState">공격을 중단하고 참가자의 PVP 덱·장비·칭호 최신화를 허용합니다.</p><label class="field" style="max-width:260px"><span>휴전 시간 (분)</span><input id="tw3TruceMinutes" type="number" min="1" max="360" value="15"><small>1~360분 · 서버 종료 시각 기준</small></label><button id="tw3Truce" style="margin-top:12px;background:linear-gradient(180deg,#167f9e,#0b4d68)">임시 휴전 발동</button></div></div></section>
    ...
    q('tw3Truce').onclick=async()=>{const durationMinutes=Math.max(1,Math.min(360,Math.round(Number(q('tw3TruceMinutes').value)||15)));q('tw3TruceMinutes').value=durationMinutes;if(!confirm(`지금부터 ${durationMinutes}분간 영토전 공격을 중단합니까?\n참가자는 휴전 중 PVP 덱과 장비·칭호를 최신화할 수 있습니다.`))return;await operation('TRUCE','admin/territory-war/truce',{durationMinutes});alert(`${durationMinutes}분 임시 휴전이 시작되었습니다.`)};
