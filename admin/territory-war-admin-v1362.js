(()=>{
 ...
 function mount(){const root=document.getElementById('territoryWarAdminRoot');if(!root||document.getElementById('territoryWarAdminV1375'))return;root.innerHTML='';const box=document.createElement('section');box.id='territoryWarAdminV1375';box.className='panel';box.innerHTML=`
 <div class="maintenanceHead"><div><small>TERRITORY WAR V1375</small><h2>영토전 운영 관리</h2><p>운영 모드 OFF 시 모집·편성·전투가 즉시 중지되며, TEST/ON 전환 시 새 모집 회차가 시작됩니다.</p></div><button id="twReload" class="ghost">새로고침</button></div>
 <div class="formgrid">
  ...
  <label class="field"><span>개별 전투 1승 코인</span><input id="twDuelCoin" type="number" min="0"></label>
  <label class="field"><span>개별 전투 1승 조각</span><input id="twDuelShards" type="number" min="0"></label>
  <label class="field"><span>개별 승리 보상 일일 최대 횟수</span><input id="twDuelLimit" type="number" min="0"><small>0이면 제한 없음</small></label>
