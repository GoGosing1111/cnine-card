(()=>{
  ...
  function mount(){
    ...
    root.innerHTML='';const box=document.createElement('section');box.id='territoryWarAdminV1402';box.className='panel';box.innerHTML=`
      ...
      <section class="panel" style="margin-top:14px;border:1px solid #2d7185;background:linear-gradient(135deg,#071e29,#091019);overflow:hidden"><div style="display:grid;grid-template-columns:minmax(220px,36%) 1fr;gap:22px;align-items:center"><img src="../assets/ui/territory-war/truce-v1.png" alt="두 진영이 전투를 멈춘 임시 휴전 작전" style="width:100%;aspect-ratio:3/2;object-fit:cover;border-radius:14px;border:1px solid #397b91"><div><small style="color:#71d8ee;letter-spacing:.16em;font-weight:900">ADMINISTRATIVE CEASEFIRE</small><h3 style="font-size:28px;margin:7px 0">임시 휴전</h3><p id="tw3TruceState">공격을 중단하고 참가자의 PVP 덱·장비·칭호 최신화를 허용합니다.</p><label class="field" style="max-width:260px"><span>휴전 시간 (분)</span><input id="tw3TruceMinutes" type="number" min="1" max="180" value="15"><small>1~180분 · 서버 종료 시각 기준</small></label><button id="tw3Truce" style="margin-top:12px;background:linear-gradient(180deg,#167f9e,#0b4d68)">임시 휴전 발동</button></div></div></section>
      <section id="tw3MassAssault" class="panel" style="margin-top:14px;border:1px solid #7d3b2d;background:linear-gradient(135deg,#160b0b,#090b10);overflow:hidden">
        <div style="display:grid;grid-template-columns:minmax(220px,36%) 1fr;gap:22px;align-items:center"><img src="../assets/ui/territory-war/mass-assault-v1.png" alt="인해전술 작전 이미지" style="width:100%;aspect-ratio:3/2;object-fit:cover;border-radius:14px;border:1px solid #8b5734"><div><small style="color:#d5a45e;letter-spacing:.18em;font-weight:900">OWNER SPECIAL OPERATION · ONCE PER ROUND</small><h3 style="font-size:28px;margin:7px 0">인해전술</h3><p id="tw3MassAssaultDesc">현재 전선 정보를 확인하고 있습니다.</p><div id="tw3MassAssaultStats" class="inlineNotice"></div><button id="tw3MassAssaultFire" class="danger" style="margin-top:12px">열세 진영 인해전술 발동</button></div></div>
      </section>`;
    root.appendChild(box);const q=id=>box.querySelector('#'+id);
    const fields={mode:'tw3Mode',battleName:'tw3BattleName',teamAName:'tw3TeamAName',teamBName:'tw3TeamBName',recruitmentHours:'tw3Recruit',preparationMinutes:'tw3Prep',roundMinutes:'tw3Round',minParticipants:'tw3MinParticipants',energyMax:'tw3EnergyMax',energyMinutes:'tw3EnergyMinutes',attackEnergyCost:'tw3EnergyCost',realtimePollSeconds:'tw3Poll',baseSiegeHp:'tw3BaseHp',outpostHpMultiplier:'tw3OutpostHp',midHpMultiplier:'tw3MidHp',gateHpMultiplier:'tw3GateHp',homeHpMultiplier:'tw3HomeHp',damageScale:'tw3DamageScale',minDamage:'tw3MinDamage',maxDamage:'tw3MaxDamage',damageVariancePercent:'tw3Variance',recentActionLimit:'tw3Recent',individualBattleWinCoin:'tw3BattleWinCoin',winnerCoin:'tw3Winner',loserCoin:'tw3Loser',drawCoin:'tw3Draw',participationShards:'tw3Shards',contributionCoinPer1000Damage:'tw3Contribution',maxContributionCoin:'tw3ContributionMax',settlementMinAttacks:'tw3MinAttacks',siegeSnapshotLimit:'tw3SnapshotLimit',siegeSnapshotAttackThreshold:'tw3SnapshotAttacks',siegeSnapshotBonusCoin:'tw3SnapshotBonus',siegeParticipationCubeThreshold:'tw3CubeAttacks',siegeParticipationCubeQuantity:'tw3CubeQuantity'};
    function setBusy(action=''){busy=action;['tw3Save','tw3ResetRecruit','tw3Start','tw3Finish','tw3Reload','tw3MassAssaultFire','tw3Truce'].forEach(id=>{const button=q(id);if(button)button.disabled=Boolean(action)})}
    function renderMassAssault(data){const plan=data.massAssault||{},button=q('tw3MassAssaultFire'),desc=q('tw3MassAssaultDesc'),stats=q('tw3MassAssaultStats');q('tw3MassAssault').hidden=!data.isOwner;if(!data.isOwner)return;desc.textContent=plan.available?`전선이 ${plan.teamName} 본진 방향으로 ${Number(plan.frontDepth||0)}칸 밀렸습니다. 봇 증원군이 ${plan.targetName}의 공성 HP를 공격합니다.`:(plan.reason||'현재 발동할 수 없습니다.');stats.innerHTML=plan.available?`<b>${plan.teamName} 전선 지원</b><span>전선 위치 ${Number(plan.frontIndex)+1}/9 · 예상 피해 ${Number(plan.damage||0).toLocaleString()} · 상대 HP ${Number(plan.targetHp||0).toLocaleString()} → ${Number(plan.hpAfter||0).toLocaleString()} · 개인 기여도 미반영</span>`:`<b>${plan.used?'사용 완료':'발동 대기'}</b><span>${plan.reason||'-'}</span>`;button.disabled=Boolean(busy)||!plan.available;button.dataset.team=plan.teamName||'';button.dataset.damage=String(plan.damage||0)}
    ...
    q('tw3Truce').onclick=async()=>{const durationMinutes=Math.max(1,Math.min(180,Math.round(Number(q('tw3TruceMinutes').value)||15)));q('tw3TruceMinutes').value=durationMinutes;if(!confirm(`지금부터 ${durationMinutes}분간 영토전 공격을 중단합니까?\n참가자는 휴전 중 PVP 덱과 장비·칭호를 최신화할 수 있습니다.`))return;await operation('TRUCE','admin/territory-war/truce',{durationMinutes});alert(`${durationMinutes}분 임시 휴전이 시작되었습니다.`)};
    q('tw3MassAssaultFire').onclick=async()=>{const button=q('tw3MassAssaultFire'),team=button.dataset.team||'열세 진영',damage=Number(button.dataset.damage||0);if(button.disabled||!confirm(`${team}을 지원하는 인해전술을 발동합니까?\n상대 공성 HP에 ${damage.toLocaleString()} 피해가 적용되며 이번 회차에는 다시 사용할 수 없습니다.`))return;await operation('MASS_ASSAULT','admin/territory-war/mass-assault');alert('인해전술이 선포되었습니다. 전체 유저 화면에 작전 팝업이 표시됩니다.')};
