(() => {
  ...
  function mount() {
    ...
    if (nav && !nav.querySelector('[data-view="monstersiege"]')) {
      ...
      b.innerHTML = '몬스터 공성전 <span class="buildBadge">6H</span>';
    ...
    if (cms && !$("#view-monstersiege")) {
      ...
      s.hidden = true;
      s.innerHTML = `<div class="sectionIntro"><div><small>ALLIANCE MONSTER SIEGE</small><h2>6시간 몬스터 공성전</h2><p>영토전과 독립된 서버 협동 공성전의 단계·체력·보상을 관리합니다.</p></div><button class="ghost" id="msaReload">새로고침</button></div><div id="msaRoot" class="panel">공성전 설정을 불러오는 중입니다.</div>`;
      cms.appendChild(s);
  ...
  async function save() {
    const phases = last.settings.phases.map((p, i) => ({
      ...
      body = {
        ...
        victoryDamagePercent: Number($("#msaVictoryDamage").value),
