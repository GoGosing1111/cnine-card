(() => {
  ...
  function render(d) {
    ...
    root.innerHTML = `<div class="maintenanceHead"><div><small>CASTLE OPERATION CONTROL</small><h2>${event ? `${event.name} · 진행 중` : "공성전 대기"}</h2><p>${event ? `종료 ${event.endsAt} · 누적 피해 ${Number(event.totalDamage || 0).toLocaleString()}` : "설정을 저장한 뒤 수동 시작할 수 있습니다."}</p>${benchmark.rounds ? `<p>최근 영토전 ${benchmark.rounds}회 평균 · 참가 ${benchmark.averageParticipants}명 · 총 공격 ${benchmark.averageAttacks}회 · 유지 ${benchmark.averageHours}시간</p>` : `<p>완료된 영토전 표본이 없어 기본 예상 전투력으로 판수를 계산합니다.</p>`}</div></div>
      ...
      <h3>몬스터 AI · 지도형 진영전</h3><div class="formgrid"><label class="field"><span>몬스터 AI</span><select id="msaAiEnabled"><option value="ON" ${cfg.monsterAiEnabled !== false ? "selected" : ""}>ON</option><option value="OFF" ${cfg.monsterAiEnabled === false ? "selected" : ""}>OFF</option></select><small>돌격대가 서버 시간 기준으로 연합 전선을 지속 공격</small></label><label class="field"><span>연합 진영 최대 HP</span><input id="msaAllianceHp" type="number" min="100000" max="2000000000" value="${cfg.allianceFortressHp}"><small>전선별 연합 진영 체력 · 진행 중 변경 시 현재 비율 보존</small></label><label class="field"><span>돌격대 공격 간격(초)</span><input id="msaAiInterval" type="number" min="15" max="300" value="${cfg.monsterAttackIntervalSeconds}"><small>15~300초 · 접속 공백도 일괄 반영</small></label><label class="field"><span>돌격대 공격력 배율(%)</span><input id="msaAiPower" type="number" min="10" max="500" value="${cfg.monsterAttackPowerPercent}"><small>전선별 돌격대 작전 피해에 공통 적용</small></label></div><div class="formgrid">${cfg.phases.map((p, i) => `<div class="field"><span>${i + 1}. ${p.ai?.skillName || "몬스터 작전"}</span><small>${p.ai?.role || "MONSTER OPERATION"} · 돌격대 작전 / 방어대 거점 주둔</small></div>`).join("")}</div>
