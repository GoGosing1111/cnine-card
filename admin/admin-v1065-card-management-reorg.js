...
(() => {
  ...
  function ensureView(){
    ...
    // V1802/V1940: FUR/ZENITH/SUPERSTAR +11~+13 패널을 눈에 띄게 (관리자 CSS 파일이 따로 없어 한 번만 주입한다)
    if(!document.getElementById('highPanelFeaturedStyle-v1802')){
      ...
      style.textContent+='#maHighEnhancementRows .furExtendedEnhancementRow{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}#maHighEnhancementRows .furExtendedEnhancementRow>div:first-child{grid-column:1/-1}#maHighEnhancementRows .furExtendedEnhancementRow label{min-width:0}#maHighEnhancementRows .furExtendedEnhancementRow input{width:100%;min-width:0}@media(max-width:760px){#maHighEnhancementRows .furExtendedEnhancementRow{grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}#maHighEnhancementRows .furExtendedEnhancementRow label span{font-size:11px;line-height:1.5}}';
  ...
  function normalizeData(d={}){
    ...
    // V1802/V1940: FUR/ZENITH/SUPERSTAR 는 중복카드·천장·고유효과 배율 항목이 추가로 붙는다.
    ...
    furHigh.extendedEnabled=d.furHigh?.extendedEnabled===true;
    const extra=[{cost:20000,duplicateCards:5,rate:15,pityThreshold:10},{cost:30000,duplicateCards:8,rate:10,pityThreshold:20}];
    furHigh.steps.push(...extra.map((defaults,i)=>({...defaults,powerBonusPercent:null,uniqueBoostPercent:null,retirementShardRefund:null,...d.furHigh?.steps?.[i+3]})));
    return {...d,grades,config,pity,maHigh,limitedHigh,furHigh,zenithHigh};
  }
  const HIGH_GRADES=['MA','LIMITED','FUR','ZENITH','SUPERSTAR'];
  const EXTENDED_HIGH_GRADES=['FUR','ZENITH','SUPERSTAR'];
  function highConfigFor(grade){
    if(grade==='LIMITED')return data.limitedHigh;
    if(grade==='FUR')return data.furHigh;
    if(grade==='ZENITH'||grade==='SUPERSTAR')return data.zenithHigh;
    return data.maHigh;
  }
    ...
    if(HIGH_GRADES.includes(grade)){
      ...
      if(high){
        $('#maHighEnhancementRows')?.querySelectorAll('input').forEach(input=>{
          ...
          if(step&&input.dataset.maHighKind)step[input.dataset.maHighKind]=grade==='FUR'&&Number(input.dataset.index)>=3&&input.value===''?null:Number(input.value);
        });
        const box=$('#maHighEnabled');
        if(box&&!$('#maHighEnhancementPanel')?.hidden)high.enabled=box.checked===true;
        if(grade==='FUR'&&$('#furExtendedEnabled'))high.extendedEnabled=$('#furExtendedEnabled').checked===true;
      }
    }
    return grade;
  }
  ...
  function render(){
    ...
    const materialLabel=['ZENITH','SUPERSTAR'].includes(current)?'마스터의 별 비용':'카드 조각 비용';
    ...
    const highPanel=$('#maHighEnhancementPanel');if(highPanel){const masterStarGrade=HIGH_GRADES.includes(current),extended=EXTENDED_HIGH_GRADES.includes(current),high=highConfigFor(current);highPanel.hidden=!masterStarGrade;
      // V1802/V1940: FUR/ZENITH/SUPERSTAR 는 +11~+13 이 본론이라 STEP 10 아래까지 스크롤하게 두지 않는다.
      ...
      if(masterStarGrade){$('#maHighEnabled').checked=high?.enabled===true;$('#maHighEnhancementRows').innerHTML=high.steps.map((r,i)=>`<div class="enhancementRow maHighEnhancementRow${current==='FUR'&&i>=3?' furExtendedEnhancementRow':''}"><div><small>${current} MASTER STAR STEP ${i+1}</small><b>★${10+i} → ★${11+i}</b></div><label><span>마스터의 별 비용</span><input data-ma-high-kind="cost" data-index="${i}" type="number" min="1" max="${extended?9999999:9999}" value="${Number(r.cost)}"></label><label><span>성공 확률 (%)</span><input data-ma-high-kind="rate" data-index="${i}" type="number" min="0" max="100" step="0.01" value="${Number(r.rate)}"></label>${extended?`<label><span>중복 카드 소모 (장)</span><input data-ma-high-kind="duplicateCards" data-index="${i}" type="number" min="0" max="99" value="${Number(r.duplicateCards||0)}"></label><label><span>천장 (연속 실패 횟수)</span><input data-ma-high-kind="pityThreshold" data-index="${i}" type="number" min="0" max="999" value="${Number(r.pityThreshold||0)}"></label><label><span>고유효과 증가 (%)</span><input data-ma-high-kind="uniqueBoostPercent" data-index="${i}" type="number" min="0" max="1000" value="${current==='FUR'&&i>=3?r.uniqueBoostPercent??'':Number(r.uniqueBoostPercent||0)}"></label>`:''}${current==='FUR'&&i>=3?`<label><span>기본 전투력 대비 증가 (%)</span><input data-ma-high-kind="powerBonusPercent" data-index="${i}" type="number" min="1" max="1000000" step="0.01" placeholder="미정" value="${r.powerBonusPercent??''}"></label>`:''}<label><span>퇴사 환급 카드 조각</span><input data-ma-high-kind="retirementShardRefund" data-index="${i}" type="number" min="0" max="10000000" value="${current==='FUR'&&i>=3?r.retirementShardRefund??'':Number(r.retirementShardRefund||0)}"></label></div>`).join('')}}
    if(highPanel&&!highPanel.hidden){
      let extension=$('#furExtendedControls');
      if(!extension){extension=document.createElement('div');extension.id='furExtendedControls';extension.className='inlineNotice';extension.innerHTML='<label class="enhancementSwitch"><input type="checkbox" id="furExtendedEnabled"><span>FUR +14·+15 운영</span></label><p>전투력·고유효과 증가율과 퇴사 환급량을 입력한 뒤 사용합니다. 미정 값은 빈칸으로 유지됩니다.</p>';highPanel.querySelector('.maintenanceHead')?.after(extension);}
      extension.hidden=current!=='FUR';$('#furExtendedEnabled').checked=data.furHigh.extendedEnabled===true;const extended=EXTENDED_HIGH_GRADES.includes(current),sharedZenith=current==='SUPERSTAR',title=highPanel.querySelector('h3'),eyebrow=highPanel.querySelector('.maintenanceHead small'),description=highPanel.querySelector('.maintenanceHead p');if(eyebrow)eyebrow.textContent=sharedZenith?'SUPERSTAR · ZENITH SHARED ENHANCEMENT':`${current} MASTER STAR ENHANCEMENT`;if(title)title.textContent=sharedZenith?'SUPERSTAR +10 → +13 · ZENITH 공용 설정':`${current} +10 → +${current==='FUR'?15:13} 고급 강화`;highPanel.classList.toggle('highPanelFeatured',extended);
      if(description)description.textContent=sharedZenith?'SUPERSTAR는 ZENITH와 비용·확률·천장·고유효과 증가·환급 설정을 동일하게 공유합니다. 이 화면의 변경은 ZENITH에도 함께 적용됩니다.':extended?'재료는 실패해도 돌려주지 않습니다. 천장은 그 횟수만큼 연속 실패하면 다음 시도를 100% 성공으로 확정합니다. 0 이면 천장 없음. 중복 카드는 강화 대상 1장을 제외한 여분에서 차감합니다.':'+10 이후 세 단계는 카드 조각 대신 마스터의 별을 사용합니다.'}
  ...
  async function save(){
    ...
    const on=[['MA',data.maHigh],['LIMITED',data.limitedHigh],['FUR',data.furHigh],['ZENITH',data.zenithHigh],['SUPERSTAR',data.zenithHigh]].filter(([,c])=>c?.enabled===true).map(([g])=>g);
    alert(`돌파·강화 설정을 저장했습니다.\n\n고급 강화 운영 중: ${on.length?on.join(', '):'없음'}`);
    render();
  }
