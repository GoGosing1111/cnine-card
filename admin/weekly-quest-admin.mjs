...
export async function mountWeeklyQuestAdmin({panel,api}){
 ...
 if(!root){root=document.createElement('section');root.id='weeklyQuestSettings';root.className='qw-admin';panel.prepend(root)}
 if(!document.getElementById('weeklyQuestAdminStyle')){const style=document.createElement('link');style.id='weeklyQuestAdminStyle';style.rel='stylesheet';style.href='weekly-quest-admin.css?v=20260924-1';document.head.append(style)}
 ...
 <p class="qw-guidance">집계는 유지되며, 지급을 ON으로 저장한 퀘스트만 보상 수령이 열립니다. 신규 퀘스트는 모두 OFF / 보상 0으로 시작합니다. 일일 보상은 아래 기존 일일 설정을 그대로 사용합니다.</p>
