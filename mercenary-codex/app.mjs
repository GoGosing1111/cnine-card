...
function renderList(){
  ...
  list.innerHTML=rows.length?rows.map(c=>`<li><button class="roster-row" type="button" data-code="${c.code}" aria-pressed="${c.code===selected}" aria-label="${esc(c.name)} ${esc(c.rank||'미정')} ${positionLabel(c)} 선택"><span class="row-portrait"><img src="${c.artOnly?asset(c.sourceArt):thumb(c.code)}" alt="" loading="lazy" width="48" height="72"></span><span class="row-identity"><small>${c.code} <i>${positionLabel(c)} · ${esc(catalog.roles[c.role].label)}</i></small><b>${esc(c.name)}</b><em>${esc(c.title)}</em></span><span class="rank-chip" data-rank="${c.rank||''}">${c.rank||'—'}</span>${favorites.has(c.code)?'<span class="row-favorite" aria-label="즐겨찾기">★</span>':''}</button></li>`).join(''):'<li class="empty-state">조건에 맞는 용병이 없습니다.<button type="button" data-reset>전체 용병 보기</button></li>';
}
...
function renderSelection(){
  ...
  if(c.artOnly){
    $('inspection').querySelector('.combat-panel').innerHTML=`<div class="panel-heading"><h2>신규 원화 공개</h2><button type="button" class="favorite-button" data-favorite aria-pressed="${favorites.has(c.code)}" aria-label="${esc(c.name)} 즐겨찾기 ${favorites.has(c.code)?'해제':'추가'}">${favorites.has(c.code)?'★':'☆'}</button></div><div class="power-summary"><span>용병 등급<small>백색 제복 · 기관총 사수</small></span><strong>${esc(c.rank)}</strong></div><dl class="role-ledger"><div><dt>이름</dt><dd>${esc(c.name)}</dd></div><div><dt>무기</dt><dd>${esc(c.weapon)}</dd></div></dl><div class="skill-empty"><b>출시 예정</b><p>원화를 먼저 만나보세요.<br>획득 방법과 전투 스킬은 추후 공개됩니다.</p></div>`;
  }
  renderMedia();
}
...
async function refresh({quiet=false}={}){
  ...
  try{const next=validateCatalog(await api('mercenary-codex',{timeoutMs:15000}));
    const changed=!catalog||catalog.revision!==next.revision||catalog.combatLink?.version!==next.combatLink?.version||catalog.artReleaseVersion!==next.artReleaseVersion;catalog=next;
