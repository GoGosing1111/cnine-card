...
export function validateCatalog(data){
  ...
  for(const c of data.cards){if(!/^V-\d{3}$/.test(c.code)||!c.name||!c.title||!POSITIONS[c.position]||!data.roles?.[c.role]||(c.rank!==null&&!RANKS.includes(c.rank))||!Array.isArray(c.skills))throw Error('용병 설정을 확인하세요.');asset(c.sourceArt);if(c.battleSprite)asset(c.battleSprite);
